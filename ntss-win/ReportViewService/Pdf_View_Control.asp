...
strPathPatId = ZeroPadByLen(strPatID, hospPatIdLen)
' add 2021-07-16 #5429:患者番号の前ゼロの扱いについて 孫 end
...
<!--	function afNextButton(intCnt, intVer, strFileName, strRep){ -->
	function afNextButton(intCnt, intVer, strFileName, strRep, hospPatIdLen){
		with(document.form1){
<!--			top.location.href = "Pdf_View_Main.asp?cnt="+intCnt+"&ver="+intVer+"&id=<%=strPatID%>&fn="+strFileName+"&repName="+strRep; -->
			top.location.href = "Pdf_View_Main.asp?cnt="+intCnt+"&ver="+intVer+"&id=<%=strPatID%>&fn="+strFileName+"&repName="+strRep+"&padlen=<%=hospPatIdLen%>";
		}
	}
	// 最新ボタン押下
<!--	function afLatestButton(intCnt, intVer, strFileName, strRep){ -->
	function afLatestButton(intCnt, intVer, strFileName, strRep, hospPatIdLen){
		with(document.form1){
<!--			top.location.href = "Pdf_View_Main.asp?cnt="+intCnt+"&ver="+intVer+"&id=<%=strPatID%>&fn="+strFileName+"&repName="+strRep; -->
			top.location.href = "Pdf_View_Main.asp?cnt="+intCnt+"&ver="+intVer+"&id=<%=strPatID%>&fn="+strFileName+"&repName="+strRep+"&padlen=<%=hospPatIdLen%>";
		}
	...
	// 表示ボタン押下
<!--	function afPDFView(){ -->
	function afPDFView(hospPatIdLen){
		with(document.form1){
<!--			top.location.href = "Pdf_View_Main.asp?cnt=<%=intCnt%>&ver="+lstVersion.value+"&id=<%=strPatID%>&fn=<%=Session("aryData")(1,intCnt)%>&repName=<%=Session("aryData")(3,intCnt)%>"; -->
			top.location.href = "Pdf_View_Main.asp?cnt=<%=intCnt%>&ver="+lstVersion.value+"&id=<%=strPatID%>&fn=<%=Session("aryData")(1,intCnt)%>&repName=<%=Session("aryData")(3,intCnt)%>"+"&padlen=<%=hospPatIdLen%>";
		}
	}
-->
</SCRIPT>
...
<FORM NAME="form1">
	<TABLE class="no-border" style="width: 100%;">
		<TR valign="top" align="center">
			<TD>
				<table class="border control-info">
					<tr>
						<td><img src="image/nikkiso.bmp"></td>
					</tr>
					<tr>
						<td>
<%If IsEmpty(Session("aryData")) = False Then%>
...
<!--							<INPUT type="button" value="最　新" onClick="Javascript:afLatestButton(<%=0%>,<%=Session("aryData")(2,0)%>,'<%=Session("aryData")(1,0)%>','<%=Session("aryData")(3,0)%>');"> -->
							<INPUT type="button" value="前　回" onClick="Javascript:afNextButton(<%=intNextCnt%>,<%=Session("aryData")(2,intNextCnt)%>,'<%=Session("aryData")(1,intNextCnt)%>','<%=Session("aryData")(3,intNextCnt)%>','<%=hospPatIdLen%>');">
							<INPUT type="button" value="次　回" onClick="Javascript:afNextButton(<%=intPrevCnt%>,<%=Session("aryData")(2,intPrevCnt)%>,'<%=Session("aryData")(1,intPrevCnt)%>','<%=Session("aryData")(3,intPrevCnt)%>','<%=hospPatIdLen%>');">　
							<INPUT type="button" value="最　新" onClick="Javascript:afLatestButton(<%=0%>,<%=Session("aryData")(2,0)%>,'<%=Session("aryData")(1,0)%>','<%=Session("aryData")(3,0)%>','<%=hospPatIdLen%>');">
<%Else%>
...
'					If objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & ZeroPad(strPatID) & "/" & Session("aryData")(3,intCnt) ) Then
					If objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & strPathPatId & "/" & Session("aryData")(3,intCnt) ) Then
...
'					If objFSO.FileExists(Server.MapPath("./" & pdfPath ) & ZeroPad(strPatID) & "/" & Session("aryData")(1,intCnt) & "0001.pdf") or _
'							objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & ZeroPad(strPatID) & "/" & Session("aryData")(1,intCnt) & ".pdf") Then
					If objFSO.FileExists(Server.MapPath("./" & pdfPath ) & strPathPatId & "/" & Session("aryData")(1,intCnt) & "0001.pdf") or _
							objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & strPathPatId & "/" & Session("aryData")(1,intCnt) & ".pdf") Then
%>							<SELECT NAME="lstVersion" style="width:4em;">
								<OPTION VALUE=1>001
							</SELECT>
							<INPUT type="button" value="表示" onClick="Javascript:afPDFView();">
...
'								objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & ZeroPad(strPatID) & "/" & Session("aryData")(1,intCnt) & ".pdf") Then
							If objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & strPathPatId & "/" & Session("aryData")(1,intCnt) & "0001.pdf") or _
								...
								End If
							End If
'						ElseIf objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & ZeroPad(strPatID) & "/" & Session("aryData")(1,intCnt) & Right("000" & j, 4) & ".pdf") Then
						ElseIf objFSO.FileExists(Server.MapPath("./" & pdfPath ) & "/" & strPathPatId & "/" & Session("aryData")(1,intCnt) & Right("000" & j, 4) & ".pdf") Then
							strFileName = Session("aryData")(1,intCnt)
							If aryComboData = "" then
								aryComboData = j
							Else
