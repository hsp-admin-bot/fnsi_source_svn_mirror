        ...
        public bool GetLatestProgramFile()
        {
            ...
            if (string.IsNullOrEmpty(strdldata) == false)
            {
                ...
                if (System.IO.File.Exists(strdlfile) == true)
                {
                    ...
                    // [DEBUG] 解凍前情報
                    long dbgFileSize = new System.IO.FileInfo(strdlfile).Length;
                    string dbgPwInfo = string.IsNullOrEmpty(DownloadFilePassword) ? "なし" : "設定あり(長さ=" + DownloadFilePassword.Length.ToString() + ")";
                    LoggingMethod(NKKLoggingLib.NKKLogging.LOGGING_CLASS.INFO,
                        "[DEBUG] 解凍開始, ファイル:" + strdlfile +
                        ", サイズ:" + dbgFileSize.ToString() + "bytes" +
                        ", パスワード:" + dbgPwInfo +
                        ", エンコード:" + EntryNameEncoding.EncodingName +
                        ", 展開先:" + DownloadDestFolder);

                    // 最新zipファイルを解凍
                    bool unzipResult = false;
                    try
                    {
                        unzipResult = TdcLib.TdcLib.UnCompressZipFile(EntryNameEncoding, strdlfile, DownloadDestFolder, string.Empty,
                                                            DownloadFilePassword);
                    }
                    catch (Exception unzipEx)
                    {
                        LoggingMethod(NKKLoggingLib.NKKLogging.LOGGING_CLASS.ERROR,
                            $"[DEBUG] 解凍中に例外発生, 例外タイプ:{unzipEx.GetType().FullName}" +
                            $", メッセージ:{unzipEx.Message}" +
                            $", InnerException:{unzipEx.InnerException?.Message ?? "(null)"}" +
                            $", StackTrace:{unzipEx.StackTrace}");
                        System.Threading.Thread.Sleep(2000);
                        return false;
                    }
                    ...
                    // [DEBUG] 解凍結果
                    LoggingMethod(NKKLoggingLib.NKKLogging.LOGGING_CLASS.INFO,
                        $"[DEBUG] 解凍結果:{unzipResult}, TdcLib.Error:{TdcLib.TdcLib.Error?.Message ?? "(null)"}");
                    ...
                    if (unzipResult)
                    {
                        ...
                        try
                        {
                            var dlFiles = System.IO.Directory.GetFiles(DownloadDestFolder, "*", System.IO.SearchOption.AllDirectories);
                            LoggingMethod(NKKLoggingLib.NKKLogging.LOGGING_CLASS.INFO,
                                "[DEBUG] 展開ファイル一覧(" + dlFiles.Length.ToString() + "件): " + string.Join(" / ", dlFiles));
                        }
                        catch (Exception listEx)
                        {
                            LoggingMethod(NKKLoggingLib.NKKLogging.LOGGING_CLASS.ERROR, $"[DEBUG] ファイル一覧取得失敗: {listEx.Message}");
                        }
