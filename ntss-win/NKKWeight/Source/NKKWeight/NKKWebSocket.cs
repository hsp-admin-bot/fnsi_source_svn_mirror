...
#region プライベートメソッド
        ...
        private void DoWork()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (String.IsNullOrEmpty(this.m_strAuthenticationKey) == false)
                        {
                            ...
                            // 名前が現在のリモートエンドポイントのホスト名と一致しない証明書を許可
                            this.m_wscl.Security.AllowNameMismatchCertificate = true;
                            // TLS1.2認証のみ許可
                            this.m_wscl.Security.EnabledSslProtocols = System.Security.Authentication.SslProtocols.Tls12;

                            //// 信頼されない認証を許可する(自己証明書を使用する場合は有効にする)
                            //this.m_wscl.Security.AllowUnstrustedCertificate = true;
                ...
                //// スレッド実行処理初回処理じゃないの場合、そして印刷処理の場合
                //if (NKKWeight.dFlag && NKKWeight.postPrintFalg())
                //{
                //    // 印刷処理を呼び出す
                //    NKKWeight.getWeightNewNo();
                //}
        ...
        private void WS_Closed(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ex != null)
                {
                    strlog = String.Format("{0}, {1}({2})", strlog, ex.Reason, ex.Code);
                }
                this.AddLogInfo(DateTime.Now, NKKLogging.LOGGING_CLASS.INFO, strlog);
