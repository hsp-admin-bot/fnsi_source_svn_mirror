...
namespace NKK.FN3.Common.Library.TcpSocket
{
    ...
    public class BaseSocket
    {
        ...
        public void Release()
        {
            // <<<<< CHG S.Yamazaki 2024/10/24 ソケット操作をする個所でtry-catchで包んでない部分に対しtry-catchで包み例外発生を防ぐ rm#13270
            //lock (m_syncSocket)
            //{
            //    if (m_runFlg == true)
            //    {
            ...
            try
            {
                lock (m_syncSocket)
                {
                    ...
                    if (m_rcvThread != null)
                    {
                        ...
                        {
                            ; // [発生が見込まれる例外、かつ、問題ない例外]なのでイチイチTRACEログに出力しない
                        }
                        catch (Exception ex)
                        {
                            ErrorWrite(ex, "");
        ...
        public int SendData(byte[] data, int size)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    ErrorWrite(e, "{0}.BaseSocket.ReceveThread 例外発生！！", System.Diagnostics.Process.GetCurrentProcess().ProcessName);
                }
