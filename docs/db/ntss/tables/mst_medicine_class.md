...
- Source sheet: `mst_medicine_class`
- Logical name: 薬剤分類マスタ
- Physical name: `mst_medicine_class`
- Prefix group: `master`
- User: `nkk5`
...
|  | 分類区分 | class_type | numeric | 2 |  |  | 0：該当なし、1：抗凝固剤、2：透析液、3：補液<br>※マスタ編集画面での編集は可能とする。<br>※区分変更時はメッセージ通知を行う。<br>（既に登録されている指示に影響する旨を通知する。）<br>※条件送信時に治療条件と区分の整合性が取れているかチェックする必要がある。 |
