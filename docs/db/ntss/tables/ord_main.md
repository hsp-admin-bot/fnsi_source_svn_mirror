...
- Source sheet: `ord_main`
- Logical name: 治療情報
- Physical name: `ord_main`
...
- [../config/recirculation-rate.md](../config/recirculation-rate.md)
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | システムで管理する一意なオーダ番号 | ord_no | bigserial |  | 1 |  | シーケンス使用 |
...
|  | 指示：装置モード | ind_device_mode | numeric | 2,0 |  |  | 治療方法マスタ.装置モード |
...
|  | 指示：治療予定指示者情報 | ind_schedule_user_info | jsonb |  |  |  | ■Json構造<br>{<br>  "ind_kur_cd_before": (Number)変更前クール(クールマスタ.クールコード、変更前がない状態：null、未登録：0),<br>  "ind_treat_start_time_before": (String)変更前治療開始予定時刻（HHMM形式、変更前がない状態：null、未登録：null）,<br>  "ind_user_id": (Number)指示者コード(利用者マスタ.利用者ID),<br>  "ind_user_last_name": (String)指示者名_姓(利用者マスタ.利用者名_姓),<br>  "ind_user_first_name": (String)指示者名_名(利用者マスタ.利用者名_名),<br>  "upd_user_id": (Number)更新者コード(利用者マスタ.利用者ID),<br>  "upd_user_last_name": (String)更新者名_姓(利用者マスタ.利用者名_姓),<br>  "upd_user_first_name": (String)更新者名_名(利用者マスタ.利用者名_名)<br>}<br>■概要<br>治療予定新規作成、スケジュール情報(クール、治療開始時刻、ベッド)の変更、治療日の移動などの操作時に登録する。 |
...
|  | 実績：登録区分 | rst_input_class | smallint |  |  |  | 1：通常(透析装置や通信サーバーなどを伴う治療)<br>2：クライアントで手入力して作成<br>3：？？？？患者スケジュール割り当て<br>4：？？？？患者名前割り当て |
...
|  | 実績：バイタル情報 | rst_vital_info | jsonb |  |  |  | ※治療記録画面と装置側のデータの持ち方によって検討が必要<br>■Json構造<br>[<br>  {<br>    "bio_moni_ctl_no": (Number)生体モニタリング管理番号, (*1)<br>    "occur_date":  (String)発生日時, (*1)(*2)<br>    "bp_class":  (String)血圧区分, (*1)(*3)<br>    "bp_max":  (Number)最高血圧,<br>    "bp_min":  (Number)最低血圧,<br>    "bp_ave":  (Number)平均血圧,<br>    "blood_sugar_level":  (Number)血糖値,<br>    "pulse":  (Number)脈拍,<br>    "temperature":  (Number)体温,<br>    "is_del":  (String)削除フラグ,<br>    "ctl_no": (Number)管理番号,<br>    "input_class": (Number)入力区分<br>  }, ・・・<br>]<br>■概要<br>(*1) 必須<br>(*2) ISO8601形式：【TDC修正】<br>(*3) 0: 設定無し、1: 前血圧、2: 後血圧<br>(*5) 血圧測定について(装置記録から取得、または測定時に記録)：【TDC追記】<br>　　　　最大管理番号は保持する必要がある(毎回配列をチェックして最大値+1は非効率)<br>(*6) 前血圧や後血圧が複数あった場合、どの管理番号の測定値をつかうかを設定する必要はないのか？：【TDC追記】 |
|  | 実績：愁訴情報 | rst_complaint_info | jsonb |  |  |  | ※治療記録画面と装置側のデータの持ち方によって検討が必要<br>■Json構造<br>[<br>  {<br>    "checkFlag":(Number)帳票出力区分,(*4)<br>    "ctl_no": (Number)管理番号, (*1)<br>    "input_class": (Number)入力区分, (*1)(*2)<br>    "row_no": (Number)行番号, ※必要か？<br>    "occur_date": (String)発生日時, (*1)(*3)<br>    "comp_cd": (Number)愁訴コード, (*1)<br>    "complaint": (String)愁訴内容 (*1)<br>  }, ・・・<br>]<br>■概要<br>(*1) 必須<br>(*2) 0: 通信サーバ、1: クライアント、2: 連携<br>(*3) ISO8601形式：【TDC修正】<br>(*4) 帳票出力区分【:帳票出力追加】 |
|  | 実績：愁訴処置情報 | rst_treatment_info | jsonb |  |  |  | ※治療記録画面と装置側のデータの持ち方によって検討が必要<br>■Json構造<br>[<br>  {<br>    "ctl_no": (Number)管理番号,<br>    "row_no": (Number)行番号,<br>    "occur_date": (String)発生日時,<br>    "treat_class": (Number)処置区分,<br>    "treat_cd": (Number)処置コード,<br>    "treat_name": (String)処置名,<br>    "medicine_cd": (Number)薬剤コード,<br>    "medicine_name": (String)薬剤名,<br>    "amount": (Number)数量,<br>    "unit": (String)単位,<br>    "procedure_cd": (Number)手技コード,<br>    "procedure_name": (String)手技名,<br>    "treat_medicine_cd": (Number)処置薬剤コード,<br>    "treat_medicine_name": (String)処置薬剤名,<br>    "oxygen_start": (String)酸素吸入開始日時,<br>    "oxygen_time": (Number)酸素吸入時間,<br>    "oxygen_amount": (Number)酸素吸入量,<br>    "oxygen_speed": (Number)酸素速度,<br>    "input_class": (Number)入力区分,<br>    "cop_order_no": (String)連携オーダ番号, (*1)<br>    "is_editable": (String)編集可能フラグ, (*2)<br>    "electrocardiogram_type": (Number)心電編集種別 (*3),<br>    "checkFlag":(Number)帳票出力区分(*5),<br>　　"linkStartDate":(String)酸素吸入関連番号(*6),<br>    "medicine_type": (Number)薬剤区分(*7),<br>    "electrocardiogram_start": (String)心電図開始日時,<br>    "over_time": (Number)経過時間,※単位分<br>  }, ・・・<br>]<br>■概要<br>(*1) 電子カルテからの予約オーダを登録する際に使用<br>(*2) 0: 編集不可、1: 編集可能<br>(*3) 処置区分が 4 (心電)の場合、有効(0: 心電開始、1: 心電終了)<br>(*4) ISO8601形式：【TDC追記】<br>(*5) 帳票出力区分【:帳票出力追加】<br>(*6) 酸素吸入終了の場合、対応の酸素吸入開始のctl_noを記入する。<br>(*7) 1: 通常薬剤、2: 調製薬剤 |
|  | 実績：愁訴処置者情報 | rst_treat_staff_info | jsonb |  |  |  | ※治療記録画面と装置側のデータの持ち方によって検討が必要<br>■Json構造<br>[<br>  {<br>    "ctl_no": (Number)管理番号,<br>    "row_no": (Number)行番号,<br>    "input_class": (Number)入力区分,<br>    "occur_date": (String)発生日時,<br>    "treat_staff_cd": (Number)処置者コード,<br>    "treat_staff_name": (String)処置者名,<br>    "cop_order_no": (String)連携オーダ番号, (*1)<br>    "is_editable": (String)編集可能フラグ (*2),<br>    "checkFlag":(Number)帳票出力区分,(*4)<br>  }, ・・・<br>]<br>■概要<br>(*1) 電子カルテからの予約オーダを登録する際に使用<br>(*2) 0: 編集不可、1: 編集可能<br>(*3) ISO8601形式：【TDC追記】<br>(*4) 帳票出力区分【:帳票出力追加】 |
...
|  | 実績装置モード | rst_device_mode | numeric | 2,0 |  |  |  |
