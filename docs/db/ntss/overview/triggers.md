...
| トリガー一覧 | col2 | col3 | ↓トリガー名記入 | col5 | ↓処理概要を記入 | col7 | col8 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| NTSSデータベース設計書.xlsm | ■ | No | トリガー名 | テーブル名 | 処理概要 | 最終更新日時 | 備考 |
|  | ■ | 1 | tg_sync_mst_machine | mst_machine | 装置マスタのレコード増減に従い、関連テーブルのレコードを増減する | 43252 | アプリケーション共通 |
|  | ■ | 2 | tg_sync_mnt_motion_record | mnt_motion_record | 装置動作記録テーブルの更新に従い、関連テーブルを更新する | 43252 | 稼働ビューア関連 |
|  | ■ | 3 | tg_sync_mnt_machine_state | mnt_machine_state | 装置状態管理のレコード更新に従い、関連テーブルのレコードを更新する | 43448 | 稼働ビューア関連 |
|  | ■ | 4 | tg_sync_mst_bed | mst_bed | ベッドマスタのレコード更新に従い、関連テーブルのレコードを更新する | 43531 | アプリケーション共通 |
|  | ■ | 5 | tg_sync_ord_main | ord_main | 治療情報のレコード更新時に、関連テーブルのレコードを更新する | 43532 | 患者経過総合ビューア |
|  |  | 6 |  |  |  |  |  |
|  |  | 7 |  |  |  |  |  |
|  |  | 8 |  |  |  |  |  |
|  |  | 9 |  |  |  |  |  |
