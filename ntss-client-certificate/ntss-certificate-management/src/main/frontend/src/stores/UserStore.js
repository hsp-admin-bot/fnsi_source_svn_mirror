  ...
  actions: {
    ...
    signIn({ commit }, user) {
      ...
      return sendRequestLogin(user).then(async response => {
        ...
        // このタブでログインしたユーザーIDをウィンドウのグローバル変数に記録する。
        // タブごとに独立したメモリ空間に保持されるため、
        // 別タブでの別ユーザーログインによる書き換えの影響を受けない。
        // リクエスト送信時にこの値を X-Expected-User ヘッダーに付与し、
        // サーバー側でセッションユーザーとの不一致を検出する際に使用する。
        window.__ntssLoginUserId = user.userId;
      });
    },
    signOut({ commit }) {
      return sendRequestLogout().then(() => {
        ...
        // ★追加：正常ログアウト時にグローバル変数をクリアする
        window.__ntssLoginUserId = null;
      });
    },
    // サインイン時刻をクリアする
    clearSignIn({ commit }) {
      ...
      commit("setSignInRestriction", null);
      commit("setSignInTimestamp", -1);
      // ★追加：セッション乗っ取り検出による強制ログアウト時にもグローバル変数をクリアする
      window.__ntssLoginUserId = null;
    }
  },
  getters: {
