...
*/
                    ...
                    switch( packetInfo->cCommType )
                    {
                        case NTSS_COMM_TYPE_NEW:    // 新通信
                            ...
                            {
                                short_set(packetInfo->cMoniData + nHeaderSize, packetInfo->nProcess[0]);
                            }
...
*/
                ...
                else if( cCmdNo[0] == 0x66 )
                {
                    ...
                    // add 強制オフライン 高 start
                    if( packetInfo->force_flg == 1 ) {
                        if(kind == 0x01 && (code == 0x04 || code == 0x05)) {
                            // 種別=0x01 && コード=0x04(前血圧測定)/0x05(後血圧測定)
                            // コード=0x01(血圧測定)
                            cCmd[nHeaderSize + 2] = (u_char)0x01;
                            code = 0x01;
                        }
                    }
                    // add 強制オフライン 高 end
