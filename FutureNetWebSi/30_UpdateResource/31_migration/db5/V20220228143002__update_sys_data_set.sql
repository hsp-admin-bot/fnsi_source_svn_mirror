...
, ind_upd_user_info AS(
  ...
  SELECT
    0 AS order_no
    , ind_schedule_user_info ->> ''ind_user_id'' AS ind_staff_cd 
    , ind_schedule_user_info ->> ''upd_user_id'' AS upd_staff_cd 
  FROM
    ord_main AS om
  WHERE
    om.ord_no = @ordNo 
...
FROM
  ...
  ) AS T', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：共通部と伝票情報の利用者番号取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
