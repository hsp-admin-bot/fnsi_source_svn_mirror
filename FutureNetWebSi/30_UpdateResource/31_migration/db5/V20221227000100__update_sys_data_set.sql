...
SELECT mst_f.value AS rtt
  ...
  WHERE mst_f.facility_setting_no = ''3007'' AND mst_f.facility_cd = ''999998''),'',''))), ''999999999999'') AS a1) AS datt
)
, do_mstmedi_cd AS (
...
WHERE ord_i.ord_no = 1133812 AND ord_i.facility_cd = ''999998''
  ...
  WHERE
    ...
    on
        ...
        on 
                mmd.class_cd = medc.class_cd
    where
        mmd.in_hospital_cd_1 is not null and medi ->> ''medicine_type'' = ''1'' and ord.ord_no =  1133812
        and ''0'' =(select count(*) from ord_main where ord_no = 1133812)
        ) as a
         GROUP BY a.medi_cd1,a.medi_unit                 
union all    
        select  
''指示薬剤del''::TEXT as detail_id,
  ...
  WHERE
    ...
    from
      ...
      LEFT OUTER JOIN mst_medicine_class AS mmdc ON mmdc.class_cd = mmd.class_cd 
    where 
        mmd.in_hospital_cd_1 is not null and medi ->> ''medicine_type'' = ''2'' and ord.ord_no =  1133812
        and ''0'' =(select count(*) from ord_main where ord_no = 1133812)
        )  as a
         GROUP BY a.medi_cd1,a.medi_unit)) AS count_all1 WHERE count_all1.medi_cd1 = count_all.medi_cd1) as medi_back,
        count_all.medi_unit 
...
SELECT 
    CASE WHEN (SELECT in_hospital_cd_1 FROM mst_medicine WHERE medicine_cd = TO_NUMBER( medi ->> ''cd'' :: text, ''999999999999'') AND mst_medicine.facility_cd = ''999998'') IS NOT NULL 
        THEN (SELECT in_hospital_cd_1 FROM mst_medicine WHERE medicine_cd = TO_NUMBER( medi ->> ''cd'' :: text, ''999999999999'') AND mst_medicine.facility_cd = ''999998'') 
        ELSE (SELECT in_hospital_cd_1 FROM mst_medicine_mix WHERE medicine_mix_cd = TO_NUMBER( medi ->> ''cd'' :: text, ''999999999999'') AND mst_medicine_mix.facility_cd = ''999998'') END AS item_cd_s,
...
, data_middle_all AS (
SELECT  
    ''指示薬剤'' :: TEXT as detail_id, count_all.medi_cd1, SUM(count_all.medi_amount :: Float) AS medi_amount, 
