...
(SELECT
    ...
    ''予約詳細'' AS detail_id, 
    --項目コード
    CASE WHEN ord.ind_bed_cd IS NULL THEN ''V9999999''
    ELSE ''V'' || lpad(ord.ind_bed_cd :: TEXT, 7, ''0'') 
    END AS e01, 
    --項目属性
...
SELECT
    ...
    --数量
    TO_CHAR(TO_NUMBER(COALESCE(ord.ind_cond_info ->''3''->>''value'', ''0''),''9999999.999'') ,''FM0999999.990'') AS e04, 
    --選択単位フラグ
    ''1'' AS e05, 
    --単位コード
...
SELECT
    ...
    COALESCE(SUBSTRING(ord.treat_date, 1, 4) || ''/'' || SUBSTRING(ord.treat_date, 5, 2) || ''/'' || SUBSTRING(ord.treat_date, 7, 2), '''') AS e03, 
    --数量
    TO_CHAR(TO_NUMBER(COALESCE(ord.ind_cond_info->''39''->>''value'', ''0''),''9999999.999'') ,''FM0999999.990'') AS e04, 
    --選択単位フラグ
    ''1'' AS e05, 
    --単位コード
...
SELECT
    ...
    COALESCE(meq.equipment_name, '''') AS e03, 
    --数量
    TO_CHAR(TO_NUMBER(equip->>''amount'',''9999999.999'') ,''FM0999999.990'') AS e04, 
    --選択単位フラグ
    CASE WHEN meq.unit IS NULL OR meq.unit = ''''
    THEN ''0'' ELSE ''1'' END AS e05, 
    --単位コード
...
SELECT 
    ...
    CASE
      ...
      WHEN ((SELECT value FROM fji_com_info WHERE key2 = ''LIQUID_SEND_FLAG'') = ''0'' AND (SELECT value FROM fji_com_info WHERE key2 = ''DIALYSIS_LIQUID_ADD_FLG'') = ''1'') OR
        ...
        THEN
          TO_CHAR(TO_NUMBER(COALESCE(ord.ind_cond_info->''17''->>''value'',''0''), ''9999999.999'') + TO_NUMBER(COALESCE(ord.ind_cond_info->''20''->>''value'',''0''), ''9999999.999''), ''FM0999999.990'')
      ELSE ''0000000.000''
    END AS e04,
    --選択単位フラグ
    ''1'' AS e05, 
...
SELECT 
    ...
    COALESCE(mmd.medicine_name, '''') AS e03,
    --数量
    TO_CHAR(TO_NUMBER(COALESCE(ord.ind_cond_info->''19''->>''value'',''0''), ''9999999.999''), ''FM0999999.990'') AS e04,
    --選択単位フラグ
    ''1'' AS e05, 
    --単位コード
...
SELECT
    ...
    --選択単位フラグ
    COALESCE((CASE ord.ind_cond_info->''25''->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END), 
    CASE
...
SELECT
    ...
    COALESCE((CASE ord.ind_cond_info->''25''->>''medicine_type'' WHEN ''2'' THEN mmx.medicine_mix_name ELSE  mmd.medicine_name END), '''') AS e03, 
    --数量
    TO_CHAR(TO_NUMBER(COALESCE(ord.rst_cond_info->''27''->>''value'', ''0''), ''9999999.999''), ''FM0999999.990'') AS e04, 
    --選択単位フラグ
    COALESCE((CASE ord.ind_cond_info->''25''->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END), 
    CASE
...
SELECT 
    ...
    COALESCE((CASE ord.ind_cond_info->''25''->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END), 
    CASE
      WHEN COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_KOU_COAG_TOTAL_UNIT_SEL''), '''') = ''2''
        THEN ''2''
      ELSE ''1''
    END) AS e05, 
    --単位コード
...
SELECT
    ...
    --項目属性
    COALESCE(mp.in_hospital_cd_a2, '''') AS e02, 
    --項目名称
    ...
    '''' AS e07, 
    --タグ名称
    COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_MEDICINE_PROCEDURE_TAG''), '''') AS e08, 
    --出力順
...
WHERE
    ord.ord_no = @ordNo
    AND (SELECT value FROM item_set_info WHERE key2 = ''ITEM_OXYGEN_PROCEDURE_FLAG'') = ''1''
UNION
SELECT
    --22薬品手技(手技あり薬剤)
    --0：注射薬剤以外
    ...
    --項目属性
    COALESCE(mmd.in_hospital_cd_2, (SELECT value FROM item_set_info WHERE key2 = ''ITEM_PROCEDURE_ATTR'')) AS e02, 
    --項目名称
    ...
    --選択単位フラグ
    CASE 
      WHEN (CASE medi->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END) IS NULL OR (CASE medi->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END) = ''''
        THEN ''0'' 
      ELSE ''1''
    END AS e05, 
    --単位コード
...
WHERE
    ord.ord_no = @ordNo
    AND (SELECT value FROM item_set_info WHERE key2 = ''ITEM_OXYGEN_PROCEDURE_FLAG'') = ''1''
    AND (CASE WHEN (SELECT value FROM fji_com_info WHERE key2 = ''MEDICINE_SEND_FLAG'') = ''1'' THEN mmd.is_shot = ''0''
              WHEN (SELECT value FROM fji_com_info WHERE key2 = ''MEDICINE_SEND_FLAG'') = ''2'' THEN mmd.is_shot = ''1''
              ELSE 1 <> 1 END)
UNION
SELECT
    --23処置薬品名(手技なし薬剤)
    ...
    COALESCE(mmd.in_hospital_cd_1, '''') AS e01, 
    --項目属性
    COALESCE(mmd.in_hospital_cd_2, COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_NON_PROCEDURE_ATTR01''), '''')) AS e02, 
    --項目名称
    COALESCE(mmd.medicine_name, '''') AS e03, 
    --数量
    TO_CHAR(TO_NUMBER(medi->>''amount'', ''9999999.999''), ''FM0999999.990'') AS e04, 
    ...
    COALESCE((CASE medi->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END), '''') AS e07, 
    --タグ名称
    COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_MEDICINE_TAG''), '''') AS e08,
    --出力順
    COALESCE((SELECT value FROM item_sort_info WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''') AS e09 
FROM
    ord_main ord
...
WHERE
    ord.ord_no = @ordNo 
    AND ((SELECT value FROM item_set_info WHERE key2 = ''ITEM_OXYGEN_PROCEDURE_FLAG'') = ''0'' 
    OR ((mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''001'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''002'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''003'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''004'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''005'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''006'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''007'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''008'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''009'')) 
    AND (mmd.procedure_cd :: TEXT <> (SELECT value FROM procedure_cd_info WHERE key2 = ''010'')) 
    ))
UNION
SELECT
    --24指示受け確認者
    ''予約詳細'' AS detail_id, 
    --項目コード
    COALESCE(info ->> ''ind_user_id'', '''') AS e01, 
    --項目属性
    COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_CHECK_STAFF_ATTR''), '''') AS e02, 
    --項目名称
    COALESCE(info ->> ''ind_user_first_name'', '''') || COALESCE(info ->> ''ind_user_last_name'', '''') AS e03, 
    --数量
    ''0000000.000'' AS e04, 
    --選択単位フラグ
    ''0'' AS e05, 
...
FROM
    ord_main ord
    cross join lateral json_array_elements (ord.ind_ind_comment_info :: json) info
WHERE
    ord.ord_no = @ordNo
...
order by all_cost.e09', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通：透析予約繰り返し部', '2020-05-08 16:22:41', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}]');
