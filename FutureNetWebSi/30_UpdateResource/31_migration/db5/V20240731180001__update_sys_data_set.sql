...
VALUES(1705, 'WITH in_out_class AS (SELECT (CASE
   ...
   , current_new_data AS (SELECT info ->> ''in_out''       AS in_out
                          ...
                          FROM pat_unique patu
                                   CROSS JOIN LATERAL jsonb_array_elements(in_out_visit_history_info) WITH ordinality AS tmp(info, json_idx)
                          WHERE pat_id = @patId
                            AND facility_cd = ''@facilityCd''
                            AND is_del = ''0''
                          ORDER BY json_idx DESC
                          LIMIT 1)
...
WHERE pat_id = @patId
  ...
  AND ''@isDie'' <> ''1''
  AND ''@inOutClass'' <> ''''
    AND exists_flag = ''0''', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡以外)', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, NULL);