...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (233, 'WITH facility_cd_info AS (
    SELECT 
        facility_name,
...
SELECT
    ...
    p.reg_date,
    p.up_date,
		p.letter_info->>''to_facility_cd'' as to_facility_cd,
		p.letter_info->>''to_medical_institution_cd'' as to_medical_institution_cd,
	  case when  p.letter_info->>''to_facility_cd'' IS NOT NULL AND p.letter_info->>''to_facility_cd'' <> '''' then p.letter_info->>''to_facility_cd'' else 
		(case when p.letter_info->>''to_medical_institution_cd'' IS NOT NULL AND p.letter_info->>''to_medical_institution_cd'' <> '''' then  p.letter_info->>''to_medical_institution_cd''
		else '''' end )  end as  medical_institution_cd
