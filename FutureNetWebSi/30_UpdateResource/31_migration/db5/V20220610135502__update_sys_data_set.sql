DELETE FROM "ntss"."sys_data_set" where "sql_cd" in (1015,1211,1212);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1015, 'SELECT
  ...
  LIMIT 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)日機装)連携設定[患者個人情報]の取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
, cource_ward_info AS (
  SELECT 
    CASE WHEN (SELECT take_cource_flg FROM take_cource_info) = ''1'' AND (''@inOutClassOut'') = ''1'' -- ''1''：入院
...
WHERE
  is_del = ''0'' 
  AND pat_id = @patId', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_患者基本情報の修正', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": 1015, "field_name": "check_value", "replace_var": "@inOutClassOut"}]');
