DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (-87,-88,-89,-68);
...
FROM
    ...
    ) AS T ', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析レポート：施設内職員ID取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-88, '
SELECT
	( CASE @userId::text 
	WHEN '''' THEN 
	@defaultUserNo 
	...
	END ) AS disp_user_id ', 1, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析レポート：施設内職員ID取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -87, "field_name": "staff_cd_comm", "replace_var": "@userId"}, {"sql_cd": -87, "field_name": "default_user_no", "replace_var": "@defaultUserNo"}]');
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-89, '
SELECT
	( CASE @userId::text 
	WHEN '''' THEN 
	@defaultUserNo 
	...
	FROM mst_user_authentication 
	WHERE user_id::text = @userId )
	END ) AS disp_user_id ', 1, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析レポート：施設内職員ID取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -87, "field_name": "staff_cd_data", "replace_var": "@userId"}, {"sql_cd": -87, "field_name": "default_user_no", "replace_var": "@defaultUserNo"}]');
...
WHERE 
    user_id = @userId', 1, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析レポート：施設内職員ID取得', '2022-08-25 06:50:20.979', CURRENT_TIMESTAMP, '[{"sql_cd": -61, "field_name": "staff_cd_comm", "replace_var": "@userId"}]');
