...
WHERE sql_cd IN (-1102002, -1102011, -1102012, -1103001);
...
-- SQL: -1102002 end', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析指示_処置項目情報取得', '2025-07-01 17:38:01.233', current_timestamp, '[{"sql_cd": -1102004, "field_name": "pat_personal_info", "replace_var": "@patPersonalInfo"}]'::jsonb);
INSERT INTO ntss.sys_data_set (sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES(-1102011, 'WITH coop_ini_info AS (
    --連携設定から取得
    SELECT
        COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v'') AS value,
        info ->> ''key1'' AS key1,
...
SELECT
  ...
  CASE
    WHEN t1.tre_info ->> ''treat_class'' = ''3''
      ...
      -9999
    ELSE
      (t1.tre_info ->> ''treat_medicine_cd'')::integer
  END AS mst_cd,
...
UNION ALL
  ...
  SELECT
    imi.temp_no,
    33000 + imi.medicine_type as medicine_type,
    33000 + COALESCE(t.timing_code_order, 0) AS timing_code_order,
    33000 + COALESCE(p.procedure_code_order, 0) AS procedure_code_order,
    33000 + COALESCE(imi.interval_no, 0) AS interval_no,
    33000 + COALESCE(mst_medi.medi_code_order, 0) AS medi_code_order,
    33000 + COALESCE(mst_medi.medi_class_code_order, 0) AS medi_class_code_order,
  ...
  SELECT
    ti.temp_no,
    50000 + ti.medicine_type as medicine_type,
    50000 + COALESCE(t.timing_code_order, 0) AS timing_code_order,
    50000 + COALESCE(p.procedure_code_order,0) AS procedure_code_order,
    50000 + COALESCE(ti.interval_no, 0) AS interval_no,
    50000 + COALESCE(mst_medi.medi_code_order, 0) AS medi_code_order,
    50000 + COALESCE(mst_medi.medi_class_code_order, 0) AS medi_class_code_order,
...
, medi_union_2_with_sorted AS (
    ...
    from medi_union_2
    order by oxgen_flg, 
    case when (SELECT a1 FROM medi_order_data WHERE no2 = 1) <> ''5'' then pro_hosp_cd end,
    sort_num
)
, equip_union AS (
-- 医療材料情報（吸着カラム,1次膜,2次膜,医療材料情報）
