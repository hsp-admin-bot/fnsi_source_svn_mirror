...
SELECT
  ...
  fin.unit
FROM
  FINAL fin', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析指示_処置項目情報取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -1102004, "field_name": "pat_personal_info", "replace_var": "@patPersonalInfo"}]'::jsonb);


INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-1102004, 'SELECT
...
WHERE
  ppm.pat_id = @patId
  AND ppm.is_del = ''0''', 3, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析指示_患者個人情報の透析困難コード取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);