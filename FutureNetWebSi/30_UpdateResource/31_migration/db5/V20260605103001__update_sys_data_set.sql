DELETE FROM "ntss"."sys_data_set" where sql_cd in (16,159);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (16, 'WITH plan_time AS (
	SELECT
		...
		latest_dw.cond_dw || '' Kg'' AS cond_dw,
		CASE 
			WHEN om.ind_cond_info::json #>> ''{3, value}'' = ''-1'' 
			THEN latest_dw.cond_dw || '' Kg''
			ELSE om.ind_cond_info::json #>> ''{3, value}'' || '' Kg''
		END AS cond_tg_wei,
		CASE 
			WHEN om.rst_dialysis_state = ''0'' 
			THEN mst_treatment.treatment_name
			ELSE om.ind_treatment_name 
		END AS cond_tre_nm,
	...
	INNER JOIN mst_treatment 
		ON om.ind_treatment_cd = mst_treatment.treatment_cd
...
EquipmentList_Tmp AS (
	...
	(
		...
		LEFT OUTER JOIN mst_procedure mp ON to_number( medi ->> ''procedure_cd'', ''9999999999'' ) = mp.procedure_cd
			...
			AND mp.is_disp = ''1''
		WHERE
				om.medicine_type = ''2'' AND mdx.medicine_mix_cd IS NOT NULL
		UNION ALL--透析液
		SELECT
			@medsort AS disp_order,
			NULL AS equipment_cd,
...
result_all AS (
	...
	(
		SELECT
			...
			NULL AS dia_order,
			NULL AS room_bed_group,
			NULL AS dialysis_room_group,
			spi.spitz_order
		FROM
		(
			SELECT M
		...
		WHERE
			...
			AND P.reg_order_class IN (@regOrderClassList)
			AND 1 in (@inspectIds)
		group by
			treat_date,
			ord_main.ind_kur_cd,
  ...
  UNION ALL
	...
	(
		...
		LEFT OUTER JOIN room_bed AS rb2 ON rbg2.room_bed_group_cd = rb2.room_bed_code
		WHERE
			P.reg_order_class IN (@regOrderClassList)
