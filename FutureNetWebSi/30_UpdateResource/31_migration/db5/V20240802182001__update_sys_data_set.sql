...
VALUES(-63, 'WITH pdf_path_info AS (
SELECT
                0 AS order_no,
...
SELECT
  pdf_path_info.pdf_file_path
  || ''/''
  || CASE 
     WHEN len > 0 THEN LPAD(LTRIM(data_info.hosp_pat_id) , len, ''0'') 
     ELSE LTRIM(data_info.hosp_pat_id)
     END
  || ''/''
...
WHERE pdf_path_info.order_no = 0', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '富士通）PDFファイルパスの取得', '2022-04-04 16:37:06.279', CURRENT_TIMESTAMP, NULL);