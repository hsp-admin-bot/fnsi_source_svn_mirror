DELETE  FROM sys_data_set WHERE sql_cd IN (45,4,141,190,74,97,166,95,100,167,170,8,165);
...
from
  ...
  left join ord_material_save as save on (save.supplies_base_no = ord.ord_no and ord.facility_cd = save.facility_cd and ord.cd  = save.supplies_cd and save.supplies_source_class = ''1'' and save.ind_rst_class =''2'')
    LEFT JOIN ( select
...
from
  ord_tbl as ord
  inner join dialyzer_tbl as dia on ord.cd = dia.dialyzer_cd::text and equip_type = ''1'' AND dia.dialyzer_cd IN (@diaIds)
UNION ALL
select
...
from
  ord_tbl as ord
  inner join equipment_tbl as eqp on ord.cd = eqp.equipment_cd::text and equip_type = ''0'' AND eqp.class_cd IN (@eqIds)
...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (95, 'with ord_tbl as (
  select
    facility_cd,
    pat_id,
    rst_bed_cd,
...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (100, '
with latest_otc as(
...
DELETE  FROM sys_data_set WHERE sql_cd IN (167);
