...
UNION ALL
  SELECT
    ...
    MIN(imi.timing_cd) AS timing_cd,
    imi.procedure_cd AS procedure_cd,
    MIN(imi.interval_no) AS interval_no,
...
UNION ALL
  SELECT
    ...
    MIN(ti.timing_cd) AS timing_cd,
    ti.procedure_cd AS procedure_cd,
    MIN(ti.interval_no) AS interval_no,
...
CROSS JOIN ini_value
WHERE ind_medi_table.procedure_cd IS NULL 
OR (
  CASE
    -- ▼治療日が A/B の両開始日を満たしている場合（より新しい方を優先）
    WHEN om.treat_date >= mp.in_hosp_a_startdate
     AND om.treat_date >= mp.in_hosp_b_startdate THEN
      CASE
        -- Aの方が新しければA系の施設CDを参照
    ...
    WHEN om.treat_date >= mp.in_hosp_a_startdate THEN
      CASE ini_value.hosp_get_mst_procedure
        WHEN ''1'' THEN mp.in_hospital_cd_a1
        WHEN ''2'' THEN mp.in_hospital_cd_a2
      END
    -- ▼治療日がBの開始日だけを満たしている場合
    WHEN om.treat_date >= mp.in_hosp_b_startdate THEN
      CASE ini_value.hosp_get_mst_procedure
        WHEN ''1'' THEN mp.in_hospital_cd_b1
        WHEN ''2'' THEN mp.in_hospital_cd_b2
      END
...
SELECT
  MAX(pro_medi_table.temp_no) as temp_no,
  MAX(pro_medi_table.medicine_type) as medicine_type,
  MAX(pro_medi_table.interval_no) as interval_no,
  MAX(pro_medi_table.timing_cd) as timing_cd,
...
LEFT JOIN mst_procedure mst
  ON mst.procedure_cd = pro_medi_table.procedure_cd AND mst.facility_cd = @facilityCd
CROSS JOIN ini_value
WHERE
...
, medi_union_2_with_sorted as (
    ...
    from medi_union_2
    LEFT JOIN mst_medi mmd ON mst_cd = mmd.medicine_cd::text
...
SELECT
  ...
  END AS hosp_cd,
  LEAST(TRUNC(amount, 4)::FLOAT8, 99999.9999)::text AS amount,
  unit
FROM
  final_data
WHERE
...
-- SQL: -1103003 end', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'セコム連携 透析実績連携 処置実績ファイル_オーダーインデックス', '2025-07-16 14:50:48.578', current_timestamp, '[{"sql_cd": -1102004, "field_name": "pat_personal_info", "replace_var": "@patPersonalInfo"}]'::jsonb);