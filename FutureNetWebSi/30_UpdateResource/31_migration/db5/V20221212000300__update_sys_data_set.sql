DELETE FROM ntss.sys_data_set WHERE sql_cd IN (1012,1013,1101,1102,1103,1401,1501,-2400);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-2400, 'select
 @pat_id as patid,
 @ord_no as ordno,
...
where
  is_del = ''0''
and pat_id = @pat_id', 3, '[{}]', '0', '{"applications": [4]}', NULL, '定時一括送信機能（患者プロファイル用）', '2020-03-11 18:05:37.059', CURRENT_TIMESTAMP, '[{"sql_cd": -2401, "field_name": "ord_no", "replace_var": "@ord_no"}, {"sql_cd": -2401, "field_name": "pat_id", "replace_var": "@pat_id"}]');
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1012, 'WITH name_info AS ( 
  SELECT
...
SET
  ...
  , pat_sex = TO_NUMBER(''@patSexOut'', ''FM9999999999999999'') 
  ...
  , pat_blood_type_serovar = CASE ''@patBloodTypeSerovar'' 
    ...
    ELSE TO_NUMBER(''@patBloodTypeSerovar'', ''FM9999999999999999'') 
    END
  , in_out_class = TO_NUMBER(''@inOutClassOut'', ''FM9999999999999999'') 
  , is_die = NULLIF(''@isDie'', '''')
  , die_cd = CASE ''@dieCd'' 
    WHEN '''' THEN NULL 
    ELSE TO_NUMBER(''@dieCd'', ''FM99999999999999999999999999999999'') 
...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1102, 'WITH name_info AS ( 
  SELECT
    ''@patLastName'' ::TEXT AS patLastName
    , ''@patFirstName'' ::TEXT AS patFirstName
    , ''@patLastNmKana'' ::TEXT AS patLastNmKana
...
VALUES ( 
  ...
  , CASE ''@patBloodTypeRh'' 
    WHEN '''' THEN 0 
    ELSE TO_NUMBER(''9'', ''FM9999999999999999'') 
    END
  , CASE ''@patBloodTypeSerovar'' 
    WHEN '''' THEN 0 
    ELSE TO_NUMBER(''@patBloodTypeSerovar'', ''FM9999999999999999'') 
  ...
  , NULLIF(''@remoteMonitorUserId'', '''')
  , NULLIF(''@remoteMonitorUserPw'', '''')
)', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報の取得の新規', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, '[{"sql_cd": 1001, "field_name": "dial_diff_com_info", "replace_var": "@dialDiffComInfo"}]');
...
WHERE
  is_del = ''0'' 
  AND hosp_pat_id = ''@hospPatId''  
  AND facility_cd = ''@facilityCd''', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報の取得の修正', '2022-01-07 18:21:46', CURRENT_TIMESTAMP, '[{"sql_cd": 1009, "field_name": "up_base_disease_flg", "replace_var": "@upBaseDiseaseFlg"}]');
...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1501, 'select
  pat_id,
  fn_pat_id,
  hosp_pat_id,
  nkk_pat_id,
