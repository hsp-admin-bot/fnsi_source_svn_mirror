delete from ntss.sys_data_set where sql_cd in (7401,7405, 7406);
...
ORDINALITY WHERE pea1.pat_id = pea2.pat_id AND value->>''item_cd'' = ''@examResultInfo.itemCd'' AND pea2.reg_order_class = ''@regOrderClass'' )::text, ''result''::text], ''"@examResultInfo.result"'') 
 END
WHERE  pat_id = @patId 
  AND facility_cd = ''@facilityCd''
  AND	is_del = ''0''
