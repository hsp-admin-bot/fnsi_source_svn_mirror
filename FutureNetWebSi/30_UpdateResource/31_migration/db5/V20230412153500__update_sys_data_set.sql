DELETE FROM "ntss"."sys_data_set" where sql_cd IN (-2040,-2041,-2050,-2100,-2120,-2130,-2300,-2164, -2110, -2170, -2180, -2190, -2220, -2280, -2250, -2450, -2270, -2420, -2430);
...
WHERE ntss_db5_om.is_del = ''0''
    ...
    AND to_date( @toDate, ''YYYYMMDDHH24MISS'' )
		AND ntss_db5_mst_b.in_hospital_cd_1 IS NOT NULL
    AND ntss_db5_mst_m.machine_serial IS NOT NULL
    AND ntss_db5_mm.occur_date IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2050,'SELECT
    '''' AS hosppatid --患者ID
...
FROM
	...
	INNER JOIN ntss.mnt_motion_record ntss_db5_om_mnt_mr ON ntss_db5_om_mnt_mr.motion_record_no = ntss_db5_om.rst_machine_no
...
WHERE
	...
	'''' AS hosppatid ,
	ntss_db5_om.pat_id AS patid,
	ntss_db5_om.treat_date AS dialysisdate ,
	to_char ( ntss_db5_om.rst_start_date, ''HH24MISS'' ) AS dialysistime ,
CASE

	WHEN ntss_db5_om.rst_dialysis_state IN ( ''0'', ''1'', ''2'' ) THEN
CASE

		WHEN ntss_db5_om.treat_date IS NOT NULL
		AND to_char ( ntss_db5_om.rst_start_date, ''HH24MISS'' ) IS NOT NULL
		AND ntss_db5_om.ind_treat_start_time IS NOT NULL THEN
			to_char ( ( ntss_db5_om.treat_date || '' '' || ntss_db5_om.ind_treat_start_time ) :: TIMESTAMP, ''yyyy-mm-dd hh24:mi:ss'' ) ELSE NULL
		END
			WHEN ntss_db5_om.rst_dialysis_state IN ( ''3'', ''4'', ''5'', ''6'' ) THEN
		CASE

				WHEN ntss_db5_om.rec_set_date IS NOT NULL THEN
				to_char ( ntss_db5_om.rec_set_date, ''YYYY-MM-DD hh24:mi:ss'' ) ELSE NULL
			END
			END AS startplandate ,
		CASE

				WHEN ntss_db5_om.rst_cond_send_date IS NULL THEN
				''0'' ELSE ''1''
			END AS enterflg ,
			to_char ( ntss_db5_om.rst_cond_send_date, ''YYYY-MM-DD hh24:mi:ss'' ) AS enterdate ,
		CASE

				WHEN ntss_db5_om_mnt_mr.machine_record_cd = ''F407''
				AND ntss_db5_om_mnt_mr.event_reg_date IS NULL THEN
					''0'' ELSE ''1''
				END AS machinecheckflg ,
			CASE

					WHEN ntss_db5_om_mnt_mr.machine_record_cd = ''F407'' THEN
					to_char ( ntss_db5_om_mnt_mr.event_reg_date, ''YYYY-MM-DD hh24:mi:ss'' ) ELSE NULL
				END AS machinecheckdate ,
			CASE

					WHEN ntss_db5_om.rst_start_date IS NULL THEN
					''0'' ELSE ''1''
				END AS dialsisstartflg ,
				to_char ( ntss_db5_om.rst_start_date, ''YYYY-MM-DD hh24:mi:ss'' ) AS dialsissstartdate ,
			CASE

					WHEN ntss_db5_om_mnt_mr.machine_record_cd = ''4000''
					AND ntss_db5_om_mnt_mr.event_reg_date IS NULL THEN
						''0'' ELSE ''1''
					END AS offwaterflg ,
				CASE

						WHEN ntss_db5_om_mnt_mr.machine_record_cd = ''4000'' THEN
						to_char ( ntss_db5_om_mnt_mr.event_reg_date, ''YYYY-MM-DD hh24:mi:ss'' ) ELSE NULL
					END AS offwaterdate ,
				CASE

						WHEN ntss_db5_om.rst_end_date IS NULL THEN
						''0'' ELSE ''1''
					END AS wastefluidflg ,
					to_char ( ntss_db5_om.rst_end_date, ''YYYY-MM-DD hh24:mi:ss'' ) AS wastefluiddate ,
				CASE

						WHEN ntss_db5_om.rst_weight_info #>> ''{weight_after_date}'' IS NULL
					THEN
							''0'' ELSE ''1''
						END AS weightafterflg ,
					CASE

							WHEN ntss_db5_om.rst_weight_info #>> ''{weight_after_date}'' is not NULL
						THEN
								to_char (
									(
										ntss_db5_om.rst_weight_info #>> ''{weight_after_date}'')::timestamp, ''yyyy-mm-dd hh24:mi:ss'')
									ELSE NULL
									END AS weightafterdate ,
								CASE

										WHEN ntss_db5_om.rec_set_date IS NULL THEN
										''0'' ELSE ''1''
									END AS recoverybtnflg ,
									to_char ( ntss_db5_om.rec_set_date, ''YYYY-MM-DD hh24:mi:ss'' ) AS recoverybtndate ,
									to_char ( ntss_db5_om.up_date, ''YYYY-MM-DD hh24:mi:ss'' ) AS updates
								FROM
									ntss.ord_main ntss_db5_om
									LEFT JOIN ntss_db5_om_mnt_mr ON ntss_db5_om_mnt_mr.ord_no = ntss_db5_om.ord_no
								WHERE
...
WHERE
		ntss_db5_mst_b.in_hospital_cd_1 IS NOT NULL
    AND ntss_db5_mst_m.machine_serial IS NOT NULL
    AND ntss_db5_mm.occur_date IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2180,'with ntss_db5_om_1 as (
        ...
        WHERE ntss_db5_os.treat_date BETWEEN SUBSTR(@fromDate,0,9) AND SUBSTR(@toDate,0,9)
						...
						AND ntss_db5_om_mst_k.kur_cd IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2270,'SELECT
	'''' AS hosppatid --患者ID
...
WHERE
	...
	AND ntss_db5_mst_e.in_hospital_cd1 IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2120,'SELECT
    '''' AS hosppatid                             --患者ID
    , ntss_db5_om.pat_id AS patid
    , ntss_db5_os.treat_date AS dialysisdate    --透析日
  ...
  , (-2100,'WITH ntss_db5_om_key AS(
                ...
                ,
                        CASE
                                ...
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''9'' THEN ''999'' --穿刺針(A針)
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''10'' THEN ''998'' --穿刺針(V針)
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''11'' THEN ''997'' --穿刺針(SN)
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''12'' THEN ''029'' --シングルニードル使用
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''13'' THEN ''996'' --血液回路
                        ...
                        CASE
                                ...
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''9'' THEN ''穿刺針(A針)'' --穿刺針(A針)
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''10'' THEN ''穿刺針(V針)'' --穿刺針(V針)
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''11'' THEN ''穿刺針(SN)'' --穿刺針(SN)
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''12'' THEN ''シングルニードル使用'' --シングルニードル使用
                                WHEN CAST(ntss_db5_om_key.keys AS CHAR(10))= ''13'' THEN ''血液回路'' --血液回路
...
WHERE
    ...
    AND ntss_db5_om.pat_id IS NOT NULL
		AND ntss_db5_mst_list.fnw_code IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid,userid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2170,'with ntss_db5_om_1 as (
        ...
        WHERE
            ...
            AND ntss_db5_os.treat_date BETWEEN SUBSTR(@fromDate,0,9) AND SUBSTR(@toDate,0,9)
						...
						AND ntss_db5_os.treat_date IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2280,'SELECT
	'''' AS hosppatid --患者ID
...
WHERE
  ...
  , (-2250,'WITH ntss_db5_om_rti_json AS (
    SELECT
        ntss_db5_om_rti_json ::json ->> ''occur_date'' AS occur_date
        , ntss_db5_om_rti_json ::json ->> ''medicine_type'' AS medicine_type
        , ntss_db5_om_rti_json ::json ->> ''treat_name'' AS treat_name
        , ntss_db5_om_rti_json ::json ->> ''medicine_name'' AS medicine_name
        , ntss_db5_om_rti_json ::json ->> ''amount'' AS amount
        , ntss_db5_om_rti_json ::json ->> ''unit'' AS unit
        , ntss_db5_om_rti_json ::json ->> ''procedure_name'' AS procedure_name
        , ntss_db5_om_rti_json ::json ->> ''treat_medicine_cd'' AS treat_medicine_cd
        , ntss_db5_om_rti_json ::json ->> ''procedure_cd'' AS procedure_cd
        , ntss_db5_om_rti_json ::json ->> ''ctl_no'' AS ctl_no
        , om.ord_no
    FROM
        ord_main om
        cross join lateral json_array_elements(om.rst_treatment_info ::json) ntss_db5_om_rti_json
    WHERE
        om.is_del = ''0''
        AND om.facility_cd = @facilityCd
        AND om.up_date BETWEEN to_date(@fromDate, ''YYYYMMDDHH24MISS'') AND to_date(@toDate, ''YYYYMMDDHH24MISS'')
        AND om.pat_id = ''43851''
), ntss_db5_om_rci_json AS (
    SELECT
        ntss_db5_om_rci_json ->> ''occur_date'' AS occur_date
        , ntss_db5_om_rci_json ->> ''comp_cd'' AS comp_cd
        , ntss_db5_om_rci_json ->> ''complaint'' AS complaint
        , ntss_db5_om_rci_json ->> ''medicine_type'' AS medicine_type
        , ntss_db5_om_rci_json ->> ''ctl_no'' AS ctl_no
        , om.ord_no
    FROM
        ord_main om
        cross join lateral json_array_elements(om.rst_complaint_info ::json) ntss_db5_om_rci_json
    WHERE
        om.is_del = ''0''
        AND om.facility_cd = @facilityCd
        AND om.up_date BETWEEN to_date(@fromDate, ''YYYYMMDDHH24MISS'') AND to_date(@toDate, ''YYYYMMDDHH24MISS'')
        AND om.pat_id = ''43851''
), ntss_db5_om_tsi_json AS (
    SELECT
        SUBSTR(ntss_db5_om_tsi_json ->> ''treat_staff_name'', 0, 10) AS treat_staff_name
        , ntss_db5_om_tsi_json ->> ''ctl_no'' AS ctl_no
        , om.ord_no AS ord_no
    FROM
        ord_main om
        cross join lateral json_array_elements(om.rst_treat_staff_info ::json) ntss_db5_om_tsi_json
    WHERE
        om.is_del = ''0''
        AND om.facility_cd = @facilityCd
        AND om.up_date BETWEEN to_date(@fromDate, ''YYYYMMDDHH24MISS'') AND to_date(@toDate, ''YYYYMMDDHH24MISS'')
...
SELECT
        rti.occur_date AS occur_date_rti
        , rti.medicine_type AS medicine_type_rti
        , rti.treat_name AS treat_name
        , rti.medicine_name AS medicine_name
        , rti.amount AS amount
        , rti.unit AS unit
        , rti.procedure_name AS procedure_name
        , rti.treat_medicine_cd AS treat_medicine_cd
        , rti.procedure_cd AS procedure_cd
        , rti.ctl_no AS ctl_no
        , rci.occur_date AS occur_date_rci
        , rci.comp_cd AS comp_cd
        , rci.complaint AS complaint
        , rci.medicine_type AS medicine_type_rci
        , tsi.treat_staff_name AS treat_staff_name
        , rti.ord_no
FROM
    ntss_db5_om_rti_json rti
    INNER JOIN ntss_db5_om_tsi_json tsi
        ON rti.ctl_no = tsi.ctl_no
        AND rti.ord_no = tsi.ord_no
    INNER JOIN ntss_db5_om_rci_json rci
        ON rti.ctl_no = rci.ctl_no
        AND rti.ord_no = rci.ord_no
)
SELECT
	'''' AS hosppatid --患者ID
	,ntss_db5_om.pat_id AS patid
	,COALESCE(to_char(to_timestamp(ntss_json_list.occur_date_rci, ''YYYY-MM-DD hh24:mi:ss''), ''YYYY-MM-DD hh24:mi:ss''),
			to_char(to_timestamp(ntss_json_list.occur_date_rti, ''YYYY-MM-DD hh24:mi:ss''), ''YYYY-MM-DD hh24:mi:ss'')) AS occurdate --発生日時
	,CASE
		WHEN ntss_json_list.medicine_type_rti = ''2'' THEN ''0''
		WHEN ntss_json_list.medicine_type_rti = ''1'' THEN ''1''
		WHEN ntss_json_list.medicine_type_rti IS NULL THEN ''2''
		WHEN ntss_json_list.medicine_type_rti = ''3'' THEN ''3''
	 END AS measureclass --区分
	,ntss_json_list.comp_cd AS reqcode --愁訴コード
	,ntss_json_list.complaint AS complaint --愁訴内容
	,ntss_json_list.treat_name AS treatname --処置名
	,CASE
		WHEN ntss_json_list.medicine_type_rci = ''2'' THEN ntss_db5_mst_mm.in_hospital_cd_1
		WHEN ntss_json_list.medicine_type_rti = ''1'' THEN ntss_db5_mst_m.in_hospital_cd_1
	 END AS medicinecd1 --薬剤コード1
	 ,CASE
		WHEN ntss_json_list.medicine_type_rci = ''2'' THEN ntss_db5_mst_mm.in_hospital_cd_2
		WHEN ntss_json_list.medicine_type_rti = ''1'' THEN ntss_db5_mst_m.in_hospital_cd_2
	 END AS medicinecd2 --薬剤コード2
	 ,ntss_json_list.medicine_name AS medicinename --薬剤名称
	 ,ntss_json_list.amount AS amount --数量
	 ,ntss_json_list.unit AS unit --単位
	 ,ntss_json_list.procedure_name AS procedurename --手技名
	 ,ntss_db5_mst_p.in_hospital_cd_a1 AS procedurecd1 --手技コード1
	 ,ntss_db5_mst_p.in_hospital_cd_a2 AS procedurecd2 --手技コード2
	 ,ntss_json_list.treat_staff_name AS treatpersonname --処置者名
	 ,to_char(ntss_db5_om.up_date, ''YYYY-MM-DD hh24:mi:ss'') AS updates --更新日時
FROM
	ord_main ntss_db5_om
    LEFT JOIN ntss_json_list
    ON ntss_json_list.ord_no = ntss_db5_om.ord_no
	LEFT JOIN mst_medicine_mix ntss_db5_mst_mm
	ON cast(ntss_db5_mst_mm.medicine_mix_cd AS char(4)) = ntss_json_list.treat_medicine_cd
	AND ntss_db5_mst_mm.facility_cd = @facilityCd
	AND ntss_db5_mst_mm.up_date BETWEEN to_date( @fromDate, ''YYYYMMDDHH24MISS'' )
	AND to_date( @toDate, ''YYYYMMDDHH24MISS'' )
	LEFT JOIN mst_medicine ntss_db5_mst_m
	ON cast(ntss_db5_mst_m.medicine_cd AS char(4)) = ntss_json_list.treat_medicine_cd
	AND ntss_db5_mst_m.facility_cd = @facilityCd
	AND ntss_db5_mst_m.up_date BETWEEN to_date( @fromDate, ''YYYYMMDDHH24MISS'' )
	AND to_date( @toDate, ''YYYYMMDDHH24MISS'' )
	LEFT JOIN  mst_procedure ntss_db5_mst_p
	ON cast(ntss_db5_mst_p.procedure_cd AS char(4)) = ntss_json_list.procedure_cd
...
WHERE ntss_db5_om.is_del = ''0''
	...
	AND ntss_db5_om.up_date BETWEEN to_date( @fromDate, ''YYYYMMDDHH24MISS'' )
	AND to_date( @toDate, ''YYYYMMDDHH24MISS'' )
    AND ntss_json_list.occur_date_rci IS NOT NULL
    AND ntss_json_list.occur_date_rti IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null);
