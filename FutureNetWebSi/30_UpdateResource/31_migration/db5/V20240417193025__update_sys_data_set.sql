...
, type_1_f5 AS (
    SELECT
        ...
        , SUBSTR(op_pd_json ->> ''Rp'', 3) AS ctlno
        , op_pd_json ->> ''F5'' AS f5
    FROM
        ord_prescription op
        CROSS JOIN LATERAL json_array_elements(op.prescription_detail ::json) op_pd_json
    WHERE
        op.is_del = ''0''
        AND op.facility_cd = @facilityCd
        AND op_pd_json ->> ''type'' = ''1''
        AND @fromDate <= op.issue_date AND op.issue_date < @toDate
)
SELECT
    '''' AS hosppatid                             --患者ID
    ...
    , SUBSTR(ntss_db5_op_pd_json ->> ''Rp'', 3) AS ctlno --項目番号
    , CASE
    ...
    , CASE
        WHEN ntss_db5_op_pd_json ->> ''type'' = ''1''
            THEN ntss_db5_op_pd_json ->> ''F5''
        WHEN ntss_db5_op_pd_json ->> ''type'' IN (''2'', ''4'', ''5'')
            THEN CAST(CAST(type_1_f5.f5 AS numeric) * CAST(ntss_db5_op_pd_json ->> ''F5'' AS numeric) AS text)
        WHEN ntss_db5_op_pd_json ->> ''type'' = ''3''
            THEN type_1_f5.f5
        END AS dosage                           --用量
    ...
    , CAST(ntss_db5_op.expiration_date AS DATE) - CAST(ntss_db5_op.issue_date AS DATE) AS daycount --調剤日数
...
FROM
    ...
    LEFT JOIN type_1_f5
        ON type_1_f5.ord_prescription_no = ntss_db5_op.ord_prescription_no
        AND type_1_f5.ctlno = SUBSTR(ntss_db5_op_pd_json ->> ''Rp'', 3)
WHERE
