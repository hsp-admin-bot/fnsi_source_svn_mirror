delete from "sys_data_set" where "sql_cd" in (1102,1103);
...
pat_last_name =  case when split_part( ''@patLastName'', ''　'', 2 ) is null or split_part( ''@patLastName'', ''　'', 2 ) = '''' then 
    personal_info_encrypt(split_part( ''@patLastName'', '' '', 1 ))
  else 
    personal_info_encrypt(split_part( ''@patLastName'', ''　'', 1 ))
  end,
pat_first_name = COALESCE(NULLIF(case when split_part( ''@patFirstName'', ''　'', 2 ) is null or split_part( ''@patFirstName'', ''　'', 2 ) = '''' then 
...
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1102, 'insert into pat_personal_main (
  ...
  NULLIF(''@nkkPatId'',''''),
  NULLIF(''@facilityCd'',''''),
  case when split_part( ''@patLastName'', ''　'', 2 ) is null or split_part( ''@patLastName'', ''　'', 2 ) = '''' then 
    personal_info_encrypt(split_part( ''@patLastName'', '' '', 1 ))
  else 
    personal_info_encrypt(split_part( ''@patLastName'', ''　'', 1 ))
  end,
  case when split_part( ''@patFirstName'', ''　'', 2 ) is null or split_part( ''@patFirstName'', ''　'', 2 ) = '''' then 
    personal_info_encrypt(split_part( ''@patFirstName'', '' '', 2 ))
  else 
    personal_info_encrypt(split_part( ''@patFirstName'', ''　'', 2 ))
  end,
