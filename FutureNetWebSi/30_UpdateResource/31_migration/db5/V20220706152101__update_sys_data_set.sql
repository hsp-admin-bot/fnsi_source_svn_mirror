DELETE FROM "ntss"."sys_data_set" where "sql_cd" IN (1705,1716);
...
WHERE
  ...
  AND facility_cd = ''@facilityCd'' 
  AND is_del = ''0'' 
	AND (SELECT is_die FROM isDie) <> 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡以外)', '2022-06-22 08:26:30.149', CURRENT_TIMESTAMP, NULL);
...
WHERE
  ...
  AND (SELECT exists_flag FROM data_exists_info) = ''0'' 
	AND (SELECT is_die FROM isDie) <> 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡以外)', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, NULL);
