DELETE FROM "ntss"."sys_data_set" where "sql_cd" in (-498);INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-498, 'WITH do_order_data_from AS (
...
FROM
  (
    SELECT--血液回路情報
    ''血液回路'' AS detail_id,
		case when meq.in_hospital_cd_1 is null then meq.equipment_cd::text else meq.in_hospital_cd_1 end AS e01,
  ...
  UNION
    SELECT--ダイアライザ
    ''ダイアライザ'' AS detail_id,
		case when mdz.in_hospital_cd_1 is null then mdz.dialyzer_cd::text else mdz.in_hospital_cd_1 end AS e01,
  ...
  UNION
    SELECT--医材情報
    ''医材'' AS detail_id,
		case when meq.in_hospital_cd_1 is null then meq.dialyzer_cd::text else meq.in_hospital_cd_1 end AS e01,
