  ...
  SELECT
     ...
     ''VE1'' AS sbt_key,
	    CASE (SELECT bed_conv FROM bed_conv)
					WHEN 1 THEN
						( ''V'' || lpad( mbd.in_hospital_cd_1 :: TEXT, 7, ''0'' ) )
					WHEN 2 then 
						( ''V'' || lpad( mbd.in_hospital_cd_2 :: TEXT, 7, ''0'' ) )
					ELSE
						( ''V'' || lpad( mbd.in_hospital_cd_1 :: TEXT, 7, ''0'' ) )
				END AS e01,--項目コード
          ...
          CASE
            ...
            WHEN ord.rst_in_out_class = ''0''
              THEN 
                CASE
                  --名称が一致する設定が存在しない場合
                  WHEN (SELECT value FROM conv_teart_item_send_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0) = '''' 
                    OR (SELECT value FROM conv_teart_item_send_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0) IS NULL 
                    THEN
                      --治療項目マスタ．院内コードをセットする
                      CASE
                        --患者の入外区分が外来の場合
                        ...
                        ELSE ''''
                      END
                  ELSE (SELECT value FROM conv_teart_item_send_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0)
                END  
            --[透析実績履歴．入外区分]が入院の場合
            WHEN ord.rst_in_out_class = ''1''
              THEN 
                CASE
                  --名称が一致する設定が存在しない場合
                  WHEN (SELECT value FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0) = '''' 
                    OR (SELECT value FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0) IS NULL
                    THEN
                      --治療項目マスタ．院内コードをセットする
                      CASE
                        --患者の入外区分が外来の場合
                        ...
                        ELSE ''''
                      END
                  ELSE (SELECT value FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0)
                END
            ELSE ''''
          END
      ELSE ''''
          ...
          CASE
            ...
            WHEN ord.rst_in_out_class = ''0''
              THEN 
                CASE
                  --名称が一致する設定が存在しない場合
                  WHEN (SELECT value FROM conv_teart_item_send_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1) = '''' 
                    OR (SELECT value FROM conv_teart_item_send_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1) IS NULL 
                    --治療項目マスタ．治療方法名称をセットする
                    THEN COALESCE(mtt.treatment_name, '''')
                  ELSE (SELECT value FROM conv_teart_item_send_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1)
                END  
            --[透析実績履歴．入外区分]が入院の場合
            WHEN ord.rst_in_out_class = ''1''
              THEN 
                CASE
                  --名称が一致する設定が存在しない場合
                  WHEN (SELECT value FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1) = '''' 
                    OR (SELECT value FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1) IS NULL
                    --治療項目マスタ．治療方法名称をセットする
                    THEN COALESCE(mtt.treatment_name, '''')
                  ELSE (SELECT value FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1)
                END
...
WHERE
  all_cost.sortTag IS NOT NULL AND all_cost.sortTag <> ''''
order by all_cost.sortTag', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）実績繰り返し部', '2020-04-24 19:15:25.001',CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}]');
