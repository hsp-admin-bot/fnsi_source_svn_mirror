...
FROM (
	...
	, null as medi_timing
	, null as medi_proc
	, concat(cond_dl_am, CASE WHEN om.medicine_type = ''1'' THEN md.unit_second ELSE mdx.unit END) as num_unit
	, null as cond_va_dir
	, null as cond_va
	, null as equip_pnc_cls
	...
	, null as medi_timing
	, null as medi_proc
	, om.ind_cond_info::json#>>''{22,value}'' || CASE WHEN om.medicine_type = ''1'' THEN COALESCE(md.unit_second,'''') ELSE COALESCE(mdx.unit,'''') END as num_unit
	, null as cond_va_dir
	, null as cond_va
	, null as equip_pnc_cls
	...
	--抗凝固剤(調製薬剤)
				SELECT 10 as disp_order, om.treat_date, kr.kur_cd, kr.kur_name, COALESCE(bd.bed_name,''未登録'') as bed_name,om.ind_bed_cd AS bed_cd, om.pat_id
	, COALESCE(mdc.class_name,'''') as kind, mmx.medicine_mix_name AS Name
  , (TO_NUMBER( om.ind_cond_info :: json #>> ''{26,value}'', ''99999999.99'' ) + TO_NUMBER( om.ind_cond_info :: json #>> ''{28,value}'', ''99999999.99'' ) )
    ...
    , null as function_class, null as area, null as ufr, null as koa, null as material, null as wetdry
	, ''AntiCoagulan'' as class_name, null as Anticoagulant_name, om.ord_no, mmx.in_hospital_cd_1, mmx.in_hospital_cd_2
	, null AS equip_circuit
	, TO_NUMBER(om.ind_cond_info::json#>>''{26,value}'',''99999999.99'') || om.unit as cond_ac_shot
	, TO_NUMBER(om.ind_cond_info::json#>>''{27,value}'',''99999999.99'') || CASE WHEN om.unit IS NULL THEN '''' ELSE om.unit || ''/h'' END as cond_ac_spd
	, TO_NUMBER(om.ind_cond_info::json#>>''{28,value}'',''99999999.99'') || om.unit as cond_ac_dur_total
	...
	, null as equip_pnc_cls
							FROM
								...
								LEFT OUTER JOIN mst_medicine_class mdc ON mmx.class_cd=mdc.class_cd and mdc.is_del = ''0'' and mdc.is_disp = ''1''
              	LEFT OUTER JOIN mst_kur kr ON om.ind_kur_cd=kr.kur_cd and kr.is_del = ''0''
	              LEFT OUTER JOIN mst_bed bd ON om.ind_bed_cd=bd.bed_cd and bd.is_del = ''0'' and bd.is_disp = ''1''
							WHERE
								om.ord_no IN ( @ordNos )
								AND om.ind_cond_info :: json #>> ''{25,value}'' IS NOT NULL
								AND mmx.class_cd IN ( @medIds )
	UNION ALL
	--投薬(薬剤)
	...
	--投薬(調製薬剤)
	SELECT 11 as disp_order, om.treat_date, kr.kur_cd, kr.kur_name, COALESCE(bd.bed_name,''未登録'') as bed_name,om.ind_bed_cd AS bed_cd,om.pat_id
	, COALESCE(mdc.class_name,'''') as kind,mdx.medicine_mix_name as Name
	, to_number(om.amount, ''9999999999.999'') as Amount
	, COALESCE(mdx.unit,'''') AS Unit
    , null as function_class, null as area, null as ufr, null as koa, null as material, null as wetdry, ''Medicine'' as class_name, null as Anticoagulant_name, om.ord_no, mdx.in_hospital_cd_1, mdx.in_hospital_cd_2
	, null AS equip_circuit
	, null as cond_ac_shot
	, null as cond_ac_spd
	, null as cond_ac_dur_total
	...
	FROM ord_medi as om
    LEFT OUTER JOIN (
    SELECT medicine_mix_cd, info->>''cd'' as cd, info->>''amount'' as amount, unit as unit, medicine_mix_name, in_hospital_cd_1, in_hospital_cd_2
	...
	LEFT OUTER JOIN mst_medicine_class mdc ON to_number(mdx.cd,''9999999999'')=mdc.class_cd and mdc.is_del = ''0'' and mdc.is_disp = ''1''
	LEFT OUTER JOIN mst_kur kr ON om.ind_kur_cd=kr.kur_cd and kr.is_del = ''0''
	LEFT OUTER JOIN mst_bed bd ON om.ind_bed_cd=bd.bed_cd and bd.is_del = ''0'' and bd.is_disp = ''1''
	LEFT OUTER JOIN mst_medicate_timing mt ON to_number(medi->>''timing_cd'', ''9999999999'') = mt.medicate_timing_cd and mt.is_del = ''0'' and mt.is_disp = ''1''
	LEFT OUTER JOIN mst_procedure mp ON to_number(medi->>''procedure_cd'', ''9999999999'') = mp.procedure_cd and mp.is_del = ''0'' and mp.is_disp = ''1''
