...
xml_dispose AS (
  SELECT
    ...
    (xpath(''@DISPOSE_CTL_NO'', dispose_info.dispose_node))[1]::text as xml_cost_no,
    (xpath(''TENCD/text()'', dispose_info.dispose_node))[1]::text as xml_tencd,
    (xpath(''TKJNAM/text()'', dispose_info.dispose_node))[1]::text as xml_tkjnam,
    (xpath(''AMOUNT/text()'', dispose_info.dispose_node))[1]::text as xml_amount,
    (xpath(''UNIT/text()'', dispose_info.dispose_node))[1]::text as xml_unit
  FROM xml_base
  CROSS JOIN LATERAL (
    SELECT
      dispose_node,
...
xml_equip AS (
  SELECT
    ...
    xml_base.is_xml_valid,
    equip_info.equip_no,
    (xpath(''EQUIP_INFO/@CTL_NO'', equip_info.equip_node))[1]::text as xml_cost_no,
    (xpath(''EQUIP_INFO/EQUIP_CD/text()'', equip_info.equip_node))[1]::text as xml_equip_cd,
    (xpath(''EQUIP_INFO/EQUIP_NAME/text()'', equip_info.equip_node))[1]::text as xml_equip_name,
    (xpath(''EQUIP_INFO/EQUIP_CLASS_NAME/text()'', equip_info.equip_node))[1]::text as xml_equip_class_name,
    (xpath(''EQUIP_INFO/PUNCTURE_CLASS/text()'', equip_info.equip_node))[1]::text as xml_puncture_class,
    (xpath(''EQUIP_INFO/AMOUNT/text()'', equip_info.equip_node))[1]::text as xml_amount,
    (xpath(''EQUIP_INFO/UNIT/text()'', equip_info.equip_node))[1]::text as xml_unit
  FROM xml_base
  CROSS JOIN LATERAL (
    SELECT
      equip_node,
      row_number() OVER () AS equip_no
    FROM unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO'', xmlparse(document xml_base.xml_text))) AS equip_node
  ) AS equip_info
  WHERE xml_base.xml_text IS NOT NULL AND xml_base.xml_text != ''''
),
...
WHERE
  ...
  OR
  -- フォールバック：XMLが無効な場合はcontentデータのみ
  (xml_base.is_xml_valid = false AND content_equip.row_order IS NOT NULL)
ORDER BY
  CASE
    WHEN COALESCE(xml_base.is_xml_valid, false) = true THEN
      xml_equip.equip_no
    ELSE
      content_equip.row_order
  END;', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'SSI 実績送信', '2025-09-13 19:58:07.067', '2025-09-13 19:58:07.068', '[{"sql_cd": -501102, "field_name": ["detail_id", "e01", "e02", "e03", "e04", "e05", "e06", "cost_no"], "replace_var": "content"}]'::jsonb);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-504006, '-- =====================================================
...
LEFT JOIN content_medicine cm ON true
WHERE
  -- XMLデータ優先：XMLが有効な場合はXMLデータのみ
