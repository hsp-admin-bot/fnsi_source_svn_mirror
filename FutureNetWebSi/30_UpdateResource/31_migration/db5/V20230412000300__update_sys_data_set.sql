DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (7409, 9208);
...
infect_tbl AS (
  SELECT
	  CASE WHEN EXISTS (SELECT tmpVal1 FROM mstSet1_tbl WHERE tmpVal1 = ''@exam_result_info.result'') THEN
		  ''2''
		ELSE 
		  CASE WHEN EXISTS (SELECT tmpVal2 FROM mstSet2_tbl WHERE tmpVal2 = ''@exam_result_info.result'') THEN
		    ''1''
		  ELSE
			  ''0''
			END
...
WHERE
  facility_cd = ''@facilityCd'' 
	AND pat_id = @patId', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の検査結果(検査結果情報更新)', '2023-04-10 23:29:43.109', CURRENT_TIMESTAMP, NULL);
