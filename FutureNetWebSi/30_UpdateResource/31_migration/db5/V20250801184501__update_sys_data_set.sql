...
SELECT
  ...
  CASE
    ...
    ELSE
      CASE t1.medi_info ->> ''medicine_type''
        WHEN ''1'' THEN ROUND((medi_info ->> ''amount'')::NUMERIC, 4)
        WHEN ''2'' THEN
          CASE mst_mix.solvent
            WHEN ''0'' THEN
              ROUND((medi_info ->> ''amount'')::NUMERIC * mst_mix.amount::NUMERIC, 4)
            WHEN ''1'' THEN
              ROUND(mst_mix.amount::NUMERIC, 4)
          END
        ELSE 0
      END
  END AS amount,
...
SELECT
  ...
  CASE
    ...
    ELSE
      CASE t1.tre_info ->> ''medicine_type''
        WHEN ''1'' THEN ROUND((t1.tre_info ->> ''amount'')::NUMERIC, 4)
        WHEN ''2'' THEN
          CASE mst_mix.solvent
            WHEN ''0'' THEN
              ROUND((t1.tre_info ->> ''amount'')::NUMERIC * mst_mix.amount::NUMERIC, 4)
            WHEN ''1'' THEN
              ROUND(mst_mix.amount::NUMERIC, 4)
          END
        ELSE 0
...
SELECT
  ...
  END AS hosp_cd,
  ROUND(amount, 4)::FLOAT8::TEXT AS amount,
  unit
FROM
  final_data
WHERE
