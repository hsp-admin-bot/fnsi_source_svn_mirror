...
WHERE sql_cd IN (8104, -500094, -500100, -500102);
...
input_data AS (
  SELECT
    ...
    CASE
      WHEN (SELECT device_mode FROM device_mode) IN (''0'',''1'')
      THEN NULL
      ELSE CASE
             WHEN (SELECT is_new FROM is_new)
             THEN COALESCE(
    ...
    END                                             AS indCondInfo019,
     ...
     -- 020 補液量
    CASE
        WHEN (SELECT device_mode FROM device_mode) IN (''0'',''1'',''10'')
        THEN ''0.0''
        ELSE TO_CHAR(
                 LEAST(
                     (SELECT max_liters
                     ...
                     CASE
                       ...
                       THEN (SELECT replenisher_amount::NUMERIC
                                     FROM calc_value )
    ...
    -- 022 補液使用数
   CASE
       WHEN (SELECT device_mode FROM device_mode) IN (''0'',''1'',''10'') THEN ''0''
   ...
   END AS indCondInfo022,
    ...
    TO_CHAR(
     ...
     -- 031 IP ワンショット量 [0–20] g
    COALESCE(
      TO_CHAR(
            COALESCE(
              NULLIF(''@ind_off_water_info.ind_off_water_info.weight_5'','''')::NUMERIC / 10,
              0
            ),
        ''FM9999.0''
      ),
...
WHERE
  ord_no = @ordNo', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)SSIのオーダ受け(指示：治療条件情報 の更新)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -500013, "field_name": "cd", "replace_var": "@indCondInfo.005.value"}, {"sql_cd": -500025, "field_name": "equipment_cd", "replace_var": "@indCondInfo.008.value"}, {"sql_cd": -500015, "field_name": "equipment_cd", "replace_var": "@indCondInfo.010.value"}, {"sql_cd": -500017, "field_name": "medicine_cd", "replace_var": "@indCondInfo.015.value"}, {"sql_cd": -500027, "field_name": "equipment_cd", "replace_var": "@indCondInfo.006.value"}, {"sql_cd": -500037, "field_name": "equipment_cd", "replace_var": "@indCondInfo.011.value"}, {"sql_cd": -500029, "field_name": "medicine_cd", "replace_var": "@indCondInfo.019.value"}, {"sql_cd": -500035, "field_name": "medicine_cd", "replace_var": "@indCondInfo.025.value"}]'::jsonb);
INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-500094, 'WITH pat_limit AS (
    SELECT
        (device_set_info#>>''{"ope","dev","A","179"}'')::NUMERIC AS blood_flow_limit
    FROM
        pat_main
    WHERE
        pat_id = @patId
...
WHERE
  ...
  (
    ...
    AND (SELECT blood_flow_limit FROM pat_limit) >= (NULLIF(@indCondInfo.014.value, ''''))::int
  );', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)SSIの入力値チェック(血流量)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-500100, 'WITH pat_limit AS (
    SELECT
        (device_set_info#>>''{"ope","dev","A","182"}'')::NUMERIC AS temperature_upper_limit,
        (device_set_info#>>''{"ope","dev","A","183"}'')::NUMERIC AS temperature_lower_limit
    FROM
        pat_main
    WHERE
        pat_id = @patId
...
WHERE
  ...
  OR
  (
    (SELECT temperature_lower_limit FROM pat_limit) <= (COALESCE(NULLIF(@indCondInfo.018.value, '''')::NUMERIC, 0) / 10)
    AND (SELECT temperature_upper_limit FROM pat_limit) >= (COALESCE(NULLIF(@indCondInfo.018.value, '''')::NUMERIC, 0) / 10)
  );', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)SSIの入力値チェック(透析液温度)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-500102, 'WITH ssi_order_treat_set_name AS ( 
  SELECT
    info->>''key2'' AS key2
    , COALESCE(NULLIF(info->>''value'', ''''), info->>''default_v'') AS VALUE 
  FROM
...
FROM (
  SELECT
    ind_cond_info->''21''->>''value'' AS fr_ba,
    0 AS is_ssi_order_treat_set_name,
    0 AS sort_order
  FROM
    mst_treatment_set
  ...
  UNION ALL
  SELECT
    mst_treatment_set.ind_cond_info->''21''->>''value'' AS fr_ba,
    1 AS is_ssi_order_treat_set_name,
    treatment_set_order.sort_order
  FROM
    mst_treatment_set
...
ssi_order_recv_info AS ( 
  ...
  WHERE
    ...
    AND COALESCE(info->>''key0'','''') = @key0
    AND info->>''key1'' = ''SSI_ORDER_RECV''
    AND info->>''key2'' IN (''IN_HOSPITAL_CD'', ''SUPLIQUID_BEFORE_CD'', ''SUPLIQUID_AFTER_CD'', ''USE_SUPLIQUID'')
),
ind_cond_info021 AS (
  SELECT
    CASE
      WHEN (SELECT VALUE FROM ssi_order_recv_info WHERE key2 = ''USE_SUPLIQUID'') = ''1''
      THEN CASE
        WHEN LTRIM(@ind_off_water_info.ind_off_water_info.weight_4, ''0'')
             = LTRIM((SELECT VALUE FROM ssi_order_recv_info WHERE key2 = ''SUPLIQUID_AFTER_CD''), ''0'')
        THEN ''0''
        WHEN LTRIM(@ind_off_water_info.ind_off_water_info.weight_4, ''0'')
             = LTRIM((SELECT VALUE FROM ssi_order_recv_info WHERE key2 = ''SUPLIQUID_BEFORE_CD''), ''0'')
        THEN ''1''
        ELSE (SELECT fr_ba FROM fr_ba_tbl)
      END
      ELSE (SELECT fr_ba FROM fr_ba_tbl)
    END                                             AS indCondInfo021
),
...
max_upper_limit_info AS ( 
    ...
    CASE 
      ...
      ELSE
        CASE 
          ...
          WHEN device_mode in (8) THEN 
            COALESCE((SELECT pat_max_limit_ohf_a FROM pat_limit), 999.0)
          ELSE
            999.0
        END
  ...
  where
    ...
    and facility_cd = @facilityCd
    and ((
        CASE (SELECT VALUE FROM ssi_order_recv_info WHERE key2 = ''IN_HOSPITAL_CD'')
          WHEN ''1'' THEN in_hospital_cd_a1 = (SELECT VALUE FROM ssi_order_treat_info) AND COALESCE(in_hospital_cd_a1,'''') <> ''''
          WHEN ''2'' THEN in_hospital_cd_a2 = (SELECT VALUE FROM ssi_order_treat_info) AND COALESCE(in_hospital_cd_a2,'''') <> ''''
          WHEN ''3'' THEN in_hospital_cd_a3 = (SELECT VALUE FROM ssi_order_treat_info) AND COALESCE(in_hospital_cd_a3,'''') <> ''''
          WHEN ''4'' THEN in_hospital_cd_a4 = (SELECT VALUE FROM ssi_order_treat_info) AND COALESCE(in_hospital_cd_a4,'''') <> ''''
...
SELECT
  1 AS flg
WHERE
  (SELECT replenisher_speed FROM calc_value) > (SELECT max_upper_limit FROM max_upper_limit_info)', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)SSIの入力値チェック(補液速度)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -500011, "field_name": "ind_treatment_cd", "replace_var": "@indTreatmentCd"}]'::jsonb);