...
select
  case when ord.ind_bed_cd is null then ''9999999''
  else 
    case when (select bed_code_kbn from bed_code_info) = ''1''
    then COALESCE(bed_tbl.in_hospital_cd_1, ''9999999'')
		when (select bed_code_kbn from bed_code_info) = ''2''
		then COALESCE(bed_tbl.in_hospital_cd_2, ''9999999'')
    else ''9999999''
    end
