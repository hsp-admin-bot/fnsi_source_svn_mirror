DELETE FROM sys_data_set WHERE sql_cd in (1703, 1714);
...
WHERE
  ...
  AND facility_cd = ''@facilityCd'' 
  AND is_del = ''0''
  AND (SELECT exists_flag FROM data_exists_info) = ''0'' ', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_身体情報', '2020-05-25 18:21:40.841', '2022-03-25 16:11:22.636', NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1714, 'WITH exam_date_tmp AS ( 
  SELECT 
    COALESCE(NULLIF(''@physicalInfo.examDate'', ''''), ''@physicalInfo.examDate_Date'') AS exam_date
) 
, exam_date_info AS ( 
  SELECT
    CASE WHEN NULLIF(exam_date, '''') IS NULL THEN TO_CHAR(CURRENT_TIMESTAMP, ''YYYY-MM-DD'') ELSE exam_date END AS exam_date
...
, data_new_info AS (
  SELECT 
    ...
    NULL AS ctl_no,
    TRIM(NULLIF(''@physicalInfo.height'', ''''), ''0'') AS height,
    TRIM(NULLIF(''@physicalInfo.chestDia'', ''''), ''0'') AS chest_dia,
    (SELECT exam_date FROM exam_date_info) AS exam_date,
    TRIM(NULLIF(''@physicalInfo.breastDia'', ''''), ''0'') AS breast_dia,
    TRIM(NULLIF(''@physicalInfo.ctrWeight'', ''''), ''0'') AS ctr_weight,
    NULLIF(''@facilityCd'', '''') AS facility_cd,
    COALESCE(NULLIF((SELECT order_class FROM order_class_info), ''0''), ''3'') AS order_class,
    (SELECT indicator_cd FROM indicator_info) AS indicator_cd,
...
WHERE
  ...
  AND facility_cd = ''@facilityCd'' 
  AND is_del = ''0''
  AND (SELECT exists_flag FROM data_exists_info) = ''0'' ', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_身体情報', '2020-05-25 18:21:40.841', '2022-03-28 16:23:44.304', NULL);
