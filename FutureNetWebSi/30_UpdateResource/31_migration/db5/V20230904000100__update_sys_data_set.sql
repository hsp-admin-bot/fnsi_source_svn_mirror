delete from ntss.sys_data_set where sql_cd in (-512,-496);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-496, 'WITH do_order_data_from AS (
...
ORDER BY cond_info_jyun,
    ...
    login_ord_mix
limit 135)
SELECT ((select count(1) from order_all)/15 + CASE WHEN ((select count(1) from order_all)%15) > 0 THEN 1 ELSE 0 END)  AS total_cnt ', 2, '[{}]', '1', '{"applications": [4]}', NULL, '日機装)実績）薬剤繰り返し部の総ページ', '2020-05-22 12:43:46',CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-512, 'WITH do_order_data_from AS (
SELECT ROW_NUMBER () OVER () AS no2, datt.a1
FROM (SELECT TO_NUMBER((unnest(string_to_array((
SELECT mst_f.value AS rtt
  FROM mst_facility_setting AS mst_f
...
', 2, '[{}]', '0', '{"applications": [4]}', '{"classes": []}', 'NKK)  透析実績：ヘッダ（削除）', '2022-08-22 12:05:22.245',CURRENT_TIMESTAMP, NULL);
