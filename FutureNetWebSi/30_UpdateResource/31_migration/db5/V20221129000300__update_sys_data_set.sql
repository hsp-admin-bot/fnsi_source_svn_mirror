DELETE FROM ntss.sys_data_set where sql_cd in (-18,-661);
...
(select mix_count from IND_COND_INFO)*mmm.amount_unit as e02,
mmm.unit as e03,
''0'' as e04
...
', 2, '[{}]', '1', '{"applications": [4]}', NULL, '抗凝固剤', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
,IND_COND_INFO AS (select ord.ord_no,
ord.ind_cond_info:: json ->''25'' ->> ''value'' as mix_cd,
to_number(ord.ind_cond_info:: json ->''26'' ->> ''value'', ''999999'')+
to_number(ord.ind_cond_info:: json ->''28'' ->> ''value'', ''999999'') as mix_count
from ord_main ord where ord.ord_no = @ordNo)
, do_mstmedi_cd AS (
...
''指示薬剤''::TEXT as detail_id,
  ...
  from 
    ...
    on
        ...
        left join 
                mst_medicine_class as medc on mmd.class_cd = medc.class_cd
    where
