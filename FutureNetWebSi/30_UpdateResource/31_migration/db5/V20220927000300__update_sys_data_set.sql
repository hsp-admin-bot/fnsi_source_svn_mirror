...
(CASE WHEN ''1''=(SELECT
  ...
  WHERE
    ...
    AND is_del = ''0''
    AND info ->> ''key1'' = ''DIALYSISSEND''
    AND info ->> ''key2'' = ''CREATE_NUMBER_FUNCTION'') THEN 
      (COALESCE(rst_cond_info-> ''3'' ->> ''value'', ''0'') :: FLOAT)::text
			else 
			(COALESCE(rst_cond_info-> ''3'' ->> ''value'', ''0'') :: FLOAT * 100)::text
      END) AS e05,
	''kg'' AS  e06,
	5 AS syoumouhinOrder
...
-- 	 (COALESCE(rst_weight_info ->> ''weight_before'', ''0'') :: FLOAT * 100)::text AS e05,
 (CASE WHEN ''1''=(SELECT
  ...
  WHERE
    ...
    AND is_del = ''0''
    AND info ->> ''key1'' = ''DIALYSISSEND''
    AND info ->> ''key2'' = ''BEFORE_WEIGHT_CD'') THEN 
     (COALESCE(rst_weight_info ->> ''weight_before'', ''0''):: FLOAT)::text
			else 
		  (COALESCE(rst_weight_info ->> ''weight_before'', ''0''):: FLOAT * 100)::text
      END) AS e05,
	''kg'' AS  e06,
    5 AS syoumouhinOrder
   from ord_main where ord_no =@ordNo
  ...
  WHERE
    ...
    AND info ->> ''key1'' = ''DIALYSISSEND''
    AND info ->> ''key2'' = ''CREATE_NUMBER_FUNCTION'') THEN 
(COALESCE(rst_weight_info ->> ''weight_after'', ''0'') :: FLOAT)::text
			else 
			(COALESCE(rst_weight_info ->> ''weight_after'', ''0'') :: FLOAT * 100)::text
      END) AS e05,
	''kg'' AS  e06,
    5 AS syoumouhinOrder
   from ord_main where ord_no = @ordNo
