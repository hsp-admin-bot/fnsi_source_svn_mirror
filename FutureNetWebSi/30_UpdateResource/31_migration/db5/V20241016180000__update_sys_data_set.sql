DELETE FROM sys_data_set sds WHERE sql_cd  = -103;

INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-103, 'WITH sch_start_time_info AS (
  SELECT
    0 AS order_no 
...
, sch_start_time AS (-- 透析項目送信
 SELECT
  CASE WHEN (SELECT start_time_kbn FROM sch_start_time_info) = ''0''
    THEN SUBSTRING(mk.kur_standard_start_time,1,2) || '':'' || SUBSTRING(mk.kur_standard_start_time,3,2)
    ELSE SUBSTRING(ord.ind_treat_start_time,1,2) || '':'' || SUBSTRING(ord.ind_treat_start_time,3,2)
    END AS sch_start_time,
    treat_date,
    ind_cond_info->''1''->>''value'' AS treat_time
 FROM
    do_ord_main ord
LEFT OUTER JOIN
    mst_kur mk
...
SELECT
    ...
    --項目名称
    TO_CHAR(TO_TIMESTAMP(treat_date || '' '' || sch_start_time || '':00'', ''YYYYMMDD HH24:MI:SS'') + (interval ''1minute'' * TO_NUMBER(treat_time,''9999'')) ,''HH24:MI'') AS e03,
    --数量
    ''0000000.000'' AS e04,
    --選択単位フラグ
    ''0'' AS e05,
