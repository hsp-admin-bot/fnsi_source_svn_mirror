...
WHERE sql_cd IN (-11, -505);

INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-11, 'with KOU_COAG_RESOLVE_MODE_cd AS(
SELECT
...
SELECT
  ...
  CASE WHEN ord.rst_fn_dialysis_no > 0 THEN
      ord.rst_fn_dialysis_no 
    ELSE
      ord.ord_no 
    END AS dialysis_no,--透析番号
...
WHERE
  ord.ord_no =  @ordNo', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '汎用）実績）透析条件', '2020-03-17 15:42:41.000', CURRENT_TIMESTAMP, NULL);
INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-505, 'WITH ord_main_restore_info AS (
  (SELECT
    ...
    ord_i.rst_fn_dialysis_no AS dialysis_no,--透析番号
...
SELECT
  ord.ord_no AS ord_no,
  CASE WHEN ord.dialysis_no > 0 THEN
      ord.dialysis_no 
    ELSE
      ord.ord_no 
    END AS dialysis_no,--透析番号
