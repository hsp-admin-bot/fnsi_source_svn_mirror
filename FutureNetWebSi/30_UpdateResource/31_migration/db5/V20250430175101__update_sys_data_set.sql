DELETE FROM "ntss"."sys_data_set" where sql_cd in (3,45,95,166,248,249,250);
...
-- 指定患者の車いす情報を取得
 ...
 b AS (
    ...
    , to_number(mni_monitor.monitor_data::json->>''4'', ''9999'') AS 残り時間_透析完了
    from d
    inner join mni_monitor on (d.bio_moni_ctl_no = mni_monitor.bio_moni_ctl_no) and mni_monitor.facility_cd = d.facility_cd
    where d.data_type = 1
), f AS (
    select e.*
    , e.経過時間 + e.残り時間_除水完了 AS 予測時間_除水
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (248, 'select
	om.ord_no
	, om.treat_date
  , om.rst_start_date
  , om.rst_end_date
...
and is_del = ''0''
and rst_dialysis_state <>''0''
', 2, '[]', '0', '{"applications": [1]}', '{"classes": [3]}', NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (249, 'WITH monitorData AS (
    SELECT
        rst_weight_info,
        monitor_data,
        bio_moni_ctl_no,
...
left join
	g
on tmp.ord_no=g.ordnob', 2, '[]', '0', '{"applications": [1]}', '{"classes": [3]}', '実績：体重情報/血圧情報 @ordNos 使用', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
FROM
	...
	time_info
	on
	DATA.ord_no = time_info.ord_no', 2, '[]', '0', '{"applications": [1]}', '{"classes": [3]}', '実績：透析条件/ベッド情報/ダイアライザ情報/実績情報 @ordNos 使用', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);