DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (9, 10, 11, 206, 207, 242, 251, 252, 253, 254);
...
FROM
  ...
  SELECT
    ...
    CAST(NULLIF(x.ind_rst_value, '''') AS DECIMAL) AS Amount,
		...
		COALESCE ( x.ind_unit, '''' ) AS receipt_unit,
		dz.in_hospital_cd_1,
		dz.in_hospital_cd_2,
		dz.in_hospital_cd_3,
		dz.in_hospital_cd_4
...
FROM
	...
	(SELECT
		...
		, Unit
		, SUM(medInfo.receipt_value) AS receipt_value
		, receipt_unit
		, in_hospital_cd_1
		, in_hospital_cd_2
		, in_hospital_cd_3
...
FROM
	...
	SELECT
		...
		9 as class_data_order,
		CASE
      		WHEN mdc.class_name IS NULL
      		THEN ''未分類'' ELSE COALESCE ( mdc.class_name, '''' )
    		END AS kind,
    		CASE
      		WHEN mdc.class_name IS NULL
      		THEN ''-1'' ELSE mdc.class_cd
    		END AS class_cd,
		''通常薬剤'' AS do_action,
		md.medicine_name AS NAME,
		TO_NUMBER(save.supplies_cd, ''99999999'' ) AS code,
    ...
    ''透析液'' AS class,
		10 as class_data_order,
		CASE
			WHEN mdc.class_name IS NULL THEN ''未分類''
			ELSE COALESCE ( mdc.class_name, '''' )
		END AS kind,
		CASE
			WHEN mdc.class_name IS NULL THEN ''-1''
			ELSE mdc.class_cd
		END AS class_cd,
		''通常薬剤'' AS do_action,
		md.medicine_name AS NAME,
