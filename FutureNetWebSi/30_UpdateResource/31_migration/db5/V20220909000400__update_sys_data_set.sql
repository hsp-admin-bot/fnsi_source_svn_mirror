DELETE  FROM sys_data_set WHERE sql_cd IN ('3109', '2020');
...
WITH user_table AS ( 
  SELECT TRIM(unnest(string_to_array(@dispUserId, '',''))) as disp_user_id 
	)
SELECT
  ''1'' AS order,
	STRING_AGG (user_id::TEXT, '','') staff_cd
FROM
	mst_user_authentication mua 
	INNER JOIN user_table ut ON mua.disp_user_id = ut.disp_user_id
WHERE
	facility_cd = @facilityCd', 1, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の初回申し込み→依頼事項登録→掲示板情報_登録', '2022-08-18 07:48:11.102', current_timestamp, '[{"sql_cd": 2021, "field_name": "disp_user_id", "replace_var": "@dispUserId"}]');
