      ...
      ( SELECT
        ...
        (
          SELECT
            ...
            ROW_NUMBER( ) OVER ( PARTITION BY ord_main.treat_date ORDER BY ord_main.ind_kur_cd ) AS row_num 
          FROM
            ord_main 
          WHERE
	...
	FROM
      ...
      ( SELECT
		...
		group by
			...
			spi.spitz_order 
		ORDER BY
		  spi.spitz_order)', 2, '[]', '1', '{"applications": [1]}', '{"classes": [8]}', 'ラベル（降順）', '2024-03-20 08:56:21', CURRENT_TIMESTAMP, NULL);
