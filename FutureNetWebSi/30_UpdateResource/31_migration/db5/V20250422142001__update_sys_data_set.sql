...
ord_staff_cd as (
SELECT 
  json_agg(row_to_json(t))::text AS value
FROM (
  SELECT
    coop_save_no,
...
from
	input_code_class
limit 1', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '担当スタッフ情報取得用', '2025-04-11 14:29:31.725', CURRENT_TIMESTAMP, NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
...
order by
	coop_save_no,
	order_units_id_suffix', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '検査情報(Order_Unitsタグ)取得用', '2025-04-11 14:29:31.725', CURRENT_TIMESTAMP, '[{"sql_cd": -307077, "field_name": "disp_user_id", "replace_var": "@dispUserId"}, {"sql_cd": -307079, "field_name": "user_name", "replace_var": "@userName"}, {"sql_cd": -307081, "field_name": "disp_user_ids", "replace_var": "@ids"}, {"sql_cd": -307082, "field_name": "user_names", "replace_var": "@names"}]'::jsonb);