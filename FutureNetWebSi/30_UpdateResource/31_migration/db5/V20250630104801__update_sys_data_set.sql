...
from
  (
    ...
    --↓画面上で追加している項目の追加処理
    union
    select ord_no, rst_start_date::text as occur_date, ''治療開始'' as complaint, null, null, ''1'' as checkFlag, null, null from ord_main where ord_no = @ordNo and rst_start_date is not null
    union
    select ord_no, rst_end_date::text as occur_date, ''治療終了'' as complaint, ''999'', ''999'', ''1'' as checkFlag, ''999'', ''999'' from ord_main where ord_no = @ordNo and rst_end_date is not null
    --↑画面上で追加している項目の追加処理
order by
      ord_no,
      occur_date) a
