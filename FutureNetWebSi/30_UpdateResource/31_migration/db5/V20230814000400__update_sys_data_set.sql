delete from ntss.sys_data_set where sql_cd in (1103,7102,9621,1013);
...
--   , pat_first_name = COALESCE(personal_info_encrypt(NULLIF((SELECT patFirstName FROM new_name_info), '''')) , pat_first_name) 
   ,pat_last_name = 
	case when (''@patLastName'' like ''% %'') then (SELECT  personal_info_encrypt(patLastName) FROM new_name_info)
  ...
  ,pat_first_name = case when (''@patFirstName''  like ''% %'') then (SELECT  personal_info_encrypt(patFirstName) FROM new_name_info)
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (7102, 'with origin_pat_contact_info as (select t.info ->> ''fax''       as fax,
...
SET
 ...
 (case when (''@patLastName'' like ''% %'') then (SELECT  personal_info_encrypt(patLastName) FROM new_name_info)
	...
	(case when (''@patFirstName''  like ''% %'') then (SELECT  personal_info_encrypt(patFirstName) FROM new_name_info)
  when ((SELECT concat(patLastName,patFirstName) FROM new_name_info) = concat(personal_info_decrypt(pat_last_name),personal_info_decrypt(pat_first_name)) and (''@patFirstName'' not like ''% %'')  )then pat_first_name 
	when ((SELECT concat(patLastName,patFirstName) FROM new_name_info) != concat(personal_info_decrypt(pat_last_name),personal_info_decrypt(pat_first_name)) and (''@patFirstName'' not like ''% %'')  ) then NULL end)	
