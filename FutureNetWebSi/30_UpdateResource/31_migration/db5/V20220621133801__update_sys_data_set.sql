delete from ntss.sys_data_set where sql_cd in ('-494','-498');
...
FROM
  ...
  WHERE
    equip ->> ''equip_type'' = ''0''
    AND (equip ->> ''class_type'' NOT IN ( ''2'', ''3'', ''5'' ) or equip ->>''class_type'' is null)
    AND ord.ord_no = @ordNo
  UNION ALL
    SELECT--医材情報
    ''ダイアライザ'' AS detail_id,
...
SELECT
    (detail_id:: text) as detail_id, left(item_cd, 8) as item_cd, sum(amounttest) as amounttest
FROM
    data_all
