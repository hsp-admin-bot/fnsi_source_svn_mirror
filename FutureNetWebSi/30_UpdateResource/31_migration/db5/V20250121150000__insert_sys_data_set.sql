delete from sys_data_set where sql_cd in (-1000026,-1000027);
...
filtered_pat AS (
    SELECT
        pat_id,
        facility_cd,
        (infect_record->>''infection_cd'')::NUMERIC AS infection_cd, -- 数値型にキャスト
        infect_record->>''infect'' AS infect,
        infect_record->>''exam_date'' AS exam_date,
        infect_record->>''up_date'' AS up_date,
        ROW_NUMBER() OVER (
            PARTITION BY pat_id, (infect_record->>''infection_cd'')::NUMERIC
            ORDER BY  infect_record->>''exam_date'' DESC, infect_record->>''up_date'' DESC
        ) AS rn
    FROM
        expanded_pat
...
WHERE
    pat.rn = 1 -- 最新のレコードを取得
    AND pat.infection_cd = @examCd',2,'[{}]','0','{"applications": [4]}',NULL,'統計調査（患者の感染症）',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,NULL);
