  ...
  select
	...
	case
      when (wst.value <> '''') then 
				...
				|| coalesce(wst.unit, '''') 
				|| (case when (wst.text is not null and wst.text <> ''0'') then (select jsonb_array_elements(wstt.initial_string::JSONB)->>''text'' LIMIT 1 OFFSET CAST(wst.text AS INTEGER) - 1)
				else '''' end)
      when (wst.memo <> '''' or wst.time <> '''' or CAST(wst.picker AS INTEGER) != 0 or CAST(wst.inspector AS INTEGER) != 0) then ''検査中''
      when (wst.text is not null and wst.text <> ''0'') then (select jsonb_array_elements(wstt.initial_string::JSONB)->>''text'' LIMIT 1 OFFSET CAST(wst.text AS INTEGER) - 1)
