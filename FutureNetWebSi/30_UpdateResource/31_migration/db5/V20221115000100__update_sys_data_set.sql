...
WHERE
 ...
 UNION
 (--  実施コメンSOAP ト A
  SELECT detail_id, sbt_key, e01, e02, e03, e04, e05, e06, e07, e08, e09, e10, e11, sortTag
 FROM
 (SELECT
  ''実績詳細'' AS detail_id,
  ''VC7'' sbt_key,
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''3'' ) AS sortTag) A
	WHERE e03 != '''' 
 ) UNION
 (--  実施コメンSOAP ト O
  SELECT detail_id, sbt_key, e01, e02, e03, e04, e05, e06, e07, e08, e09, e10, e11, sortTag
 ...
 (SELECT
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''2'' ) AS sortTag ) A
	WHERE e03 != ''''
 ) UNION
 (--  実施コメンSOAP ト P
  SELECT detail_id, sbt_key, e01, e02, e03, e04, e05, e06, e07, e08, e09, e10, e11, sortTag
 ...
 (SELECT
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''4'' ) AS sortTag ) A
	WHERE e03 != ''''
 ) UNION
 (-- レセプトメモ
 SELECT
 ...
 (SELECT
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''1'' ) AS sortTag ) A
	WHERE e03 != ''''
 ) UNION ALL
 (-- 加算・管理料
 SELECT
...
WHERE
 ...
 WHERE
  ...
  AND ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_OXYGEN_SEND_FLAG'' ) = ''1'' 
    ...
    select * from data_all where sortTag < ( SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_OXYGEN'')
   union all
   select * from data_oxygen
   union all
   select * from data_all where sortTag > ( SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_OXYGEN'' )', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）実績繰り返し部', '2020-04-24 19:15:25.001',CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -79, "field_name": "dial_diff_cd", "replace_var": "@dial_diff_cd"}]');
