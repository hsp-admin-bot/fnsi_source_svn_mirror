...
WHERE sql_cd IN (-1202002,-1202003,-1202004,-1202005,-1202006,-1202009,-1202010);
...
do_ord_main AS (
  (
    SELECT
      ...
      COALESCE(NULLIF(ord_i.ind_kur_cd, 0), (ord_i.ind_schedule_user_info ->> ''ind_kur_cd_before'')::int) as ind_kur_cd,
      ord_i.ind_cond_info -> ''1'' ->> ''value'' AS treat_times
    FROM ord_main_restore as ord_i
    WHERE ord_i.pat_id = (SELECT pat_id FROM do_pat_exam_main) AND ord_i.treat_date = TO_CHAR((SELECT reg_exam_date FROM do_pat_exam_main), ''YYYYMMDD'')
          AND ord_i.is_del = ''0''
