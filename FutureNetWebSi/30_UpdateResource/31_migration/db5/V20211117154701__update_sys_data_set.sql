delete from "sys_data_set" where "sql_cd" in (-99988,-11,-35,-36);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-99988, 'SELECT
  ''DIALYSISRST_'' || 
...
WHERE
  journal.ctl_no = @ctlNo', 2, '[]', '0', '{"applications": [4]}', NULL, 'SSI 透析実績[送信]ファイル名取得', '2021-04-20 09:19:08.001', '2021-04-20 09:19:12', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-36, 'select
  ''後・脈・血圧'' as detail_id,
  occur_date
  ...
  ,to_number(monitor_data->>''93'', ''FM999'') as body_temperature
  ,to_number(monitor_data->>''-1'', ''FM999'') as blood_glucose_level
from
  mni_monitor
where
  ord_no = @ordNo and data_type = 6 and is_del = ''0''
order by up_date desc
limit 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '汎用）透析後血圧', '2020-05-22 10:07:36', '2020-05-22 10:07:39', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-35, 'select
  ''前・脈・血圧'' as detail_id,
  occur_date
  ,to_number(monitor_data->>''90'', ''FM999'') as bp_high
  ,to_number(monitor_data->>''91'', ''FM999'') as bp_low
...
order by up_date desc
limit 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '汎用）透析前血圧', '2020-05-22 10:07:36', '2020-05-22 10:07:39', NULL);
...
--mbd.bed_cd as bed_cd,--ベッドコード
  ...
  COALESCE(TO_CHAR( ord.ord_no, ''FM000000000000'' ), '''') AS ord_no12,
