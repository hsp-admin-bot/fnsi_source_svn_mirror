...
select
     ...
     , case sv.supplies_class
         when ''01'' then (select model_number from ( select  model_number, dialyzer_cd  from  mst_dialyzer   where          is_del = ''0''         and is_disp = ''1'' and facility_cd =@facilityCd  ) dz where dialyzer_cd = TO_NUMBER(supplies_cd, ''99999999''))
