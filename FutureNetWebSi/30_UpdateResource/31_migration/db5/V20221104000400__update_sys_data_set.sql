delete from ntss.sys_data_set where sql_cd = '-61';
...
FROM (
         ...
         (SELECT ''data'' AS part, staff_cd
          FROM staff_user_info
          WHERE (SELECT setting FROM user_no_setting) IN (''2'', ''4'')
          LIMIT 1 OFFSET 1)) AS T', 2, '[{}]', '0', '{"applications": [4]}', null, '（実績）「利用者番号」に設定する値の取得', '2022-03-16 08:52:30.730', CURRENT_TIMESTAMP, null);
