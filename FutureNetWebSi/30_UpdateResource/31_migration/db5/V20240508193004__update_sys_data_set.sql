...
VALUES(1708, 'with compareDate as (select (case TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS'') >
...
WHERE is_del = ''0''
  ...
  AND treat_date not in (select treat_date
                         ...
                         WHERE is_del = ''0''
                           ...
                           AND facility_cd = ''@facilityCd''
                           AND treat_date >= TO_CHAR(TO_DATE(compareDate.die_date, ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'')
                           AND rst_dialysis_state > ''0'')', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '未来日データをコピーする。透析予定(ord_main) → ord_main_restore', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, NULL);
