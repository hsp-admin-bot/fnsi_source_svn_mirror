DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (1102,1103,1705);
...
,data_new_info AS (
  SELECT 
    ...
    null AS from_course,
    null AS from_doctor,
    (CASE inOut WHEN ''0'' THEN  ''6'' WHEN ''1'' THEN ''4'' ELSE ''6'' END) :: TEXT AS move_in_out,
    null AS to_facility,
...
WHERE
  ...
  AND (SELECT exists_flag FROM data_exists_info) = ''0'' 
	...
	AND (SELECT inOut from inOutClassNull) != ''0''', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡以外)', '2020-05-25 18:21:40.841',CURRENT_TIMESTAMP, NULL);
