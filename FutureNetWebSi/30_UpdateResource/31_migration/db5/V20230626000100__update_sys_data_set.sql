...
FROM
	...
	CASE
		...
		END AS kind,
		md.medicine_name AS NAME,
		CEIL ( CAST ( x.receipt_value AS DECIMAL ) ) AS Amount,
		COALESCE ( md.unit_second, COALESCE ( md.unit, '''' ) ) AS Unit,
		md.in_hospital_cd_1,
		md.in_hospital_cd_2,
		md.in_hospital_cd_3,
	...
	CASE
		...
		FROM
			x
			INNER JOIN mst_medicine_mix AS mdx ON mdx.medicine_mix_cd = TO_NUMBER( x.supplies_cd, ''999999999999'' )
		...
		FROM
			x
			LEFT JOIN mst_medicine_mix AS md ON ( TO_NUMBER( x.medicine_mix_cd, ''9999999999'' ) = md.medicine_mix_cd AND md.is_del = ''0'' AND md.is_disp = ''1'' AND md.class_cd IN ( @medIds ))
			LEFT JOIN mst_medicine_class AS mdc ON ( md.class_cd = mdc.class_cd AND mdc.is_del = ''0'' AND mdc.is_disp = ''1'' )
		WHERE
			x.supplies_source_class = ''1''
