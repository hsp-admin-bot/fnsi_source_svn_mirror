...
WHERE sql_cd IN (7102,9621,-403201);
...
VALUES(7102, 'with origin_pat_contact_info as (select t.info ->> ''fax''       as fax,
     ...
     nameSplit as (select split_part(REPLACE(''@otherContactInfo.lastName'', ''　'', '' ''), '' '', 1) as lastName,
                          split_part(REPLACE(''@otherContactInfo.lastName'', ''　'', '' ''), '' '', 2) as firstName),
    ...
    or (REPLACE(''@otherContactInfo.lastName'', ''　'', '' '') not like ''% %'' and concat((j ->> ''last_name'')::text,(j ->> ''first_name'')::text)=''@otherContactInfo.lastName'')))
    , replaceZipCd as (
        select
...
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(9621, 'with nameSplit as (select split_part(REPLACE(''@otherContactInfo.lastName'', ''　'', '' '') ,'' '', 1) as lastName,
                          split_part(REPLACE(''@otherContactInfo.lastName'', ''　'', '' '') ,'' '', 2) as firstName),
     oldInfo as (select (other_contact_info ->> (idx - 1)::int)::json AS oldInfo
                 ...
                 WHERE  
             ((REPLACE(''@otherContactInfo.lastName'', ''　'', '' '') like ''% %'' and (j ->> ''first_name'')::text = nameSplit.firstName
               and (j ->> ''last_name'')::text = nameSplit.lastName)
             or (REPLACE(''@otherContactInfo.lastName'', ''　'', '' '') not like ''% %'' and concat((j ->> ''last_name'')::text,(j ->> ''first_name'')::text) 
...
SET
  ...
  , pat_last_name_kana = CASE
                        ...
                        THEN (SELECT  personal_info_encrypt(patLastNmKana) FROM new_name_info)
                        ELSE pat_last_name_kana
                    END
  ,pat_first_name_kana = CASE
                        WHEN ''1'' = (SELECT name_kana_update_flg FROM check_name_update)
                        THEN (SELECT  personal_info_encrypt(NULLIF(patFirstNmKana, '''')) FROM new_name_info)
