delete from "sys_data_set" where "sql_cd" in (-101);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-101, 'WITH dialysis_item_send AS (-- 透析項目送信
 ...
 conv_teart_item_out AS ( -- 浄化方法変換（予約/実績送信用：外来）
 SELECT
  info ->> ''key2'' AS key2,
...
FROM
 (
  ...
  SELECT
   ''実績詳細'' AS detail_id,
   ''VE1'' AS sbt_key,
   ( ''V'' || lpad( ord.rst_bed_cd :: TEXT, 7, ''0'' ) ) AS e01,--項目コード
   COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_BED_NO_ATTR'' ), '''' ) AS e02,-- 項目属性
   COALESCE ( SUBSTRING ( mbd.bed_name, 1, 25 ), '''' ) AS e03,--項目名称
  ...
  SELECT
   ''実績詳細'' AS detail_id,
   ''VC1'' AS sbt_key,
--  連携設定の浄化方法送信設定が「0：装置モードで送信」の場合　※１
  CASE
    ( SELECT VALUE FROM fji_com_info WHERE key2 = ''TREAT_ITEM_SEND_FLG'' )
    
    WHEN ''0'' THEN--0：装置モードで送信」の場合
    (
    CASE
      ''1'' 
      WHEN ''1'' THEN
      COALESCE ( mtt.in_hospital_cd_a1, '''' ) ELSE COALESCE ( NULLIF ( mtt.in_hospital_cd_a2, '''' ), mtt.in_hospital_cd_a1, '''' ) 
     END 
     ) 
     WHEN ''1'' THEN-- 1：治療方法文字列で送信」の場合
     (
     CASE
       ord.rst_in_out_class --透析実績履歴．入外区分
       
       WHEN ''1'' THEN-- 入院
       (
        COALESCE (
         NULLIF ( ( SELECT VALUE FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0 ), '''' ),
        CASE
          ''1'' 
          WHEN ''1'' THEN-- 患者入院
          COALESCE ( mtt.in_hospital_cd_a1, '''' ) ELSE --患者外来
          COALESCE ( NULLIF ( mtt.in_hospital_cd_a2, '''' ), mtt.in_hospital_cd_a1, '''' ) 
         END 
         ) 
         ) ELSE (-- 外来
         COALESCE (
          NULLIF ( ( SELECT VALUE FROM conv_teart_item_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 0 ), '''' ),
         CASE
           ''1'' 
           WHEN ''1'' THEN-- 患者入院
           COALESCE ( mtt.in_hospital_cd_a1, '''' ) ELSE --患者外来 1
           COALESCE ( NULLIF ( mtt.in_hospital_cd_a2, '''' ), mtt.in_hospital_cd_a1, '''' ) 
          END 
          ) 
         ) 
      ...
      CASE
        ...
        WHEN ''0'' THEN
        ( COALESCE ( mtt.treatment_name, '''' ) ) 
        WHEN ''1'' THEN
        (
        CASE
          ord.rst_in_out_class 
          WHEN ''1'' THEN
          ( COALESCE ( NULLIF ( ( SELECT VALUE FROM conv_treat_item_send_in WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1 ), '''' ), '''' ) ) ELSE ( COALESCE ( NULLIF ( ( SELECT VALUE FROM conv_teart_item_out WHERE key2 = mtt.treatment_name LIMIT 1 OFFSET 1 ), '''' ), '''' ) ) 
         END 
         ) ELSE'''' 
        END AS e03,--治療項目名
        ''0000000.000'' AS e04,
        ...
        ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_TREAT'' ), '''' ) ) AS sortTag 
       FROM
        ord_main ord
       ...
       SELECT
        ...
        COALESCE ( SUBSTRING ( to_char( ( ord.rst_weight_info ->> ''weight_before_date'' ) :: TIMESTAMP, ''YYYY/MM/DD'' ), 1, 25 ), '''' ) AS e03,--項目名称
        ...
        ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_BEFOTE_WEIGHT'' ), '''' ) ) AS sortTag 
       FROM
        ord_main ord 
       WHERE
        ord.ord_no = @ordNo 
       ...
       SELECT
        ...
        COALESCE ( SUBSTRING ( to_char( ( ord.rst_weight_info ->> ''weight_after_date'' ) :: TIMESTAMP, ''YYYY/MM/DD'' ), 1, 25 ), '''' ) AS e03,--項目名称
       ...
       SELECT
        ...
        SUBSTRING ( to_char( ord.rst_start_date, ''YYYY/MM/DD'' ), 1, 25 ) AS e03,-- 項目名称
        to_char( TO_NUMBER( ord.rst_cond_info -> ''3'' ->> ''value'', ''999999999.999'' ), ''FM0999999.990'' ) AS e04,-- 数量
        ''1'' AS e05,-- 選択単位フラグ
       ...
       SELECT
        ...
        COALESCE ( to_char( ( pma.medical_care_info ->> ''dialysis_start_date'' ) :: TIMESTAMP, ''YYYY/MM/DD'' ), '''' ) AS e03,-- 項目名称
        ''0000000.000'' AS e04,-- 数量
        ''0'' AS e05,-- 選択単位フラグ
        '''' AS e06,-- 単位コード
        '''' AS e07,-- 単位名称
       ...
       FROM
        ord_main ord
        LEFT OUTER JOIN pat_main AS pma ON pma.pat_id = ord.pat_id 
       WHERE
        ord.ord_no = @ordNo 
       ) UNION
       (-- 障害者加算タイトル
       ...
       WHERE
        ...
        AND ord.ord_no = @ordNo 
        AND ( SELECT VALUE FROM sys_system_define WHERE ctl_no = 134 ) IS NULL 
        OR ( SELECT VALUE FROM sys_system_define WHERE ctl_no = 134 ) = ''1'' 
       ) UNION
       ...
       WHERE
        ...
        AND ord.ord_no = @ordNo 
        AND ( SELECT VALUE FROM sys_system_define WHERE ctl_no = 134 ) IS NULL 
        OR ( SELECT VALUE FROM sys_system_define WHERE ctl_no = 134 ) <> ''0'' 
       ) UNION
       ...
       SELECT
        ...
        COALESCE ( NULLIF ( mva.in_hospital_cd_2, '''' ), COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_SOLUTION_ATTR'' ), '''' ) ) AS e02,-- 項目属性
        ...
        '''' AS e09,
        ''12'' AS e10,
        COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_SOLUTION_TAG'' ), '''' ) AS e11,-- タグ名称
       ...
       SELECT
        ...
        ''VR1'' AS sbt_key,
        COALESCE ( meq.in_hospital_cd_1, '''' ) AS e01,--  項目コード
        COALESCE ( meq.in_hospital_cd_2, ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_EQUIP_ATTR'' ), '''' ) AS e02,-- 項目属性
        COALESCE ( SUBSTRING ( meq.equipment_name, 1, 25 ), '''' ) AS e03,-- 項目名称
        COALESCE ( to_char( TO_NUMBER( rei ->> ''amount'', ''999999999.999'' ), ''FM009999.000'' ), ''0'' ) AS e04,-- 数量
        ( CASE WHEN meq.unit = '''' THEN ''0'' WHEN meq.unit IS NULL THEN ''0'' ELSE''1'' END ) AS e05,-- 選択単位フラグ
        COALESCE ( meq.unit, '''' ) AS e06,-- 単位コード
        COALESCE ( meq.unit, '''' ) AS e07,-- 単位名称
        '''' AS e08,
        '''' AS e09,
       ...
       FROM
        ord_main ord
        CROSS JOIN LATERAL json_array_elements ( ord.rst_equip_info :: json ) rei
        LEFT OUTER JOIN mst_equipment AS meq ON meq.equipment_cd = TO_NUMBER( rei ->> ''cd'', ''9999999999'' )
        LEFT OUTER JOIN mst_equipment_class AS meqc ON meqc.class_cd = meq.class_cd 
       WHERE
        meqc.class_type = ''2'' --AV
       ...
       SELECT
        ...
        '''' AS e09,
        ''18'' AS e10,
        COALESCE ( meqc.class_name, '''' ) AS e11,
        ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_EQUIP'' ), '''' ) ) AS sortTag 
       FROM
        ord_main ord
        CROSS JOIN LATERAL json_array_elements ( ord.rst_equip_info :: json ) rei
       ...
       SELECT
        ...
        to_char(
         TO_NUMBER( COALESCE ( ord.rst_cond_info -> ''17'' ->> ''value'', ''0'' ), ''999999999.999'' ) + TO_NUMBER( COALESCE ( ord.rst_cond_info -> ''20'' ->> ''value'', ''0'' ), ''999999999.999'' ),
         ''FM009999.990'' -- 数量
       ...
       SELECT
        ...
        COALESCE ( SUBSTRING ( mmd.medicine_name, 1, 25 ), '''' ) e03,-- 項目名称
        COALESCE ( to_char( TO_NUMBER( ord.rst_cond_info -> ''22'' ->> ''value'', ''0'' ), ''999999999.999'' ), ''FM000099.990'' ) AS e04,-- 数量
        ''1'' e05,-- 選択単位フラグ
       ...
       FROM
        ord_main ord
        LEFT OUTER JOIN mst_medicine AS mmd ON mmd.medicine_cd = TO_NUMBER( ord.rst_cond_info -> ''20'' ->> ''value'', ''999999999999'' )
        LEFT OUTER JOIN mst_medicine_class AS mmc ON mmc.class_cd = mmd.class_cd 
       WHERE
        ord.ord_no = @ordNo 
        AND ( SELECT VALUE FROM fji_com_info WHERE key2 = ''LIQUID_SEND_FLAG'' ) = ''1'' 
        AND ( SELECT VALUE FROM int_set_medicine_resolve WHERE key2 = ''REPLACE_RESOLVE_MODE'' ) = ''0'' 
       ) UNION
       (-- 抗凝固剤初回
       SELECT
        ''実績詳細'' AS detail_id,
   ...
   to_char( TO_NUMBER( COALESCE ( ord.rst_cond_info -> ''26'' ->> ''value'', ''0'' ), ''999999999.999'' ), ''FM099999.990'' ),-- 数量
   (
    COALESCE (
    CASE
 ...
 to_char( TO_NUMBER( COALESCE ( ord.rst_cond_info -> ''27'' ->> ''value'', ''0'' ), ''999999999.999'' ), ''FM099999.990'' ) AS e04,--e4
 (
  COALESCE (
  CASE
...
WHERE
 ...
 (
  ...
  CASE 
    WHEN ( CASE ord.rst_cond_info -> ''25'' ->> ''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END ) IS NULL THEN ''0'' 
    WHEN ( CASE ord.rst_cond_info -> ''25'' ->> ''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END ) = '''' THEN ''0'' ELSE NULL 
 END,
...
WHERE
 ...
 SELECT
  ''実績詳細'' AS detail_id,
  ''VK3'' AS sbt_key,
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_BLOOD_AMT_ATTR'' ), '''' ) AS e01,-- 項目コード
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_BLOOD_AMT_CODE'' ), '''' ) AS e02,-- 項目属性
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_BLOOD_AMT_NAME'' ), '''' ) AS e03,-- 項目名称
  to_char( TO_NUMBER( ord.rst_cond_info -> ''14'' ->> ''value'', ''999999999999'' ), ''FM000999.990'' ) AS e04,-- 数量
  ''1'' AS e05,-- 選択単位フラグ
...
UNION
 ...
 SELECT
  ...
  to_char( TO_NUMBER( ord.rst_cond_info -> ''20'' ->> ''value'', ''999999999999'' ), ''FM000099.990'' ) AS e04,--数量
  ''1'' AS e05,-- 選択単位フラグ
 ...
 WHERE
  ...
  AND TO_NUMBER( ( SELECT VALUE FROM fji_com_info WHERE key2 = ''LIQUID_SEND_FLAG'' ), ''999999999999'' ) = 1 
 ) UNION
 (-- 薬品手技
 SELECT
  ''実績詳細'' AS detail_id,
  ...
  COALESCE ( mp.in_hospital_cd_a2, '''' ) AS e02,-- 項目属性
  ...
  '''' AS e09,
  ''29'' AS e10,
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_MEDICINE_PROCEDURE_TAG'' ), '''' ) AS e11,-- タグ名称
 ...
 SELECT
  ...
  ''VO2'' AS sbt_key,
  COALESCE ( mmd.in_hospital_cd_1, '''' ) AS e01,-- 項目コード
  COALESCE ( mmd.in_hospital_cd_2, ( NULLIF ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_PROCEDURE_ATTR'' ), '''' ) ), '''' ) AS e02,-- 項目属性
  COALESCE ( mmd.medicine_name, '''' ) AS e03,-- 項目名称
  COALESCE( TO_CHAR(TO_NUMBER(medi ->> ''amount'',''9999999999''),''FM099999.990'') )  AS e04,-- 数量
  (CASE WHEN mmd.unit IS NULL THEN ''0''
     WHEN mmd.unit = '''' THEN ''0''
     ELSE ''1'' END)AS e05,-- 選択単位フラグ
   COALESCE(mmd.unit,'''') AS e06,-- 単位コード
  COALESCE(mmd.unit,'''') AS e07,-- 単位名称
  '''' AS e08,
  '''' AS e09,
  ''29'' AS e10,
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_MEDICINE_TAG'' ), '''' ) AS e11,-- タグ名称
  NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE'' ), '''' ) AS sortTag 
  FROM
  ord_main AS ord
  CROSS JOIN LATERAL json_array_elements ( ord.rst_medi_info :: json ) medi
  LEFT OUTER JOIN mst_medicine AS mmd ON mmd.medicine_cd = to_number( medi ->> ''cd'', ''99999999'' ) 
 WHERE
  ord.ord_no = @ordNo 
  AND ((SELECT VALUE FROM fji_com_info  WHERE  key2 = ''MEDICINE_SEND_FLAG'') = ''2''  AND  mmd.is_shot = 1::TEXT ) 
 ) UNION
 (--  処置薬品名-手技なし薬剤
 SELECT
  ''実績詳細'' AS detail_id,
  ''VO2'' AS sbt_key,
  COALESCE ( mmd.in_hospital_cd_1, '''' ) AS e01,-- -- 項目コード
  COALESCE ( mmd.in_hospital_cd_2, ( NULLIF ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_NON_PROCEDURE_ATTR01'' ), '''' ) ), '''' ) AS e02,-- 項目属性
  COALESCE ( mmd.medicine_name, '''' ) AS e03,-- 項目名称
  ''0000000.000'' AS e04,-- 数量
  ( CASE WHEN ( mmd.unit IS NULL ) OR ( mmd.unit = '''' ) THEN ''0'' ELSE''1'' END ) AS e05,-- 選択単位フラグ
  COALESCE ( mmd.unit, '''' ) AS e06,-- 単位コード
  COALESCE ( mmd.unit, '''' ) AS e07,-- 単位名称
  '''' AS e08,
  '''' AS e09,
 ...
 FROM
  mst_medicine AS mmd
  WHERE
     ((SELECT VALUE FROM fji_com_info  WHERE  key2 = ''MEDICINE_SEND_FLAG'') = ''2'' 
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''001'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''002'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''003'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''004'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''005'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''006'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''007'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''008'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''009'' ), ''99999999'')
       AND mmd.procedure_cd <> TO_NUMBER(( SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE  key2 = ''010'' ), ''99999999'')
            AND  mmd.is_shot = 1::TEXT 
  
  ) 
  or((SELECT VALUE FROM fji_com_info  WHERE  key2 = ''MEDICINE_SEND_FLAG'' ) = ''1'' AND  ((SELECT medi_cd FROM RST_MEDI_INFO WHERE medi_cd=mmd.medicine_cd::TEXT)IS NULL ) AND mmd.is_shot = 0::TEXT ) 
 ) UNION
 (--  実施コメンSOAP ト A
 SELECT
  ''実績詳細'' AS detail_id,
 ...
 SELECT
  ...
  ''0000000.000'' AS e04,-- 数量
  to_char( TO_NUMBER( oxy ->> ''amount'', ''999999999999'' ), ''FM099999.990'' ) AS e05,-- 数量
...
WHERE
 all_cost.sortTag IS NOT NULL 
ORDER BY
 all_cost.sortTag;', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）実績繰り返し部', '2020-04-24 19:15:25.001',  CURRENT_TIMESTAMP, NULL);
