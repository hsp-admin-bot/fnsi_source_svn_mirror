...
where sql_cd in (-1102015);

INSERT INTO sys_data_set (sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES(-1102015, 'WITH RECURSIVE coop_ini_info AS (
--連携設定より取得
SELECT
...
SELECT
  RP as rp_no,
  ''01'' AS detail_id
FROM rp_series;', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析指示連携_処置依頼ファイル_処置単位のRP番号取得', '2025-06-26 17:23:48.129', '2025-07-08 15:03:28.202', '[{"sql_cd": -1102004, "field_name": "pat_personal_info", "replace_var": "@patPersonalInfo"}]'::jsonb);