DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (-51,-53,-54,-58,-59,-64,-78,-103,-194,-800010);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-51, 'WITH course_from_info AS ( 
  SELECT
    1 AS order_no
...
SELECT
  ...
  , CASE @inOut WHEN ''1'' THEN
      (CASE WHEN (SELECT ward_from FROM ward_from_info) = ''1'' 
       ...
       END)
    ELSE '''' END AS ward_cd', 2, '[]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：診療科コードと病棟コード', '2022-03-07 10:35:57.514', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}]');
...
ON
  ord.ind_kur_cd = mk.kur_cd
WHERE
  ord.ord_no = @ordNo', 2, '[]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：オーダ日付とオーダ時間取得', '2022-03-22 10:53:38.415', CURRENT_TIMESTAMP, NULL);
...
,kou_coag_procedur_falg AS (
    ...
    FROM
        ...
        WHERE
            ( key2 IN ( ''KOU_COAG_PROCEDURE_ATTR'' ) AND  (( VALUE IS NOT NULL ) AND VALUE<>'''' )) 
            OR ( key2 IN ( ''KOU_COAG_PROCEDURE_CODE'' ) AND (( VALUE IS NOT NULL ) AND VALUE<>'''')) 
            OR ( key2 IN ( ''KOU_COAG_PROCEDURE_NAME'' ) AND (( VALUE IS NOT NULL ) AND VALUE<>''''))
        ) T 
    WHERE
        cnt = 3 UNION
    SELECT
...
WHERE
    ((SELECT value FROM ind_set_medicine_resolve_info WHERE key2 = ''KOU_COAG_RESOLVE_MODE'') = ''0'' or
...
FROM
   ...
   COALESCE(mmd.unit, '''') AS e07,
    --タグ名称
   COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_MEASURE_MEDICINE_TAG''), '''') AS e08, 
       COALESCE((SELECT value FROM item_sort_info WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-'' || ''0''||''-''||''1''  AS e09 ,
    '''' as sorttag1,
    '''' as sorttag2,
        '''' as aa
FROM
...
WHERE
 ...
 --各部分の組み合わせ
    ...
    union all 
    select  detail_id::text, e01::text, e02::text, e03::text, e04::text, e05::text, e06::text, e07::text, e08::text, e09::text,sortTag1::text,sortTag2::text, aa from equip_all_copy
    union all
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-194, 'WITH do_order_data_from AS (
SELECT ROW_NUMBER () OVER () AS no2, datt.a1
FROM (SELECT TO_NUMBER((unnest(string_to_array((
SELECT mst_f.value AS rtt
  FROM mst_facility_setting AS mst_f
...
FROM data_middle_all 
    LEFT JOIN mst_medicine ON data_middle_all.e01 = mst_medicine.in_hospital_cd_1
)
, order_code_F AS (
SELECT DISTINCT ON (item_cd_f)* FROM (
  SELECT
...
SELECT
    detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09, login_ord, class_cd, medicine_type, medi_cd, timing_cd, procedure_cd, date_interval
FROM
    dataAndOrder
ORDER BY 
    e09,
    ...
    CASE WHEN (SELECT a1 FROM do_order_data_from WHERE no2 = 7) = ''0'' THEN login_ord
         ...
         WHEN (SELECT a1 FROM do_order_data_from WHERE no2 = 7) = ''5'' THEN procedure_cd
         WHEN (SELECT a1 FROM do_order_data_from WHERE no2 = 7) = ''6'' THEN date_interval END
limit 135', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通：透析予約繰り返し部', '2022-09-15 08:47:18.826', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -193, "field_name": "user_name", "replace_var": "@userName"}]');
