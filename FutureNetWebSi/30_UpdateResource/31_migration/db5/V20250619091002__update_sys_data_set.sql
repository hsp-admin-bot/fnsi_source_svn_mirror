...
WHERE
  ...
  AND (
    ...
    OR jsonb_array_length(COALESCE(pu.physical_info::jsonb, ''[]''::jsonb)) = 0
    OR (le.elem IS NOT NULL AND (le.elem->>''height'')::TEXT <> @content::TEXT)
  )
;', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)セコム)患者プロファイル　身長更新', '2025-05-18 22:33:06.096', CURRENT_TIMESTAMP, NULL);