...
select case
		...
		end || FLOOR( extract( EPOCH from (rst_end_date - rst_start_date) ) / 3600 )::int || ''H'' || FLOOR( mod( extract( EPOCH from (rst_end_date - rst_start_date) ), 3600 ) / 60 )::int || ''M '' || TO_CHAR(rst_start_date, ''HH24:MI'') || ''~'' || TO_CHAR(rst_end_date, ''HH24:MI'') as order_units_memo
from ord_main
	cross join dialysis_output_setting dos
where ord_no = @ordNo;', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, NULL, current_timestamp, current_timestamp, '[{"sql_cd": -307075, "field_name": "dial_diff_cd", "replace_var": "@dialDiffCd"}]'::jsonb);