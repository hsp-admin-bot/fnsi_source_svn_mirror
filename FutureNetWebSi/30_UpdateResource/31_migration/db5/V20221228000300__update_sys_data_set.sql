...
UNION ALL
 SELECT 
    ...
    TO_NUMBER( login_ord :: text, ''999999999999'' ) AS login_ord_s,
 ...
 AND do_medicine_mix_dis.login_ord_m = do_medicine_mix.login_ord
)
, order_code_S_MD AS (
SELECT item_cd_s, MIN(login_ord_s) AS login_ord_s
FROM order_code_S_M
GROUP BY item_cd_s ORDER BY login_ord_s
)
, order_code_S AS (
SELECT order_code_S_M.item_cd_s, 
CASE WHEN ''0'' in (SELECT a1 FROM do_order_data_from) THEN order_code_S_M.login_ord_s ELSE NULL END AS login_ord_s
, login_ord_medicine_mix, medicine_type_s, timing_cd_s, procedure_cd_s, date_interval_s
FROM order_code_S_M, order_code_S_MD
WHERE order_code_S_MD.item_cd_s = order_code_S_M.item_cd_s
AND order_code_S_MD.login_ord_s = order_code_S_M.login_ord_s
)
, dataAndOrder AS (
SELECT  
