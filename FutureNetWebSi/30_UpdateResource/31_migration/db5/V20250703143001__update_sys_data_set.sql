...
VALUES(-1102027, 'WITH RECURSIVE coop_ini_info AS (
--連携設定より取得
SELECT
...
FROM
  final_data
ORDER BY RP, RpItem', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'セコム連携 透析指示連携 処置依頼ファイル_処置項目の行数取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -1102004, "field_name": "pat_personal_info", "replace_var": "@patPersonalInfo"}]'::jsonb);