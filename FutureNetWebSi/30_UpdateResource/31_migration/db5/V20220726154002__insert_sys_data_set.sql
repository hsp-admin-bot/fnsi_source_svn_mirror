DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (-503,-504);
...
limit (SELECT
    (case WHEN (COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v''))=''0'' THEN 10 ELSE 108 END) AS staff_cd
     ...
     WHERE
        ...
        AND info ->> ''key2'' = ''EQUIP_OUTPUT'')', 2, '[{}]', '1', '{"applications": [4]}', NULL, '日機装)実績)中止時）医材繰り返し部', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
limit 135', 2, '[{}]', '1', '{"applications": [4]}', NULL, '日機装)実績)中止時）薬剤の投薬回数のSQL)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
