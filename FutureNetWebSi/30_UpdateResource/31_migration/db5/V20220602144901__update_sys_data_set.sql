DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (1014,1012,1013);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1014, 'SELECT
    CASE WHEN @patSex = TRIM(ini_info ->> ''value'') THEN TO_NUMBER(@patSex, ''FM9999999999999999'') 
       ELSE ''0''
    END AS check_value
  FROM
    mst_coop_ini AS ini 
  ...
  LIMIT 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)日機装)連携設定[性別変換項目]の取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
SET
  ...
  , pat_birthday = TRIM(REPLACE(''@patBirthday'',''/'','''' ))
...
WHERE
  ...
  AND facility_cd = ''@facilityCd''', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)日機装)患者プロファイル(profile)(XML):患者個人情報の取得の修正', '2022-01-07 18:21:46', '2022-06-02 06:16:48.51', '[{"sql_cd": 1009, "field_name": "up_base_disease_flg", "replace_var": "@upBaseDiseaseFlg"}, {"sql_cd": 1010, "field_name": "check_value", "replace_var": "@patBloodRhType"}, {"sql_cd": 1011, "field_name": "check_value", "replace_var": "@patBloodAboType"}, {"sql_cd": 1014, "field_name": "check_value", "replace_var": "@patSexOut"}]');
