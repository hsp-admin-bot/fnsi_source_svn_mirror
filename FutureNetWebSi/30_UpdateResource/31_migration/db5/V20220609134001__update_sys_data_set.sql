DELETE FROM "ntss"."sys_data_set" where "sql_cd" in (-494,-497);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-494, 'WITH do_order_data_from AS (
...
SELECT mst_f.value AS rtt
  ...
  WHERE mst_f.facility_setting_no = ''3006''),'',''))), ''999999999999'') AS a1) AS datt
)
, order_code AS (
SELECT  
...
SELECT 
    detail_id, item_cd, name, type_name, class_name, amount, amounttest, unit, login_ord, cl_cd, eq_cd
FROM  
    data_all
...
limit (SELECT
    (case WHEN (COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v''))=''0'' THEN 10 ELSE 108 END) AS staff_cd 
     ...
     WHERE
        ...
        AND info ->> ''key2'' = ''OUT_PUT_NUMBER'')', 2, '[{}]', '1', '{"applications": [4]}', NULL, '日機装)実績）医材繰り返し部', '2020-05-22 11:43:49.001', '2022-06-09 13:03:42.911', NULL);
...
SELECT mst_f.value AS rtt
  FROM mst_facility_setting AS mst_f 
  WHERE mst_f.facility_setting_no = ''3007''),'',''))), ''999999999999'') AS a1) AS datt
)
, order_code AS (
SELECT--投与薬剤情報(通常)
...
limit 135
', 2, '[{}]', '1', '{"applications": [4]}', NULL, '日機装)実績）薬剤の投薬回数のSQL)', '2022-05-09 05:52:21.853', '2022-06-09 13:03:42.911', NULL);
