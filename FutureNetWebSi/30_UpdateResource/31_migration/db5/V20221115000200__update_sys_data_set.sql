delete from "sys_data_set" where "sql_cd" in (-753,-852,-109,-242,-243);
...
WHERE
	...
	INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-242, 'select case  when @code = 1 THEN @userId ELSE (select disp_user_id AS disp_user_id
FROM 
    mst_user_authentication 
WHERE 
    user_id::TEXT = @userId
		and @code = 0
		) END;', 1, '[{}]', '0', '{"applications": [4]}', NULL, '透析予約：施設内職員ID(共通部)取得', '2022-07-08 02:16:54.402',CURRENT_TIMESTAMP, '[{"sql_cd": -243, "field_name": "staff_cd", "replace_var": "@userId"}, {"sql_cd": -243, "field_name": "code", "replace_var": "@code"}]');
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-243, 'with ordnocopy as (
...
WHERE
    ...
    WHERE
      ...
      pm.pat_id = @patId
  		...
  		AND ''1'' = (select * from ini_key)
  		UNION
      select ind_user_id::text as staff_cd, 0 as code from pat_rad_main
  		where  ''2'' = (select * from ini_key)
			and rad_result_cd = @ordNo
  		union
