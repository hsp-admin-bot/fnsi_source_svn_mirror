DELETE FROM "ntss"."sys_data_set" where sql_cd in (2,7,95,250);
...
select
  ...
  ,CASE 
    ...
    WHEN (rst_puncture_user_info->>''date'') ~ ''^\d{2}:\d{2}$''
      THEN (to_char(to_date(''20250909'',''YYYYMMDD''), ''YYYY-MM-DD'') || '' '' || (rst_puncture_user_info->>''date''))::timestamp
  ...
  ,CASE 
    ...
    WHEN (rst_return_user_info->>''date'') ~ ''^\d{2}:\d{2}$''
      THEN (to_char(to_date(''20250909'',''YYYYMMDD''), ''YYYY-MM-DD'') || '' '' || (rst_return_user_info->>''date''))::timestamp
