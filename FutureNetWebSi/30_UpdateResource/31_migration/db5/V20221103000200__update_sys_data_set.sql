DELETE from sys_data_set where sql_cd = 10;
...
FROM
	...
	SELECT
		...
		FROM
			...
			INNER JOIN mst_medicine AS md ON ( TO_NUMBER( medi ->> ''cd'', ''9999999999'' ) = md.medicine_cd AND md.is_del = ''0'' AND md.is_disp = ''1'' AND md.class_cd IN ( @medIds ))
		...
		FROM
			...
			INNER JOIN mst_equipment AS eq ON TO_NUMBER( eqi ->> ''cd'', ''9999999999'' ) = eq.equipment_cd  AND eq.class_cd IN (@eqIds)
