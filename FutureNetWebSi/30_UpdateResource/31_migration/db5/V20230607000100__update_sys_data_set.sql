delete from sys_data_set where sql_cd in (84,85,86,87,88,89,90,91,92,93,94);
...
select
  ...
  ,case when picked_result_params[30]->>''result_value'' is null or picked_result_params[30]->>''result_value'' = '''' then picked_input_params[30]->''item_json''->>''default_value''
   ...
   end as data30,es.sub_category_cd
   from 
  pe_array_agg 
...
select
  ...
  ,case when picked_result_params[10]->>''result_value'' is null or picked_result_params[10]->>''result_value'' = '''' then picked_input_params[10]->''item_json''->>''default_value''
   ...
   end as data10,es.sub_category_cd
   from
  pe_array_agg
  inner join pe_basicinfo on pe_array_agg.pat_event_cd = pe_basicinfo.pat_event_cd
  inner JOIN mst_pat_event_sub_category es on  pe_basicinfo.sub_category_name = es.sub_category_name
...
select
  ...
  ) as data30_score,es.sub_category_cd
  from
  pe_array_agg
  inner join pe_basicinfo on pe_array_agg.pat_event_cd = pe_basicinfo.pat_event_cd
  inner JOIN mst_pat_event_sub_category es on  pe_basicinfo.sub_category_name = es.sub_category_name 
...
select
  ...
  , score_total || '''' as score_total,es.sub_category_cd
  from
  pe_array_agg
...
select
  ...
  ,to_char(to_date(notice_start_date, ''YYYYMMDD''), ''YYYY/MM/DD'') || '' ～ '' || to_char(to_date(notice_end_date, ''YYYYMMDD''), ''YYYY/MM/DD'') as bbs_notice_term,es.sub_category_cd
  from
  pe_array_agg
  inner join pe_basicinfo_plus on pe_array_agg.pat_event_cd = pe_basicinfo_plus.pat_event_cd
  inner JOIN mst_pat_event_sub_category es on  pe_basicinfo_plus.sub_category_name = es.sub_category_name 
