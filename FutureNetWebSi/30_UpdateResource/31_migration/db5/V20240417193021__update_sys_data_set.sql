...
WITH 
    ...
    ,(row_number() over (PARTITION BY ord_main_head.ord_no ORDER BY ntss_db5_mst_sel.sortkey ASC, (ntss_db5_mst_list.cd)::integer))::text AS ctlno -- 項目番号
    ...
    ,ntss_db5_mst_list.ind_user_id AS userid -- 指示者
    ,ord_main_head.ord_no AS dialysisno --透析番号
FROM
    ord_main_head
    LEFT JOIN ntss_db5_om_1 ON ord_main_head.ord_no = ntss_db5_om_1.ord_no 
    LEFT JOIN ntss_db5_mst_list ON ntss_db5_mst_list.ord_no = ntss_db5_om_1.ord_no
    LEFT JOIN ntss_db5_mst_sel ON ntss_db5_mst_list.cd ::TEXT = ntss_db5_mst_sel.code
