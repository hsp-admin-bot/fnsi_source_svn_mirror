...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-13, 'SELECT
  ...
  COALESCE ( CASE  WHEN mtt.in_hospital_cd_a1 ='''' or  mtt.in_hospital_cd_a1 is NULL    THEN ''不明''  ELSE mtt.treatment_name END, '''' ) AS treatment_name,
  COALESCE ( CASE  WHEN mtt.in_hospital_cd_a1 ='''' or  mtt.in_hospital_cd_a1 is NULL    THEN ''-''  ELSE mtt.in_hospital_cd_a1 END, '''' ) AS treatment_cd,
...
then((COALESCE (
    RIGHT ( ''00'' || TRUNC( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ) / 60, 0 ), 2 ) || RIGHT ( ''00'' || MOD ( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ), 60 ), 2 ),
    ''0''
  )::INTEGER + COALESCE ( SUBSTRING ( (ord.ind_schedule_user_info->''ind_treat_start_time'')::text, 2, 4 ), ''0'' )::INTEGER) + 100 - 60) ::TEXT
  ...
  CASE WHEN
    ...
    RIGHT ( ''00'' || TRUNC( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ) / 60, 0 ), 2 ) || RIGHT ( ''00'' || MOD ( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ), 60 ), 2 ),
    ''0''
  )::INTEGER + COALESCE ( SUBSTRING ( (ord.ind_schedule_user_info->''ind_treat_start_time'')::text, 2, 4 ), ''0'' )::INTEGER >= 2400
  THEN
  LPAD((COALESCE (
    RIGHT ( ''00'' || TRUNC( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ) / 60, 0 ), 2 ) || RIGHT ( ''00'' || MOD ( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ), 60 ), 2 ),
    ''0''
  )::INTEGER + COALESCE ( SUBSTRING ( (ord.ind_schedule_user_info->''ind_treat_start_time'')::text, 2, 4 ), ''0'' )::INTEGER - 2400) ::TEXT, 4,''0'')
  ELSE
  (COALESCE (
    RIGHT ( ''00'' || TRUNC( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ) / 60, 0 ), 2 ) || RIGHT ( ''00'' || MOD ( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ), 60 ), 2 ),
    ''0''
  )::INTEGER + COALESCE ( SUBSTRING ( (ord.ind_schedule_user_info->''ind_treat_start_time'')::text, 2, 4 ), ''0'' )::INTEGER) ::TEXT
  END
END as end_time,
  COALESCE (
    RIGHT ( ''00'' || TRUNC( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ) / 60, 0 ), 2 ) || '':'' || RIGHT ( ''00'' || MOD ( TO_NUMBER( ord.ind_cond_info -> ''1'' ->> ''value'', ''999999'' ), 60 ), 2 ),
