...
, data_all AS (
SELECT data_middle_all.detail_id, data_middle_all.item_cd, data_middle_all.amount, data_middle_all.unit, data_middle_all.count, data_middle_all.cond_info_jyun 
, CASE WHEN data_middle_all.item_cd IN (SELECT item_cd_m FROM do_medicine_mix) THEN do_medicine_mix.mix_M_cd ELSE mst_medicine.medicine_cd END AS medicine_cd
, CASE WHEN data_middle_all.item_cd IN (SELECT item_cd_m FROM do_medicine_mix) THEN do_medicine_mix.class_M_cd ELSE mst_medicine.class_cd END AS class_cd
FROM data_middle_all 
