...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-11, 'SELECT
...
select 
CASE (select bed_code_conv from default_bed_code_conv) WHEN ''1'' THEN COALESCE(mbd.in_hospital_cd_1, ''9999999'')  WHEN ''2'' THEN COALESCE(mbd.in_hospital_cd_2, ''9999999'')  ELSE ''9999999'' end as bed_cd
from 
   ord_main AS ord
	 LEFT OUTER JOIN mst_bed AS mbd ON mbd.bed_cd = ord.rst_bed_cd
WHERE
  ord.ord_no = @ordNo 
	AND ord.is_del =''0'' 
	AND ord.facility_cd = @facilityCd ', 2, '[]', '0', '{"applications": [4]}', NULL, '富士通）透析実績：予約枠コード（bed_cd）', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
