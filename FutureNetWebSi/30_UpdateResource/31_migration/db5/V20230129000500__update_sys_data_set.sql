...
FROM(
 (--  実施コメンSOAP ト S
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_S'' ), '''' ) AS e02,
	case when ((petdt.input_params ::json ->> 0) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((pet.result_params ::json ->> 0) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when ((petdt.input_params ::json ->> 0) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((pet.result_params ::json ->> 0) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ...
  pet.pat_event_cd ::TEXT as sorttag1,
	...
	AND (petdt.input_params ::json ->> 0) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''S_FIELD'' ), '''' )
 )
	 UNION ALL
 (--  実施コメンSOAP ト A
  SELECT
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_A'' ), '''' ) AS e02,
	case when ((petdt.input_params ::json ->> 2) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((pet.result_params ::json ->> 2) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when ((petdt.input_params ::json ->> 2) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((pet.result_params ::json ->> 2) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''3'' ) AS sortTag,
	...
	AND pet.category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND petcg.sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
	AND (petdt.input_params ::json ->> 2) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''A_FIELD'' ), '''' )
) UNION ALL
 (--  実施コメンSOAP ト O
  SELECT
  ''実績詳細'' AS detail_id,
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_O'' ), '''' ) AS e02,
	case when ((petdt.input_params ::json ->> 1) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((pet.result_params ::json ->> 1) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when ((petdt.input_params ::json ->> 1) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((pet.result_params ::json ->> 1) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''2'' ) AS sortTag,
   pet.pat_event_cd ::TEXT as sorttag1,
	...
	AND pet.category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND petcg.sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
	AND (petdt.input_params ::json ->> 1) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''O_FIELD'' ), '''' )
) UNION ALL
 (--  実施コメンSOAP ト P
  SELECT
  ''実績詳細'' AS detail_id,
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_P'' ), '''' ) AS e02,
	case when  ((petdt.input_params ::json ->> 3) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((pet.result_params ::json ->> 3) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when ((petdt.input_params ::json ->> 3) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((pet.result_params ::json ->> 3) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''4'' ) AS sortTag,
   pet.pat_event_cd ::TEXT as sorttag1,
	...
	AND pet.category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND petcg.sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
		AND (petdt.input_params ::json ->> 3) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''P_FIELD'' ), '''' )
 )
 ) B
...
(COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')::INTEGER - 0.1) :: text AS sorttag , 
    ...
    ''抗凝固剤'' as aa,
		(SELECT ordernow FROM do_medicine_mix_in_orders1 
                            WHERE TO_NUMBER(mmxd ->> ''cd'', ''999999999999'') = medicine_cd::INTEGER ) AS ord_medicine_mix
FROM
    ord_main ord
LEFT OUTER JOIN
...
(COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')::INTEGER - 0.1) :: text AS sorttag , 
 ...
 select *, ROW_NUMBER() OVER()::INTEGER as ordernow from 
   ( select  detail_id::text, e01::text, e02::text, e03::text, e04::text, e05::text, e06::text, e07::text, e08::text, e09::text,e10::text,e11::text,sorttag::text,sortTag1::text,sortTag2::text,aa from kou_coag_procedu order by sorttag,ord_medicine_mix ) as kou_coag_procedu
 union all
