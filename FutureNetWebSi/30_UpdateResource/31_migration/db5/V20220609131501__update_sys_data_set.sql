...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1210, 'WITH patientrcv AS ( 
  SELECT
		1 AS order_no
    ,CASE TRIM(ini_info ->> ''value'') 
      WHEN '''' THEN COALESCE(NULLIF(TRIM(ini_info ->> ''default_v''), ''''), ''0'') 
      ELSE TRIM(ini_info ->> ''value'') 
      END AS hospitalization_dept_flg 
  FROM
    mst_coop_ini AS ini 
  ...
  SELECT
    2 AS order_no
    , ''1'' AS hospitalization_dept_flg 
  ORDER BY order_no ASC LIMIT 1
)
, cource_ward_info AS (
  SELECT 
		(''@medicalCareInfo.mainCourseCd'' :: TEXT) AS dialysis_course_cd
		,CASE WHEN ((SELECT hospitalization_dept_flg FROM patientrcv) = ''0'' or ((SELECT hospitalization_dept_flg FROM patientrcv) = ''1'' and (''@inOutClass'') = ''1''))-- ''1''：入院
...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1209, 'WITH patientrcv AS ( 
  SELECT
		1 AS order_no
    ,CASE TRIM(ini_info ->> ''value'') 
      WHEN '''' THEN COALESCE(NULLIF(TRIM(ini_info ->> ''default_v''), ''''), ''0'') 
...
VALUES ( 
  ...
  , CASE ''@oldUpDate_Date'' 
    ...
    END
  , NULL
)', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_患者基本情報の新規', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": 1002, "field_name": "pat_memo_info", "replace_var": "@patMemoInfo"}, {"sql_cd": 1003, "field_name": "infect_info", "replace_var": "@infectInfo"}, {"sql_cd": 1004, "field_name": "addition_info", "replace_var": "@additioninfo"}]');
