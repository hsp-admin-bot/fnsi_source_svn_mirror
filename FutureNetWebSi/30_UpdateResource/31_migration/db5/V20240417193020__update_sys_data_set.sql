...
, ind_cond_list AS (
    SELECT --ind_cond_info
        ...
        , ind_cond_info_json.value::JSON ->> ''value'' AS value
        , ind_cond_info_json.value::JSON ->> ''value_name_1'' AS value_name_1
        , ind_cond_info_json.value::JSON ->> ''unit'' AS unit
        , '''' AS valuecd2
        , ind_cond_info_json.value::JSON ->> ''medicine_type'' AS medicine_type
        , ind_cond_info_json.value::JSON ->> ''ind_user_id'' AS ind_user_id
        , '''' AS up_date
    FROM
        ntss_db5_om_temp
        CROSS JOIN lateral json_each_text(ntss_db5_om_temp.ind_cond_info::JSON) ind_cond_info_json
    WHERE
