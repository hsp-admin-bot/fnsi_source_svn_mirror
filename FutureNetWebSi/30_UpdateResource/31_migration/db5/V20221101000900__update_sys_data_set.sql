...
INSERT INTO ntss.sys_data_set (sql_cd, sql, db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES (1711, 'DELETE
FROM
  ord_main 
WHERE
  is_del = ''0'' 
  ...
  AND (treat_date > TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'')
    OR (treat_date = TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'') 
...
WHERE is_del = ''0''
  ...
  AND facility_cd = ''@facilityCd''
  AND (TO_CHAR(reg_exam_date, ''YYYYMMDD'') > TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'')
    OR (reg_exam_date = TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'') AND
...
WHERE is_del = ''0''
  ...
  AND facility_cd = ''@facilityCd''
  AND (TO_CHAR(reg_rad_date, ''YYYYMMDD'') > TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'')
    OR (reg_rad_date = TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'') AND
...
WHERE
  ...
  AND facility_cd = ''@facilityCd'' 
  AND (treat_date > TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'')
    OR (treat_date = TO_CHAR(TO_DATE(''@dieDate_Date'', ''YYYY-MM-DD HH24:MI:SS''), ''YYYYMMDD'') AND state.checkStatue))', 2, '[{}]', '0', '{"applications": [4]}', null, '未来日データをコピーする。透析予定(ord_main) → ord_main_restore', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, null);
