...
FROM
   ...
   ''0000000.000'' AS e10,
    ...
    --出力順
    COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-'' || ''0''||''-''||''0'' AS sorttag ,
   '''' as sorttag1,
   '''' as sorttag2,
   ''抗凝固剤手技'' as aa
   ...
   COALESCE((SELECT value FROM dialysis_item_send WHERE key2 = ''ITEM_MEASURE_MEDICINE_TAG''), '''') AS e11, 
    COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-'' || ''0''||''-''||''1''  AS sorttag ,
    '''' as sorttag1,
    '''' as sorttag2,
    ''抗凝固剤'' as aa
FROM
...
WHERE
 ord.ord_no = @ordNoand
  ...
  SELECT
    ...
    --タグ名称
    COALESCE((SELECT value FROM dialysis_item_send WHERE key2 = ''ITEM_MEASURE_MEDICINE_TAG''), '''') AS e11, 
    COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-'' || ''0''||''-''||''2''  AS sorttag ,
      '''' as sorttag1,
      '''' as sorttag2,
      ''抗凝固剤'' as aa
  FROM
 ...
 select * from (
  ( select  detail_id::text, e01::text, e02::text, e03::text, e04::text, e05::text, e06::text, e07::text, e08::text, e09::text,e10::text,e11::text,sorttag::text,sortTag1::text,sortTag2::text,aa, ROW_NUMBER() OVER()::INTEGER as ordernow from kou_coag_procedu order by sorttag )
 union all
