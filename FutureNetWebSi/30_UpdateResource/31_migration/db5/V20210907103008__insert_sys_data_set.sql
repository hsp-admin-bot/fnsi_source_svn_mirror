update "sys_data_set" set "memo" = '富士通' where "sql_cd" >=-100010 and "sql_cd" <=-100000;

delete from "sys_data_set" where "sql_cd" >=-200010 and "sql_cd" <=-200000;
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-200010, 'SELECT coalesce(( mcd.distribute_setting ->> ''protocolInfo'' ) :: json ->> ''hospPatIdLen'', ''0'') AS hosp_pat_id_len FROM mst_coop_distribute mcd WHERE mcd.facility_cd = @facilityCd  AND mcd.coop_cd = ''rep_dial''  AND mcd.coop_cd_index = ''listxml''  AND mcd.direction = ''S''', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2020-07-31 18:29:49', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-200009, 'select t.filename as filename from ( (select ''PatientInfo.xml'' as filename, 1 as key) union all (select ''pdfserverinfo.xml'' as filename, 2 as key) union all (select ''sample_001.xml'' as filename, 3 as key) union all (select ''sample_002.xml'' as filename, 4 as key) ) t where t.key = @key', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2020-07-31 18:29:49', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-200008, 'select journal.hosp_pat_id || lpad(trim(to_char(ord.ord_no, ''999999999999'')), 12, ''0'')  || lpad(trim(to_char(ord.rst_edition, ''9999'')), 4, ''0'') ||''.pdf'' as filename from sys_coop_journal journal  inner join ord_main ord on journal.ord_no = ord.ord_no where journal.ord_no = @ordNo and journal.direction = ''S'' and journal.ana_result = ''0'' and journal.is_del = ''0'' limit 1;', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2020-07-31 18:29:49', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-200007, 'select ''TAR'' || to_char(NOW(), ''YYYYMMDDHH24MISS'') || ''.tar'' as filename', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2020-07-31 18:29:49', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-200006, 'select
...
from
  ord_main as ord
where
  ord.ord_no = @ordNo', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2021-05-17 11:55:04.001', NULL);
...
is_del = ''0''
and
pat_id = @patId', 3, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2020-07-31 18:29:49', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-200000, 'SELECT ''05'' AS detail_id', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'CSI', '2020-07-31 18:29:49', '2020-07-31 18:29:49', NULL);
