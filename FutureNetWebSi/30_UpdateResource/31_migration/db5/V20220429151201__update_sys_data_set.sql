DELETE FROM "sys_data_set" WHERE sql_cd = -61;
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-61, 'WITH default_user_no AS (-- デフォルト利用者番号（透析実績用)
    ...
    SELECT 
        ...
        COALESCE( NULLIF ( MAX ( CASE part WHEN ''comm'' THEN staff_cd ELSE'''' END ), '''' ), '''' ) ||''%%%''||  ( SELECT staff_cd FROM default_user_no )  as staff_name_comm,
        COALESCE( NULLIF ( MAX ( CASE part WHEN ''data'' THEN staff_cd ELSE'''' END ), '''' ), '''' ) ||''%%%''||  ( SELECT staff_cd FROM default_user_no )  as staff_cd_data,
        COALESCE( NULLIF ( MAX ( CASE part WHEN ''data'' THEN staff_cd ELSE'''' END ), '''' ), '''' ) ||''%%%''||  ( SELECT staff_cd FROM default_user_no )  as staff_name_data
FROM
    (-- 0：共通部 版確定者
      SELECT ''comm'' AS part, staff_cd  FROM up_user_id_info WHERE ( SELECT setting FROM user_no_setting ) in (''0'')
    UNION
     -- 1：共通部 担当医１
     -- 3：共通部 担当医１
    ( SELECT ''comm'' AS part, staff_cd FROM staff_user_info WHERE ( SELECT setting FROM user_no_setting ) IN ( ''1'', ''3'' )  LIMIT 1 OFFSET 0 ) UNION
     -- 2：共通部 担当医２
     -- 4：共通部 担当医２
    ( SELECT ''comm'' AS part, staff_cd FROM staff_user_info WHERE ( SELECT setting FROM user_no_setting ) IN ( ''2'', ''4'' ) LIMIT 1 OFFSET 1 ) UNION
    -- 0：内容部 版確定者
      SELECT ''data'' AS part, staff_cd  FROM up_user_id_info  WHERE ( SELECT setting FROM user_no_setting ) in (''0'') UNION
    -- 1：内容部 担当医１
    -- 3：内容部 担当医１
...
WHERE 
    user_id = @userId', 1, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析レポート：施設内職員ID取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -61, "field_name": "staff_cd_comm", "replace_var": "@userId"}]');
