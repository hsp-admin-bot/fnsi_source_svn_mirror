...
VALUES(-504000, 'WITH ssi_in_hospital_cd AS ( 
  SELECT
    info->>''key2'' AS key2
...
SELECT
  ...
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mkr.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(mkr.in_hospital_cd_2, '''')
  END AS kur_cd1,--クール
  COALESCE(ord.rst_kur_name, '''') AS kur_name,--クール名
  COALESCE(ord.rst_dialysis_cnt, 0) AS dialysis_cnt,--透析回数
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mmc.in_hospital_cd_1 , 0)
    WHEN ''2'' THEN COALESCE(mmc.in_hospital_cd_2 , 0)
  END AS machine_no,--装置番号
  COALESCE(ord.rst_machine_name, '''') AS machine_name,--装置名
  COALESCE(ord.rst_course_name, '''') AS course_name,--診療科名
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mcs.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(mcs.in_hospital_cd_2, '''')
  END AS course_cd,--診療科コード１
  COALESCE(ord.rst_ward_name, '''') AS ward_name,--病棟名
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mwd.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(mwd.in_hospital_cd_2, '''')
  END AS ward_cd,--病棟コード１
  COALESCE(ord.rst_treatment_name, '''') AS treatment_name,--治療項目
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mtt.in_hospital_cd_a1, '''')
    WHEN ''2'' THEN COALESCE(mtt.in_hospital_cd_a2, '''')
...
CASE (SELECT value FROM get_bed_code_conv)
  ...
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mva.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(mva.in_hospital_cd_2, '''')
  END AS va_cd1,--シャントコード１
  ...
  COALESCE(TO_CHAR( TO_NUMBER( ord.rst_cond_info -> ''4'' ->> ''value'', ''FM99.00'' ), ''FM90.00'' ), '''') AS water_removal_amount_limit,
  COALESCE(ord.rst_cond_info -> ''5'' ->> ''value_name_1'', '''') AS dialyzer,
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(TRIM ( mdr.in_hospital_cd_1 ), '''')
    WHEN ''2'' THEN COALESCE(TRIM ( mdr.in_hospital_cd_2 ), '''')
  END AS dialyzer_cd1,--ダイアライザコード１
  ...
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqad.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(meqad.in_hospital_cd_2, '''')
  END AS ad_cd1,--吸着器コード１
  COALESCE(ord.rst_cond_info -> ''7'' ->> ''value_name_1'', '''') AS primary_film,  
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqpr.in_hospital_cd_1, '''') 
    WHEN ''2'' THEN COALESCE(meqpr.in_hospital_cd_2, '''') 
  END AS pr_cd1,--1次膜コード１
  COALESCE(ord.rst_cond_info -> ''8'' ->> ''value_name_1'', '''') AS secondary_film,
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqse.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(meqse.in_hospital_cd_2, '''')
  END AS se_cd1,--2次膜コード１
  COALESCE(ord.rst_cond_info -> ''9'' ->> ''value_name_1'', '''') AS puncture_needle_a,
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqa.in_hospital_cd_1, '''') 
    WHEN ''2'' THEN COALESCE(meqa.in_hospital_cd_2, '''') 
  END AS a_cd1,--穿刺針Aコード１
  COALESCE(ord.rst_cond_info -> ''10'' ->> ''value_name_1'', '''') AS puncture_needle_v,
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqv.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(meqv.in_hospital_cd_2, '''')
  END AS v_cd1,--穿刺針Vコード１
  COALESCE(ord.rst_cond_info -> ''11'' ->> ''value_name_1'', '''') AS puncture_needle_sn,
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqsn.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(meqsn.in_hospital_cd_2, '''')
  END AS sn_cd1,--穿刺針SNコード１
  ...
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(meqbc.in_hospital_cd_1, '''')
    WHEN ''2'' THEN COALESCE(meqbc.in_hospital_cd_2, '''')
  END AS bc_cd1,--血液回路コード１
...
then 
 COALESCE(ord.rst_cond_info -> ''26'' ->> ''unit'', '''') else '''' end AS anti_coagulant_one_shot_amount_unit,
  ...
  COALESCE(TO_CHAR( TO_NUMBER( ord.rst_weight_info ->> ''weight_before'', ''FM999.00'' ), ''FM990.00'' ), '''') AS weight_before,
  COALESCE(TO_CHAR( TO_NUMBER( ord.rst_weight_info ->> ''weight_after'', ''FM999.00'' ), ''FM990.00'' ), '''') AS weight_after,
  ...
  FROM
    ...
    LEFT OUTER JOIN mst_machine mmc ON mmc.machine_no = ord.rst_machine_no AND mmc.facility_cd = @facilityCd AND mmc.is_del = ''0'' AND mmc.is_disp = ''1''
WHERE
  ord.ord_no =  @ordNo', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'SSI連携）実績）透析条件', '2025-03-12 00:10:40.493', '2025-05-26 10:57:41.380', NULL);
