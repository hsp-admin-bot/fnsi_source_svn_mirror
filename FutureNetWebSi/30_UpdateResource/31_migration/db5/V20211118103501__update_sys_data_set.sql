delete from "sys_data_set" where "sql_cd" in (-99987,-11,-38);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-99987, 'SELECT
  ''Karte_'' || 
...
WHERE
  journal.ctl_no = @ctlNo', 2, '[]', '0', '{"applications": [4]}', NULL, 'SSI カルテ記載連携[送信]ファイル名取得', '2021-04-20 09:19:08.001', '2021-04-20 09:19:12', NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-38, 'SELECT
  CASE journal.crud 
    WHEN ''C'' THEN to_char(ord.rst_start_date, ''YYYYMMDD'') --透析開始日時
    ELSE to_char(ord.up_date, ''YYYYMMDD'') --更新日時
    END AS inp_date --入力日
  , CASE journal.crud 
    WHEN ''C'' THEN to_char(ord.rst_start_date, ''HH24MI'') --透析終了時刻
    ELSE to_char(ord.up_date, ''HH24MI'') --更新時刻
...
WHERE
  ord.ord_no = @ordNo', 2, '[{}]', '0', '{"applications": [4]}', NULL, 'SSI カルテ記載連携）入力日と入力時間と入力者取得', '2020-03-27 10:46:07', '2020-03-27 10:46:10', NULL);
