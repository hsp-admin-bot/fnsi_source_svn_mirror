DELETE FROM "ntss"."sys_data_set" where sql_cd in (- 2120, - 2130, - 2140, - 2150, - 2161, - 2162, - 2163, - 2164, - 2170, - 2180, - 2190, - 2200, - 2210, - 2010, - 2020, - 2021, - 2030, - 2040, - 2041, - 2042, - 2050, - 2060, - 2070, - 2080, - 2090, - 2100, - 2110, - 2220, - 2230, - 2231, - 2240, - 2250, - 2260, - 2270, - 2280, - 2291, - 2300, - 2450, - 2071, - 2440, - 2430, - 2420, - 2410, - 2240, - 2164, - 2160);
...
WHERE
    ...
    AND ntss_db5_mst_m.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
...
WHERE
    ...
    AND ntss_db6_ppm.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'');',3,'[]','1','{"applications": [5]}','{"classes": []}','患者情報1：患者イベント テキスト　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2162,'SELECT
...
WHERE
    ...
    AND ntss_db6_mst_opp.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')',3,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["userid,patid"]}','2026/02/01 17:51:54.726','2026/02/01 17:51:54.726',null)
           ...
           AND (
                CASE
                    WHEN @syncMode = ''update''
                        THEN (
                        (
                            ntss_db5_pm.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
                        )
                        OR (
                            ntss_db5_pm_mst_wc.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
                        )
                    )
                    else ntss_db5_pm.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
                    end
            )
...
',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid,userid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2150,'SELECT
    ...
    , '''' AS hosppatid                           --実施者コード
    , cast(
        ntss_db5_om_iic_json ->> ''upd_user_last_name'' AS char (20)
    ) || cast(
        ntss_db5_om_iic_json ->> ''upd_user_first_name'' AS char (20)
    ) AS staffname                              --実施者名
FROM
    ord_main ntss_db5_om
    LEFT JOIN ord_schedule ntss_db5_os
...
WHERE
  ...
  , (-2420,'WITH ntss_db5_mm AS (
			...
			WHERE ntss_db5_mm.facility_cd = @facilityCd
				...
				AND ntss_db5_mm.up_date BETWEEN to_timestamp( @fromDate, ''YYYYMMDDHH24MISS'' )
				AND to_timestamp( @toDate, ''YYYYMMDDHH24MISS'' )
		)
		SELECT
			ntss_db5_mst_b.in_hospital_cd_1 AS bedno --ベッド番号
  ...
  , (-2450,'WITH ntss_db5_pm_dsi AS (
			...
			WHERE ntss_db5_pm.is_del = ''0''
				...
				AND ntss_db5_pm.up_date BETWEEN to_timestamp( @fromDate, ''YYYYMMDDHH24MISS'' )
				AND to_timestamp( @toDate, ''YYYYMMDDHH24MISS'' )
		)
		SELECT
			'''' AS hosppatid --患者ID
...
AND ntss_db5_mnt_ws.up_date BETWEEN to_timestamp( @fromDate, ''YYYYMMDDHH24MISS'' )

AND to_timestamp( @toDate, ''YYYYMMDDHH24MISS'' );',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": [""]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2270,'SELECT
...
WHERE
	...
	AND ntss_db5_pxm.up_date BETWEEN to_timestamp( @fromDate, ''YYYYMMDDHH24MISS'' )
...
WHERE
    ...
    );',3,'[]','1','{"applications": [5]}','{"classes": []}','患者基本情報：@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid","doctorcd1","doctorcd2","staffcd1","staffcd2","dialdiffcd","basediseasecd","transportcd"]}','2021/02/26 17:51:54','2021/02/26 17:51:54',null)
  , (-2020,'SELECT
    '''' AS hosppatid                             --患者ID
    , '''' AS names                               --氏名
...
WHERE
    ntss_db5_pm.is_del = ''0''
    AND ntss_db5_pm.facility_cd = @facilityCd
    AND ntss_db5_pm.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'');',2,'[]','1','{"applications": [5]}','{"classes": []}','患者情報1：患者イベント テキスト　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2220,'SELECT
    '''' AS hosppatid                             --患者ID
    , ntss_db5_om.pat_id AS patid
    , ntss_db5_om.treat_date AS dialysisdate    --透析日
...
FROM
    (
        ...
        WHERE
            1 = 1
            AND ntss_db5_om_1.facility_cd = @facilityCd
            AND ntss_db5_om_1.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
        GROUP BY
            ntss_db5_om_1.ord_no
            , ntss_db5_om_1.pat_id
            , ntss_db5_om_1.treat_date
...
',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2030,'SELECT
		...
		AND to_timestamp( @toDate , ''YYYYMMDDHH24MISS'' );',3,'[]','1','{"applications": [5]}','{"classes": []}','患者情報1：患者イベント テキスト　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2040,'SELECT
    '''' AS hosppatid --患者ID
  ...
  , (-2042,'SELECT
		...
		WHERE
			...
			AND ntss_db5_mst_p.up_date
				BETWEEN to_timestamp( @fromDate, ''YYYYMMDDHH24MISS'' )
				AND to_timestamp( @toDate, ''YYYYMMDDHH24MISS'' );',3,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["userid"]}','2021/02/26 17:51:54.726','2021/08/31 17:51:54.726',null)
  , (-2050,'SELECT
    '''' AS hosppatid                             --患者ID
    ...
    AND (
        CASE
            WHEN @syncMode = ''update''
                ...
                OR (
                    ntss_db5_mst_mi.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
                )
            )
            else ntss_db5_pm.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
            end
    )
    AND ntss_db5_mst_mi.in_hospital_cd_1 IS NOT NULL;',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2060,'with ntss_db5_mst_add as (
    SELECT
        ...
        ,ntss_db5_mst_addition.up_date AS up_date
    ...
    AND (
        CASE
            WHEN @syncMode = ''update''
                ...
                OR (
                    ntss_db5_mst_add.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')
                )
            )
            else ntss_db5_pm.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'' ) AND to_timestamp( @toDate , ''YYYYMMDDHH24MISS'' )
            end
...
FROM
    ...
    LEFT JOIN ntss.mst_medicine ntss_db5_op_mst_m
...
WHERE
    ...
    AND ntss_db5_op.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS'')',2,'[]','1','{"applications": [5]}','{"classes": []}','患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}','2021/02/26 17:51:54.726','2021/02/26 17:51:54.726',null)
  , (-2280,'SELECT
