...
SELECT
  CASE
    WHEN ord.ind_bed_cd IS NULL THEN ''9999999''
    ELSE 
      CASE 
        WHEN (SELECT bed_code_kbn FROM bed_code_info) = ''1'' 
          THEN COALESCE(mb.in_hospital_cd_1, ''9999999'') 
        WHEN (SELECT bed_code_kbn from bed_code_info) = ''2'' 
          THEN COALESCE(mb.in_hospital_cd_2, ''9999999'') 
        ELSE ''9999999'' 
      END 
...
WHERE
  ...
  ord.is_del = ''0''', 2, '[]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：予約枠コード', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
