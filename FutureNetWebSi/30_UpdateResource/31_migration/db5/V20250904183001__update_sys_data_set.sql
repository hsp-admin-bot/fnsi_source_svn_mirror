...
VALUES(9629, 'WITH tabooAllergy AS (
	SELECT
		( idx - 1 ) AS idx,
		ms ->> ''memo'' AS memo,
		ms ->> ''ctl_no'' AS ctl_no,
...
SELECT COALESCE (
	...
	FROM (SELECT ROW_NUMBER() OVER (order by disp_order) AS no,*
		FROM (
			select DISTINCT ON (CONTENT, memo_class)
			memo,
			ctl_no,
			CONTENT,
			disp_order :: INTEGER,
...
WHERE is_del = ''0''
  AND pat_id = @patId 
  AND facility_cd = ''@facilityCd''', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)日機装の患者プロファイル(アレルギー情報)', '2023-05-31 17:24:39.000', CURRENT_TIMESTAMP, NULL);
