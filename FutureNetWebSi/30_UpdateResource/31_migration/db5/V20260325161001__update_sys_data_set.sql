...
WHERE sql_cd IN (-604167,-604168,-604175,-604178);
...
SELECT
  ...
  case (mix_infoes ->> ''solvent'')
    when ''1''
    then (mix_infoes ->> ''amount'') :: numeric --数量固定薬剤
    else (mix_infoes ->> ''amount'') :: numeric * (medi ->> ''amount'') :: numeric --数量固定でない薬剤
    end as amount,
  ''06'' AS detail_id
FROM
  ord_main AS ord
...
SELECT
  ...
  CASE (mix_infoes ->> ''solvent'')
    WHEN ''1''
    THEN (mix_infoes ->> ''amount'') :: numeric --数量固定薬剤
    ELSE (mix_infoes ->> ''amount'') :: numeric * (info.tmedi ->> ''amount'') :: numeric --数量固定でない薬剤
    END AS amount,
  ''08'' AS detail_id
FROM
  rst_treatment_info AS info
