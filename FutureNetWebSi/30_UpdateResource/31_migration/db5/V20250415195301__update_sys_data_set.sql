...
, inspection_records as (
  ...
  WHEN wst.value <> '''' THEN
    (
      CASE
        WHEN position(''.'' in wst.value) = 0 THEN wst.value || ''.'' || repeat(''0'', wstt.decimal_digits)
        WHEN length(split_part(wst.value, ''.'', 2)) < wstt.decimal_digits THEN
          wst.value || repeat(''0'', wstt.decimal_digits - length(split_part(wst.value, ''.'', 2)))
        ELSE wst.value
      END
      || coalesce(wst.unit, '''')
      || (
        CASE
          WHEN wst.text IS NOT NULL AND wst.text <> ''0'' THEN
            (
              SELECT jsonb_array_elements(wstt.initial_string::jsonb)->>''text''
              LIMIT 1 OFFSET CAST(wst.text AS INTEGER) - 1
            )
          ELSE ''''
        END
      )
    )
  WHEN (wst.memo <> '''' OR wst.time <> '''' OR CAST(wst.picker AS INTEGER) != 0 OR CAST(wst.inspector AS INTEGER) != 0)
    THEN ''検査中''
  WHEN wst.text IS NOT NULL AND wst.text <> ''0'' THEN
    (
      SELECT jsonb_array_elements(wstt.initial_string::jsonb)->>''text''
