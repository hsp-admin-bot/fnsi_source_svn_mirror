DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (1714, 1715);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1714, 'WITH exam_date_tmp AS ( 
    SELECT CASE WHEN ''@physicalInfo.ctr'' <> '''' THEN NULLIF(''@physicalInfo.examDate_CTR_GMTDate'', '''') ELSE 
       (CASE WHEN ''@physicalInfo.height'' <> '''' THEN NULLIF(''@physicalInfo.examDate_height_GMTDate'', '''') ELSE '''' END ) 
    END AS exam_date,
      CASE WHEN ''@physicalInfo.examDate_CTR_GMTDate'' = ''@physicalInfo.examDate_height_GMTDate'' THEN 0 ELSE 1 END AS do_height_flag
) 
, exam_date_info AS ( 
  SELECT
...
, data_new_info AS (
  SELECT 
    ...
    NULL AS memo,
    NULL AS ctl_no,
    CASE WHEN (SELECT do_height_flag FROM exam_date_tmp) = 0 THEN TRIM(NULLIF(''@physicalInfo.height'', ''''), ''0'') ELSE NULL END AS height,
...
WHERE
  ...
  AND (''@physicalInfo.ctr'' <> '''' OR ''@physicalInfo.height'' <> '''')', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_身体情報', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, NULL);
...
, data_check AS (
  SELECT
    CASE WHEN info ->> ''exam_date'' = (SELECT exam_date FROM exam_date_info) THEN 0 ELSE 1 END AS height_date_flag
  FROM
    pat_unique patu 
    CROSS JOIN LATERAL json_array_elements(patu.physical_info ::json) AS info 
  WHERE
...
WHERE
  ...
  AND (SELECT height_date_flag FROM data_check) = 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_身体情報', '2022-06-10 10:47:32.927', CURRENT_TIMESTAMP, NULL);
