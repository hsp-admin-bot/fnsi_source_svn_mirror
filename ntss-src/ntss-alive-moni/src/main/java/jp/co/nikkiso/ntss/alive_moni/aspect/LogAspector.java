package jp.co.nikkiso.ntss.alive_moni.aspect;
...
@Aspect
@Component("LogAspectorAliveMoni")
...
// #9698 アプリケーションログの内容修正 20260328 add yangxuewang start
  @Pointcut(value = "within(@org.springframework.web.bind.annotation.RestController *) && within(jp.co.nikkiso.ntss.alive_moni.resource..*)")
  ...
  private void saveLog(ProceedingJoinPoint joinPoint, Object result, long costTime, Exception exception) {
    EventLogMessage eventLogMessage = aspectorLogEventLogMessage(joinPoint,result,costTime,exception,"ntss-alive-moni");
    LogLevel level;
    if(exception!=null){
...
// #9698 アプリケーションログの内容修正 20260328 add yangxuewang end
  @Pointcut(value = "execution (* jp.co.nikkiso.ntss.alive_moni.service..*.*(..))")
...
// #9698 アプリケーションログの内容修正 20260327 add yangxuewang start
  ...
  public Object cutDaoPoint(ProceedingJoinPoint pjp) throws Throwable {
    ...
    try {
      ...
      map.put("LogAspectorAliveMoni","SQL-LOG");
      ...
      logService.log(LogLevel.INFO,eventLogMessage , "", LoggingConstant.SERVICE_NAME.REMS, null);
    } catch (Exception e) {
      map.put("Exception",e.getMessage());
      eventLogMessage.setLogMessage(toJson(map));
      logService.log(LogLevel.ERROR, eventLogMessage, "", LoggingConstant.SERVICE_NAME.REMS, null);
      throw e;
    }
    return object;
  }
