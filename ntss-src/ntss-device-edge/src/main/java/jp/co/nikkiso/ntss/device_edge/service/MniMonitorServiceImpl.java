...
//            if (!Objects.isNull(dev)) {
            if (node.hasNonNull("dev")) {
              JsonNode dev = node.get("dev");
              String treatMode = dev.get("15").textValue();
              if (!Objects.isNull(treatMode) && treatMode.equals("9") && dataType.equals((short)1)) {
      ...
      // del FNSI-バグ 通信サーバ #8176 高 start
      // mntMachineStateDao.updateMachineState(state);
      // del FNSI-バグ 通信サーバ #8176 高 end
