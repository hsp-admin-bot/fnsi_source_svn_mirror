...
set
    ...
    ind_sch_info = CASE WHEN tmp.indSchInfo IS NULL THEN ind_sch_info ELSE  jsonb_merge_recursive(ind_sch_info, CAST(tmp.indSchInfo AS jsonb)) END,
...
--     mod 10495 無期限治療条件変更時のpat_treatment_patternの更新バグ 関 end
    ...
    ind_tare_info = CASE WHEN tmp.indTareInfo IS NULL THEN ind_tare_info ELSE jsonb_merge_recursive(ind_tare_info, CAST(tmp.indTareInfo AS jsonb)) END,
    ind_off_water_info = CASE WHEN tmp.indOffWaterInfo IS NULL THEN ind_off_water_info ELSE jsonb_merge_recursive(ind_off_water_info, CAST(tmp.indOffWaterInfo AS jsonb)) END,
    ind_device_set_info = CASE WHEN tmp.indDeviceSetInfo IS NULL THEN ind_device_set_info ELSE jsonb_merge_recursive(ind_device_set_info, CAST(tmp.indDeviceSetInfo AS jsonb)) END,
