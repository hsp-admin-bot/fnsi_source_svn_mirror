WITH ord_main_temp AS (
  ...
  from ord_main
  where ord_no = /*ordNoTemp*/'9928776'
)
,cond_data_ord AS (
  SELECT
...
,ord_main_data AS (
  SELECT ord_no,
    ...
    up_date
  from ord_main
  where ord_no in /*ordNoList*/('9928776','9967237','9967239')
)
,ord_data_all as (
  SELECT
    ...
    omd.facility_cd,
    omd.pat_id,
    omd.treat_date AS supplies_base_date,
    omd.ord_no AS supplies_base_no,
    omd.reg_date,
    omd.up_date,
    oda.*
  FROM (
  ...
  ) oda cross join ord_main_data omd
)
