...
from
 ord_main A
 INNER JOIN shr_pat s ON s.facility_cd = A.facility_cd AND s.pat_id = A.pat_id
 inner join (
   (select distinct
     (o.treat_date) as treat_date
   from
     ord_main o
     INNER JOIN shr_pat s ON s.facility_cd = o.facility_cd AND s.pat_id = o.pat_id
   where
     o.treat_date >= /*baseDate*/'20260310'
   order by
     o.treat_date
   limit
     /*period*/0
