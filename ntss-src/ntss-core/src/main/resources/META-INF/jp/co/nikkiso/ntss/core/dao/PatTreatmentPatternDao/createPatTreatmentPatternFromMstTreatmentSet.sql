    ...
    COALESCE(
      jsonb_agg(
        jsonb_build_object(
          'cd', (e->>'cd')::numeric,
          'amount', e->>'amount',
          'equip_type', COALESCE((e->>'equip_type')::int, 0)
        ) || user_info.user_json
      ),
      '[]'::jsonb) AS ind_equip_info
  FROM treat_set ts
    CROSS JOIN LATERAL jsonb_array_elements(COALESCE(ts.ind_equip_info::jsonb, '[]'::jsonb)) e
...
-- medi start --
,medi_lock AS (
  SELECT medi_info_no FROM medicine_latest_no
  WHERE facility_cd = /*dto.facilityCd*/'NKKSBR'
    AND pat_id = /*dto.patId*/'160030'
  FOR UPDATE
)
,medi_json AS (
  SELECT
...
,final_base as (
  SELECT
    ...
    e.key,
    CASE
      WHEN e.key IN ('15','19','25') AND jsonb_typeof(e.value) = 'object' AND NOT jsonb_exists(e.value::jsonb, 'medicine_type') THEN
        e.value
        || jsonb_build_object(
             'medicine_type',
             'null'::jsonb
...
SELECT
  ts.treatment_cd as ind_treatment_cd
  ,sj.ind_schedule_user_info as ind_sch_info
  ,cj.ind_cond_info
  ,mj.ind_medi_info
  ,ej.ind_equip_info
