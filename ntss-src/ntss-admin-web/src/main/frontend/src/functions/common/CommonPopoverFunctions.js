...
export const popoverPostShow = function(event) {
  ...
  if (upTriObj.length > 0 && popoverObj.length > 0 && contentObj.length > 0) {
    ...
    if (popoverObj[0].style.visibility !== "hidden") {
      ...
      upTriObj[0].classList.remove('disp_target_popover__arrow');
      popoverObj[0].classList.remove('disp_target_popover');
      contentObj[0].classList.remove('disp_target_popover__content');
    } else if (popoverObj[0].style.visibility === "hidden") {
      // リサイズ処理の為、要素判別用のclassを付与する
      upTriObj[0]?.classList?.add('disp_target_popover__arrow');
      popoverObj[0]?.classList?.add('disp_target_popover');
      contentObj[0]?.classList?.add('disp_target_popover__content');
      // 表示時初回リサイズ
      popoverResize();
...
const popoverResize = function(event) {
  const upTriObj = document.getElementsByClassName("disp_target_popover__arrow");
  const popoverObj = document.getElementsByClassName("disp_target_popover");
  const contentObj = document.getElementsByClassName("disp_target_popover__content");
 ...
 */
const popoverEventResize = function(event) {
