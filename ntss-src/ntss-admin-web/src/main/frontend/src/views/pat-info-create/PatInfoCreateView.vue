...
import MainComponent from "@/components/pat-info/PatInfoCreateContent";
...
export default {
  ...
  mixins: [ViewHelper],
  //add 8397 【デグレ】患者情報編集中に画面遷移した際、内容破棄確認メッセージが出ない 周安寧 start
  async beforeRouteLeave(to, from, next) {
    try {
      if (to.name != "signin" && this.hasEditedComponent) {
      ...
      getErrorMessage('PatInfoCreateView.vue','beforeRouteLeave',error);
      next();
    }
  },
  //add 8397 【デグレ】患者情報編集中に画面遷移した際、内容破棄確認メッセージが出ない 周安寧 end
  data() {
    return {
  ...
  methods: {
    ...mapMutations("pat-info", ["setIsPatInfoChaned", "resetEditedComponent"]),
  }
};
</script>
