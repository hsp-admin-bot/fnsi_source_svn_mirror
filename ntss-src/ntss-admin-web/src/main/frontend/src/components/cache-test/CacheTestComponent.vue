...
<template>
  <div class='main-content-area'>
    <div>
      ...
      <!-- Script変更確認用ダイアログ -->
      <message-dialog
        :visible.sync="isCacheTestDialog"
        :message-cd="22010001"
...
export default {
  ...
  data() {
    return {
      ...
      stringParams: [],
      css_src_test_module: require("./css_sample/test_module.css"),
      image_src_test_module: require("./img_sample/test_module.png"),
      audio_src_test_module: require("./audio_sample/test_module.wav"),
  ...
  methods: {
    ...
    paddingText(input){
      document.getElementById("js-input").value = this.paddingInput(input);
    },

    // 非モジュール化CSS適用
    readCss() {
      let testNoModule = document.createElement("link");
      testNoModule.rel = "stylesheet";
      testNoModule.href = "css/css_sample/test_no_module.css";
      document.head.appendChild(testNoModule);
    },
    ...
    testBtn_03() {
      let thisObj = this;
      webPushSubscribe(this.publicKey)
      .then(response => {
