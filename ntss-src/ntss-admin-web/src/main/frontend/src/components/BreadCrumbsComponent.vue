...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      // }
      histories: {
        handler: 'createHistoryList',
        deep: true
      },
      historyList:{
        handler(newVal,oldVal){
            this.$nextTick(()=>{
              let domdata = this.$refs.mbx
              let data = []
              domdata.forEach(ele=>{
    ...
    created() {
      this.depth = this.getHistory().find(
        e => e.historyKey === this.historyKey
      ).depth;
    ...
    methods: {
      ...
      // this.createHistoryList();
        ...
        const shownHistories = JSON.parse(JSON.stringify(this.histories))
          .reverse()
          .filter((value, index) => {
        ...
        let histories = JSON.parse(JSON.stringify(this.histories))
        let routerName = this.keepHistories[0]?.routerName || "";
        // パンくずリストに表示する対象要素を決定する
        histories.forEach(item => {
          if ((['INFO', 'PAT_EVENT' , 'EXAM_RECORD_DETAIL'].includes(item.historyKey)) && ((item.historyKey == 'PAT_EVENT') || (item.historyKey == 'INFO')) && (routerName === 'exam-record-detail')) {
          item.depth = 1,
        ...
        } else if (shownHistories[0].depth === this.depth) {
        ...
        const targetRouters = ["pat-viewer", "pat-calendar", "bbs-info", "split-graph", "multi-pat-list", "facility-calendar"];
        ...
        this.$nextTick(() => {
          ...
          //   `breadcrumb-content-${this.depth}`
          // ).scrollLeft = 1000;
          if (document.getElementById(`breadcrumb-content-${this.depth}`) && document.getElementById(`breadcrumb-content-${this.depth}`).scrollLeft) {
            document.getElementById(
              `breadcrumb-content-${this.depth}`
            ).scrollLeft = 1000;
          }
        ...
        this.historyList.forEach((item, index) => {
          if (['EXAM_RECORD_LIST' , 'EXAM_RECORD_DETAIL'].includes(item.historyKey) && (item.historyKey == 'EXAM_RECORD_DETAIL') && (this.routerName == 'exam-record') && (routerName === 'exam-record-detail')) {
      ...
      async goBack(routerName) {
        ...
        if (routerName == 'view-log' ) {
          this.$parent.$parent.$refs.mainComponent.createTab(this.$parent.$parent.$refs.mainComponent.getCondition);
          this.$parent.$parent.$refs.mainComponent.onGetTabData();
          await this.$parent.$parent.$refs.mainComponent.getUser();
        }
        if (routerName == 'facility-calendar' && this.historyList.length === 1 ) {
          this.$parent.$parent.$refs.mainComponent.createCalendarContentsForMasterLayoutSearch();
        }
        if (routerName == 'split-graph' && this.historyList.length === 1 ) {
          this.$parent.$parent.$refs.mainComponent.init();
        }
        if (routerName == 'pat-info' && this.historyList.length === 1) {
          this.$parent.$parent.$refs.mainComponent.$refs.cardList?.refreshData();
        }
        if (routerName == 'pat-info-create' && this.historyList.length === 1) {
          this.$parent.$parent.$refs.mainComponent.$refs.cardListCreate?.refreshData();
        }
        if (routerName == 'bbs-info') {
          // 施設カレンダー＞掲示板、掲示板 の掲示板パンくずリスト押下時はsearch()を実行する。掲示板＞各機能の場合はsearch()が存在しないのでtypeerror発生
          if (typeof this.$parent.$parent.$refs.mainComponent.search === "function") {
            this.$parent.$parent.$refs.mainComponent.search();
          }
        }
        ...
        if (routerName == 'pat-prescription') {
          this.$parent.$parent.$refs.mainComponent.$refs.detail?.refresh();
        }
        // 画面分割される画面で元の画面となるルート名
        const splitPageRouterName = ['status-list', 'operation-viewer-general-machines', 'device-edge-operation',
          'operation-viewer-admin-facilities', 'operation-viewer-admin-machines',
          'operation-viewer-admin-motion-record', 'operation-viewer-general-motion-record', 'pat-group'];
        if (routerName == this.historyList[this.historyList.length - 1].routerName ) {
