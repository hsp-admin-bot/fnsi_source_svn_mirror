...
<template>
  ...
  <!-- <v-ons-row> -->
    <v-ons-row :class="getIsUseFlagSecondPass ? 'cell-disabled' : ''">
  <!-- mod 9664補液及び透析液仕様修正します yangqingzhe end -->
    <v-ons-col class="action-condition-column">2次膜</v-ons-col>
    <common-master-selector
      ...
      :extraParams="{ treatDate: mstExtraParams.treatDate, classType: filmClassType, initValue: displayInputValue.initCd, actualName: rstNameForCd }"
      ...
      :btnDisabled="getIsUseFlagSecondPass || !getItemAuthorized('Indication', 'default_authority')"
...
export default {
  ...
  computed: {
    ...
    // add 8204 周安寧 start
    ...mapGetters("pat-viewer-treat-cond", {getIsUseFlagSecondPass: "getIsUseFlagSecondPass"}),
    // add 8204 周安寧 end
    rstDialysisState() {
  ...
  methods: {
    ...
    async createPopoverData() {
      ...
      const popover = await buildMasterPopover(
        ...
        getErrorMessage("IndTreatCondSecondPass.vue", "createPopoverData", error);
        throw error;
      });
    ...
    async createPopoverDataLegacy() {
      ...
      const filterParam = item => {
        return item.classType === 5 || item.classType === 6;
      };
      ...
      const contentMapping = item => {
        return {
          ...
          text: rstName && item.equipmentCd == this.localSelectedCd ? rstName : getPrefix({
            ...
            normalClassType: [CODES.EQUIPMENT_CLASS.ADSORBER.classType, CODES.EQUIPMENT_CLASS.SEPARATOR.classType],
            ...item
          }) + item.equipmentName,
          // #10659 禁忌、アレルギー、削除済み、分類不一致、期限切れ、削除済み含むの接頭文字対応 linjunfeng end
        };
