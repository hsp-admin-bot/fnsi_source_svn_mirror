...
export default {
  ...
  watch: {
    ...
    isSingleNeedle: {
      handler(val) {
        this.refreshKey++;
        const currentInit = this.getNeedleInit[this.type];
        if (currentInit == null) {
          this.setNeedleInit({
            type: this.type,
            value: this.displayInputValue.initValue
          });
        }
      },
      immediate: true
  ...
  methods: {
    ...
    async createPopoverData() {
      ...
      const popover = await buildMasterPopover(
        ...
        getErrorMessage("IndTreatCondNeedleSN.vue", "createPopoverData", error);
        throw error;
      });
