...
<template>
  <div>
    <base-content
      :disp-data-list="ufrProgramDataList"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("pat-viewer", ["getDataListKeepUFRProgram", "getPatIdKeep", "getPatIdKeepChgFlg"]),
  ...
  async created() {
    ...
    this.convertUfrProgramData({
      listIndex: this.rowIndex,
      selectLayoutCd: this.selectedLayoutCd
    }).then(ufrProgramDataListLet => {
      this.ufrProgramDataList = ufrProgramDataListLet;
    }).finally(() => {
  ...
  methods: {
    ...mapActions("pat-viewer", [
      "convertUfrProgramData",
      "getMstRecordInState"
    ]),
    // add 1006-398 指示の切り替わりポイントを赤くする 陳 start
    ...mapActions("pat-viewer", ["setPatIdKeep", "setDataListKeepUFRProgram", "setPatIdKeepChgFlg"]),
    // add 1006-398 指示の切り替わりポイントを赤くする 陳 end
    ...
    onSubTitleClick(event, rowInfo) {
      ...
      // settingData.headerTitle = MODAL_TITLE["UFRプログラム"];
      settingData.headerTitle = MODAL_TITLE["除水プログラム"];
      ...
      // モーダル表示
      this.showUfrProgramEditModal({
        // ベースからslotで表示される子コンポーネント
        dispComponentId: "pat-device-program-ufr",
