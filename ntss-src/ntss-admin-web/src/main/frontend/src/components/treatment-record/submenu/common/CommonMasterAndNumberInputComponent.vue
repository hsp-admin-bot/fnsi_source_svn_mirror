...
<template>
  ...
  <v-ons-row v-if="typeNo === 0">
    <v-ons-col class="title">
      <label class="theme">
        {{ themeLable + (modelValue.name != null ? modelValue.name : "") }}
      </label>
    </v-ons-col>
    <v-ons-col class="num-value">
      ...
      <v-ons-button
        ...
        @click="createPopoverData(modelValue.cd)"
    ...
    <div style="display: flex; flex-wrap: wrap; align-items: center; width: 100%;">
      ...
      <div style="display: flex; justify-content: flex-end; align-items: center; flex: 1; min-width: 200px;">
        <custom-input-number-pro
          ...
          :min="inputMin"
          :step="step"
          :disabled="disabled || !modelValue.name"
          :invalidArray="invalidArrayMethod()"
          @handlerInput="handlerInput"
          @blur="handlerBlur"
          :required="modelValue.cd && required"
        />
        ...
        <common-master-selector
          ...
          :initItem="{ text: modelValue.name, value: modelValue.cd, unit: unitName }"
          :editItem="{ text: modelValue.name, value: modelValue.cd, unit: unitName }"
          :extraParams="{ treatDate: treatDate, rstInfo: { rstName: modelValue.name, rstUnit: unitName }, medicineType: medicineType }"
...
export default {
  ...
  props: {
    ...
    /** 1:通常薬剤 2:調製薬剤（MEDICATION_TREATMENT_RECORD の init 振り分け用） */
    medicineType: {
      type: [Number, String],
      default: null
    }
  ...
  data() {
    return {
      ...
      // 車いす重量をg⇒kgに変換して表示する必要があるので初期化の際も必要 (this.modelValue.value / this.base) 
      // Mixinのthis.decimalLengthはこの時点では評価されないので代わりにdecimalPlacesで小数部桁数を取得する
      currentValuePro: this.modelValue.value != null ? (this.modelValue.value / this.base).toFixed(BigNumber(this.step).decimalPlaces()) : this.modelValue.value,
    }
  },
  watch: {
    "modelValue.cd" (val) {
      if (!val) {
        ...
        this.currentValuePro = this.modelValue.value != null ? (this.modelValue.value / this.base).toFixed(this.decimalLength) : null;
      }
      this.$emit(
        "update:modelValue",
        new MasterAndNumber(val, this.modelValue.name, (typeof this.currentValuePro === "number" || (typeof this.currentValuePro === "string") && this.currentValuePro !== "") ? (this.currentValuePro * this.base) : null));
      this.customInputNumberProKey++;
    }
  },
  mounted () {
    this.initNum = this.modelValue
  },
  methods: {
    invalidArrayMethod() {
      const invalidNumber = 0;
      if (!this.modelValue.cd) {
        return [];
      }
    ...
    updateInput(data) {
      ...
      this.$emit(
        "update:modelValue",
        new MasterAndNumber(data.value, data.text, this.modelValue.value));
      if(Object.prototype.hasOwnProperty.call(data, "fnValue")) {
        if(Object.prototype.hasOwnProperty.call(data.fnValue, "薬剤区分")) {
