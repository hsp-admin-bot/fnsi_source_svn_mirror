...
export default function (args) {
  ...
  if (!("map" in Array.prototype)) {
    Array.prototype.map = function(mapper, that) {
      const other = new Array(this.length);
      for (let i = 0, n = this.length; i < n; i++)
  ...
  Map.prototype.fitSize = function() {
    ...
    if (!this.options.width && !this.options.height) {
      ...
      this.options.height =
        (this.base.height * this.options.width) / this.base.width;
    } else if (!this.options.width && this.options.height) {
      this.options.width =
        (this.base.width * this.options.height) / this.base.height;
    }
    ...
    if (
      ...
      this.size.height =
        (this.options.width * this.base.height) / this.base.width;
    } else {
      this.size.width =
        (this.options.height * this.base.width) / this.base.height;
  ...
  Map.prototype.addEvent = function() {
    const self = this;
    const _target = this.element;

    if (
      (_ua.Pointer && !isWinDesktop) ||
      (_ua.MSPointer && !isWinDesktop) ||
      _ua.Touch
    ) {
      if (_ua.Pointer || _ua.MSPointer) {
        getComputedStyle(_target)["-ms-touch-action"] = "none";
        getComputedStyle(_target)["touch-action"] = "none";
      }

      _target.addEventListener(_start, function(e) {
        let point = e;

        self.pointer = {
          x: point.touches[0].pageX - _target.getBoundingClientRect().left,
          y: point.touches[0].pageY - _target.getBoundingClientRect().top
        };
        self.render();
        if (self.isHovering()) {
          e.preventDefault();
        ...
        _target.addEventListener(_move, function(e) {
          point = e;
      ...
      _target.addEventListener("mousemove", function(e) {
        ...
        self.pointer = {
          x: point.pageX - _target.getBoundingClientRect().left,
          y: point.pageY - _target.getBoundingClientRect().top
        };
        self.render();
      });

      _target.addEventListener("mousedown", function() {
        if (
          self.data.code !== null &&
          self.data.name != null &&
          "onSelect" in self.options
      ...
      _target.addEventListener("mouseout", function() {
        self.pointer = null;
        self.render();
      });
    }
  ...
  Map.prototype.findPrefectureByCode = function(code) {
    const results = this.options.prefectures.filter(function(p) {
      return p.code == code;
    });
  ...
  Map.prototype.findAreaBelongingToByCode = function(code) {
    const results = this.options.areas.filter(function(a) {
  ...
  Map.prototype.brighten = function(hex, lum) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      c = Math.round(
        Math.min(Math.max(0, parseInt(c + c * lum)), 255)
      ).toString(16);
  ...
  Map.prototype.getName = function(prefecture_or_area) {
    switch (
      this.isArea(prefecture_or_area)
        ? this.options.areaNameType
        : this.options.prefectureNameType
    ) {
      case "short":
  ...
  MapCanvas.prototype.renderPrefectureMap = function() {
    ...
    this.options.prefectures.forEach(function(prefecture) {
      ...
      const area = this.findAreaBelongingToByCode(prefecture.code);
      if (area) {
  ...
  MapCanvas.prototype.renderAreaMap = function() {
    ...
    this.options.areas.forEach(function(area) {
      ...
      area.prefectures.forEach(function(code) {
        const prefecture = this.findPrefectureByCode(code);
        if (prefecture) {
          this.drawPrefecture(prefecture);
        } else {
    ...
    prefecture.path.forEach(function(p) {
      ...
      if (this.options.movesIslands) {
        OFFSET = {
          X:
            OFFSET.X +
            (this.isNanseiIslands(p)
              ? this.NanseiIslands.left
              : -this.NanseiIslands.width),
  ...
  MapCanvas.prototype.drawName = function() {
    ...
    const drawsArea =
      this.options.showsAreaName &&
      (!this.options.showsPrefectureName || this.options.selection == "area");

    if (drawsArea) {
      this.options.areas.forEach(function(area) {
        const center = { x: 0, y: 0, n: 0 };
        area.prefectures.forEach(function(code) {
          const prefecture = this.findPrefectureByCode(code);
          const _center = this.getCenterOfPrefecture(prefecture);
          center.n++;
      ...
      this.options.prefectures.forEach(function(prefecture) {
        const center = this.getCenterOfPrefecture(prefecture);
  ...
  MapCanvas.prototype.drawText = function(prefecture_or_area, point) {
    const context = this.element.getContext("2d");
    const area = this.isArea(prefecture_or_area)
      ? prefecture_or_area
      : this.findAreaBelongingToByCode(prefecture_or_area.code);
    const drawsArea =
      this.options.showsAreaName &&
      (!this.options.showsPrefectureName || this.options.selection == "area");
    ...
    if (this.options.fontColor && this.options.fontColor == "areaColor") {
      ...
      const color = area.color ? area.color : this.options.color;
      const hvColor =
        area.color && area.hoverColor
          ...
          ? this.options.hoverColor
          : this.brighten(this.options.color, 0.2);
    ...
    context.font = `${
      this.options.fontSize ? this.options.fontSize : this.element.width / 100
    }px '${this.options.font ? this.options.font : "Arial"}'`;
    ...
    for (let i = 0; i < 5; i++)
      context.fillText(
        this.getName(prefecture_or_area),
        (point.x * this.element.width) / this.base.width,
        (point.y * this.element.height) / this.base.height
      );
    context.restore();
  };
  ...
  MapCanvas.prototype.getCenterOfPrefecture = function(prefecture) {
    ...
    const path = prefecture.path[0];

    if (this.options.movesIslands) {
      OFFSET = {
        X:
          OFFSET.X +
          (this.isNanseiIslands(path)
            ? this.NanseiIslands.left
            : -this.NanseiIslands.width),
        Y:
          OFFSET.Y + (this.isNanseiIslands(path) ? this.NanseiIslands.top : 0)
      };
    }
    if ("coords" in path) {
      let i = 0;
      for (;;) {
        //while (true) {
        let x = path.coords[i * 2 + 0];
        let y = path.coords[i * 2 + 1];
  ...
  MapCanvas.prototype.drawCoords = function(coords, OFFSET) {
    ...
    for (;;) {
      ...
      if (i == 0) {
        context.moveTo(
          (x * this.element.width) / this.base.width,
          (y * this.element.height) / this.base.height
        );
      } else {
        context.lineTo(
          (x * this.element.width) / this.base.width,
  ...
  MapCanvas.prototype.drawIslandsLine = function() {
    ...
    if (this.options.movesIslands) {
      ...
      context.lineTo(
        (150 * this.element.width) / this.base.width,
        (200 * this.element.height) / this.base.height
      );
  ...
  MapCanvas.prototype.setProperties = function(prefecture, area) {
    ...
    const pointerIsOn =
      this.pointer && context.isPointInPath(this.pointer.x, this.pointer.y);
    ...
    if (pointerIsOn) {
      ...
      if (area.hoverColor) context.fillStyle = area.hoverColor;
      else if (this.options.hoverColor)
        context.fillStyle = this.options.hoverColor;
      else context.fillStyle = this.brighten(context.fillStyle, 0.2);
    }

    this.element.style.cursor =
      this.data.code == null ? "default" : "pointer";
  };
  ...
  let japanMap = new MapCanvas(options);
  options.field.appendChild(japanMap.element);
  japanMap.render(); // IE and Safari doesn't render properly when rendered before appending to the parent.
  japanMap.addEvent(); // iPad 1st + iOS5 doesn't work if this sentence is put before "target.append".
