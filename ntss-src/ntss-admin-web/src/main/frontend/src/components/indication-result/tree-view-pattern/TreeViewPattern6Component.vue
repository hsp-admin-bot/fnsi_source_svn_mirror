/**
 * パターン６（日付、予実、カテゴリ）表示コンポーネント.
 */
<template>
  <div class="tree-row" @click="onClick(model)">
    <!-- mod FNSI-No.342 患者イベント、検査結果、検査予定、一般撮影検査予定、処方の表示、機能遷移に対応 李 start -->
    <!-- <div class="tree-col">
      {{ model.categoryName }} {{ model.treatmentName }} {{ model.kurName }} {{ model.treatTime }} {{ model.bedName }}
    </div> -->
    ...
    <div v-else-if="model.type && model.type == 'pat_event'" class="tree-col">
      ...
      <!-- {{ '患者イベント' }} {{ model.subCategoryName }} {{ model.eventStartTime + ' 〜 ' }} {{ model.eventEndTime }}-->
      {{ '患者イベント' }} {{ model.subCategoryName }} {{ model.eventStartTime}}
    ...
    <div v-else-if="model.type && model.type == 'prescription'" class="tree-col">
      {{ '処方' }} {{ model.prescriptionType }} {{ model.count + '剤' }}
    </div> -->
