/**
 * パターン１（カテゴリ、予実・日付）表示コンポーネント.
 */
<template>
  <div class="tree-row" @click="onClick(model)">
    <!-- mod FNSI-No.342 患者イベント、検査結果、検査予定、一般撮影検査予定、処方の表示、機能遷移に対応 李 start -->
    <!-- <div class="tree-col">
      {{ model.indRstTypeName }} {{ model.treatDate }} {{ model.treatmentName }} {{ model.kurName }} {{ model.treatTime }} {{ model.bedName }}
    </div> -->
    ...
    <div v-else-if="model.type && model.type == 'pat_event'" class="tree-col">
      ...
      <!--{{ '【イベント】' }} {{ model.treatDate }} {{ model.subCategoryName }} {{ model.eventStartTime + ' 〜 ' }} {{ model.eventEndTime }}-->
      {{ '【イベント】' }} {{ model.treatDate }} {{ model.subCategoryName }} {{ model.eventStartTime}} 
    ...
    <!-- <div v-else-if="model.type && model.type == 'in_schedule'" class="tree-col">
      {{ '【予定】' }} {{ model.treatDate }} {{ model.count + '項目' }}
    </div>
    <div v-else-if="model.type && model.type == 'in_result'" class="tree-col">
      {{ '【結果】' }} {{ model.treatDate }} {{ model.count + '項目' }}
    </div>
    ...
    <div v-else-if="model.type && model.type == 'prescription'" class="tree-col">
      {{ '【' + model.issueState + '】' }} {{ model.treatDate }} {{ model.prescriptionType }} {{ model.count + '剤' }}
    </div>
