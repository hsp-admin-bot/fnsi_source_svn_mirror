...
export default {
  ...
  mutations: {
    ...
    setScrollPositions(state, scrollPosition) {
  ...
  actions: {
    ...
    setScrollPositions({ commit }, scrollPosition) {
      commit("setScrollPositions", scrollPosition);
    },
    ///**
    // * データ削除
    // * @param {*} param0
  ...
  getters: {
    getMachineAndBedList(state) {
      if (state.bedList) {
        ...
        // 無い側のコードは-1で埋める。
        const tmpBedList = bedList.map(
          (bed, index) =>{
            return {
              machineNo: bed.machineNo ? bed.machineNo : -1,
              bedCd: bed.bedCd,
              name: bed.bedName,
              // 在宅フラグ isHomeDialysis をつける
              isHomeDialysis: bed.isHomeDialysis == 1 ? true : false,
              sortNo: index,
              /* add by chamaojia 2023-06-27 [8894] 戻り値の追加  --start */
              fnBedNo: bed ? bed.fnBedNo : 0
              /* add by chamaojia 2023-06-27 [8894] 戻り値の追加  --end */
            };
          }
        );
        const tmpAllocatedCdList = tmpBedList.filter(
          bed => bed.machineNo > 0
        ).map(
          bed => {
            if (bed.machineNo > 0) {
              return bed.machineNo;
            }
          }
        );
        // machine.machineName !== "004" && machine.machineName !== "005" の判定は以前の処理(mod redmine 4654 未配置ベッドの並び順が不正 ベッドマスタ順＋機械室 宋qy)から引き継いています
        const tmpMachineList = state.machineList.filter(machine => {
          return machine.isDel === "0" && machine.isDisp === "1" && !tmpAllocatedCdList.includes(machine.machineNo) &&
                 (machine.machineName !== "004" && machine.machineName !== "005" )
        }).map(
          (machine, index) =>{
            return {
              ...
              sortNo: tmpBedList.length + index,
              /* add by chamaojia 2023-06-27 [8894] 戻り値の追加  --start */
              fnBedNo: 0
              /* add by chamaojia 2023-06-27 [8894] 戻り値の追加  --end */
            };
        ...
        return tmpBedList.concat(tmpMachineList);
      } else {
        return [];
      }
    },
    ...
    getBackGroundImage: state => state.selectedBedLayout.backgroundImage,
