...
public class MongoDbConfig {
  ...
  public MongoClient mongo(ObjectProvider<MongoClientSettingsBuilderCustomizer> builderCustomizers,
    ...
    MongoClientSettings mongoClientSettings = MongoClientSettings
      ...
      // MongoDB Driver 5.x の STREAM モードは旧 DocumentDB 非対応 → POLL に固定
      .applyToServerSettings(builder -> builder
          .serverMonitoringMode(ServerMonitoringMode.POLL)
          .heartbeatFrequency(10, SECONDS))
      .build();

    MongoClient realClient = new MongoClientFactory(
        builderCustomizers.orderedStream().collect(Collectors.toList()))
      .createMongoClient(mongoClientSettings);

    // DocumentDB は endSessions コマンド非対応（error 303）
    // close() をプロキシでラップしてシャットダウン時の例外ログを抑制する
        ...
        (proxy, method, args) -> {
          if ("close".equals(method.getName())) {
            try {
              ...
              log.debug("Suppressed on MongoClient.close(): {}", t.getMessage());
            }
            return null;
          }
          try {
