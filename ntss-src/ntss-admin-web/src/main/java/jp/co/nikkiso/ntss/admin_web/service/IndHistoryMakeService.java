...
/**
 * 患者経過総合ビューアの指示履歴を登録するサービスクラス
 */
@Service
public class IndHistoryMakeService {

  // ThreadLocalを使用してリクエスト単位で制御
  private static final ThreadLocal<Set<String>> requestProcessedLogDates =
  ...
  // Journal作成用キュー（リクエスト終了時に処理）
  private static final ThreadLocal<List<IndHistory>> pendingJournalCreations =
      new ThreadLocal<List<IndHistory>>() {
          @Override
          protected List<IndHistory> initialValue() {
...
//  }
  ...
  public void createPlanHistory(
    ...
    for (String key : keysList) {
      //指示履歴に登録するパラメータを作成
      indHistory = this.createPlanHistoryParams(
        bodyData,
        key,
        ordMainMap.get(key),
        weeksArray,
  ...
  public void createMoveHistory(
    ApiEntityOrdMain.ValiMoveTreatPlan bodyData,
    OrdMain ordMain,
    List<Integer> weeksArray) {
    ...
    for (String key : keysList) {
      ...
      if (key.equals("治療予定") || key.equals("スケジュール変更")) {
        //指示履歴に登録するパラメータを作成
        indHistory = this.createMoveHistoryParams(
          bodyData,
          ordMain,
          key,
          ordMainMap.get(key),
        ...
        //治療日変更のみのため、”未登録”の内容が変わらないので、履歴に登録しない。
        //del 8339 2023-02-07 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
//        indHistory.setLogContent(indHistory.getLogContent().replace("未登録", ""));
        //del 8339 2023-02-07 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
...
//        indHistory.setOrdNo(ordMain.getOrdNo().toString());
   ...
   * @param weeksArray 編集対象曜日
   */
  public void createCopyHistory(
    ApiEntityOrdMain.ValiCopyTreatPlan bodyData,
    OrdMain ordMain,
...
//    this.setIsIncludingMedicine(bodyData.getIs_including_medicine());
    ...
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
    // クールがコピー元の設定値なので、一旦保留して、変更内容を設定した後再設定する
    Integer copyIndKurCd = ordMain.getIndKurCd();
    // 対象がクール，治療開始時刻，ベッドにて、内容がコピー元の値で記録される。→　NG。正しくはコピー先の値にする事。
    ordMain.setIndKurCd(Objects.isNull(bodyData.getInd_kur_cd()) ? null : Integer.valueOf(bodyData.getInd_kur_cd()));
    ordMain.setIndTreatStartTime(indTreatStartTime);
    ordMain.setIndBedCd(Objects.isNull(bodyData.getInd_bed_cd()) ? null : Integer.valueOf(bodyData.getInd_bed_cd()));
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    ...
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
    ordMain.setIndKurCd(copyIndKurCd);
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm end

    //対象項目毎に指示履歴を登録
    for (String key : keysList) {
      //指示履歴に登録するパラメータを作成
      indHistory = this.createCopyHistoryParams(
        bodyData,
        ordMain,
        key,
        ordMainMap.get(key),
...
//      indHistory.setOrdNo(ordMain.getOrdNo().toString());
   ...
   *
   * @param bodyData   指示履歴作成に必要な値一覧
   * @param ordMainList 中止するの各項目の値一覧
   * @param weeksArray 中止する治療日の曜日
   */
...
//      //中止する各項目の値一覧を保持するマップ
//      Map<String, String> ordMainMap = this.createKeyValues(ordMain, bodyData.getFacility_cd());
//
//      //中止する項目のキー名を取得
//      List<String> keysList = this.gettingKeylist(ordMainMap);
    Map<String, List<String>> ordMainMap = ordMainList.stream()
      // #10959 システム内でstatic変数を使っている箇所の洗い出し mod yangxuewang start
      .map(o -> createKeyValues(o, bodyData.getFacility_cd(), true))
      // #10959 システム内でstatic変数を使っている箇所の洗い出し mod yangxuewang end
    ...
    List<String> keysList = ordMainMap.keySet().stream().toList();
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end

      //対象項目毎に指示履歴を登録
      for (String key : keysList) {
        //指示履歴に登録するパラメータを作成
        indHistory = this.createDeleteHistoryParams(
          bodyData,
          key,
          ordMainMap.get(key),
          weeksArray);
...
//    }
   ...
   * @param weeksArray 中止する治療日の曜日
   */
  public void createDeleteHistoryByDeleteIndPlanPatInfo(
    ApiEntityOrdMain.ValiDeleteIndPlanPatInfo bodyData,
    List<OrdMain> ordMainList,
    List<Integer> weeksArray,
    String dialysisDateFrom,
    String dialysisDateTo) {
    if("99991231".equals(dialysisDateTo)){
      dialysisDateTo = "";
    }
    //指示履歴用パラメータ
    IndHistory indHistory;
    ...
    for (OrdMain ordMain : ordMainList) {
      ...
      for (String key : keysList) {
        //指示履歴に登録するパラメータを作成
        indHistory = this.createDeleteHistoryParamsByDeleteIndPlanPatInfo(
          bodyData,
          key,
          ordMainMap.get(key),
          weeksArray,
...
//        indHistory.setOrdNo(ordMain.getOrdNo().toString());
   ...
   */
  public void createWeekPatternHistory(
    ApiEntityOrdMain.ValiWeekPattern bodyData,
    HashMap<Short, List<Short>> changWeekList,
    List<Integer> srcDelWeekList) {
...
//    String endDate = StringUtils.isEmpty(bodyData.getEnd_date()) ? "" : bodyData.getEnd_date().replaceAll("-", "");
   ...
   * @param type       操作区分
   */
  public void planCreateHistoryExecute(IndHistory indHistory, String key, String type) {
    //対象、内容を文字列→配列に変換
...
//if (! paramContentArray[i].equals("未登録")) {
        ...
        // add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
        case "治療条件":
          String[] contents = indHistory.getLogContent().split("→");
          String subContents = String.join(",", Arrays.stream(contents[0].split(","))
              .filter(content -> !"dummy".equals(content)).toArray(String[]::new));
          if (!StringUtils.hasText(subContents)) {
            break;
          }
          if (contents.length > 1) {
            indHistory.setLogContent(String.join("→", subContents, contents[1]));
          } else {
            indHistory.setLogContent(subContents);
          }
          indHistory.setLogTarget(gettingEditTarget(String.valueOf(i+1)));
          //指示履歴を登録
          this.createHistoryExecute(indHistory, type);
          break;
        // add 11555 指示履歴への記録の残り方が仕様と異なる zkm end
   ...
   * @param weeksArray    編集対象曜日
   */
  public void createMethodHistory(
    ApiEntityOrdMain.ValiCreateTreatPlan bodyData,
    OrdMain ordMain,
    OrdMain targetOrdMain,
    List<Integer> weeksArray) {
...
// add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
  ...
  public void createMethodHistory(
    ApiEntityOrdMain.ValiCreateTreatPlan bodyData,
    OrdMain ordMain,
    List<OrdMain> changeOrdMainList,
    List<Integer> weeksArray) {
    ...
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
    keysList = keysList.stream().filter(key -> !"スケジュール変更".equals(key)).toList();
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    ...
    if (treatMethodFlag.equals("0")) {
      ...
      if (indHistory.getLogContent().contains("→") && (! indHistory.getLogContent().split("→")[0].equals(indHistory.getLogContent().split("→")[1]))) {
        // add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
        JSONArray paramContentArray = new JSONArray(indHistory.getLogContent());
        indHistory.setLogContent(paramContentArray.get(0).toString());
        // add 11555 指示履歴への記録の残り方が仕様と異なる zkm end
        this.createHistoryExecute(indHistory, "2");
      }
      // add #7327 治療方法マスタ操作時の動作がおかしい 王永吉 start
...
// add 11555 指示履歴への記録の残り方が仕様と異なる zkm end
   ...
   */
  public void createMstTreatmentModifyHistory(
          String facilityCd,
          String patId,
          String indTreatmentName,
          Long indUserId, //指示者コード
          Long updUserId, //更新者コード
          TreatMethodChangeHelper treatCondSettingDiff) {
    createMstTreatmentModifyHistory(facilityCd, patId, indTreatmentName, indUserId, updUserId, treatCondSettingDiff, null);
  }
  ...
  public void createMstTreatmentModifyHistory(
    ...
    if (today == null || today.isEmpty()) {
        today = new SimpleDateFormat(FORMAT_DATE).format(new Date());
    }
    //改行のための変数を取得
    String br = System.getProperty("line.separator");

    // YYYYMMDD形式の日付を作成（treatment_start_date用）
    String treatmentStartDate = today.substring(0, 8); // yyyyMMddHHmmssSSS → yyyyMMdd
    ...
    // 開始日（YYYYMMDD形式）
    indHistoryBaseParams.setTreatmentStartDate(treatmentStartDate);
    // 終了日
    ...
    if(treatCondSettingDiff.hasChangeToAdd()){
      ...
      for (TreatMethodChangeHelper.ItemAndValue itemAndValue : treatCondSettingDiff.getToAddCtlNoList()) {
        ...
        String itemNumber = itemAndValue.getItem(); // 治療条件項目番号（"1"〜"39"）
        String treatCondItemName = this.gettingEditTarget(itemNumber);
        String treatCondItemValue = itemAndValue.getValue();

        // 値を表示用の形式に変換
        if (StringUtils.hasLength(treatCondItemValue)) {
          // JSONObjectを作成してgettingValueメソッドで変換
          JSONObject itemData = new JSONObject();
          itemData.put("value", treatCondItemValue);
          treatCondItemValue = this.gettingValue(itemNumber, itemData, facilityCd);
        } else {
          treatCondItemValue = "未登録";
        }

        // logTargetを項目名に設定
        indHistory.setLogTarget(treatCondItemName);
        indHistory.setLogClass("新規");

        // sortNoを項目名からsortMapで取得）
        Integer sortNo = sortMap.get(treatCondItemName);
        if (sortNo == null) {
          sortNo = Integer.parseInt(itemNumber) * 10;
        }
        indHistory.setSortNo(sortNo);

        // logContentを変換後の項目値に設定
        indHistory.setLogContent(treatCondItemValue);
    ...
    if(treatCondSettingDiff.hasChangeToUpd()){
      ...
      for (TreatMethodChangeHelper.ItemAndValue itemAndValue : treatCondSettingDiff.getToUpdCtlNoList()) {
        ...
        // sortNoを項目名からsortMapで取得（10刻みの値）
        Integer sortNo = sortMap.get(treatCondItemName);
        if (sortNo == null) {
          // sortMapに存在しない項目の場合は、項目番号 * 10をsortNoとして使用
          sortNo = Integer.parseInt(itemNumber) * 10;
        }
        indHistory.setSortNo(sortNo);
    ...
    if(treatCondSettingDiff.hasChangeToDel()){
      ...
      for (TreatMethodChangeHelper.ItemAndValue itemAndValue : treatCondSettingDiff.getToDelCtlNoList()) {
        ...
        String itemNumber = itemAndValue.getItem();
        String treatCondItemName = this.gettingEditTarget(itemNumber);
        String oldValue = itemAndValue.getValue(); // 元の値（設定されていない場合はnull）
        ...
        if (StringUtils.hasLength(oldValue)) {
          logContent = oldValue + "→中止";
        } else {
          logContent = treatCondItemName + "→中止";
        }
        indHistory.setLogContent(logContent);
    ...
    if(!indHistoryList.isEmpty()){
      indHistoryService.createBatch(indHistoryList);
    }
  }
   ...
   */
  public void createScheduleHistory(
    ...
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    for (OrdMain ordMain : ordMainList) {
//      indHistory = this.createScheduleHistoryParams(bodyData, weeksArray, ordMain);
      indHistory = this.createScheduleHistoryParams(bodyData, weeksArray, ordMainList);
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//        if (paramContentArray[i].contains("→") && (paramContentArray[i].split("→")[0].equals(paramContentArray[i].split("→")[1]))) {
        ...
        //パラメータの対象、内容を設定
        indHistory.setLogTarget(paramTargetArray[i]);
//        indHistory.setLogContent(paramContentArray[i]);
        indHistory.setLogContent(paramContentArray.get(i).toString());
        //mod 8339 2023-02-05 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 end
        //指示履歴時刻設定
        indHistory.setLogDate(setLogDate);
...
//    }
  ...
  /**
   * 指示履歴登録処理(スケジュール変更)
   * createScheduleHistoryのオーバーロードメソッド（設定パラメーターを指定する）
   *
   * @param bodyData    (指示履歴作成に必要な値一覧)
   * @param flag        (1.新規、2.変更,3.削除。ここでは2しか無い)
   * @param weeksArray  編集対象曜日
   * @param ordMainList 更新対象となる治療情報
   * @param paramTarget 設定パラメーター
   */
  public void createScheduleHistory(
    ApiEntityOrdMain.ValiUpdateIndSchedule bodyData,
    String flag,
...
//        indHistory.setLogContent(paramContentArray[i]);
   ...
   * @param ordMainList 更新対象となる治療情報
   */
  public void createConditionHistory(
    ApiEntityOrdMain.ValiUpdateIndCond bodyData,
    String flag,
    List<Integer> weeksArray,
    List<OrdMain> ordMainList) {
    //指示履歴用パラメータ
...
//      indHistory = this.createConditionHistoryParams(bodyData, weeksArray, ordMain);
      indHistory = this.createConditionHistoryParams(bodyData, weeksArray, ordMainList);
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end

      //パラメータの対象、内容を文字列→配列に変換
      // mod #2856：DWの指示履歴登録対応 韓 start
      String[] paramTargetArray = Objects.isNull(indHistory.getLogTarget())
        ? null
        : indHistory.getLogTarget().split(",");
      //mod 8339 2023-02-05 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
//      String[] paramContentArray = Objects.isNull(indHistory.getLogContent())
//        ? null
//        : indHistory.getLogContent().split(",");
      // mod #9372 患者情報から身体情報(DW，目標体重)を登録して保存すると処理中のままになる zy start
//      JSONArray paramContentArray = Objects.isNull(new JSONArray(indHistory.getLogContent()))?null:new JSONArray(indHistory.getLogContent());
...
//    }
   ...
   */
  public void createMedicineHistory(
    ApiEntityOrdMain.ValiOrdMedi bodyData,
    String flag,
    List<Integer> weeksArray,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    OrdMain ordMain) {
    List<OrdMain> ordMains) {
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//    IndHistory indHistory = this.createMedicineHistoryParams(bodyData, flag, weeksArray, ordMain);
   ...
   * @param ordMainList 更新対象となる治療情報
   */
  public void createEquipmentHistory(
    ApiEntityOrdMain.ValiOrdEquip bodyData,
    String flag,
    List<Integer> weeksArray,
    List<OrdMain> ordMainList) {
    //指示履歴用パラメータ
...
//      }
   ...
   * @param flag       (1.新規、2.変更,3.削除)
   * @param weeksArray 編集対象曜日
   * @param oldIndContents 編集前内容
   */
  // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//  public void createCommentHistory(ApiEntityOrdMain.ValiCommentCreate bodyData, String flag, List<Integer> weeksArray) {
  public void createCommentHistory(ApiEntityOrdMain.ValiCommentCreate bodyData, String flag, List<Integer> weeksArray, List<String> oldIndContents) {
  // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    //指示履歴用パラメータを定義し、各値を入力
    IndHistory indHistory = new IndHistory();
    String setLogDate = new SimpleDateFormat(FORMAT_DATE).format(new Date());
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    indHistory = this.createCommentHistoryParams(bodyData, weeksArray);
  ...
  public boolean isToMongo(){
    //return "mongo".equals(this.profiles);
  ...
  public void createHistoryExecute(IndHistory indHistory, String flag) {
    // デバッグログ（呼び出し確認）
    EventLogMessage debugLogMessage = new EventLogMessage();
    debugLogMessage.setFacilityCd(indHistory != null ? indHistory.getFacilityCd() : "UNKNOWN");
    debugLogMessage.setLogMessage("[DEBUG] createHistoryExecute called. flag=" + flag + ", logDate=" +
        (indHistory != null ? indHistory.getLogDate() : "null") + ", thread=" + Thread.currentThread().getId());
    logService.log(LogLevel.INFO, debugLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);
    ...
    if (mongoTemplate != null) {
      //指示履歴登録処理
      indHistoryService.create(indHistory);

      // Journal作成用にキューに追加（リクエスト終了時に処理）
      if (indHistory.getLogDate() != null) {
        List<IndHistory> pendingList = pendingJournalCreations.get();
        pendingList.add(indHistory);
        EventLogMessage eventLogMessage = new EventLogMessage();
        eventLogMessage.setFacilityCd(indHistory.getFacilityCd());
        eventLogMessage.setLogMessage("createHistoryExecute: indHistory added to queue. logDate=" + indHistory.getLogDate() +
            ", queueSize=" + pendingList.size() + ", thread=" + Thread.currentThread().getId());
  ...
  public void createHistoryExecuteBatch(List<IndHistory> indHistoryList, String flag) {
    ...
    if (mongoTemplate != null) {
      ...
      if (addedCount > 0) {
        EventLogMessage eventLogMessage = new EventLogMessage();
        eventLogMessage.setLogMessage("createHistoryExecuteBatch: indHistory batch added to queue. addedCount=" + addedCount +
            ", queueSize=" + pendingList.size() + ", thread=" + Thread.currentThread().getId());
   ...
   */
  public IndHistory createPlanHistoryParams(
    ApiEntityOrdMain.ValiCreateTreatPlan bodyData,
    String key,
    String ordMainValue,
    List<Integer> weeksArray,
    String indTreatmentCd) {
    ...
    //クールコード
    // String indKurCd =
    //   Objects.isNull(bodyData.getTarget_kur_cd())
    //     ? ""
    //     : bodyData.getTarget_kur_cd();
    String indKurCd =
      Objects.isNull(bodyData.getInd_kur_cd())
        ? ""
        : bodyData.getInd_kur_cd();
    // mod 9339 特殊浄化に治療方法変更してもNa注入プログラムが入りのままとなる。 関  end
    //指示者コード
...
//    String paramContent = this.createplanContent(key, ordMainValue, facilityCd, "1");
   ...
   * @return indHistory    指示履歴用パラメータ
   */
  public IndHistory createMoveHistoryParams(
    ApiEntityOrdMain.ValiMoveTreatPlan bodyData,
    OrdMain preOrdMain,
    String key,
    String ordMainValue,
    ...
    //治療方法コード
    String indTreatmentCd =
      Objects.isNull(preOrdMain.getIndTreatmentCd())
        ? ""
        : preOrdMain.getIndTreatmentCd().toString();
    //クールコード
    String indKurCd =
      Objects.isNull(preOrdMain.getIndKurCd())
        ? ""
        : preOrdMain.getIndKurCd().toString();
    //指示者コード
    ...
    //設定する内容
    //mod 8339 2023-02-06 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
//    String paramContent = this.createMoveContent(key, ordMainValue, startDate, dateTo);
   ...
   * @return indHistory    指示履歴用パラメータ
   */
  public IndHistory createCopyHistoryParams(
    ApiEntityOrdMain.ValiCopyTreatPlan bodyData,
    OrdMain ordMain,
    String key,
    String ordMainValue,
    ...
    //治療方法コード
    String indTreatmentCd =
      Objects.isNull(ordMain.getIndTreatmentCd())
        ? ""
        : ordMain.getIndTreatmentCd().toString();
    //クールコード
    String indKurCd =
      Objects.isNull(ordMain.getIndKurCd())
        ? ""
        : ordMain.getIndKurCd().toString();
    //指示者コード
    String indUserId = bodyData.getInd_user();
    //更新者コード
    String updUserId = bodyData.getUpd_user();
...
//    String paramContent = this.createplanContent(key, ordMainValue, facilityCd, "1");
   ...
   * @return indHistory    指示履歴用パラメータ
   */
  public IndHistory createDeleteHistoryParamsByDeleteIndPlanPatInfo(
    ApiEntityOrdMain.ValiDeleteIndPlanPatInfo bodyData,
    String key,
    String ordMainValue,
    List<Integer> weeksArray,
...
//      bodyData.getEnd_date().replaceAll("-", "");
    //治療方法コード
    String indTreatmentCd =
      Objects.isNull(bodyData.getTreatment_cd())
        ? ""
        : bodyData.getTreatment_cd();
    //クールコード
    String indKurCd =
      Objects.isNull(bodyData.getKur_cd())
        ? ""
        : bodyData.getKur_cd();
    //指示者コード
...
//    String paramContent = this.createplanContent(key, ordMainValue, facilityCd, "3");
   ...
   * @return indHistory    指示履歴用パラメータ
   */
  public IndHistory createDeleteHistoryParams(
    ApiEntityOrdMain.ValiDeleteTreatPlan bodyData,
    String key,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    String ordMainValue,
    List<String> ordMainValues,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    List<Integer> weeksArray) {
...
//    String paramContent = this.createplanContent(key, ordMainValue, facilityCd, "3");
  ...
  /**
   * 治療予定・曜日パターン変更用、履歴用パラメーター作成処理
   *
   * @param bodyData     必要パラメータの記載されたJson文字列
   * @param key          Json文字列の、登録する各項目のキー名(治療予定、治療方法、治療条件、スケジュール変更、医療材料、投与薬剤、指示コメントのどれか)
   * @param ordMainValue Json文字列の、登録する各項目の値(キー名に対応する値)
   * @param weeksArray   登録を行う曜日
   * @param type         操作区分(1,新規 2,変更 3, 削除)
   * @return indHistory    指示履歴用パラメータ
   */
  public IndHistory createWeekPatternHistoryParams(
    ApiEntityOrdMain.ValiCreateTreatPlan bodyData,
    String key,
    String ordMainValue,
    List<Integer> weeksArray,
    ...
    //治療方法コード
    String indTreatmentCd =
      Objects.isNull(bodyData.getTreatment_set_cd())
        ? ""
        : bodyData.getTreatment_set_cd();
    //クールコード
    // mod 9339 特殊浄化に治療方法変更してもNa注入プログラムが入りのままとなる。 関  strat
    // String indKurCd =
    //   Objects.isNull(bodyData.getTarget_kur_cd())
...
//    String paramContent = this.createplanContent(key, ordMainValue, facilityCd, type);
   ...
   */
  public Map<String, Object> createMethodHistoryParams(
    ApiEntityOrdMain.ValiCreateTreatPlan bodyData,
    String key,
    String ordMainValue,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
...
//      this.createMethodContent(targetOrdMain, key, ordMainValue, treatMethodFlag, facilityCd);
    Map<String, List<String>> paramContentMap = this.createMethodContent(changeOrdMainList, key, ordMainValue, treatMethodFlag, facilityCd, indHistory.getTreatmentWeekday());
...
//    String paramContent = String.join(",", paramContentMap.get("内容"));
   ...
   * @param bodyData   必要パラメータの記載されたJson文字列
   * @param weeksArray 編集対象曜日
   * @param ordMains    更新対象治療情報
   * @return indHistory 指示履歴用パラメータ
   */
  public IndHistory createScheduleHistoryParams(
    ApiEntityOrdMain.ValiUpdateIndSchedule bodyData,
    List<Integer> weeksArray,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    OrdMain ordMain) {
    ...
    String endDate = "false".equals(bodyData.getIs_deadline()) ?
      "" :
      bodyData.getInd_end_date().replaceAll("-", "");
    //治療方法コード
...
//    String indKurCd =
//      Objects.isNull(ordMain.getIndKurCd()) || ordMain.getIndKurCd() == 0
//        ? "0"
//        : String.valueOf(ordMain.getIndKurCd());
//    //変更前治療開始時刻
...
//    //変更前ベッドコード
//    String indBedCd =
//      Objects.isNull(ordMain.getIndBedCd()) || ordMain.getIndBedCd() == 0
//        ? "0"
//        : String.valueOf(ordMain.getIndBedCd());
    // del 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    //変更後クールコード
    String editIndKurCd = bodyData.getEdit_ind_kur_cd();
    //変更後治療開始時刻
    String editIndTreatStartTime =
      Objects.isNull(bodyData.getEdit_ind_treat_start_time())
        ? "未登録"
    ...
    /**対象 */
    //設定する対象
    String paramTarget = "クール,治療開始時刻,ベッド";
    indHistory.setLogTarget(paramTarget);

    /**内容 */
    //設定する内容
    String paramContent = this.createScheduleContent(
      facilityCd,
      // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//      indKurCd,
//      indTreatStartTime,
//      indBedCd,
  ...
  /**
   * スケジュール変更用、履歴用パラメーター作成処理
   * createScheduleHistoryParamsのオーバーロードメソッド（設定パラメーターを指定する）
   *
   * @param bodyData    必要パラメータの記載されたJson文字列
   * @param weeksArray  編集対象曜日
...
//      e.printStackTrace();
    ...
    // 変更前 治療日(スケジュール表呼び出し用)
    String treatDate = ordMain.getTreatDate();
    // 変更後 治療日(スケジュール表呼び出し用)
    String editTreatDate = bodyData.getEdit_ind_treat_date();
   ...
   * @param bodyData   必要パラメータの記載されたJson文字列
   * @param weeksArray 編集対象曜日
   * @param ordMains    更新対象治療情報一覧
   * @return indHistory 指示履歴用パラメータ
   */
  public IndHistory createConditionHistoryParams(
    ApiEntityOrdMain.ValiUpdateIndCond bodyData,
    List<Integer> weeksArray,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    OrdMain ordMain) {
...
//    indHistory.setOrdNo(ordMain.getOrdNo() == null ? null : ordMain.getOrdNo().toString());
   ...
   * @return indHistory 指示履歴用パラメータ
   */
  public IndHistory createMedicineHistoryParams(
    ApiEntityOrdMain.ValiOrdMedi bodyData,
    String flag,
    List<Integer> weeksArray,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    OrdMain targetOrdMain) {
    List<OrdMain> targetOrdMains) {
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    //指示履歴用パラメータ
...
//      List<Map<String, String>> indInfoMedicineList = this.changeListMap(targetOrdMain.getIndMediInfo());
    ...
    String weekDay = indHistory.getTreatmentWeekday();
    // add 11555 指示履歴への記録の残り方が仕様と異なる zkm start
    if (!flag.equals("1")) {
   ...
   */
  //mod FNSI-5910 劉全航 start
  public IndHistory createEquipmentHistoryParams(
    ApiEntityOrdMain.ValiOrdEquip bodyData,
    String flag,
    List<Integer> weeksArray,
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
...
//      ordMain);
  ...
  // public void createEquipmentHistoryParams(
  //   ApiEntityOrdMain.ValiOrdEquip bodyData,
  //   String flag,
  //   List<Integer> weeksArray,
  //   OrdMain ordMain){
  ...
  //       if(cd.equals(targetEquipment)){
  //         amount = indInfoEquipmentList.get(i).get("amount");
  //       }
  //     }
  //     equipmentInfo.put("amount", amount);
  //     //指示者ID
  //     String indUserId = equipmentInfo.get("ind_user_id").toString();
  //     //更新者ID
  //     String updUserId = equipmentInfo.get("upd_user_id").toString();
  //     //穴埋め有無
  //     String autoInsert = bodyData.getAuto_insert();
  //     //削除の場合、数量を更新対象から取得(当日の治療情報の投与薬剤数量が自動的に格納されているため)
  //     /**指示履歴用パラメータに各値を入力*/
  ...
  //   }else{
  //     //医療材料内容
  //     JSONArray equipmentInfoList = new JSONArray(bodyData.getInd_info());
  //     for(int i = 0; i < equipmentInfoList.length(); i ++){
  //       JSONObject equipmentInfo = equipmentInfoList.getJSONObject(i);
  //       //指示者ID
  //       String indUserId = equipmentInfo.get("ind_user_id").toString();
  //       //更新者ID
  //       String updUserId = equipmentInfo.get("upd_user_id").toString();
  //       //穴埋め有無
  //       String autoInsert = equipmentInfo.get("auto_insert").toString();
  //       //削除の場合、数量を更新対象から取得(当日の治療情報の投与薬剤数量が自動的に格納されているため)
  //       /**指示履歴用パラメータに各値を入力*/
  //       /** 患者コード、開始日、終了日、曜日、治療方法、クール、指示者、更新者を設定*/
  //       indHistory = this.setBasicParams(
   ...
   */
  // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//  public IndHistory createCommentHistoryParams(ApiEntityOrdMain.ValiCommentCreate bodyData, List<Integer> weeksArray) {
  public IndHistory createCommentHistoryParams(ApiEntityOrdMain.ValiCommentCreate bodyData, List<Integer> weeksArray, List<String> oldIndContents) {
  // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    //指示履歴用パラメータ
    IndHistory indHistory = null;
    ...
    //設定する内容を取得
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    String paramContent = this.createCommentContent(flag, contentNum, indContent);
...
//        }
   ...
   */
  //mod 8339 2023-02-06 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
//  public String createMoveContent(String key, String ordMainValue, String dateFrom, String dateTo) {
  public String createMoveContent(String key, String ordMainValue, String dateFrom, String dateTo,OrdMain preOrdMain,ApiEntityOrdMain.ValiMoveTreatPlan bodyData) {
    //変更後クールコード
    String indKurCd =
      StringUtils.isEmpty(bodyData.getInd_kur_cd())
        ? "0"
        : bodyData.getInd_kur_cd();
    //変更後ベッドコード
    String indBedCd =
      StringUtils.isEmpty(bodyData.getInd_bed_cd())
        ? "0"
        : String.valueOf(bodyData.getInd_bed_cd());
    //変更後クールコードをクール名称に変換
...
//      e.printStackTrace();
      ...
      EventLogMessage eventLogMessage = new EventLogMessage();
      eventLogMessage.setLogMessage(ExcetionStackTraceToString(e));
      if (preOrdMain != null && preOrdMain.getFacilityCd() != null) {
    ...
    switch (key) {
      ...
      default:
        ...
        //mod 8339 2023-02-06 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
//        String[] valueArray = ordMainValue.split(",");
        JSONArray valueArray = new JSONArray(ordMainValue);
        //各項目の内容をリストに追加
//        for (String scheduleValue : valueArray) {
        for (int i = 0; i <valueArray.length() ; i++) {
          //内容を作成
//          String value = preChangeDate + scheduleValue + "→" + nextChangeDate + scheduleValue;
          String value="";
          if (0==i) {
...
//    OrdMain targetOrdMain, String key, String ordMainValue, String treatMethodFlag, String facilityCd) {
    List<OrdMain> changeOrdMainList, String key, String ordMainValue, String treatMethodFlag, String facilityCd, String weekDay) {
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//          paramContentMap = this.methodMedicineContent(targetOrdMain, ordMainValue, facilityCd);
          paramContentMap = this.methodMedicineContent(changeOrdMainList, ordMainValue, facilityCd, weekDay);
          // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
          break;
...
//    String preIndCondInfoValue, String ordMainValue, String facilityCd) {
//
//    //治療条件の内容に記載する文章のリスト
//    List<String> paramContentList = new ArrayList<>();
//    //操作区分リスト
...
//    // JSONObject preIndCondInfo = new JSONObject(targetOrdMain.getIndCondInfo());
...
//        if (i >= 15 && i <= 18) {
//          // add #9973 Resolve null exception for key 20240117 ztc start
//          if(preIndCondInfo.optJSONObject("15") != null) {
//          // add #9973 Resolve null exception for key 20240117 ztc end
//            preMedicineCd = preIndCondInfo.getJSONObject("15").get("value");
//            preMedicineType = preIndCondInfo.getJSONObject("15").get("medicine_type");
//            preMedicineUnit = preIndCondInfo.getJSONObject(String.valueOf(i)).get("unit");
//          }
//          //補液編集の場合
//        } else if (i >= 19 && i <= 24) {
//          // add #9973 Resolve null exception for key 20240117 ztc start
//          if(preIndCondInfo.optJSONObject("19") != null) {
//          // add #9973 Resolve null exception for key 20240117 ztc end
//            preMedicineCd = preIndCondInfo.getJSONObject("19").get("value");
//            // mod #9973 Resolve null exception for key 20240117 ztc start
////            preMedicineType = preIndCondInfo.getJSONObject("15").get("medicine_type");
//            preMedicineType = preIndCondInfo.getJSONObject("19").get("medicine_type");
...
//        } else if (i >= 25 && i <= 38) {
//          // add #9973 Resolve null exception for key 20240117 ztc start
//          if(preIndCondInfo.optJSONObject("25") != null) {
//          // add #9973 Resolve null exception for key 20240117 ztc end
//            preMedicineCd = preIndCondInfo.getJSONObject("25").get("value");
//            // mod #9973 Resolve null exception for key 20240117 ztc start
////            preMedicineType = preIndCondInfo.getJSONObject("15").get("medicine_type");
//            preMedicineType = preIndCondInfo.getJSONObject("25").get("medicine_type");
//            // mod #9973 Resolve null exception for key 20240117 ztc end
//            preMedicineUnit = preIndCondInfo.getJSONObject(String.valueOf(i)).get("unit");
//          }
//        }
...
//      }
//
//      JSONObject preIndCond = null;
//      try {
//        preIndCond = preIndCondInfo.getJSONObject(String.valueOf(i));
//      } catch (Exception e) {
//        //治療条件が存在しない
//        preIndCond = new JSONObject();
//      }
//
//      //変更前の治療条件の値を取得
//      String preCondValue =
//        this.gettingValue(String.valueOf(i), preIndCond, facilityCd, preMedicineCd, preMedicineType);
//      //変更前の値の単位
//      String preCondUnit =
//        preCondValue.equals("未登録") || preCondValue.equals("DWと同じ")
//          ? ""
//          : this.gettingCondUnit(String.valueOf(i), facilityCd, preMedicineCd, preMedicineType, preMedicineUnit);
//
//      //変更後の治療条件の値
//      String condValue = "未登録";
//      //変更後の値の単位
//      String condUnit = "";
//
//      //新たに登録する治療条件が存在するか判定
...
//
//        //変更後の治療条件の値を取得
//        condValue = this.gettingValue(String.valueOf(i), indCond, facilityCd, medicineCd, medicineType);
//        //変更後の値の単位
//        condUnit =
//          condValue.equals("未登録") || condValue.equals("DWと同じ")
//            ? ""
...
//
//      //内容を作成
//      String content = preCondValue + preCondUnit + "→" + condValue + condUnit;
//      //値をリストに追加
//      paramContentList.add(content);
//    }
//
...
//   */
//  public Map<String, List<String>> methodScheduleContent(
...
//    //mod 8339 2023-02-06 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 張 start
////    String[] ordMainArray = ordMainValue.split(",");
...
////    String editIndKurName = ordMainArray[0];
...
////    String editIndTreatStartTime = ordMainArray[1];
...
////    String editIndBedName = ordMainArray[2];
...
//    OrdMain targetOrdMain, String ordMainValue, String facilityCd) {
  public Map<String, List<String>> methodMedicineContent(List<OrdMain> changeOrdMainList, String ordMainValue, String facilityCd, String weekDay) {
  // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    //投与薬剤の内容に記載する文章のリスト
    List<String> paramContentList = new ArrayList<>();
    //操作区分リスト
...
//          previousMedicineInfo, facilityCd, "3", nums, initDate, dateInterval, targetOrdMain);
        String comment = this.createMedicineContent(
          previousMedicineInfo, facilityCd, "3", nums, initDate, dateInterval, previousIndMedicineList, weekDay);
        // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//            medicineInfo, facilityCd, "1", nums, initDate, dateInterval, targetOrdMain);
          String comment = this.createMedicineContent(
            medicineInfo, facilityCd, "1", nums, initDate, dateInterval, previousIndMedicineList, weekDay);
          // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
          paramContentList.add(comment);
...
//    String indKurName = indKurCd.equals("0") ? "未登録" : this.changekur(indKurCd, facilityCd);
    //変更前クールコードをクール名称に変換
    List<String> kurs = ordMains.stream().map(o -> Objects.isNull(o.getIndKurCd()) ? "0" : o.getIndKurCd().toString())
      .filter(o -> !editIndKurCd.equals(o)).distinct()
      .map(o -> "0".equals(o) ? "未登録" : changekur(o, facilityCd)).distinct().toList();
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    ...
    //内容一覧に追加
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    paramContentList.add(editIndKurName + "→" + editIndKurName);
    paramContentList.add((CollectionUtils.isEmpty(kurs) ? editIndKurName
      : String.join(",", kurs)) + "→" + editIndKurName);
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//    indTreatStartTime = Objects.isNull(indTreatStartTime) ? "未登録" : indTreatStartTime;
    SimpleDateFormat TreatTimeFormat = new SimpleDateFormat("HHmm");
    String finalEditIndTreatStartTime = editIndTreatStartTime;
    List<String> treatStartTimes = ordMains.stream().map(o -> {
      if (Objects.isNull(o.getIndTreatStartTime())) {
        return "未登録";
      } else {
        Date treatTimeDate = null;
        try {
          treatTimeDate = TreatTimeFormat.parse(o.getIndTreatStartTime());
        } catch (ParseException e) {
          // #9700 イベントログに出るべきではないもの、判読不可能なログがある 20260402 del yangxuewang start
//      e.printStackTrace();
          // #9700 イベントログに出るべきではないもの、判読不可能なログがある 20260402 del yangxuewang end
        ...
        return new SimpleDateFormat("HH:mm").format(treatTimeDate);
      }
    }).filter(o -> !finalEditIndTreatStartTime.equals(o)).distinct().toList();
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    ...
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    paramContentList.add(indTreatStartTime + "→" + editIndTreatStartTime);
    paramContentList.add((CollectionUtils.isEmpty(treatStartTimes) ? editIndTreatStartTime
      : String.join(",", treatStartTimes)) + "→" + editIndTreatStartTime);
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end

    //変更前ベッドコードをベッド名称に変換
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    String indBedName = indBedCd.equals("0") ? "未登録" : this.changeBed(indBedCd, facilityCd);
    List<String> beds = ordMains.stream().map(o -> Objects.isNull(o.getIndBedCd()) ? "0" : o.getIndBedCd().toString())
      .filter(o -> !editIndBedCd.equals(o)).distinct()
      .map(o -> "0".equals(o) ? "未登録" : changeBed(o, facilityCd)).distinct().toList();
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    ...
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    paramContentList.add(indBedName + "→" + editIndBedName);
    paramContentList.add((CollectionUtils.isEmpty(beds) ? editIndBedName
      : String.join(",", beds)) + "→" + editIndBedName);
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//    paramContent = String.join(",", paramContentList);
  ...
  /**
   * 指示履歴に登録する、スケジュール変更内容を作成する処理
   * createScheduleContentのオーバーロードメソッド（設定パラメーターを指定する）
   *
   * @param facilityCd            施設コード
   ...
   * @param editIndTreatStartTime 変更後治療開始時刻
   * @param editIndBedCd          変更後ベッドコード
   * @param treatDate             変更前治療日
   * @param editTreatDate         変更後治療日
   * @param paramTarget           設定パラメーター
   * @return スケジュール変更内容
   */
  public String createScheduleContent(
    ...
    //設定するパラメーター
    String[] paramTargetArray = paramTarget.split(",");

    if (Arrays.asList(paramTargetArray).contains("クール")) {
      //変更前クールコードをクール名称に変換
      String indKurName = indKurCd.equals("0") ? "未登録" : this.changekur(indKurCd, facilityCd);
      //変更後クールコードをクール名称に変換
      String editIndKurName = editIndKurCd.equals("0") ? "未登録" : this.changekur(editIndKurCd, facilityCd);
    ...
    if (Arrays.asList(paramTargetArray).contains("治療開始時刻")) {
      //値がnullの場合は"未登録"と表記を修正
      indTreatStartTime = Objects.isNull(indTreatStartTime) ? "未登録" : indTreatStartTime;
      editIndTreatStartTime = Objects.isNull(editIndTreatStartTime) ? "未登録" : editIndTreatStartTime;
      //内容一覧に追加
    ...
    if (Arrays.asList(paramTargetArray).contains("ベッド")) {
      //変更前ベッドコードをベッド名称に変換
      String indBedName = indBedCd.equals("0") ? "未登録" : this.changeBed(indBedCd, facilityCd);
      //変更後ベッドコードをベッド名称に変換
      String editIndBedName = editIndBedCd.equals("0") ? "未登録" : this.changeBed(editIndBedCd, facilityCd);
    ...
    if (Arrays.asList(paramTargetArray).contains("治療日")) {
      SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
      //内容一覧に追加
      Date preDate = null;
      Date nextDate = null;
      try {
        preDate = sdf.parse(treatDate);
        nextDate = sdf.parse(editTreatDate);
      } catch (ParseException e) {
        // #9700 イベントログに出るべきではないもの、判読不可能なログがある 20260402 del yangxuewang start
//      e.printStackTrace();
        // #9700 イベントログに出るべきではないもの、判読不可能なログがある 20260402 del yangxuewang end
      ...
      String indTreatDate = new SimpleDateFormat("yyyy/MM/dd ").format(preDate);
      String editIndTreatDate = new SimpleDateFormat("yyyy/MM/dd ").format(nextDate);
      paramContentList.add(indTreatDate + "→" + editIndTreatDate);
    }
...
//    paramContent = String.join(",", paramContentList);
   ...
   * @param facilityCd    施設コード
   * @param ordMains       編集前の値一覧
   * @param targetNumList 編集を行った対象項目の項目番号の一覧
   * @return 治療条件内容
   */
  public String createTreatCondContent(
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//    JSONObject indInfo, String facilityCd, OrdMain ordMain, List<Integer> targetNumList) {
    JSONObject indInfo, String facilityCd, List<OrdMain> ordMains, List<Integer> targetNumList) {
    // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
    //設定する内容
    String paramContent = null;
    //編集を行った対象項目一覧
...
//    JSONObject ordMainCond = new JSONObject(indCondInfoTmp);
    ...
    for (int num : targetNumList) {
      ...
      // if (! ordMainCond.has(String.valueOf(num))) {
      // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm start
//      if (! ordMainCond.has(String.valueOf(num)) && !(num == 39)) {
      if (ordMainConds.stream().noneMatch(o -> o.has(String.valueOf(num))) && !(num == 39) && !(num == 9) && !(num == 10) && !(num == 11)) {
        // mod 11555 指示履歴への記録の残り方が仕様と異なる zkm end
...
//        }
//      } catch (Exception e) {
//        //薬剤が存在しない:薬剤系が変更されていないことにより取得できないと判断し、編集前データの値を使用する
//        editMedicineCd = preMedicineCd;
//        editMedicineType = preMedicineType;
//        editMedicineUnit = indInfo.getJSONObject(String.valueOf(num)).get("unit");
//      }
//      // mod 11019 治療条件の編集箇所のみをOFFで保存すると500エラーが発生する 関 start
//      String content = "";
...
//        paramContentList.add("null");
//      }
//      // mod 11019 治療条件の編集箇所のみをOFFで保存すると500エラーが発生する 関 end
      String postContent = getConditionValUnit(facilityCd, num, indInfo);

      List<String> preContents = ordMains.stream().map(o -> {
        JSONObject ordMainCond = null == o.getIndCondInfo() ? new JSONObject() : new JSONObject(o.getIndCondInfo());
...
//      if (num == 3) {
//        // TODO #10196 del valueDw for now
//        if (condInfo.has("indicator_start_date") && condInfo.has("value_dw")) {
//
//          if (Objects.equals(condInfo.opt("indicator_start_date"), null) &&
//            ! Objects.equals(condInfo.opt("value_dw"), null)) {
//            // 身体情報画面上に「目標体重」が変更なしの場合、登録目標から外す
//            paramContentList.remove(content);
//          }
//          String editValueDw =
//            Objects.equals(condInfo.opt("value_dw"), null)
//              ? null
//              : condInfo.opt("value_dw").toString();
//          if (! Objects.equals(editValueDw, null)) {
//            String initValueDw =
//              Objects.equals(condInfo.opt("init_value_dw"), null)
//                ? null
//                : condInfo.opt("init_value_dw").toString();
//            if (! editValueDw.equals(initValueDw)) {
//              initValueDw = Objects.equals(initValueDw, null) ? "未登録" : initValueDw;
//              String initCondUnitDw =
//                initValueDw.equals("未登録")
//                  ? ""
//                  : this.gettingCondUnit(String.valueOf(num), facilityCd, editMedicineCd, editMedicineType, editMedicineUnit);
//              String editCondUnitDw =
//                this.gettingCondUnit(String.valueOf(num), facilityCd, editMedicineCd, editMedicineType, editMedicineUnit);
//              paramContentList.add(initValueDw + initCondUnitDw + "→" + editValueDw + editCondUnitDw);
//            }
//          } else {
...
//    paramContent = paramContentList.size() > 0 ? String.join(",", paramContentList) : null;
   ...
   */
  // mod #2856：DWの指示履歴登録対応 韓 start
  public String createTreatCondTarget(List<Integer> targetNumList, JSONObject indInfo) {
    // mod #2856：DWの指示履歴登録対応 韓 end
    //設定する対象
    String paramTarget = null;
    //編集を行った対象項目一覧
    ...
    for (int num : targetNumList) {
EventLogMessage eventLogMessage = new EventLogMessage();
eventLogMessage.setLogMessage("DEBUG_対象項目のnum:" + String.valueOf(num));
logService.log(LogLevel.INFO, eventLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);
      //項目番号に該当する項目名称を取得
  ...
  public String createMedicineContent(
    ...
    switch (flag) {
      ...
      case "2":
        ...
        //登録する投与間隔の文章を作成(変更が無い場合は登録する値のみ表示)
        String intervalRight = hasWeekDay ? dayInterval + "（" + weekDay + "）" : dayInterval;
...
//      paramContent = "指示コメント" + contentNum + "→中止";
   ...
   */
  // #10959 システム内でstatic変数を使っている箇所の洗い出し mod yangxuewang start
  public Map<String, String> createKeyValues(OrdMain ordMain, String facilityCd, Boolean isIncludingMedicine) {
    // #10959 システム内でstatic変数を使っている箇所の洗い出し mod yangxuewang end
    //登録する各項目の値一覧を格納するMap
    ...
    String indTreatmentCd =
      Objects.isNull(ordMain.getIndTreatmentCd())
        ? ""
        : String.valueOf(ordMain.getIndTreatmentCd());
    //治療方法コードをmapに追加
...
//      e.printStackTrace();
    ...
    //スケジュール変更の各項目の値をmapに追加
//    ordMainMap.put(
//      "スケジュール変更",
//      String.valueOf(indKurName + "," + indTreatStartTime + "," + indBedName));
    ordMainMap.put(
      "スケジュール変更",
      JSONObject.valueToString(dataList));
//mod 8339 2023-02-05 名前に半角カンマが含まれる治療方法と紐づく治療方法セットを用いた際の動作不正 end
    //登録する治療条件を取得
...
//    /* add #9355  by zhangruixue 2023-09-06 --end */
   ...
   * @return 指示履歴用パラメータ
   */
  public IndHistory setBasicParams(
    String patId,
    String startDate,
    String endDate,
    List<Integer> weeksArray,
    ...
    /**指示者、更新者*/
    //セットする指示者氏名
    String paramIndUserName = this.changeUserName(indUserId, facilityCd);
    //セットする更新者氏名
    String paramUpdUserName = this.changeUserName(updUserId, facilityCd);
   ...
   * @return 曜日番号に対応する曜日
   */
  public String changeWeekday(List<Integer> weeksArray) {
    //曜日番号に対応する曜日
    String weekDay = null;
    //対応する曜日を格納したリスト
   ...
   * @return 変換した曜日名
   */
  public String getWeekName(String strNo) {
    String rtnName = "";
    switch (strNo.trim()) {
      case "1":
        rtnName = "月";
   ...
   */
  public String gettingEditTarget(String targetNum) {
    //対象項目名称
   ...
   * @return 治療条件対象の値
   */
  public String gettingValue(String targetKey, JSONObject objectTarget, String facilityCd) {
    return gettingValue(targetKey, objectTarget, facilityCd, null, null);
  }
   ...
   * @return 治療条件対象の値
   */
  public String gettingValue(String targetKey, JSONObject objectTarget, String facilityCd, Object medicineCd, Object medicineType) {
    //治療条件対象の値
   ...
   * @return 治療条件の単位
   */
  public String gettingCondUnit(String targetKey, String facilityCd, Object medicineCd, Object medicineType, Object medicineUnit) {
    //治療条件の単位
    String unit = "";
    ...
    switch (targetKey) {
      ...
      case "38":
        unit = "分前";
        break;
      // add 11019 治療条件の編集箇所のみをOFFで保存すると500エラーが発生する 関 start
      //DW
      case "39":
        unit = "kg";
        break;
      // add 11019 治療条件の編集箇所のみをOFFで保存すると500エラーが発生する 関 end
   ...
   */
  public String changeTreatment(String indTreatmentCd, String facilityCd) {
...
//      List<MstTreatment> mstTreatmentList = mstTreatmentDao.selectAll(selectOptions, mstTreatment);
      List<MstTreatment> mstTreatmentList = MasterCacheHandler.get().getMstTreatmentInfo(facilityCd);
   ...
   */
  public String changekur(String kurCd, String facilityCd) {
...
//    List<MstKur> mstKurList = mstKurDao.selectByFacilityCd(selectOptions, facilityCd, "0");
    List<MstKur> mstKurList = MasterCacheHandler.get().getMstKurInfo(facilityCd);
...
//      return "すべて";
    ...
    // add #11166 治療予定のクールが0のみ、"未登録"を戻る zkm start
    if (indKurCdlist.size() == 1 && indKurCdlist.get(0).equals(0)) {
      return "未登録";
    }
    // add #11166 治療予定のクールが0のみ、"未登録"を戻る zkm end
   ...
   */
  public String changeUserName(String userCd, String facilityCd) {
    //指示者コード・更新者コードに対応する利用者氏名
    String paramIndUserName = null;
    //指示者・更新者コードの比較のため、nullの場合は空文字に変換
    String checkedUserCd = Objects.isNull(userCd) ? "" : userCd;
    //マスタ取得処理
//    SelectOptions selectOptions = SelectOptions.get();
//    List<MstPersonalUser> mstPersonalUserList = mstPersonalUserDao.selectAll(selectOptions, facilityCd, "0");
//    //利用者IDに対応する指示者氏名を取得
//    for (int i = 0 ; i < mstPersonalUserList.size() ; i++) {
//      //マスタ利用者IDを数値→文字列に変換
//      String mstUserId = String.valueOf(mstPersonalUserList.get(i).getUserId());
//      //指示者ID(更新者ID)と比較し、利用者IDに対応する利用者氏名を取得
//      if(checkedUserCd.equals(mstUserId)){
//        paramIndUserName = mstPersonalUserList.get(i).getUserLastName() +" "+ mstPersonalUserList.get(i).getUserFirstName();
//      }
//    }
...
//      MstPersonalUser mstPersonalUserList = mstPersonalUserDao.selectById(userId);
      MstPersonalUser mstPersonalUserList = MasterCacheHandler.get().getMstPersonalUser(userId);
   ...
   */
  public String changeEquipment(String equipmentCd, String facilityCd) {
   ...
   */
  public String gettingMedicineUnit(String medicineCd, String facilityCd, String medicineType, String unitType) {
    //投与薬剤単位
    String unit = null;
    ...
    if (checkedMedicineType.equals("1") && Objects.nonNull(medicineCd) && ! (checkedMedicineCd.equals("null"))) {
      // 対象が薬剤マスタの場合
      // modify 10150_9664 by kangjie 20240523 start  update API error：NumberFormatException->For input string:""
//      MstMedicine medicine = mstMedicineDao.selectByCd(facilityCd, Integer.parseInt(checkedMedicineCd));
      ...
      // modify 10150_9664 by kangjie 20240523 end  update API error：NumberFormatException->For input string:""
      if (medicine != null) {
...
//      MstMedicineMix medicineMix = mstMedicineMixDao.selectByCd(facilityCd, Integer.parseInt(checkedMedicineCd));
   ...
   */
  public String gettingEquipmentUnit(String equipmentCd, String facilityCd) {
    //医療材料単位
    String unit = null;
   ...
   */
  public String changeMedicine(String medicineCd, String medicineType, String facilityCd) {
    //投与薬剤コードに対応する投与薬剤名
    String medicineName = "";
    //投与薬剤コードの比較のため、nullの場合は空文字に変換
    String checkedMedicineCd = Objects.isNull(medicineCd) ? "" : medicineCd;
   ...
   * @return 投与間隔の名称
   */
  public String changeInterval(String intervalNum) {
    //投与間隔の名称
   ...
   */
  public String changeProcedure(String procedureCd, String facilityCd) {
    //手技コードに対応する手技名称
    String procedureName = "";
   ...
   */
  public String changeTiming(String timingCd, String facilityCd) {
    //投与薬剤コードに対応する投与薬剤名
    String timingName = "";
   ...
   */
  public String changeBed(String bedCd, String facilityCd) {
    //ベッドコードに対応するベッド名
    String bedName = "";
...
//    List<MstBed> mstBedList = mstBedDao.selectByFacilityCd(selectOptions, facilityCd, "1", "0");
    List<MstBed> mstBedList = MasterCacheHandler.get().getMstBedInfo(facilityCd);
   ...
   */
  public String changeVA(String vaCd, String facilityCd) {
    //VAコードに対応するVA名
    String vaName = "";
   ...
   */
  public String changeDialyzer(String dialyzerCd, String facilityCd) {
   ...
   */
  public List<Integer> gettingTargetNum(JSONObject indInfo) {
   ...
   */
  public String changeMedicineValue(String targetValue, Object medicineCd, Object medicineType, String facilityCd, String unitType) {
    ...
    if (checkedMedicineType.equals("1") && Objects.nonNull(medicineCd) && ! (checkedMedicineCd.equals("null"))) {
      ...
      if (medicine != null) {
        if (unitType.equals("0")) {
          ...
          // 指示単位
          //mod FNSI 劉全航 start
//          medicineDecPoint = medicine.getUnitDecimalPoint();
   ...
   * @return 薬剤数量(小数点桁数制御後)
   */
  public String changeMedicineValue(String targetValue, Integer decPoint) {
   ...
   * @return key一覧
   */
  public List<String> gettingKeylist(Map<String, String> keyMap) {
    //登録する項目のキー名を取得
  ...
  /* add #5482 by zhangruixue 2023-03-02 スケジュール --start */
  /**
   * 指示履歴登録処理(スケジュール変更) Batch
   * createScheduleHistoryのオーバーロードメソッド（設定パラメーターを指定する） Batch
   *
   * @param bodyData    (指示履歴作成に必要な値一覧)
   * @param flag        (1.新規、2.変更,3.削除。ここでは2しか無い)
   * @param weeksArray  編集対象曜日
   * @param paramTarget 設定パラメーター
   */
  public List<IndHistory> createScheduleHistoryBatch(
    ApiEntityOrdMain.ValiUpdateIndSchedule bodyData,
    String flag,
    List<Integer> weeksArray,
    List<OrdMain> ordMainList,
    ...
    for (OrdMain ordMain : ordMainList) {
      ...
      for (int i = 0; i < paramTargetArray.length; i++) {
        ...
        indHistory.setLogDate(setLogDate);
        //指示履歴登録処理
        indHistory = this.createHistoryListBatch(indHistory, flag);
...
//        indHistoryList.add(indHistory);
   ...
   * @param flag       フラグ(1.新規、2.変更,3.削除、4.治療日変更)
   */
  public IndHistory createHistoryListBatch(IndHistory indHistory, String flag) {
    //操作区分の値を設定
    switch (flag) {
      //新規の場合
      case "1":
  ...
  /* add #5482 by zhangruixue 2023-03-02 スケジュール --end */
...
//   */
//  public void getBvUfcContent(JSONObject bvufc,List<String> resultContentList){
...
//   * @param resultContentList
//   */
//  public void getUfrContent(JSONObject ufr,List<String> resultContentList){
...
//   * @param resultContentList
//   */
//  public void getDcContent(JSONObject dc,List<String> resultContentList){
...
//   * @param resultContentList
//   */
//  public void getNaContent(JSONObject na,List<String> resultContentList){
...
//   * @param resultContentList
//   */
//  public void getQbqdContent(JSONObject qbqd,List<String> resultContentList){
...
//   * @param resultContentList
//   */
//  public void getIhdfContent(JSONObject ihdf,List<String> resultContentList){
//    JSONObject ihdfDev = getJSONObject(ihdf,"dev");
//    JSONObject ihdfA = getJSONObject(ihdfDev, "A");
//    if(ihdfA != null){
////      Object value = ihdfA.get("432");
////      String usedFlag = "";
////      if (value instanceof String) {
////        usedFlag = (String) value;
////      }
...
//      // del #11166 467-469がmst_device_set_info_default.device_set_infoに管理して、ord_mainに削除されましたので、処理不要となる zkm start
////      resultContentList.add(addResultContent(ihdfA,"I-HDF", "467","TMPゼロ補正開始時間:"));
////      resultContentList.add(addResultContent(ihdfA,"I-HDF", "468","TMPゼロ補正時間:"));
////      resultContentList.add(addResultContent(ihdfA,"I-HDF", "469","VA確認報知基準値(アクセス内圧力比率):"));
//      // del #11166 467-469がmst_device_set_info_default.device_set_infoに管理して、ord_mainに削除されましたので、処理不要となる zkm end
//    }
//
//  }
//
...
//   * @param resultContentList
//   */
//  public void getDiaContent(JSONObject dia,List<String> resultContentList){
...
//
//
//  public List<String> creatDeviceSetInfoContent(String jsonString){
...
//   * @return
//   */
//  public String convertBigDecimal(String bigDecimalStr,int digit){
...
//   * @param ordMainList 更新対象となる治療情報
//   */
//  public void createDeviceSetInfoHistory(
//    ApiEntityDeviceSetInfo.ValiDeviceSetInfo bodyData,
//    String flag,
//    List<Integer> weeksArray,
//    List<OrdMain> ordMainList) {
//    //指示履歴用パラメータ
...
//   * @return indHistory 指示履歴用パラメータ
//   */
//  public IndHistory createDeviceSetInfoHistoryParams(ApiEntityDeviceSetInfo.ValiDeviceSetInfo bodyData, List<Integer> weeksArray, OrdMain ordMain) {
//
//    //指示履歴用パラメータ
//    IndHistory indHistory = new IndHistory();
//    //装置設定内容
...
//
//    //画面から取得した対象項目の項目番号の一覧を取得
////    List<Integer> targetNumList = this.gettingTargetNum(deviceSetInfo, ordMain);
//
//    /**画面から取得した各値を取得*/
//    //患者コード
//    String patId = bodyData.getPat_id();
...
//   * @return 治療条件内容
//   */
//  public String createDeviceSetInfoContent(ApiEntityDeviceSetInfo.ValiDeviceSetInfo bodyData, OrdMain ordMain) {
//    //設定する内容
//    String paramContent = null;
//    //編集を行った対象項目一覧
//    List<String> paramContentList = new ArrayList<>();
...
//   * @return
//   */
//  public String getSelectString(String param,int flag){
//
//    String resultString = "";
//    if(flag == 0){
...
//   * @return
//   */
//  public String getPowerString(String param,int flag){
//    String resultString = "";
//    if(flag == 0){
//      String[] parts = param.split(":");
//      if (parts.length > 1) {
...
//   * @return
//   */
//  public String get340PowerString(String param,int flag){
//
//    String resultString = "";
//    if(flag == 0){
//      String[] parts = param.split(":");
...
//   * @return
//   */
//  public String getOFF_ONString(String param,int flag){
//    String resultString = "";
//    if(flag == 0){
//      String[] parts = param.split(":");
//      if (parts.length > 1) {
...
//   * @return
//   */
//  public String getQB_QDString(String param,int flag){
//    String resultString = "";
//    if(flag == 0){
//      String[] parts = param.split(":");
//      if (parts.length > 1) {
...
//   * @return
//   */
//  public String getHD_ECUMString(String param,int flag){
//    String resultString = "";
//    if(flag == 0){
//      String[] parts = param.split(":");
//      if (parts.length > 1) {
...
//  }
//
//  public JSONObject getJsonByImageFlag(JSONObject bodyData,String imagFlg){
...
//   * @param bodyData       指示履歴作成に必要な値一覧
//   */
//  public void createIndTareInfoHistory(
//    ApiEntityOrdMain.ValiCreateTreatPlan bodyData,OrdMain ordMain,String indTareInfo,List<Integer> weeksArray,String TareName,String flag) {
//    //指示履歴用パラメータを定義し、各値を入力
//    IndHistory indHistory = new IndHistory();
//    String setLogDate = new SimpleDateFormat(FORMAT_DATE).format(new Date());
//    //患者コード
...
//   */
//  public List<String> createIndTareInfoContent(String oldIndTareInfo,String newIndTareInfo,String flag) {
//    Gson gson =new Gson();
//    List<String> paramContentList = new ArrayList<>();
...
// del 11555 指示履歴への記録の残り方が仕様と異なる zkm end
  ...
  /**
   * Journal作成API呼び出し
   * @param params IndHistory
   * @return ResponseEntity
   * @throws URISyntaxException
   */
  public ResponseEntity<?> callCreateJournal(IndHistory params) throws URISyntaxException {
    String today = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
    JournalCreateRequestPayload payload = new JournalCreateRequestPayload();
    PatPersonalMain patPersonalMain = patPersonalMainDao.selectById(Long.parseLong(params.getPatId()));
    payload.setFacilityCd(params.getFacilityCd());
    payload.setPatId(Long.parseLong(params.getPatId()));
    payload.setHospPatId(patPersonalMain.getHosp_pat_id());
    payload.setOrdNo(Long.parseLong(params.getLogDate()));
    payload.setBaseDate(today);
    payload.setCrud("C");
    payload.setOpeCd("004000");
    payload.setCoopCdIndex("");
    payload.setUserId(params.getCreatedUserId());
  ...
  /**
   * リクエスト終了時にThreadLocalをクリーンアップ
   * すべてのindHistory登録完了後にJournal作成APIを呼び出し
   */
  public void clearRequestCache() {
    long threadId = Thread.currentThread().getId();
    EventLogMessage eventLogMessage = new EventLogMessage();
    eventLogMessage.setLogMessage("clearRequestCache called. thread=" + threadId);
    logService.log(LogLevel.INFO, eventLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);

    try {
      // キューに蓄積されたindHistoryに対してJournal作成APIを呼び出し
      List<IndHistory> pendingList = pendingJournalCreations.get();
      eventLogMessage = new EventLogMessage();
      if (!pendingList.isEmpty()) {
        eventLogMessage.setFacilityCd(pendingList.get(0).getFacilityCd());
      }
      eventLogMessage.setLogMessage("clearRequestCache: pendingList size=" + pendingList.size() + ", thread=" + threadId);
      logService.log(LogLevel.INFO, eventLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);

      Set<String> processedLogDates = new HashSet<>();
      int journalCallCount = 0;

      for (IndHistory indHistory : pendingList) {
        if (indHistory.getLogDate() != null &&
            processedLogDates.add(indHistory.getLogDate())) {
          try {
            eventLogMessage = new EventLogMessage();
            eventLogMessage.setFacilityCd(indHistory.getFacilityCd());
            eventLogMessage.setLogMessage("clearRequestCache: calling callCreateJournal for logDate=" + indHistory.getLogDate() + ", thread=" + threadId);
            logService.log(LogLevel.INFO, eventLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);
            this.callCreateJournal(indHistory);
            journalCallCount++;
          } catch (URISyntaxException exception) {
            eventLogMessage = new EventLogMessage();
            eventLogMessage.setFacilityCd(indHistory.getFacilityCd());
            eventLogMessage.setLogMessage("clearRequestCache: callCreateJournal failed for logDate=" + indHistory.getLogDate() + ", error=" + exception.getMessage() + ", thread=" + threadId);
...
//          exception.printStackTrace();
      ...
      eventLogMessage.setLogMessage("clearRequestCache: journal calls completed. callCount=" + journalCallCount + ", thread=" + threadId);
      logService.log(LogLevel.INFO, eventLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);

    } finally {
      // ThreadLocalをクリーンアップ
      requestProcessedLogDates.remove();
      pendingJournalCreations.remove();
      EventLogMessage finalLogMessage = new EventLogMessage();
      finalLogMessage.setLogMessage("clearRequestCache: ThreadLocal cleanup completed. thread=" + threadId);
      logService.log(LogLevel.INFO, finalLogMessage, null, LoggingConstant.SERVICE_NAME.FNSI, null);
    }
  }
}
