...
@Service
public class BVGraphService extends AbstractBVMSService<BVMSFilterDTO, BVGraphDTO> {
  ...
  @Override
  public BVGraphDTO getGraph(Long ordNo, BVMSFilterDTO inputDTO) throws NotExistException {
  ...
  @Override
  public BVGraphDTO getGraphByUploadFile(Long ordNo, MultipartFile file, BVMSFilterDTO inputDTO) {
...
//      e.printStackTrace();
    ...
    return getGraph(ordNo, getBVMSGraphDTOFromUploadFile(file), inputDTO);
  }

    private BVGraphDTO getGraph(Long ordNo, BVMSGraphDTO graphDTO, BVMSFilterDTO filter) {
        BVGraphDTO bvGraphData = new BVGraphDTO();
        List<ErrorCellDTO> errorCellDTOs = graphDTO.getErrorCells();
        if (errorCellDTOs != null && !errorCellDTOs.isEmpty()) {
            bvGraphData.setErrorCells(errorCellDTOs);
            return bvGraphData;
        }

        BVGraphDTO filterBVGrapDTO = apdapterService.adaptBVGrapDTO(graphDTO, filter);
        return filterBVGrapDTO;
    }
