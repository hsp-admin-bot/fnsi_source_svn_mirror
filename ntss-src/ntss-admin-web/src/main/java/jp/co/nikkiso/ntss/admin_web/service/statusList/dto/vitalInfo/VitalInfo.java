...
//      e.printStackTrace();
  ...
  private void setItems(JsonNode jsonNodeArray) {
    ...
    if (beforeNodes.size() > 0) {
      int latestIdx_before = Util.getLatestOccurDateIndex(beforeNodes);
    ...
    if (afterNodes.size() > 0) {
      int latestIdx_after = Util.getLatestOccurDateIndex(afterNodes);
    ...
    if (temperatureNodes.size() > 0) {
      int latestIdx_temperature = Util.getLatestOccurDateIndex(temperatureNodes);
  ...
  private VitalInfoItem createVitalInfoItem(JsonNode jsonNode) {
    ...
    rtn.occurDate = DateTimeUtils.dateStringToDate_iso8601(occurDate_node.asText());
