...
@Service
public class RRGraphService extends AbstractBVMSService<RRGraphFilterDTO, RRGraphDTO> {
  ...
  // #9700 イベントログに出るべきではないもの、判読不可能なログがある 20260402 add yangxuewang end
    ...
    @Override
    public RRGraphDTO getGraph(Long ordNo, RRGraphFilterDTO inputDTO) throws NotExistException {
    ...
    @Override
    public RRGraphDTO getGraphByUploadFile(Long ordNo, MultipartFile file, RRGraphFilterDTO filter) {
      //add FNSI-8360 ljx start
      // #9700 イベントログに出るべきではないもの、判読不可能なログがある 20260402 mod yangxuewang start
      String facilityCd = "";
      try {
        facilityCd = getFacilityCdByOrdNo(ordNo);
...
//      e.printStackTrace();
    ...
    private RRGraphDTO getGraph(Long ordNo, BVMSGraphDTO rawDataDTO, RRGraphFilterDTO filter) {
        ...
        RRGraphDTO filterRRGraphDTO = apdapterService.adaptRRGraphDTO(rawDataDTO, filter);
        return filterRRGraphDTO;
    }
