...
public class SysSystemDefineServiceImpl implements SysSystemDefineService {
  ...
  public boolean IsProcServer(SysSystemDefineCtlNo ctlNo, String ipAddress) {
    ...
    if (true == StringUtils.isEmpty(ipAddress)) {
      ...
      eventLogMessage.setLogMessage("データ自動収集監視：処理実施対象サーバーか確認するためのローカルIPアドレスが null または空文字");
    ...
    if (null == lstSysSystemDefine) {
      // エラー(false を返す)
      EventLogMessage eventLogMessage = new EventLogMessage();
      eventLogMessage.setLogMessage("データ自動収集監視：システム設定情報の取得に失敗　管理番号[" + ctlNo.getNo() + "]");
      //FNSI-修正 ログ対応 xiebzh add start
      eventLogMessage.setInvokeClass(this.getClass().getName());
      //FNSI-修正 ログ対応 xiebzh add end
      logService.log(LogLevel.ERROR, eventLogMessage, null, SERVICE_NAME.FNSI, null);
      ...
      // エラー(false を返す)
      EventLogMessage eventLogMessage = new EventLogMessage();
      eventLogMessage.setLogMessage("データ自動収集監視：システム設定情報の取得件数が0件　管理番号[" + ctlNo.getNo() + "]");
      //FNSI-修正 ログ対応 xiebzh add start
      eventLogMessage.setInvokeClass(this.getClass().getName());
      //FNSI-修正 ログ対応 xiebzh add end
      logService.log(LogLevel.ERROR, eventLogMessage, null, SERVICE_NAME.FNSI, null);
    ...
    try {
      ...
      EventLogMessage eventLogMessage = new EventLogMessage();
      eventLogMessage.setLogMessage("データ自動収集監視：システム設定情報のJsonデータ展開処理に失敗　Jsonデータ[" + lstSysSystemDefine.get(0).getValue() + "]");
      //FNSI-修正 ログ対応 xiebzh add start
      eventLogMessage.setInvokeClass(this.getClass().getName());
      //FNSI-修正 ログ対応 xiebzh add end
      logService.log(LogLevel.ERROR, eventLogMessage, null, SERVICE_NAME.FNSI, null);
    ...
    if (false == ipAddress.equals(map.get("ip_address"))) {
      ...
      EventLogMessage eventLogMessage = new EventLogMessage();
      eventLogMessage.setLogMessage("データ自動収集監視：処理対象外サーバー　システム設定[" + map.get("ip_address") + "]、確認サーバー[" + ipAddress + "]");
      //FNSI-修正 ログ対応 xiebzh add start
      eventLogMessage.setInvokeClass(this.getClass().getName());
      //FNSI-修正 ログ対応 xiebzh add end
