DELETE  FROM sys_data_set WHERE sql_cd IN (-192, -193, -194, -195, -196, -197);
...
limit 1
', 2, '[{}]', '0', '{"applications": [4]}', NULL, '実績）担当医先頭１名(担当医→版確定者）', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-193, 'select
...
and
  is_del = ''0''
', 3, '[]', '0', '{"applications": [4]}', NULL, 'スタッフ名取得用　@ordno使用', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -192, "field_name": "staff_cd", "replace_var": "@userId"}]');
...
order by all_cost.e09', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通：透析予約繰り返し部', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -193, "field_name": "user_name", "replace_var": "@userName"}]');
...
FROM
  ...
  ) AS T', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：共通部と伝票情報の利用者番号取得', '2022-02-28 14:34:34.866', '2022-04-18 18:05:20.486', NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-196, 'SELECT 
    disp_user_id AS disp_user_id 
FROM 
    mst_user_authentication 
WHERE 
    user_id = @userId', 1, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：施設内職員ID(共通部)取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -195, "field_name": "staff_cd_comm", "replace_var": "@userId"}]');
...
FROM 
    mst_user_authentication 
WHERE 
    user_id = @userId', 1, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）透析予約：施設内職員ID(伝票情報部)取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -195, "field_name": "staff_cd_data", "replace_var": "@userId"}]');
