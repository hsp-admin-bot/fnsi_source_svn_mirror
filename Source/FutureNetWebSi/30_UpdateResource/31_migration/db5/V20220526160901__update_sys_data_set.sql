DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (1011,1102,1103);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1011, 'SELECT
    CASE WHEN @patBloodTypeAbo = TRIM(ini_info ->> ''value'') THEN TO_NUMBER(@patBloodTypeAbo, ''FM9999999999999999'') 
       ELSE ''0''
    END AS check_value
  FROM
    mst_coop_ini AS ini 
...
)', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報の取得の新規', '2020-05-25 18:21:40.841',CURRENT_TIMESTAMP, '[{"sql_cd": 1001, "field_name": "dial_diff_com_info", "replace_var": "@dialDiffComInfo"}, {"sql_cd": 1010, "field_name": "check_value", "replace_var": "@patBloodRhType"}, {"sql_cd": 1011, "field_name": "check_value", "replace_var": "@patBloodAboType"}]');
...
WHERE
  ...
  AND facility_cd = ''@facilityCd''', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報の取得の修正', '2022-01-07 18:21:46',CURRENT_TIMESTAMP, '[{"sql_cd": 1009, "field_name": "up_base_disease_flg", "replace_var": "@upBaseDiseaseFlg"}, {"sql_cd": 1010, "field_name": "check_value", "replace_var": "@patBloodRhType"}, {"sql_cd": 1011, "field_name": "check_value", "replace_var": "@patBloodAboType"}]');
