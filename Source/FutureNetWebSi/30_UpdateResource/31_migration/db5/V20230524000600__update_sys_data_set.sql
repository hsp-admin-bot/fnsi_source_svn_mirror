DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (7409);
...
mstSet1_tbl AS (
  SELECT unnest(regexp_split_to_array(value, '','')) AS tmpVal1 FROM mstSet_1
),
mstSet_2 AS (
  SELECT
 B.facility_setting_no As facility_setting_no,
...
mstSet2_tbl AS (
  SELECT unnest(regexp_split_to_array(value, '','')) AS tmpVal2 FROM mstSet_2
),
infect_tbl AS (
  SELECT
	  CASE WHEN EXISTS (SELECT tmpVal1 FROM mstSet1_tbl WHERE tmpVal1 = ''@examResultInfo.result'') THEN
		  ''2''
		ELSE 
		  CASE WHEN EXISTS (SELECT tmpVal2 FROM mstSet2_tbl WHERE tmpVal2 = ''@examResultInfo.result'') THEN
		    ''1''
		  ELSE
			  ''0''
			END
...
WHERE
  facility_cd = ''@facilityCd'' 
	AND pat_id = @patId', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)日機装の検査結果(検査結果情報更新)', '2023-04-10 23:29:43.109', CURRENT_TIMESTAMP, NULL);
 ...
 SET
  ...
  ELSE
	  CASE WHEN exam_result_info IS NOT NULL AND exam_result_info <> ''[]'' THEN
		  jsonb_set(exam_result_info, array[(SELECT ORDINALITY::INT - 1 FROM pat_exam_main pea2, jsonb_array_elements(exam_result_info) WITH
...
WHERE  pat_id = @patId 
  ...
  AND	is_del = ''0''
	AND @item_cd != ''0''
	AND reg_exam_date = to_timestamp(''@resultExamDate_Date'', ''yyyy-MM-dd hh24:mi:ss'')
