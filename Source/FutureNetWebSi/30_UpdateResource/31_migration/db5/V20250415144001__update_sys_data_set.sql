...
select case
		...
		when 0 then 
			case when (select value from pat_staff_cd)::text is not null then (select value from pat_staff_cd)::text
				else (select value from ord_staff_cd)::text
			end
		when 1 then 
			case when (select value from ord_staff_cd)::text is not null then (select value from ord_staff_cd)::text
				else (select value from treatment_staff_cd)::text
			end
		when 2 then 
			case when (select value from treatment_staff_cd)::text is not null then (select value from treatment_staff_cd)::text
				else ''''
			end
	end as staff_cd
from input_code_class
...
select case
		when input_code_class.value = ''0'' then case
			when (select value from pat_staff_cd)::text is not null then (select value from pat_staff_cd)::text
			else (select value from ord_staff_cd)::text
		end
