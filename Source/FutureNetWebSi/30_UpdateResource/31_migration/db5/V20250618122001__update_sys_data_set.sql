DELETE FROM "ntss"."sys_data_set" where sql_cd in (-18, -181, -497, -503);
...
(TO_CHAR(
     (CASE WHEN mix ->> ''solvent'' = ''1'' THEN 1 ELSE ici.mix_count END) * 
     TO_NUMBER(mix ->> ''amount'', ''999999999999.99''),''FM99990.00'')
  ) as e02,
mmd.unit as e03
...
inner join IND_COND_INFO ici on mmm. medicine_mix_cd = to_number(ici.mix_cd, ''999999'')
) AS midd
UNION ALL
    select mmd.in_hospital_cd_1 as e01,
(TO_CHAR(
...
json_array_elements (mmm.mix_info :: json) mix left join mst_medicine as mmd 
on mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
inner join IND_COND_INFO ici on mmm. medicine_mix_cd = to_number(ici.mix_cd, ''999999'') ) as a 
...
''指示薬剤'' :: TEXT as detail_id, a.medi_cd1
  ...
  from 
      ...
      ''指示薬剤'' as detail_id,
        ...
        to_char(((CASE WHEN mmxd ->> ''solvent'' = ''1'' THEN 1 ELSE to_number( medi ->> ''amount'', ''99999.99'' ) END)
              * to_number( mmxd ->> ''amount'', ''99999.99'' )), ''FM99990.00'' ) as medi_amount,
        mmd.unit as medi_unit,
        mp.pricedure_name as procedure_name,
        mp.in_hospital_cd_a1 as procedure_cd1,
        mmd.in_hospital_cd_1 as medi_cd1,
...
SELECT  
    ''指示薬剤'' :: TEXT as detail_id, count_all.medi_cd1, (SELECT middle_data.medi_amount FROM middle_data WHERE middle_data.medi_cd1 = count_all.medi_cd1
                                                  and middle_data.medi_unit IS NOT DISTINCT FROM count_all.medi_unit) as medi_amount,
  ...
  from 
   ...
   WHERE
      mmd.in_hospital_cd_1 is not null and medi ->> ''medicine_type'' = ''2'')  AS a ) AS count_all1 WHERE count_all1.medi_cd1 = count_all.medi_cd1 
                                                                                                   and count_all1.medi_unit IS NOT DISTINCT FROM count_all.medi_unit
        ) as medi_back
...
json_array_elements (mmm.mix_info :: json) mix left join mst_medicine as mmd 
on mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
inner join IND_COND_INFO_ord_main ici on mmm.medicine_mix_cd = to_number(ici.mix_cd, ''999999'')
) AS midd
...
json_array_elements (mmm.mix_info :: json) mix left join mst_medicine as mmd 
on mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
inner join IND_COND_INFO_ord_main ici on mmm.medicine_mix_cd = to_number(ici.mix_cd, ''999999'') ) as a 
    where a.e04 = (SELECT case when (select staff_cd from KOU_COAG_RESOLVE_MODE_cd) IS NULL or 
            (select staff_cd from KOU_COAG_RESOLVE_MODE_cd) = ''''
            then  ''0'' else (select staff_cd from KOU_COAG_RESOLVE_MODE_cd)
            END)
...
json_array_elements (mmm.mix_info :: json) mix left join mst_medicine as mmd 
on mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
inner join IND_COND_INFO_ord_res ici on mmm. medicine_mix_cd = to_number(ici.mix_cd, ''999999'')
) AS midd
...
json_array_elements (mmm.mix_info :: json) mix left join mst_medicine as mmd 
on mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
inner join IND_COND_INFO_ord_res ici on mmm. medicine_mix_cd = to_number(ici.mix_cd, ''999999'') ) as a 
    where a.e04 = (SELECT case when (select staff_cd from KOU_COAG_RESOLVE_MODE_cd) IS NULL or 
            (select staff_cd from KOU_COAG_RESOLVE_MODE_cd) = ''''
            then  ''0'' else (select staff_cd from KOU_COAG_RESOLVE_MODE_cd)
            END)
   ...
   from 
    ...
    where 
        ...
        and ''0'' =(select count(*) from do_ord_main)
                                   ...
                                   and count_all1.medi_unit IS NOT DISTINCT FROM count_all.medi_unit) as medi_back,
        count_all.medi_unit, count_all.medi_type, count_all.order_m
FROM (
...
--     meq.unit AS e05,
 ...
 UNION ALL
   SELECT--投与薬剤情報(調製)
    ...
    to_char(((CASE WHEN mmxd ->> ''solvent'' = ''1'' THEN 1 ELSE to_number( medi ->> ''amount'', ''99999.99'' ) END)
              * to_number( mmxd ->> ''amount'', ''99999.99'' )), ''FM99990.00'' ) as e04,
    mmd.unit AS e05,
    mp.in_hospital_cd_a1 AS e06,
    medi ->> ''procedure_name'' AS e07,
    ''0'' AS e08
  ...
  UNION ALL
    ...
    SELECT--処置薬剤情報(調製)
      ...
      tmedi ->> ''treat_medicine_name'' AS e02,
      mmdc.class_name AS e03,
      to_char(((CASE WHEN mmxd ->> ''solvent'' = ''1'' THEN 1 ELSE to_number( tmedi ->> ''amount'', ''99999.99'' ) END)
              * to_number( mmxd ->> ''amount'', ''99999.99'' )), ''FM99990.00'' ) as e04,
      mmd.unit AS e05,
      mp.in_hospital_cd_a1 AS e06,
      tmedi ->> ''procedure_name'' AS e07,
    ...
    FROM
      ...
      CROSS JOIN LATERAL json_array_elements ( ord.rst_treatment_info :: json ) tmedi
      LEFT OUTER JOIN mst_procedure AS mp ON mp.procedure_cd = TO_NUMBER( tmedi ->> ''procedure_cd'', ''999999999999'' )
      LEFT OUTER JOIN mst_medicine_mix AS mmx ON mmx.medicine_mix_cd = TO_NUMBER( tmedi ->> ''treat_medicine_cd'', ''999999999999'' )
      CROSS JOIN LATERAL json_array_elements ( mmx.mix_info :: json ) mmxd
      LEFT OUTER JOIN mst_medicine AS mmd ON mmd.medicine_cd = TO_NUMBER( mmxd ->> ''cd'', ''999999999999'' )
      LEFT OUTER JOIN mst_medicine_class AS mmdc ON mmdc.class_cd = mmd.class_cd 
    WHERE
      tmedi ->> ''medicine_type'' = ''2'' 
    UNION ALL
        ...
        FROM (
            SELECT * FROM --抗凝固剤
                ...
                SELECT *, ''1'' AS e00 FROM (
                        SELECT mmd.in_hospital_cd_1 AS e01,
                                                             ...
                                                             (TO_CHAR(
															     (CASE WHEN mix ->> ''solvent'' = ''1'' THEN 1 ELSE rci.mix_count END) * 
															     TO_NUMBER(mix ->> ''amount'', ''999999999999.99''),''FM99990.00'')
															  ) as e04,
                                                             mmd.unit AS e05
                        FROM mst_medicine_mix AS mmm cross join lateral
                                 jsON_array_elements (mmm.mix_info :: jsON) mix left join mst_medicine AS mmd 
                                 ON mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
                             inner join RST_COND_INFO rci on mmm.medicine_mix_cd = to_number(rci.mix_cd, ''999999'') ) AS midd
            ...
            UNION ALL
                ...
                FROM mst_medicine_mix AS mmm cross join lateral
                         jsON_array_elements (mmm.mix_info :: jsON) mix left join mst_medicine AS mmd 
                         ON mmd.medicine_cd = TO_NUMBER (mix ->> ''cd'',''999999999999'')
                     inner join RST_COND_INFO rci on mmm.medicine_mix_cd = to_number(rci.mix_cd, ''999999'') ) AS a 
...
SELECT
  ...
  (SELECT middle_data.amount FROM middle_data WHERE middle_data.item_cd = all_cost.e01 and middle_data.e05 IS NOT DISTINCT FROM all_cost.e05) AS amount,
  ...
  UNION ALL
    ...
    UNION ALL
        ...
        AND (SELECT mix_medicine_type FROM RST_COND_INFO) = ''2''
                             ...
                             and all_cost1.e05 IS NOT DISTINCT FROM all_cost.e05) count
 ...
 UNION ALL
  ...
  UNION ALL
    ...
    SELECT--処置薬剤情報(調製)
      ...
      ''0'' AS e08,
      ''5'' AS e09,
      tmedi ->> ''medicine_type'' AS medi_type
    FROM
      do_ord_main AS ord
      CROSS JOIN LATERAL json_array_elements ( ord.rst_treatment_info :: json ) tmedi
      LEFT OUTER JOIN mst_procedure AS mp ON mp.procedure_cd = TO_NUMBER( tmedi ->> ''procedure_cd'', ''999999999999'' )
...
FROM
    data_all ORDER BY item_cd, unit, cond_info_jyun) AS order_middle 
)
SELECT
