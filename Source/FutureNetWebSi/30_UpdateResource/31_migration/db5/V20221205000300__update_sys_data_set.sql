DELETE FROM ntss.sys_data_set WHERE sql_cd IN (-18,-497,-503);
...
''指示薬剤''::TEXT as detail_id,
  ...
  from 
      ...
      (case when mmd.unit_second is null then to_number(medi ->> ''amount'',''FM99999.99'') else (case  when mmd.is_exchange = ''0'' then to_number(medi ->> ''amount'',''FM99999.99'') / mmd.unit_converted_amount * mmd.unit_converted_amount_second   when mmd.is_exchange = ''1'' then trunc( to_number(medi ->> ''amount'',''FM99999.99'') / mmd.unit_converted_amount * mmd.unit_converted_amount_second + 0.9 ,0) when mmd.is_exchange = ''2'' then 1 else  to_number(medi ->> ''amount'',''FM99999.99'') end) end) as res_amount,
        ...
        CASE WHEN medi ->> ''medicine_type'' = ''2'' THEN mmx.in_hospital_cd_1 ELSE mmd.in_hospital_cd_1 END as medi_cd1,
                CASE WHEN medi ->> ''medicine_type'' = ''2'' THEN mmx.in_hospital_cd_2 ELSE mmd.in_hospital_cd_2 END as medi_cd2,
                CASE WHEN medi ->> ''medicine_type'' = ''2'' THEN mmx.in_hospital_cd_3 ELSE mmd.in_hospital_cd_3 END as medi_cd3,
                CASE WHEN medi ->> ''medicine_type'' = ''2'' THEN '''' ELSE mmd.in_hospital_cd_4 END as medi_cd4
    from
        ord_main  as ord
    cross join lateral
...
, do_get_type AS (
SELECT mmx.in_hospital_cd_1 AS cd_1, mmx.medicine_mix_cd AS mix_cd, mmx.class_cd AS class_cd
FROM ord_main AS ord 
    cross join lateral json_array_elements (ord.ind_medi_info :: json) medi     
        LEFT OUTER JOIN mst_medicine_mix AS mmx ON mmx.medicine_mix_cd = TO_NUMBER( medi ->> ''cd'', ''999999999999'' )
  WHERE ord.ord_no = @ordNo AND medi ->> ''medicine_type'' = ''2''  
)
, data_all AS (
SELECT data_middle_all.detail_id, data_middle_all.medi_cd1, data_middle_all.medi_amount, data_middle_all.medi_back, data_middle_all.medi_unit, 
        CASE WHEN data_middle_all.medi_cd1 in (SELECT cd_1 FROM do_get_type) THEN (SELECT mix_cd FROM do_get_type WHERE cd_1 = data_middle_all.medi_cd1) ELSE mst_medicine.medicine_cd END, 
        CASE WHEN data_middle_all.medi_cd1 in (SELECT cd_1 FROM do_get_type) THEN (SELECT class_cd FROM do_get_type WHERE cd_1 = data_middle_all.medi_cd1) ELSE mst_medicine.class_cd END
FROM data_middle_all 
