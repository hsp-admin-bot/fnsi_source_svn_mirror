...
,select_rst_medi AS ( --薬剤
  ...
  FROM (
    ...
    SELECT
      ...
      , CASE WHEN t2.mmxd ->> ''solvent'' = ''1'' THEN
          TO_NUMBER (t2.mmxd ->> ''amount'', ''FM999999999.999999999'')
        ELSE
          CASE
            -- 数量 どちらかが入力されいない場合はNULL
            WHEN NULLIF(ord.rst_cond_info -> ''26'' ->> ''value'', '''') IS NULL
            AND NULLIF(ord.rst_cond_info -> ''28'' ->> ''value'', '''') IS NULL THEN NULL
          ELSE
            (TRUNC(COALESCE(NULLIF(ord.rst_cond_info -> ''26'' ->> ''value'', '''')::numeric, 0),2) +
            TRUNC(COALESCE(NULLIF(ord.rst_cond_info -> ''28'' ->> ''value'', '''')::numeric, 0),2))
            * TO_NUMBER (t2.mmxd ->> ''amount'', ''FM999999999.999999999'')
          END 
        end AS amount --使用量
    FROM do_ord_main ord
