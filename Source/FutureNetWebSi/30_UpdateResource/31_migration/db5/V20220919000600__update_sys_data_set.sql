DELETE  FROM "ntss"."sys_data_set" WHERE sql_cd IN ('-81','-101','-103','-194');
...
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-194, 'WITH item_sort_info AS (
  --連携設定「項目情報部出力順（予約/実績送信用）」の設定値
  SELECT
    info->>''key2'' AS key2 
    , COALESCE(NULLIF(info->>''value'', ''''), info->>''default_v'') AS value 
...
, device AS (
		SELECT device_mode
		FROM mst_treatment mst JOIN ord_main_restore ord 
		ON ord.ind_treatment_cd = mst.treatment_cd 
		AND ord.ord_no = @ordNo
		ORDER BY ord.del_date DESC LIMIT 1
)
...
WHERE 
 all_cost.e09 IS NOT NULL AND all_cost.e09 <> ''''
 AND all_cost.e01 != '''' AND all_cost.e01 IS NOT NULL
order by all_cost.e09', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通：透析予約繰り返し部', '2022-09-15 08:47:18.826', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -193, "field_name": "user_name", "replace_var": "@userName"}]');
