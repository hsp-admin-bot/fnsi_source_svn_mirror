...
FROM (
	...
	, null as medi_timing
	, null as medi_proc
	, om.ind_cond_info::json#>>''{22, value}'' || CASE WHEN om.medicine_type = ''1'' THEN COALESCE(md.unit,'''') ELSE COALESCE(mdx.unit,'''') END as num_unit
	, null as cond_va_dir
	, null as cond_va
	, null as equip_pnc_cls
	...
	, COALESCE(mdc.class_name,'''') as kind,CASE WHEN om.medicine_type = ''1'' THEN md.medicine_name WHEN om.medicine_type = ''2'' THEN mdx.medicine_mix_name ELSE '' '' END as Name
	...
	, CASE WHEN om.medicine_type = ''1'' THEN om.amount || COALESCE(md.unit,'''') ELSE to_number(om.amount, ''9999999999.999'') * mdx.amount_unit || COALESCE(mdx.unit,'''') END as num_unit
	, null as cond_va_dir
	, null as cond_va
	, null as equip_pnc_cls
	FROM ord_medi as om
