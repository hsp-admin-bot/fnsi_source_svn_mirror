...
(-1200000,-1201003,-1201004,-1201005,-1201006);

INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-1200000, 'WITH main_staffs AS (
  SELECT 
...
disp_users AS (
  SELECT
    ua.disp_user_id,
    ROW_NUMBER() OVER (ORDER BY ms.disp_order) AS rn
  FROM main_staffs ms
  LEFT JOIN user_auth_list ua ON ms.staff_cd = ua.user_id
),
--設定関連
default_doctor as (
    -- デフォルト担当医
    select coalesce(nullif(info->>''value'', ''''), info->>''default_v'') as disp_user_id
    from mst_coop_ini as ini
        cross join lateral json_array_elements(ini.coop_ini_info::json) info
    where facility_cd = @facilityCd
        and is_del = ''0''
        and coalesce(info->>''key0'', '''') = @key0
        and info->>''key1'' = ''SX_DOCTOR_INFO''
        and info->>''key2'' = ''DEFAULT_DOCTOR''
)
SELECT
  COALESCE(
    (SELECT disp_user_id FROM disp_users WHERE rn = 1),
    (SELECT disp_user_id FROM disp_users WHERE rn = 2),
    (SELECT disp_user_id FROM default_doctor),
    ''''
  ) AS disp_user_id
', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, '{"classes": []}'::jsonb, 'SX汎用_担当医', '2025-05-23 17:36:09.485', CURRENT_TIMESTAMP, '[{"sql_cd": -1100003, "field_name": "user_list", "replace_var": "@userList"}]'::jsonb);
...
,select_rst_medi AS ( --薬剤
  ...
  FROM (
    SELECT
      ...
      , TO_CHAR(
          TO_NUMBER(COALESCE(ord.rst_cond_info -> ''22'' ->> ''value'', ''0''), ''FM99999999.00'')
        , ''FM000000V99'') AS amount --使用量
    ...
    SELECT
      ...
      , to_char(1 * TO_NUMBER (t2.mmxd ->> ''amount'', ''FM99999999.99''), ''FM000000V99'' ) AS amount --使用量
    FROM do_ord_main ord
    CROSS JOIN lateral jsonb_each(ord.rst_cond_info) AS info
    ...
    WHERE
      ...
      AND (SELECT value FROM kou_coag_resolve_mode) IN (''2'', ''3'') --''1'',''2''?
    UNION ALL
    SELECT
      --投与薬剤(通常)
      ...
      , TO_CHAR(
        TO_NUMBER(COALESCE(t.medi ->> ''amount'', ''0''), ''FM000000.0000'')
      , ''FM000000V99'') AS amount
    FROM do_ord_main ord
...
sum_count as (
  select
    MIN (ttm.e09) as no
    , ttm.e01 as hosp_cd
    , SUM(ttm.amount::numeric) AS amount
    , (select count(tt.e01)
...
select
    ...
    ,RIGHT(TO_CHAR(FLOOR(sm.amount), ''FM999999999999''), 8) as amount
    ,tt.e05 as Unit
    ,sm.count as count
