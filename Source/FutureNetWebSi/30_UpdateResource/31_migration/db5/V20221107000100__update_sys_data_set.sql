...
--普通部分
data_commmon as(
  select detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09,e10,e11,sorttag,''普通''::text as  aa from data_all where medicine_cd is null and equipment_cd is null ORDER BY  sortTag
)
,
--医疗材料部分
data_all_meq as(
...
select * from med_order
 union all
 select * from  data_commmon where sorttag >= ( case when (SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE'') is null  then ''28''
  else (((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE'')::INTEGER +1 )::text)end)
