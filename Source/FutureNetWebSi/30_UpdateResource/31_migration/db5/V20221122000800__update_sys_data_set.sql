...
INSERT INTO ntss.sys_data_set (sql_cd, sql, db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES (3102, 'WITH take_cource_info AS (SELECT 1       AS order_no,
                          ...
                          LIMIT 1),
     witch_ctl_no as (select take_cource_flg as ctl_no from take_cource_info),
     ...
     ctl_no_for_1_or_2 as (select (case witch_ctl_no.ctl_no
                                       when ''1'' then 1
                                       when ''2'' then (case
                                                        when total_number_of_docs.total < 2
                                                            then total_number_of_docs.total + 1
                                                        else 2 end)
                                       else ''2'' end) as ctl_no
                           from total_number_of_docs,
                                witch_ctl_no)
UPDATE pat_main
SET charge_staff_info = (case
                        ...
                        {
                          "ctl_no": '' || total_number_of_docs.total + 1 || '',
