...
SELECT
  ...
  (SELECT value FROM coop_ini_info WHERE key1 = ''SCM_COMMON'' AND key2 = ''MEDICINE_SEND_TYPE'') AS hosp_get_mst_send_type
  )
, mst_medi_mix AS (
--調整薬剤マスタ
SELECT
...
select
  ...
  ini_unit.value AS unit
  ,mi.pro_hosp_cd
FROM
  get_items gi
INNER JOIN medi_indo_mi_cut AS mi ON gi.med_code = LPAD(mi.hosp_cd_trimmed, 6,'' '')
  AND gi.technique = LPAD(RIGHT(mi.pro_hosp_cd, 2), 2,'' '')
LEFT JOIN ini_unit ON mi.unit = ini_unit.key2
CROSS JOIN ini_value
WHERE
  mi.is_shot = ''1'' and 
  mi.is_disp = ''1'' and
  gi.rp_no::numeric = @rpNo and
  ini_value.hosp_get_mst_send_type = ''0''
...
select
  ...
  gi.med_no::numeric AS medi_no,
  mi.hosp_cd_trimmed AS medi_cd,
  SUM(ROUND(mi.amount, 2)::FLOAT8)::text AS amount,
...
WHERE
  ...
  mi.is_disp = ''1'' and
  gi.rp_no::numeric = @rpNo and
  ini_value.hosp_get_mst_send_type = ''1''
  
GROUP BY gi.rp_no, gi.med_no, mi.hosp_cd_trimmed, mi.pro_hosp_cd, ini_unit.value

ORDER BY rp_no, medi_no', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析実績連携', '2025-06-03 08:56:02.129', CURRENT_TIMESTAMP, NULL);
