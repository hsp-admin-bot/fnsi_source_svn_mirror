...
data_all_med as(
select * from data_all where medicine_cd is not null
),
data_all_meq as(
select * from data_all where medicine_cd is null
),
order_qcode_F AS (
...
SELECT DISTINCT ON (item_cd_s)* FROM (
  ...
  ORDER BY item_cd_s, login_ord_s asc) AS order_code_middle_QB
   ...
   FROM
	 ...
	 equip_order as(
SELECT  detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09, ''医疗材料''::text as category
FROM dataequipOrder
ORDER BY e09,
...
med_order as(
SELECT
    detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09,''薬剤''::text as  category
FROM
    dataAndOrder
ORDER BY 
    e09,
...
order by e09
', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通：透析予約繰り返し部', '2020-05-08 16:22:41', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -37, "field_name": "user_name", "replace_var": "@userName"}]');
