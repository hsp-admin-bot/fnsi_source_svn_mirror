...
SET
  ...
  , pat_last_name = CASE WHEN pat_last_name <> personal_info_encrypt(''ini_dial'') AND pat_first_name <> personal_info_encrypt(''only'') THEN personal_info_encrypt((SELECT patLastName FROM new_name_info)) ELSE pat_last_name END 
  , pat_first_name = CASE WHEN pat_last_name <> personal_info_encrypt(''ini_dial'') AND pat_first_name <> personal_info_encrypt(''only'') THEN COALESCE(personal_info_encrypt(NULLIF((SELECT patFirstName FROM new_name_info), '''')), pat_first_name) ELSE pat_first_name END 
