...
VALUES(-2190, '--性能検証のため一時的なアップ。ロールバックには下部のコメントアウトブロックを使用する
...
, ntss_db5_om_temp_pat as (
    select distinct pat_id
    from ntss_db5_om_temp
)
, ntss_db5_pu AS (
    SELECT
        pu_temp.pat_id
        ...
        , phy_temp ->> ''dw'' AS dw
        , phy_temp ->> ''indicator_cd'' AS indicator_cd
        , ROW_NUMBER () OVER (PARTITION BY pu_temp.pat_id ORDER BY phy_temp ->> ''exam_date'' DESC, phy_temp ->> ''ctl_no'' DESC) AS rowno
    FROM
        pat_unique pu_temp
        join ntss_db5_om_temp_pat t 
        on t.pat_id = pu_temp.pat_id
        CROSS JOIN lateral jsonb_array_elements(pu_temp.physical_info ::jsonb) phy_temp
    WHERE
        pu_temp.facility_cd = @facilityCd
        AND pu_temp.is_del = ''0''
...
FROM
    ...
    LEFT JOIN ntss_db5_ptp
        ...
        AND ntss_db5_ptp.treat_week = ntss_db5_om_temp.treat_week
        AND ntss_db5_ptp.ind_treatment_cd = ntss_db5_om_temp.ind_treatment_cd;
', 2, '[]'::jsonb, '1', '{"applications": [5]}'::jsonb, '{"classes": []}'::jsonb, '患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid,userid"]}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -2518}]'::jsonb);
