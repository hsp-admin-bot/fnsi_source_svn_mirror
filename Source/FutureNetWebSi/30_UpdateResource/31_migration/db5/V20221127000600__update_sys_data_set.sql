...
INSERT INTO ntss.sys_data_set (sql_cd, sql, db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES (1802, 'WITH check_taboo_allergy_cd as (select (case position('','' in ''@tabooAllergyInfo.tabooAllergyCd'') when 0 then false else true end) as ctac)
  ,tabooAllergyCdInfo AS (SELECT (case when ctac then split_part(''@tabooAllergyInfo.tabooAllergyCd'', '','', 1) else '''' end) AS cd
                                 , (case when ctac then split_part(''@tabooAllergyInfo.tabooAllergyCd'', '','', 2) else ''5'' end) AS type
                                 , (case when ctac then split_part(''@tabooAllergyInfo.tabooAllergyCd'', '','', 3) else ''@tabooAllergyInfo.tabooAllergyCd'' end) AS hospital_cd from check_taboo_allergy_cd)
...
, tabooAllergyInfo AS ( 
  ...
  FROM
    ...
    INNER JOIN newTabooAllergyInfo AS new ON new.taboo_allergy_cd = ms->>''taboo_allergy_cd'' and ms ->> ''taboo_allergy_cd'' <> ''''
  WHERE
    A.is_del = ''0'' 
...
--  AND (SELECT idx FROM tabooAllergyInfo) IS NULL', 2, '[{}]', '0', '{"applications": [4]}', null, '(受信用)富士通__禁忌・アレルギー情報_更新', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, null);
