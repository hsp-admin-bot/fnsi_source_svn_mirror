...
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-2501, 'WITH ntss_db5_mst_b AS (
    SELECT
...
re_loop_rate_table AS ( --再循環率
    ...
    FROM (
        SELECT
            ntss_db5_om.ord_no
            , ntss_db5_om.rst_weight_info #>> ''{recrcl_rt, "valid_no"}'' AS valid_no
            , ntss_db5_om.rst_weight_info #> ''{recrcl_rt}'' AS recrcl_rt
        ...
        AND ntss_db5_om.rst_weight_info #> ''{recrcl_rt}'' <> ''null''
    ) AS ntss_db5_om
    CROSS JOIN lateral jsonb_each_text(ntss_db5_om.recrcl_rt::jsonb) json_rr
    WHERE json_rr.key = ntss_db5_om.valid_no
)
SELECT
    '''' AS hosppatid --患者ID
    ...
    ,CASE WHEN (ntss_db5_om.rst_end_date IS NULL OR ntss_db5_om.rst_start_date IS NULL) THEN 0
        ELSE round(date_part(''epoch'',ntss_db5_om.rst_end_date - ntss_db5_om.rst_start_date)::NUMERIC / 60)
        END AS dialysistime --透析時間
    ...
    ,CONCAT(ntss_db5_om.rst_charge_user_info #>> ''{user_last_name_1}''
        ,''　''
        , ntss_db5_om.rst_charge_user_info #>> ''{user_first_name_1}'') AS charge1name --担当者１
    ,CONCAT(ntss_db5_om.rst_charge_user_info #>> ''{user_last_name_2}''
        ,''　''
        , ntss_db5_om.rst_charge_user_info #>> ''{user_first_name_2}'') AS charge2name --担当者２
    ...
    ,CONCAT(ntss_db5_om.rst_puncture_user_info #>> ''{user_last_name_1}''
        ,''　''
        , ntss_db5_om.rst_puncture_user_info #>> ''{user_first_name_1}'') AS puncture1name --穿刺者１
    ,CONCAT(ntss_db5_om.rst_puncture_user_info #>> ''{user_last_name_2}''
        ,''　''
        , ntss_db5_om.rst_puncture_user_info #>> ''{user_first_name_2}'') AS puncture2name --穿刺者２
    ...
    ,CONCAT(ntss_db5_om.rst_return_user_info #>> ''{user_last_name_1}''
        ,''　''
        , ntss_db5_om.rst_return_user_info #>> ''{user_first_name_1}'') AS collect1name --回収者１
    ,CONCAT(ntss_db5_om.rst_return_user_info #>> ''{user_last_name_2}''
        ,''　''
        , ntss_db5_om.rst_return_user_info #>> ''{user_first_name_2}'') AS collect2name --回収者２
    ...
    ,ntss_db5_om.rst_weight_info #>> ''{kt_v_measure}'' AS ktvmeasure --Kt/v測定値
...
FROM
    ...
    ON re_loop_rate_table.ord_no = ntss_db5_om.ord_no
WHERE
