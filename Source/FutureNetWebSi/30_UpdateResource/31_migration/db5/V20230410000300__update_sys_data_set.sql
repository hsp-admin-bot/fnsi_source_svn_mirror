DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (9207, 9208);
...
ELSE
  ...
  ELSE
	  CASE WHEN keepCnt = 0 THEN
		  newJsonb
		ELSE
      ((SELECT jsonb_agg(infectJobj) FROM pat_main CROSS JOIN jsonb_array_elements(infect_info) infectJobj WHERE facility_cd = ''999998'' AND pat_id = 39931 AND infectJobj->>''infection_cd'' != ''1040'') || newJsonb)::jsonb
		END
  END
END
FROM
