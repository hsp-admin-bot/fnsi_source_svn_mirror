DELETE FROM "ntss"."sys_data_set" where sql_cd in (239, 241);
...
med_reg_sort AS (
	...
	FROM
		ordMS_tbl
	WHERE
		supplies_class in (''12'', ''14'', ''20'', ''21'', ''22'')		
	ORDER BY reg_date,reg_order ASC
),
med_sort AS (
	SELECT
...
SELECT
	...
	, supplies_class AS receipt_class_cd
...
(SELECT
	...
	, CASE 
		...
		WHEN oms.supplies_class = ''10'' THEN ''抗凝固剤''
		ELSE '''' 
	END AS receipt_class
	, NULL AS receipt_kind_cd
	, CASE 
...
WHERE
	oms.supplies_class in (''08'', ''09'', ''10'')
ORDER BY oms.supplies_class ASC)
UNION ALL
(SELECT
	ord_no
...
LEFT JOIN equ_reg_sort mer ON mer.code = equinfo.receipt_cd AND mer.idx = 1
LEFT JOIN equ_sort eq ON eq.code = equinfo.receipt_cd
LEFT JOIN equ_class_sort eqc ON eqc.code = equinfo.class_cd
...
FROM(
	...
	WHERE
		oms.supplies_class in (''12'', ''14'', ''20'', ''21'', ''22'')
	GROUP BY 
		supplies_base_no
		, oms.class_cd
		, oms.supplies_cd
