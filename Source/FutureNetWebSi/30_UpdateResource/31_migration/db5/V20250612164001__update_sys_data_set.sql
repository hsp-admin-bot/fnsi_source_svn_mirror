...
WHERE sql_cd IN (-1202007,-1202008);
...
, exam_inhosp AS ( --検査セットマスタの使用院内コード番号
    SELECT *
    FROM coop_ini_info
    WHERE key1 = ''SX_EXAM_SCHE_INFO''
        AND key2 = ''EXAM_INHOSP''
)
, exam_set_inhosp AS ( --検査項目マスタの使用院内コード番号
    SELECT *
    FROM coop_ini_info
    WHERE key1 = ''SX_EXAM_SCHE_INFO''
...
, exam_data AS (
  ...
  CROSS JOIN LATERAL jsonb_array_elements(exam_order_info) info
...
group by
  ...
  , exam_full.exam_row
order by
  exam_row', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'SX_血液検査依頼検査項目', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
...
WHERE
  ...
  AND CASE (SELECT value FROM output_setting)
    ...
    WHEN ''2'' THEN mei.is_in_hospital = ''0'' --院外のみ
    ELSE true
    END', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'SX_血液検査依頼項目数', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
