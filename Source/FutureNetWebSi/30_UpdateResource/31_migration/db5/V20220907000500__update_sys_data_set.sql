delete from ntss.sys_data_set where sql_cd in ('1203','1705');
INSERT INTO ntss.sys_data_set (sql_cd, sql, db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES (1705, 'WITH inOutClass AS (SELECT (CASE
                                WHEN ''@inOutClass'' = ''1''
                                    THEN ''1''
                                ELSE ''0''
    END) AS inOut)
   ...
   , current_new_data AS (
    -- 当日を含む過去の最新データを取得する
    SELECT ((info ->> ''in_out'')::TEXT) AS in_out
   ...
   , json_data AS (SELECT json_build_object(
                   ...
                   FROM data_info)
UPDATE pat_unique
SET in_out_visit_history_info = (SELECT array_to_json(ARRAY_AGG(new_data)) FROM json_data)
...
WHERE pat_id = @patId
  ...
  AND ''@isDie'' <> ''1''
  AND ''@inOutClass'' <> ''''', 2, '[{}]', '0', '{"applications": [4]}', null, '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡以外)', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, null);
INSERT INTO ntss.sys_data_set (sql_cd, sql, db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES (1203, 'WITH take_cource_info AS ( 
  SELECT
    1 AS order_no
