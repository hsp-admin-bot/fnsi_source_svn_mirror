...
WHERE
    ord.ord_no = @ordNo
    AND ((SELECT value FROM ind_set_medicine_resolve_info WHERE key2 = ''KOU_COAG_RESOLVE_MODE'') = ''0'' or
...
SELECT
    ...
    --数量
    TO_CHAR(TO_NUMBER(ord.ind_cond_info->''@ordNo''->>''value'', ''9999999.999''), ''FM0999999.990'') AS e04, 
    --選択単位フラグ
    ''1'' AS e05, 
    --単位コード
...
WHERE
    ord.ord_no = @ordNo
    AND TO_NUMBER(ord.ind_cond_info->''@ordNo''->>''value'',''999999999999'') > 1
...
SELECT
    ...
    --項目コード
		CASE WHEN LENGTH(TRIM(mmd.in_hospital_cd_1)) > 8 THEN RIGHT(TRIM(mmd.in_hospital_cd_1), 8) ELSE RPAD(TRIM(mmd.in_hospital_cd_1), 8, '''') END AS e01,
    --項目属性
    COALESCE(mmd.in_hospital_cd_2, COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_KOU_COAG_ONESHOT_ATTR''), '''')) AS e02, 
    --項目名称
    COALESCE(mmd.medicine_name, '''') AS e03,
    --数量
    COALESCE( TO_CHAR((TO_NUMBER(ord.ind_cond_info->''26''->>''value'', ''9999999.999'') * TO_NUMBER(mmxd ->> ''amount'', ''9999999.999'' )), ''FM0999999.990'' ) )  AS e04,
    --選択単位フラグ
    CASE 
      WHEN (CASE ord.ind_cond_info->''25''->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END) IS NULL OR 
           (CASE ord.ind_cond_info->''25''->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END) = ''''
   ...
   COALESCE(mmd.unit, '''') AS e07,
    ...
    --出力順
--     COALESCE((SELECT value FROM item_sort_info WHERE key2 = ''ITEM_KOU_COAG_ONESHOT''), '''') AS e09 ,
       COALESCE((SELECT value FROM item_sort_info WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-'' || ''0'' AS e09 ,
    '''' as sorttag1,
    '''' as sorttag2
FROM
    ord_main ord
