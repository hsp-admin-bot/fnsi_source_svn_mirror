...
INSERT INTO ntss.sys_data_set (sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES(-1102033, 'WITH raw_data AS (
...
WHERE EXISTS (
  SELECT 1 FROM rows
);', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'セコム連携 透析指示連携 注射依頼(削除電文用)', '2025-07-29 18:23:10.538', current_timestamp, '[{"sql_cd": -1102029, "field_name": "content_json", "replace_var": "@contentJson"}]'::jsonb);