...
(CASE 
    ...
    WHERE 
      user_id::TEXT = @doctorCd::TEXT
		)
END) AS user_id
FROM mst_user_authentication LIMIT 1
', 1, '[]', '0', '{"applications": [4]}', '{"classes": []}', NULL, '2022-06-11 14:01:34', CURRENT_TIMESTAMP, '[{"sql_cd": -109, "field_name": "doctor_cd", "replace_var": "@doctorCd"}, {"sql_cd": -109, "field_name": "default_doctor_cd", "replace_var": "@defaultDoctorCd"}]');
