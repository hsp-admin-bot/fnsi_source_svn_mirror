...
VALUES(1705, 'WITH in_out_class AS (SELECT (CASE
                                  WHEN ''@inOutClass'' = ''1''
                                      THEN ''1''
                                  ELSE ''0''
    END) AS in_out)
...
WHERE pat_id = @patId
  ...
  AND ''@isDie'' <> ''1''
  AND ''@inOutClass'' <> ''''
  AND (exists_flag = ''0'' OR in_out_visit_history_info IS NULL OR in_out_visit_history_info = ''[]'')', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)富士通、日機装標準(xml)、CSIの患者プロファイル_固有情報_入外・転入出情報(死亡以外)', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, '[{"sql_cd": -100001, "field_name": "in_out_class", "replace_var": "@ppmInOutClass"}]');