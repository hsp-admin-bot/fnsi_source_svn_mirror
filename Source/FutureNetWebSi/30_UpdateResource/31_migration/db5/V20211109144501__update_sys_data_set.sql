delete from "sys_data_set" where "sql_cd" in (-400001,-99992,-13);
...
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-99992, 'SELECT
  ''DIASCH-'' || 
  TO_CHAR(ord_no, ''FM9999999999'') || ''-'' ||
  TO_CHAR(CURRENT_TIMESTAMP, ''YYYYMMDDHH24MISS'') ||
  ''.dat'' AS filename
FROM
  sys_coop_journal AS journal 
WHERE
  journal.ctl_no = @ctlNo', 2, '[]', '0', '{"applications": [4]}', NULL, '日機装 透析予約[送信]ファイル名取得', '2021-04-20 09:19:08.001', '2021-04-20 09:19:12', NULL);
