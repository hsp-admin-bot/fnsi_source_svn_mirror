DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (1717);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1717, 'WITH
inOutClass AS (
  SELECT (CASE WHEN ''@inOutClass'' = ''1'' THEN ''1'' ELSE ''0'' END) AS inOut
)
,data_new_info AS (
  SELECT 
    null AS ctl_no,
...
WHERE
  ...
  AND facility_cd = ''@facilityCd'' 
  AND is_del = ''0''
  AND (SELECT exists_flag FROM data_exists_info) = ''0'' ', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡以外)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
