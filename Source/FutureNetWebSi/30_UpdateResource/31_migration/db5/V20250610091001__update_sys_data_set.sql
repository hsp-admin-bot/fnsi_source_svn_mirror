...
new_array AS (
  SELECT
    ...
    (si.in_hospital_cd)::int                   AS in_hospital_cd
  FROM split_info si
  -- raw_state → state_val
...
-- 6) JSONオブジェクト化して in_hospital_cd順にソート
updated_array AS (
  SELECT
    jsonb_build_object(
      ''infect'',       na.infect,
...
WHERE
  ...
  AND pm.is_del    = ''0'';
', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)セコム)患者プロファイル　感染症更新', '2025-05-18 22:33:06.096', CURRENT_TIMESTAMP, NULL);