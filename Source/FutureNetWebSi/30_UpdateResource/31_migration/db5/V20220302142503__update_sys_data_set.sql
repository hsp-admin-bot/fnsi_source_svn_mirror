delete from "sys_data_set" where "sql_cd" in (3204,3203,1905,1104);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (3204, 'WITH up_disease_date_flg_info AS (
...
, disease_date_info_tmp AS (
  SELECT COALESCE(NULLIF(''@medicalHstInfo.diseaseDate'', ''''), TO_CHAR(CURRENT_TIMESTAMP, ''YYYYMMDD'')) AS disease_date
)
, disease_date_info AS (
  SELECT 
    SUBSTR(disease_date, 1, 8) AS disease_date
    , SUBSTR(disease_date, 1, 4) AS disease_year
    , SUBSTR(disease_date, 5, 2) AS disease_month
    , SUBSTR(disease_date, 7, 2) AS disease_day
  FROM 
    disease_date_info_tmp
  WHERE 
    (SELECT up_disease_date_flg FROM up_disease_date_flg_info) = ''0''
  UNION
  SELECT 
...
, data_info AS ( 
  ...
  SELECT
    ...
    CASE WHEN new_data.disease_cd IS NULL 
      THEN ''0''
      ELSE ''1''
  ...
  FROM
    pat_unique patu
    CROSS JOIN LATERAL json_array_elements ( patu.medical_hst_info :: json ) AS info 
    LEFT JOIN (SELECT ''@medicalHstInfo.diseaseCd'' ::TEXT AS disease_cd) AS new_data ON new_data.disease_cd = (info->>''disease_cd''::TEXT) AND (''@medicalHstInfo.diseaseCd'' ::TEXT) != ''''
  WHERE
    pat_id = @patId 
    AND facility_cd = ''@facilityCd'' 
    AND is_del = ''0'' 
...
WHERE
  pat_id = @patId 
  AND facility_cd = ''@facilityCd'' 
  AND is_del = ''0''', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の初回申し込み→既往歴情報(原疾患登録)', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (3203, 'WITH up_base_disease_flg_info AS (
...
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1905, 'UPDATE pat_coop_detail 
SET
  save_3 = json_build_object(''des_cd'', COALESCE(NULLIF(''@save3.desCd'', ''''), (save_3->>''des_cd''))
      , ''diff_cd'', COALESCE(NULLIF(''@save3.diffCd'', ''''), (save_3->>''diff_cd''))
      , ''add_cd'', COALESCE(NULLIF(''@save3.addCd'', ''''), (save_3->>''add_cd''))
      , ''treat_cd'', (save_3->>''treat_cd'')
      , ''comment'', COALESCE(NULLIF(''@save3.comment'', ''''), (save_3->>''comment''))
      , ''dw'', COALESCE(NULLIF(''@save3.dw'', ''''), (save_3->>''dw''))
      , ''ind_dr'', (save_3->>''ind_dr'')
      , ''start_date'', (save_3->>''start_date'')
    )
  , up_date = CURRENT_TIMESTAMP
WHERE
  ...
  AND save_2->>''ord_no''::TEXT = ''@save2.ordNo''', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者連携情報更新(SAVE_3のdes_cd、diff_cd、add_cd、comment、dw)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
WHERE
  facility_cd = ''@facilityCd'' 
  AND pat_id = @patId
  AND is_del = ''0''', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報→原疾患コードの設定', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, NULL);
