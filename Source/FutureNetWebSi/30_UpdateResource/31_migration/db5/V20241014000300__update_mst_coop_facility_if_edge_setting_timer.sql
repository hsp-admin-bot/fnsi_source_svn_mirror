WITH updated_data AS (
    SELECT
        ...
        jsonb_agg(
            ...
            ) AS updated_if_edge_setting
    FROM (
             SELECT
                 facility_cd,
                 jsonb_array_elements(if_edge_setting -> 'timer') AS elem,
                 CASE facility_cd
                     WHEN 'nkknkk' THEN
                         'NKK'
                     WHEN 'F_hosp' THEN
...
SET if_edge_setting = jsonb_set(A.if_edge_setting, '{timer}', updated_data.updated_if_edge_setting, FALSE)
FROM updated_data
