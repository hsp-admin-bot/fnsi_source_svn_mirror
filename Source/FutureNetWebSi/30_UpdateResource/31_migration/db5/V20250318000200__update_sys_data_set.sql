DELETE FROM "ntss"."sys_data_set" where sql_cd in (9,11,16,17,206,242,243,244);
  ...
  spi AS (
	...
	FROM
		...
		WHERE
			...
			AND om.ind_cond_info :: json #>> ''{13,value}'' IS NOT NULL
			AND om.is_del = ''0''
			AND eq.class_cd IN ( @eqIds ) UNION ALL--医材
				SELECT
					CASE WHEN @dataTypeOrder = ''0'' THEN 2
           WHEN @dataTypeOrder = ''1'' THEN 3
           WHEN @dataTypeOrder = ''2'' THEN 1
...
result_all AS (
 --採血管
		...
		FROM
			...
			CROSS JOIN LATERAL json_array_elements ( P.order_exam_set_info :: json) infoExam
			...
			LEFT OUTER JOIN mst_exam_set AS mest CROSS JOIN LATERAL json_array_elements ( mest.exam_item_info :: json) infoExamInfo
			 on infoExam ->> ''set_cd'' = mest.exam_set_cd :: TEXT
			AND mest.is_del = ''0''
			AND mest.is_disp = ''1''
...
SELECT * FROM result_all as res @orderBy		', 2, '[]', '1', '{"applications": [1]}', '{"classes": [8]}', 'ラベル（降順）', '2024-03-20 08:56:21', CURRENT_TIMESTAMP, NULL);
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (242, 'WITH dz AS (
	SELECT
		*
	FROM
		mst_dialyzer
...
FROM
	...
	SELECT * from result_all as res @orderBy	', 2, '[]', '1', '{"applications": [1]}', '{"classes": [4]}', '器材準備リスト（降順）', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
SELECT * from result_all as res @orderBy	', 2, '[]', '1', '{"applications": [1]}', '{"classes": [6]}', '配布リスト(器材)（SQL11降順）', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
FROM
	...
	SELECT * from result_all as res @orderBy', 2, '[]', '1', '{"applications": [1]}', '{"classes": [6]}', '配布リスト(器材)（SQL206降順）', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
