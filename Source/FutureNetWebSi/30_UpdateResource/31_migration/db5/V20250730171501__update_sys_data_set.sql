...
, file_info AS (
  ...
  FROM datas
  JOIN res_data    ON res_data.ctl_no = datas.ctl_no
  JOIN treat_data  ON treat_data.ctl_no = datas.ctl_no
  JOIN inj_data    ON inj_data.ctl_no = datas.ctl_no
  JOIN med_data    ON med_data.ctl_no = datas.ctl_no
)
, create_memo AS (
SELECT json_build_object(
  ...
  ''memo'',
    ...
    ''#K|'' || @sendStatus || ''|'' || coalesce(med_data.cols[5], '''') || ''|'' || coalesce(file_info.inj_day, '''') || ''|'' || coalesce(file_info.inj_time, ''''),
