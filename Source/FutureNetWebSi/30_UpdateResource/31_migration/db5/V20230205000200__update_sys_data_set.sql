DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (-11,-110,-505,-507);
...
--   COALESCE(ord.rst_cond_info -> ''26'' ->> ''unit'', '''') AS anti_coagulant_one_shot_amount_unit,
 case when (select (SELECT staff_cd FROM KOU_COAG_RESOLVE_MODE_cd)) in (''0'',''1'') then 
...
WHERE
  ord.ord_no =  @ordNo', 2, '[{}]', '0', '{"applications": [4]}', NULL, 'NKK)  実績）透析条件（削除）', '2022-08-01 14:31:32.443',CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-11, 'with KOU_COAG_RESOLVE_MODE_cd AS(
SELECT
    COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v'') AS staff_cd
  FROM
    mst_coop_ini AS ini
