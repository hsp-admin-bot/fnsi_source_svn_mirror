DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (9207, 9208, 7408, 7409);
    ...
    ELSE
      ...
      ELSE
	      CASE WHEN keepCnt = 0 THEN
		      newJsonb
		    ELSE
          ((SELECT jsonb_agg(infectJobj) FROM pat_main CROSS JOIN jsonb_array_elements(infect_info) infectJobj WHERE facility_cd = ''999998'' AND pat_id = 39931 AND infectJobj->>''infection_cd'' != ''@infectCd'') || newJsonb)::jsonb
		    END
      END
    END,
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (9207, 'SELECT
 exam_item_cd,
 infection_cd
FROM
  mst_exam_item
WHERE
  facility_cd = @facilityCd
	...
	AND infection_cd IS NOT NULL
	AND is_del = ''0''', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)NECの検査結果(検査結果情報更新)', '2023-04-10 23:29:43.109', CURRENT_TIMESTAMP, NULL);
...
WHERE
  facility_cd = ''@facilityCd'' 
	AND pat_id = @patId', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)NECの検査結果(検査結果情報更新)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (7408, 'SELECT
 exam_item_cd,
 infection_cd
FROM
  mst_exam_item
WHERE
  facility_cd = @facilityCd
	AND exam_item_cd = @examResultInfo.itemCd
	AND infection_cd IS NOT NULL
	AND is_del = ''0''', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)NECの検査結果(検査結果情報更新)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
