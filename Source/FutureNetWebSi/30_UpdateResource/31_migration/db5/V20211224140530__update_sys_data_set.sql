...
,string_agg(distinct CASE WHEN om.treat_week =1 THEN ''月'' 
...
FROM  
ord_main 
where facility_cd = ''998998''
AND pat_id=''817''
AND	treat_date BETWEEN to_char(cast(''20211117'' as timestamp)
, ''YYYYMMDD'') AND to_char(cast(''20211117'' as timestamp)::timestamp + ''6 day'', ''YYYYMMDD'' )
ORDER BY treat_week DESC)  om

 GROUP BY om.pat_id;
 
 
 SELECT COUNT(om.treat_date) AS dialysis_conunt
,string_agg(distinct CASE WHEN om.treat_week =1 THEN ''月'' 
WHEN om.treat_week =3 THEN ''水''
WHEN om.treat_week =4 THEN ''木''
WHEN om.treat_week =5 THEN ''金''
