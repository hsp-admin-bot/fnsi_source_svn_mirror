DELETE FROM sys_data_set sds WHERE sql_cd  IN (-53, -103, -194);
...
', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '富士通）透析予約：予約時間取得', '2022-02-23 16:10:41.424', CURRENT_TIMESTAMP, NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
...
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-194, 'WITH sch_start_time_info AS (
  SELECT
    0 AS order_no 
    , COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v'') AS start_time_kbn 
  FROM
