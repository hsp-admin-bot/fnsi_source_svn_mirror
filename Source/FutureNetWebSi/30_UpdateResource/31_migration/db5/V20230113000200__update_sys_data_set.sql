...
FROM(
 (--  実施コメンSOAP ト S
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_S'' ), '''' ) AS e02,
	case when (input_params ::json ->> 0) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''S_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 0) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((result_params ::json ->> 0) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when (input_params ::json ->> 0) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''S_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 0) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((result_params ::json ->> 0) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ...
  pat_event_cd ::TEXT as sorttag1,
	''1'' as sorttag2
	FROM pat_event 
	WHERE ord_no = @ordNo
	AND facility_cd = @facilityCd
	AND is_del = ''0''
	AND category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
	AND ((input_params ::json ->> 0) ::json->> ''field_name'') = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''S_FIELD'' ), '''' )
 )
	 UNION ALL
 (--  実施コメンSOAP ト A
  SELECT
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_A'' ), '''' ) AS e02,
	case when (input_params ::json ->> 2) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''A_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 2) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((result_params ::json ->> 2) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when (input_params ::json ->> 2) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''A_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 2) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((result_params ::json ->> 2) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''3'' ) AS sortTag,
	...
	AND category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
	AND ((input_params ::json ->> 2) ::json->> ''field_name'') = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''A_FIELD'' ), '''' )
) UNION ALL
 (--  実施コメンSOAP ト O
  SELECT
  ''実績詳細'' AS detail_id,
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_O'' ), '''' ) AS e02,
	case when (input_params ::json ->> 1) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''O_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 1) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((result_params ::json ->> 1) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when (input_params ::json ->> 1) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''O_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 1) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((result_params ::json ->> 1) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''2'' ) AS sortTag,
   pat_event_cd ::TEXT as sorttag1,
	...
	AND category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
	AND ((input_params ::json ->> 1) ::json->> ''field_name'') = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''O_FIELD'' ), '''' )
) UNION ALL
 (--  実施コメンSOAP ト P
  SELECT
  ''実績詳細'' AS detail_id,
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_P'' ), '''' ) AS e02,
	case when (input_params ::json ->> 3) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''P_FIELD'' ), '''' ) 
	AND ((input_params ::json ->> 3) ::json->> ''item_json'')::json->> ''is_formatting'' = ''0''
	then REGEXP_REPLACE(LEFT((result_params ::json ->> 3) ::json->> ''result_value'',50),E''[\\n\\r]'','' '',''g'')
	when (input_params ::json ->> 3) ::json->> ''field_name''= COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''P_FIELD'' ), '''' )
	 AND ((input_params ::json ->> 3) ::json->> ''item_json'')::json->> ''is_formatting'' = ''1''
	then REGEXP_REPLACE(REGEXP_REPLACE(((result_params ::json ->> 3) ::json->> ''result_value'')::text,''<br />'','' '',''g''),''<[^<]*?>'','''',''g'')
	end as e03,
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''4'' ) AS sortTag,
   pat_event_cd ::TEXT as sorttag1,
	...
	AND category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''CATEGORY'' ), '''' )
	AND sub_category_name = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''SUB_CATEGORY'' ), '''' )
	AND ((input_params ::json ->> 3) ::json->> ''field_name'') = COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''P_FIELD'' ), '''' )
 )
 ) B
...
SELECT equip_all_copy.detail_id, equip_all_copy.sbt_key,equip_all_copy.e01, equip_all_copy.e02, equip_all_copy.e03, equip_all_copy.e04, equip_all_copy.e05, equip_all_copy.e06, equip_all_copy.e07, equip_all_copy.e08, equip_all_copy.e09,equip_all_copy.e10,equip_all_copy.e11,equip_all_copy.sortTag
       ,sortTag1,sortTag2,sorttagclass, mmd.medicine_cd, mmd.class_cd::text as mdclass_cd 
FROM equip_all_copy 
...
union all
SELECT equip_all_copy.detail_id, equip_all_copy.sbt_key,equip_all_copy.e01, equip_all_copy.e02, equip_all_copy.e03, equip_all_copy.e04, equip_all_copy.e05, equip_all_copy.e06, equip_all_copy.e07, equip_all_copy.e08, equip_all_copy.e09,equip_all_copy.e10,equip_all_copy.e11,equip_all_copy.sortTag
       ,sortTag1,sortTag2,sorttagclass, mmx.medicine_mix_cd as medicine_cd , equip_all_copy.sorttagclass as mdclass_cd  
FROM equip_all_copy 
    LEFT JOIN mst_medicine mmd ON equip_all_copy.e01 = mmd.in_hospital_cd_1 and equip_all_copy.e03 = mmd.medicine_name
		LEFT JOIN mst_medicine_mix mmx ON equip_all_copy.e01 = mmx.in_hospital_cd_1 and equip_all_copy.e03 = mmx.medicine_mix_name
...
(SELECT  detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09,e10,e11,sorttag, sortTag1,sortTag2, aa from equip_order))			 
 , order_code_F AS (--薬剤ない手技のない治療条件1,3场合
   ...
   FROM
     data_all_copy
     LEFT OUTER JOIN do_mstmedi_cd ON medi_code = data_all_copy.medicine_cd
     LEFT OUTER JOIN do_mstmedi_class_cd ON medi_class_code ::text  = data_all_copy.mdclass_cd
		 where sorttag2 = ''2'' or sorttag2 = ''3''
 ...
 union all
   ...
   FROM
     data_all_copy
		 LEFT OUTER JOIN mst_medicine_mix AS mmx ON mmx.medicine_mix_cd = data_all_copy.medicine_cd
...
ORDER BY item_cd_s, login_ord_s asc
 ...
 FROM
     data_all_copy,order_code_S,order_code_G where order_code_S.item_cd_s = e01 and  order_code_S.item_cd_s is not null
		 and order_code_G.item_cd_f = e01 and  order_code_S.medi_no = data_all_copy.sortTag1)  AS order_code_middle_B
