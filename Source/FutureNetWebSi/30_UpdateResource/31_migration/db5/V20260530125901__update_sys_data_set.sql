DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (95, 250, 208, 209);
...
select
  ...
  mst_dialyzer.ufr_warning_max,
  mst_dialyzer.ufr_warning_min,
  mst_dialyzer.ufr_warning_reduction,
  ...
  mst_dialyzer.in_number,
  mst_dialyzer.use_start_date,
  mst_dialyzer.use_end_date,
  ...
  ,CASE 
    WHEN (rst_puncture_user_info->>''date'') ~ ''^\d{4}/\d{2}/\d{2}$''
      THEN (rst_puncture_user_info->>''date'')::timestamp
    WHEN (rst_puncture_user_info->>''date'') ~ ''^\d{2}:\d{2}$''
      THEN (to_char(to_date(''19700101'',''YYYYMMDD''), ''YYYY-MM-DD'') || '' '' || (rst_puncture_user_info->>''date''))::timestamp
    WHEN (rst_puncture_user_info->>''date'') ~ ''^\d{4}-\d{2}-\d{2}T''
      THEN (rst_puncture_user_info->>''date'')::timestamp
  ...
  ,CASE 
    WHEN (rst_return_user_info->>''date'') ~ ''^\d{4}/\d{2}/\d{2}$''
      THEN (rst_return_user_info->>''date'')::timestamp
    WHEN (rst_return_user_info->>''date'') ~ ''^\d{2}:\d{2}$''
      THEN (to_char(to_date(''19700101'',''YYYYMMDD''), ''YYYY-MM-DD'') || '' '' || (rst_return_user_info->>''date''))::timestamp
    WHEN (rst_return_user_info->>''date'') ~ ''^\d{4}-\d{2}-\d{2}T''
      THEN (rst_return_user_info->>''date'')::timestamp
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (250, 'WITH DATA AS (
	with ord_tbl as (
		select
			...
			to_timestamp(treat_date, ''yyyymmdd'') + ''1 days - 1 milliseconds'' as treat_date_end
		from ord_main
		where ord_no in (@ordNos)
		and is_del = ''0''
	), bed_group_tbl AS (
		SELECT DISTINCT ON (ord.rst_bed_cd) 
			ord.facility_cd,
			ord.ord_no,
			ord.rst_bed_cd,
			m.room_bed_group_name AS bed_group_name
		FROM 
			ord_tbl ord
		LEFT JOIN 
			mst_room_bed_group m
			ON m.bed_list @> (''['' || ord.rst_bed_cd || '']'')::jsonb
		WHERE
			m.group_class = 1
			AND m.is_del = ''0''
			AND m.is_disp = ''1''
		ORDER BY
			ord.rst_bed_cd, m.room_bed_group_cd
	), room_tbl AS (
		SELECT DISTINCT ON (ord.rst_bed_cd) 
			ord.facility_cd,
			ord.ord_no,
			ord.rst_bed_cd,
			m.room_bed_group_name AS room_name
		FROM 
			ord_tbl ord
		LEFT JOIN 
			mst_room_bed_group m
	...
	-- 指定患者、基準日以前のDWがある身体情報を取得
		SELECT work_tbl.* FROM (
			select  
				...
				info->>''pre_scale_lower'' as pre_scale_lower,
				ord.treat_date_end,
				ROW_NUMBER() OVER (PARTITION BY ord.treat_date_end ORDER BY info->>''exam_date'' DESC) AS rn
			from
				ord_tbl ord 
				JOIN pat_unique on pat_unique.pat_id = ord.pat_id
				cross join lateral
					json_array_elements (pat_unique.physical_info :: json) info
			where
				pat_unique.is_del = ''0''
				and (info->>''exam_date'')::timestamp <= ord.treat_date_end
			and
				info->>''dw'' is not null
		) work_tbl
		WHERE
	...
	-- 指定患者の車いす情報を取得
		...
		from
			ord_tbl ord 
			LEFT JOIN mst_wheel_chair mwc on mwc.pat_id = ord.pat_id
			LEFT JOIN wheel_sort ms on mwc.wheel_chair_cd = ms.code
		where
			mwc.is_disp = ''1''
		and
			mwc.is_del = ''0''
...
-- 指定患者、基準日以前のDWがある身体情報を取得
	...
	FROM
		ord_material_save oms
	WHERE
		oms.supplies_base_no in (@ordNos)
		AND supplies_source_class = ''0''
		AND supplies_class = ''06''
		AND ind_rst_class=''2''
...
from
  ...
  left join bed_group_tbl on ord.ord_no = bed_group_tbl.ord_no -- 実績
  left join room_tbl on ord.ord_no = room_tbl.ord_no -- 実績
...
FROM
	...
	time_info
	on
	DATA.ord_no = time_info.ord_no', 2, '[]', '0', '{"applications": [1]}', '{"classes": [3]}', '実績：透析条件/ベッド情報/ダイアライザ情報/実績情報 @ordNos 使用', '2025-04-23 16:14:27.115', CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (208, 'with input_params_expand as(
	SELECT 
		pat_event_cd
		, event_start_date
		, (event_start_date || '' '' || COALESCE(event_start_time::text, ''''))::timestamp as event_start_datetime
		, data_pic_name
		, json_info ->>''file_path'' as data1_pic1_file_path
	from (
		SELECT 
			pat_event_cd
			, event_start_date
			, event_start_time
			, info->>''va_name'' as data_pic_name
			, info->>''result_value'' as resul_json
		FROM pat_event 
...
select * from (
	(
		select a.*,parms_5.resul_json as make_start_date,treat_date  from input_params_expand a 
	...
	UNION ALL
	(
		select a.*,parms_5.resul_json as make_start_date,treat_date from input_params_expand as a 
		INNER JOIN order_main on order_main.va =a.data_pic_name 
		LEFT JOIN input_params_expand_5 parms_5 on parms_5.pat_event_cd = a.pat_event_cd
...
LIMIT 1', 2, '[{"preview": "2011/05/20", "can_calc": "0", "data_code": "event_start_datetime", "data_name": "開始日時", "data_type": "DateTime", "conv_table": [], "data_class": "VA情報", "field_name": "event_start_datetime", "disp_format": "yyyy/MM/dd HH:mm", "data_category": "実績", "facility_table": "", "facility_filter_type": "0"}, {"preview": "2011/05/20", "can_calc": "0", "data_code": "make_start_date", "data_name": "造設日", "data_type": "DateTime", "conv_table": [], "data_class": "VA情報", "field_name": "make_start_date", "disp_format": "yyyy/mm/dd", "data_category": "実績", "facility_table": "", "facility_filter_type": "0"}, {"preview": "va.jpg", "can_calc": "0", "is_image": "true", "data_code": "data1_pic1_file_path", "data_name": "画像", "data_type": "byte[]", "conv_table": [], "data_class": "VA情報", "field_name": "data1_pic1_file_path", "disp_format": "", "filter_type": "Category", "data_category": "実績", "facility_table": "", "facility_filter_type": "0"}]', '0', '{"applications": [1]}', '{"classes": [1, 2, 3, 9]}', 'VA情報 増設日 画像(実績)', '2023-04-23 09:37:00', CURRENT_TIMESTAMP, NULL);
