...
FROM(
 (--  実施コメンSOAP ト S
  SELECT detail_id, sbt_key, e01, e02, e03
...
when position( ''</'' in ((result_params ::json ->> 0) ::json->> ''result_value'')) = ''0''
	then ((result_params ::json ->> 0) ::json->> ''result_value'')::text
	else
-- ((xpath(''/span/text()'',xmlparse(content(result_params ::json ->> 0) ::json->> ''result_value'')))[1])::text
regexp_replace(((result_params ::json ->> 0) ::json->> ''result_value'')::text,''<[^<]*?>'','''',''g'')
end	as e03,
