DELETE FROM "ntss"."sys_data_set" where sql_cd in (2,6,53,54,55,72,75,96,99,103,106,118,141,161,174,201,202,218,224,228);  
...
from
  (
    ...
    --↓画面上で追加している項目の追加処理
    union
    select ord_no, rst_start_date::text as occur_date, ''治療開始'' as complaint, null, null from ord_main where ord_no = @ordNo and rst_start_date is not null
    union
    select ord_no, rst_end_date::text as occur_date, ''治療終了'' as complaint, ''999'', ''999'' from ord_main where ord_no = @ordNo and rst_end_date is not null
    --↑画面上で追加している項目の追加処理
order by
      ord_no,
      occur_date) a
...
row_no
  ...
  (
    ...
    order by
      ...
      occur_date,
      row_no) c
  on COALESCE(a.ord_no,b.ord_no) = c.ord_no and COALESCE(a.occur_date,b.occur_date) = c.occur_date and COALESCE(a.row_no, b.row_no) = c.row_no and COALESCE(a.ctl_no, b.ctl_no) = c.ctl_no
where
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (72, 'with 
pat_info as (SELECT pat_id,ord_no from ord_main where ord_no = @ordNo),
input_params_expand as
(
  select
    pat_event_cd
    ,json_idx
  ...
  where
		pat_event.ord_no = pat_info.ord_no and pat_event.pat_id = pat_info.pat_id  
    and pat_event.is_del = ''0''
    and pat_event.use_type = 2 and pat_event.ord_no = @ordNo
)
...
select @ordNo as ord_no,array_water_removal_rst[1] as water_removal_rst_prev
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (106, 'select * from (
select
...
where
  ord_no = @ordNo
 and data_type = 1 and is_del = ''0'' ) a   ORDER BY a.mon1 asc , a.occur_date::timestamp  asc
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (141, 'select a.f_medi_cd,a.medicine_name as f_medi_name,a.f_medi_amount,a.medicine_unit as f_medicine_unit,array_agg(a.f_week)  as f_week,a.ord_no from (
select
