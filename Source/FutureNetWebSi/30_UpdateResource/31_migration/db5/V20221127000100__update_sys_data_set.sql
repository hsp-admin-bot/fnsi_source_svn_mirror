...
from data_all_meq where aa = ''治医療材料'' order by sorttag1)
 ...
 --各部分の組み合わせ
    ...
    select * from  data_commmon where e09 > ( case when (SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_OXYGEN'') is null  then ''28''
        ...
        ORDER BY e09
',2,'[{}]','0','{"applications": [4]}',NULL,'富士通：透析予約繰り返し部','2022/11/19 5:34:21.059',CURRENT_TIMESTAMP,'[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -151, "field_name": "check_user1_name", "replace_var": "@checkUser1Name"}, {"sql_cd": -151, "field_name": "check_user2_name", "replace_var": "@checkUser2Name"}]');
