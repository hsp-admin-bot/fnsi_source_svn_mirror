...
WHERE
 ...
 (SELECT
  ...
  ( NULLIF ( ( SELECT VALUE FROM dialyis_item_sort WHERE key2 = ''ITEM_COMMENT_IMPLEMENTATION'' ), '''' )|| ''-'' || ''3'' ) AS sortTag
	FROM pat_event WHERE ord_no = @ordNo) A
	WHERE e03 != '''' 
 ) UNION
 (--  実施コメンSOAP ト O
  SELECT detail_id, sbt_key, e01, e02, e03, e04, e05, e06, e07, e08, e09, e10, e11, sortTag
 ...
 (SELECT
  ...
  LEFT(REPLACE(replace(substr((result_params ::json ->> 0) ::json->> ''result_value'', 
	POSITION(''>﻿'' in (result_params ::json ->> 0) ::json->> ''result_value'' )+1),''﻿</span>'',''''),''﻿'',''''),50) AS e03,--e3
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  '''' AS e07,--e7
