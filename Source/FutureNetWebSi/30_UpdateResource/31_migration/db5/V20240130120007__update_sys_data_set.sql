        ...
        AND (
            CASE
                WHEN @syncMode = ''update''
                    THEN (ntss_db5_om.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS''))OR (ntss_db5_om.up_date BETWEEN to_timestamp(@fromDate, ''YYYYMMDDHH24MISS'') AND to_timestamp(@toDate, ''YYYYMMDDHH24MISS''))
                    ELSE (CURRENT_DATE - INTERVAL ''1 YEAR'') < to_timestamp(ntss_db5_om.treat_date, ''YYYYMMDDHH24MISS'')::timestamp AND to_timestamp(ntss_db5_om.treat_date, ''YYYYMMDDHH24MISS'')::timestamp <= CURRENT_DATE
                END
        )
        AND ntss_db5_om.pat_id IS NOT NULL
        ORDER BY patid,dialysisdate,dialysisno,ctlno;', 2, '[]'::jsonb, '1', '{"applications": [5]}'::jsonb, '{"classes": []}'::jsonb, '患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}', '2021-02-26 17:51:54.726', CURRENT_TIMESTAMP, NULL);