...
INSERT INTO ntss.sys_data_set (sql_cd, sql, db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info) VALUES (-61, 'WITH default_user_no AS (-- デフォルト利用者番号（透析実績用)
    ...
    LIMIT 1),
     ...
     mst_user_authenticator as (--常勤医
         ...
         union
         SELECT 2 as no, COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v'') AS staff_cd
         FROM mst_coop_ini AS ini
                  CROSS JOIN LATERAL json_array_elements(ini.coop_ini_info :: json) info
         WHERE facility_cd = @facilityCd
           AND is_del = ''0''
...
FROM (
         ...
         (SELECT ''comm'' AS part, staff_cd
          FROM staff_user_info
          WHERE (SELECT setting FROM user_no_setting) IN (''1'')
          LIMIT 1 OFFSET 0)
         UNION
         -- 3：共通部 版確定者
         (SELECT ''comm'' AS part, staff_cd
          FROM up_user_id_info
          WHERE (SELECT setting FROM user_no_setting) IN (''3'')
          LIMIT 1 OFFSET 0)
         UNION
         -- 2：共通部 担当医２
         (SELECT ''comm'' AS part, staff_cd
          FROM staff_user_info
          WHERE (SELECT setting FROM user_no_setting) IN (''2'')
          LIMIT 1 OFFSET 1)
         UNION
         -- 4：共通部 版確定者
         (SELECT ''comm'' AS part, staff_cd
          FROM up_user_id_info
          WHERE (SELECT setting FROM user_no_setting) IN (''4'')
          LIMIT 1 OFFSET 0)
         UNION
         -- 5：共通部：常勤医
         (SELECT ''comm'' AS part, staff_cd
