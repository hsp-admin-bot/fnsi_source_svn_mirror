DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (-497,-18,-181);
...
''指示薬剤del''::TEXT as detail_id,
  ...
  WHERE
    ...
    AND info ->> ''key2'' = ''CREATE_NUMBER_FUNCTION'') and sum(medi_amount::Float)*100<100 then 
        (case when sum(medi_amount::Float)*100 >10 
        then ''0''||((sum(medi_amount::Float)*100) :: INTEGER :: TEXT) 
        else  ''00''||((sum(medi_amount::Float)*100) :: INTEGE', 2, '[{}]', '1', '{"applications": [4]}', NULL, '指示)中止時）投与薬剤コード', '2022-06-18 05:06:30.626', CURRENT_TIMESTAMP, NULL);
