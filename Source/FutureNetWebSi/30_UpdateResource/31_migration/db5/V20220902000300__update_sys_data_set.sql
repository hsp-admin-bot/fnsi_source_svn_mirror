DELETE  FROM sys_data_set WHERE sql_cd IN (-498);
...
SELECT
  ...
  COALESCE(all_cost.e05, ''0'') AS amount,
	COALESCE(all_cost.e05, ''0'') ::INTEGER AS amounttest,
...
select --目標体重出力
	...
	NULL AS e03,
	NULL AS  e04,
	(COALESCE(rst_cond_info-> ''3'' ->> ''value'', ''0'') ::INTEGER * 100)::text AS e05,
	''kg'' AS  e06,
	5 AS syoumouhinOrder
 ...
 select --前体重出力
	...
	NULL AS e03,
	NULL AS  e04,
	 (COALESCE(rst_weight_info ->> ''weight_before'', ''0'') ::INTEGER * 100)::text AS e05,
	''kg'' AS  e06,
    5 AS syoumouhinOrder
   from ord_main where ord_no =@ordNo
 ...
 select --後体重出力
	...
	NULL AS e03,
	NULL AS  e04,
	 (COALESCE(rst_weight_info ->> ''weight_after'', ''0'') ::INTEGER * 100)::text AS e05,
	''kg'' AS  e06,
    5 AS syoumouhinOrder
   from ord_main where ord_no =@ordNo
