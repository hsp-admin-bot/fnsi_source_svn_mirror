...
WHERE
  ...
  (-- 希望開始時刻
       ...
       WHERE
        ((SELECT VALUE FROM dialysis_send WHERE key2 = ''ADD_TITLE_SEND_FLG'' ) = ''1'') AND  ''ljg'' <> ''''
       ) UNION
       (--  障害者加算:患者基本情報から取得
        SELECT
        ''実績詳細'' AS detail_id,
...
WHERE
 ...
 SELECT
  ...
  COALESCE ( ( SELECT LEFT((result_params ::json ->> 2) ::json->> ''result_value'',50) FROM pat_event WHERE ord_no = @ordNo), '''' ) AS e03,--e3
 ...
 SELECT
  ...
  COALESCE ( ( SELECT LEFT((result_params ::json ->> 1) ::json->> ''result_value'',50) FROM pat_event WHERE ord_no = @ordNo), '''' ) AS e03,--e3
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  '''' AS e07,--e7
 ...
 SELECT
  ...
  COALESCE ( ( SELECT LEFT((result_params ::json ->> 3) ::json->> ''result_value'',50) FROM pat_event WHERE ord_no = @ordNo), '''' ) AS e03,--e3
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  '''' AS e07,--e7
 ...
 SELECT
  ...
  COALESCE ( ( SELECT LEFT((result_params ::json ->> 0) ::json->> ''result_value'',50) FROM pat_event WHERE ord_no = @ordNo), '''' ) AS e03,--e3
  ''0000000.000'' AS e04,--e4
  ''0'' AS e05,--e5
  '''' AS e06,--e6
  '''' AS e07,--e7
...
(-- 酸素吸入手技
 SELECT
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_OXYGEN_PROCEDURE_TAG'' ), '''' ) AS e11,-- タグ名称
	COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_OXYGEN''), '''')|| ''-'' || ''1'' AS sortTag
 FROM
  ord_main AS ord
...
UNION ALL
 ...
 SELECT
  ...
  COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_OXYGEN''), '''')|| ''-'' || ''2'' AS sortTag
 FROM
  ord_main AS ord
  CROSS JOIN LATERAL json_array_elements ( ord.rst_treatment_info :: json ) oxy 
 WHERE
...
union all select * from data_all where medicine_cd is null and equipment_cd is null
order by sortTag
)
,
--医疗材料部分
...
union all select * from  med_order
order by sortTag
