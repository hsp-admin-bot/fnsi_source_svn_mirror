...
WHERE sql_cd in(8104,-500099,-500106,8109);
...
WHERE
  ...
  OR
  (
    0 <= (COALESCE(NULLIF(@indMediInfo.amount, '''')::NUMERIC, 0) / 100)
    AND 999999 >= (COALESCE(NULLIF(@indMediInfo.amount, '''')::NUMERIC, 0) / 100)
  );
', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)SSIの入力値チェック(投薬オーダ情報の数量)', '2025-12-14 19:42:09.757', '2025-12-14 19:42:09.757', NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
  ...
  ind_medi_info = CASE WHEN (SELECT (SELECT filtered_rst_medi_info FROM isExistsData) is not null and (ind_medi_info <> ''[]'')) THEN ''@indMediInfoValue'' 
    ELSE 
      CASE WHEN (SELECT (SELECT filtered_ind_medi_info FROM filteredValue) is null) THEN 
        ...
        "amount":'',(COALESCE(NULLIF(''@indMediInfo.amount'', '''')::NUMERIC, 0) / 100),'',
        "comment":null,
