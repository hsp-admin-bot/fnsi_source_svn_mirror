...
, inspection_records as (
  select
    ...
    wst.*,
		case
      when (wst.value <> '''') then wst.value::text || coalesce(wst.unit, '''') || (case when (wst.text <> ''0'' and wst.text is not null) then (select jsonb_array_elements(wstt.initial_string::JSONB)->>''text'' LIMIT 1 OFFSET CAST(wst.text AS INTEGER) - 1)
				else '''' end)
