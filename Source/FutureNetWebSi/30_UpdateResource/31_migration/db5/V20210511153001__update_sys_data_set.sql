delete from "sys_data_set" where "sql_cd" = -100006 or "sql_cd" = -100008;
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-100008, 'select journal.hosp_pat_id || lpad(trim(to_char(ord.ord_no, ''999999999999'')), 12, ''0'')  || lpad(trim(to_char(ord.rst_edition, ''9999'')), 4, ''0'') ||''.pdf'' as filename from sys_coop_journal journal  inner join ord_main ord on journal.ord_no = ord.ord_no where journal.ord_no = @ordNo and journal.direction = ''S'' and journal.ana_result = ''0'' and journal.is_del = ''0'' limit 1;', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'report test', '2020-07-31 18:29:49.294', '2020-07-31 18:29:49.294', NULL);
...
from
  ord_main as ord
where
  ord.ord_no = @ordNo', 2, '[]', '0', '{"applications": []}', '{"classes": []}', 'report test', '2020-07-31 18:29:49.294', CURRENT_TIMESTAMP, NULL);
