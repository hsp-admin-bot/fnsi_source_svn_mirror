...
margin_time_0 as (
    -- 透析前マージン時間
    select coalesce(nullif(info->>''value'', ''0''), info->>''default_v'') as value
...
select
  COALESCE(CASE reg_order_class    
    WHEN ''1'' THEN 
      -- 透析前
      TO_CHAR(
        TO_TIMESTAMP((SELECT kur_standard_start_time FROM standard_start_time), ''YYYYMMDDHH24MISS'')
...
select
  ...
  COALESCE(CASE reg_order_class    
    WHEN ''1'' THEN coalesce( (SELECT kbn FROM dialysis_kbn_0) , ''1'')
    WHEN ''2'' THEN coalesce( (SELECT kbn FROM dialysis_kbn_1) , ''2'')
    WHEN ''0'' THEN coalesce( (SELECT kbn FROM dialysis_kbn_2) , ''0'')
