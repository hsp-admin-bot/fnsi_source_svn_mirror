delete from "sys_data_set" where "sql_cd" in (-55);
...
, ord_no_info AS (
  SELECT
    TO_CHAR((CASE WHEN rst_fn_dialysis_no IS NOT NULL AND rst_fn_dialysis_no > 0 THEN rst_fn_dialysis_no ELSE ord_no END), ''FM0999999999999999999'') AS ord_no
  FROM
    ord_main AS ord
  WHERE
    ord_no = @ordNo
...
SELECT
  ...
  , CASE WHEN COALESCE(NULLIF((SELECT document_no_setting FROM document_no_setting_info), ''''), ''0'') = ''0'' 
    ...
    END AS ord_no
FROM ord_no_info', 2, '[]', '0', '{"applications": [4]}', NULL, '富士通）透析レポートの文書番号を取得', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
