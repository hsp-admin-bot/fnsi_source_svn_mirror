DELETE FROM "ntss"."sys_data_set" WHERE sql_cd IN (10);
...
FROM
	...
	SELECT
		...
		CEIL(cast( om.ind_cond_info :: json #>> ''{17,value}''  AS DECIMAL)  )  AS Amount,
	...
	FROM
		...
		INNER JOIN mst_medicine AS md ON (
			cast( om.ind_cond_info :: json #>> ''{15,value}'' AS DECIMAL) = md.medicine_cd
			AND md.is_del = ''0''
			AND md.is_disp = ''1''
	...
	SELECT
		...
		CEIL( cast( om.ind_cond_info :: json #>> ''{26,value}'' as DECIMAL) + cast( om.ind_cond_info :: json #>> ''{28,value}'' as DECIMAL ) )AS Amount,
