...
VALUES(-101, 'WITH dialysis_item_send AS (-- 透析項目送信
 SELECT
  info ->> ''key2'' AS key2,
...
FROM(
 (--  実施コメンSOAP ト S
  ...
  COALESCE ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_CONDUCTED_COMMENT_ATTR_S'' ), '''' ) AS e02,
	case when ((pet.result_params ::json ->> data_soap_inikey.ordernow::INTEGER-1) ::json->> ''format_class'') = ''0'' or
	(((pet.result_params ::json ->> data_soap_inikey.ordernow::INTEGER-1) ::json->> ''format_class'') = ''1'' and data_soap_inikey.is_formatting = ''0'' )
then REGEXP_REPLACE(LEFT((pet.result_params ::json ->> data_soap_inikey.ordernow::INTEGER-1) ::json->>     ''result_value'',50),E''[\\n\\r]'','' '',''g'')
  when (((pet.result_params ::json ->> data_soap_inikey.ordernow::INTEGER-1) ::json->> ''format_class'') = ''1'' and
data_soap_inikey.is_formatting = ''1'' )
...
(--  ''treat_class'' WHEN ''0''-手技あり薬剤
 SELECT
  ''実績詳細'' AS detail_id,
  ''VO2'' AS sbt_key,
	 COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.in_hospital_cd_1 ELSE mmd.in_hospital_cd_1 END), '''') AS e01, 
   COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.in_hospital_cd_2 ELSE mmd.in_hospital_cd_2 END), COALESCE((SELECT value FROM              dialysis_item_send   WHERE key2 = ''ITEM_MEDICINE_ATTR''), '''')) AS e02, 
	 COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.medicine_mix_name ELSE mmd.medicine_name END), '''') AS e03, 
   COALESCE( TO_CHAR(TO_NUMBER(treat_medi ->> ''amount'',''9999999.999''),''FM0999999.990'') )  AS e04,-- 数量
    CASE 
      WHEN (CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.unit ELSE mmd.unit END) IS NULL OR 
           (CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.unit ELSE mmd.unit END) = ''''
        THEN ''0'' 
      ELSE ''1''
    END AS e05, -- 選択単位フラグ
  COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.unit ELSE mmd.unit END), '''') AS e06,  --単位コード
  COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.unit ELSE mmd.unit END), '''') AS e07, --単位名称
  '''' AS e08,
  '''' AS e09,
  ''29'' AS e10,
  COALESCE ( (SELECT VALUE FROM dialysis_item_send  WHERE key2 = ''ITEM_MEASURE_MEDICINE_TAG'' ), '''' ) AS e11,-- タグ名称
	COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''') AS sortTag,
	treat_medi ->> ''ctl_no''  as sorttag1,
	''2'' as sorttag2,
	  (treat_medi ->>''treat_medicine_cd'') as mdcd,
		(treat_medi ->>''class_cd'') as mdclass
  FROM
  ord_main AS ord
   cross join lateral json_array_elements (ord.rst_treatment_info  :: json) treat_medi 
LEFT OUTER JOIN
    mst_medicine_mix mmx
ON
    mmx.medicine_mix_cd = TO_NUMBER(treat_medi ->> ''treat_medicine_cd'',''999999999999'')
LEFT OUTER JOIN
    mst_medicine mmd
ON
    mmd.medicine_cd = TO_NUMBER(treat_medi ->> ''treat_medicine_cd'',''999999999999'')
LEFT OUTER JOIN
    mst_procedure mp
ON
    mp.procedure_cd = TO_NUMBER(treat_medi ->> ''procedure_cd'',''999999999999'')
WHERE
    ord.ord_no = @ordNo


    AND treat_medi ->>''medicine_type'' = ''1'' 
    AND treat_medi ->> ''treat_class'' IN(''0'',''1'')
    AND treat_medi ->> ''procedure_cd'' IS NOT NULL
    AND (SELECT value FROM DIALYSIS_ITEM_PROCEDURE_TAG WHERE key2 = mp.in_hospital_cd_a1) IS NOT NULL 
    AND (SELECT value FROM DIALYSIS_ITEM_PROCEDURE_TAG WHERE key2 = mp.in_hospital_cd_a1) <> ''''
 ) UNION ALL
(--  愁訴処置-処置薬品名-手技なし薬剤
SELECT
  ...
  ''VO2'' AS sbt_key,
	 COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.in_hospital_cd_1 ELSE mmd.in_hospital_cd_1 END), '''') AS e01, 
   COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.in_hospital_cd_2 ELSE mmd.in_hospital_cd_2 END), COALESCE((SELECT value FROM              dialysis_item_send   WHERE key2 = ''ITEM_NON_PROCEDURE_ATTR''), '''')) AS e02, 
	 COALESCE((CASE treat_medi ->>''treat_class'' WHEN ''0'' THEN mmx.medicine_mix_name ELSE mmd.medicine_name END), '''') AS e03, 
   COALESCE( TO_CHAR(TO_NUMBER(treat_medi ->> ''amount'',''9999999.999''),''FM0999999.990'') )  AS e04,-- 数量
...
WHERE
    ...
    AND treat_medi ->>''medicine_type'' = ''1'' 
    AND treat_medi ->> ''treat_class'' IN(''0'',''1'')
    AND (treat_medi ->> ''procedure_cd'' IS  NULL
    or(treat_medi ->> ''procedure_cd'' IS  Not NULL and ((SELECT value FROM DIALYSIS_ITEM_PROCEDURE_TAG WHERE key2 = mp.in_hospital_cd_a1) IS NULL 
   or (SELECT value FROM DIALYSIS_ITEM_PROCEDURE_TAG WHERE key2 = mp.in_hospital_cd_a1) = '''')))
 )
union all
(--  愁訴処置-処置薬品名-手技あり薬剤
...
WHERE
    ...
    AND treat_medi ->>''medicine_type'' = ''2'' 
    AND treat_medi ->> ''treat_class'' IN(''0'',''1'')
    AND treat_medi ->> ''procedure_cd'' IS NOT NULL
    AND (SELECT value FROM DIALYSIS_ITEM_PROCEDURE_TAG WHERE key2 = mp.in_hospital_cd_a1) IS NOT NULL 
    AND (SELECT value FROM DIALYSIS_ITEM_PROCEDURE_TAG WHERE key2 = mp.in_hospital_cd_a1) <> ''''
...
UNION ALL
SELECT
	 (case when treat_medi ->>''medicine_type'' = ''1'' then  
     (SELECT in_hospital_cd_1 FROM mst_medicine WHERE medicine_cd = TO_NUMBER( treat_medi ->> ''treat_medicine_cd'' :: text, ''999999999999''))
	 else  (SELECT in_hospital_cd_1 FROM mst_medicine_mix WHERE medicine_mix_cd = TO_NUMBER( treat_medi ->> ''treat_medicine_cd'' :: text, ''999999999999'')) 
	 end ) as item_cd_s,
     CASE WHEN ''0'' in (SELECT a1 FROM do_order_data_from) THEN TO_NUMBER( json_idx :: text, ''999999999999'' ) ELSE NULL END AS login_ord_s,
     CASE WHEN ''2'' in (SELECT a1 FROM do_order_data_from) THEN TO_NUMBER( treat_medi ->> ''medicine_type'' :: text, ''999999999999'' ) ELSE NULL END AS medicine_type_s,
     NULL AS timing_cd_s,
     CASE WHEN (''5'' in (SELECT a1 FROM do_order_data_from)
		 ) THEN
		   TO_NUMBER( procedure_code_order :: text, ''999999999999'' ) 
		 ELSE NULL END AS procedure_cd_s,
     CASE WHEN ''6'' in (SELECT a1 FROM do_order_data_from) THEN TO_NUMBER( treat_medi ->> ''date_interval'' :: text, ''999999999999'' ) ELSE NULL END AS date_interval_s,
		  treat_medi ->> ''ctl_no'' as medi_no,
			treat_medi ->> ''medicine_type'' ::text as medicine_type
   FROM
 
     ord_main AS ord
     CROSS JOIN LATERAL json_array_elements(ord.rst_treatment_info  :: json) with ordinality as tmp(treat_medi , json_idx)
LEFT OUTER JOIN do_mst_procedure ON procedure_code = TO_NUMBER(treat_medi ->> ''procedure_cd'' :: text, ''999999999999'' )
 WHERE ord.ord_no = @ordNo
  ...
  ORDER BY 
   ...
   ''調製手技''::text as aa,
    ...
    COALESCE ( mp.in_hospital_cd_a2, ( NULLIF ( ( SELECT VALUE FROM dialysis_item_send WHERE key2 = ''ITEM_PROCEDURE_ATTR'' ), '''' ) ), '''' )::text AS e02,-- 項目属性
     ...
     COALESCE ( (SELECT VALUE FROM DIALYSIS_ITEM_PROCEDURE_TAG  WHERE key2 = mp.in_hospital_cd_a1 ), '''' )::text AS e11,-- タグ名称
     COALESCE((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')::text AS sortTag,
   (treat_medi->>''ctl_no'')::text as sortTag1,
   ''1''::text as sortTag2,
   ''单体手技''::text as aa,
	  med_order.ordernow::INTEGER  as ordernow
    from 
     ord_main ord
   	CROSS JOIN LATERAL json_array_elements ( ord.rst_treatment_info :: json ) treat_medi
   	LEFT OUTER JOIN mst_procedure AS mp ON mp.procedure_cd = TO_NUMBER( treat_medi ->> ''procedure_cd'', ''999999999999'' )
		LEFT OUTER JOIN med_order ON med_order.sorttag1= (treat_medi->>''ctl_no'') and (med_order.sorttag2 = ''2'' or med_order.sorttag2 = ''3'')
    WHERE
    ord.ord_no = @ordNo
   ...
   ''調製手技''::text as aa,
    ...
    from 
     ord_main ord
   	CROSS JOIN LATERAL json_array_elements ( ord.rst_treatment_info :: json ) treat_medi
   	LEFT OUTER JOIN mst_procedure AS mp ON mp.procedure_cd = TO_NUMBER( treat_medi ->> ''procedure_cd'', ''999999999999'' )
		LEFT OUTER JOIN med_order ON med_order.sorttag1= (treat_medi->>''ctl_no'') and (med_order.sorttag2 = ''22'' or med_order.sorttag2 = ''33'')
    WHERE
    ord.ord_no = @ordNo
 ...
 select * from ( (
  ...
  (
   SELECT
    ...
    COALESCE( TO_CHAR((TO_NUMBER(treat_medi ->> ''amount'', ''9999999.999'') * TO_NUMBER(mmxd ->> ''amount'', ''9999999.999'' )), ''FM0999999.990'' ) )  AS e04,-- 数量
  
      CASE 
        WHEN (CASE treat_medi->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END) IS NULL OR 
             (CASE treat_medi->>''medicine_type'' WHEN ''2'' THEN mmx.unit ELSE mmd.unit END) = ''''
          THEN ''0'' 
        ELSE ''1''
      END AS e05, -- 選択単位フラグ
     COALESCE(mmd.unit, '''''''') AS e06,  --単位コード
    ...
    COALESCE ( (SELECT VALUE FROM dialysis_item_send  WHERE key2 = ''ITEM_MEASURE_MEDICINE_TAG'' ), '''' ) AS e11,-- タグ名称
  	...
  	''22'' as sorttag2,
		''調製'' as aa,
			(SELECT login_ord_in_mm FROM do_medicine_mix_in_orders 
                            WHERE TO_NUMBER( treat_medi ->> ''treat_medicine_cd'' :: text, ''999999999999'') = medicine_mix_cd 
                            AND TRIM(mmd.in_hospital_cd_1) = item_cd_mm)::text AS ord_medicine_mix
    FROM
  ord_main AS ord
        CROSS JOIN LATERAL json_array_elements(ord.rst_treatment_info :: json) with ordinality as tmp(treat_medi, json_idx)
        LEFT OUTER JOIN mst_procedure AS mp ON mp.procedure_cd = TO_NUMBER( treat_medi ->> ''procedure_cd'', ''999999999999'' )
        LEFT OUTER JOIN mst_medicine_mix AS mmx ON mmx.medicine_mix_cd = TO_NUMBER( treat_medi ->> ''treat_medicine_cd'', ''999999999999'' )
        CROSS JOIN LATERAL json_array_elements ( mmx.mix_info :: json ) mmxd
        LEFT OUTER JOIN mst_medicine AS mmd ON mmd.medicine_cd = TO_NUMBER( mmxd ->> ''cd'', ''999999999999'' )
  WHERE
      ord.ord_no = @ordNo
   ...
   SELECT
    ...
    COALESCE ( (SELECT VALUE FROM dialysis_item_send  WHERE key2 = ''ITEM_MEASURE_MEDICINE_TAG'' ), '''' ) AS e11,-- タグ名称
  	...
  	''33'' as sorttag2,
		''調製'' as aa,
		(SELECT login_ord_in_mm FROM do_medicine_mix_in_orders 
                            WHERE TO_NUMBER( treat_medi ->> ''cd'' :: text, ''999999999999'') = medicine_mix_cd 
                            AND TRIM(mmd.in_hospital_cd_1) = item_cd_mm)::text AS ord_medicine_mix
    FROM
  ord_main AS ord
        CROSS JOIN LATERAL json_array_elements(ord.rst_treatment_info :: json) with ordinality as tmp(treat_medi, json_idx)
   ...
   ''抗凝固剤手技'' as aa,
     ...
     FROM kou_coag_procedur_falg
        ...
        (SELECT value FROM int_set_medicine_resolve WHERE key2 = ''KOU_COAG_RESOLVE_MODE'') = ''2'')       
                AND EXISTS(SELECT 1 FROM ord_main ord  WHERE ord.ord_no = @ordNo and (rst_cond_info->''25''->>''value'' is NOT NULL) AND (rst_cond_info->''25''->>''medicine_type'' = ''2'') )
        ) T
 ...
 union all
  ( select  detail_id::text, e01::text, e02::text, e03::text, e04::text, e05::text, e06::text, e07::text, e08::text, e09::text,e10::text,e11::text,
   ...
   sorttag::text,sortTag1::text,sortTag2::text,aa,ROW_NUMBER() OVER()::INTEGER as ordernow from data_oxygen)
 	...
 	order by sorttag,ordernow', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '富士通）実績繰り返し部', '2020-04-24 19:15:25.001', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -79, "field_name": "dial_diff_cd", "replace_var": "@dial_diff_cd"}]'::jsonb);