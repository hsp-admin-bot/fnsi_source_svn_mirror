...
UNION ALL
  SELECT
    ...
    MIN(pro_medi_table.timing_cd) AS timing_cd,
...
SELECT
  pro_medi_table.temp_no as temp_no,
  pro_medi_table.medicine_type as medicine_type,
  pro_medi_table.interval_no as interval_no,
  pro_medi_table.timing_cd as timing_cd,
  ''投与薬剤/愁訴処置情報(薬剤）'' AS title,
  mst_cd,
  hosp_cd,
  pro_medi_table.amount AS amount,
  pro_medi_table.unit AS unit,
  ...
  END AS pro_hosp_cd,
  CASE
    WHEN pro_medi_table.procedure_cd = -9999 THEN true
    ELSE false
  END AS oxgen_flg
FROM
  (SELECT
