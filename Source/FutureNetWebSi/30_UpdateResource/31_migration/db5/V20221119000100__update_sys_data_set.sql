delete from ntss.sys_data_set where sql_cd in (-103, -150, -151);
...
FROM
	pat_ind_approve 
WHERE
	ord_no = @ordNo', 2, '[{}]', '0', '{"applications": [4]}', NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-151, 'SELECT
(
CASE
	WHEN @checkUser1Cd = '''' THEN
	'''' 
	ELSE (
		SELECT
...
from
  mst_personal_user
where
  user_id::TEXT = @checkUser2Cd
and
  is_disp = ''1''
and
  is_del = ''0''
 ) END) AS check_user2_name', 3, '[{}]', '0', '{"applications": [4]}', NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '[{"sql_cd": -150, "field_name": "check_user1_cd", "replace_var": "@checkUser1Cd"}, {"sql_cd": -150, "field_name": "check_user2_cd", "replace_var": "@checkUser2Cd"}]');
...
SELECT
    ...
    ''予約詳細'' AS detail_id, 
    --項目コード
    COALESCE(pia.check_user1_cd :: TEXT, '''') AS e01, 
    --項目属性
...
SELECT
    ...
    ''予約詳細'' AS detail_id, 
    --項目コード
    COALESCE(pia.check_user2_cd :: TEXT, '''') AS e01, 
    --項目属性
    COALESCE((SELECT value FROM item_set_info WHERE key2 = ''ITEM_CHECK_STAFF_ATTR''), '''') AS e02, 
    --項目名称
...
, 
--找出是药剂或者医疗材料的在后面带上code
data_all AS (
SELECT data_middle_all.detail_id, data_middle_all.e01, data_middle_all.e02, data_middle_all.e03, data_middle_all.e04, data_middle_all.e05, data_middle_all.e06, data_middle_all.e07, data_middle_all.e08, data_middle_all.e09, mmd.medicine_cd, mmd.class_cd as mdclass_cd ,mmq.equipment_cd,mmq.class_cd as mqclass_cd
...
--药剂部分
data_all_med as(
select detail_id, e01,e02,e03,data_all.e04,e05,e06,e07,e08,e09,medicine_cd,mdclass_cd ,equipment_cd,mqclass_cd 
			 from data_all,medi_data where (e01 = medi_data.item_cd_s)
order by e09
),
--普通部分
data_commmon as(
  select detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09,''普通''::text as  aa from data_all where medicine_cd is null and equipment_cd is null ORDER BY  e09
)
,
--医疗材料部分
data_all_meq as(
...
SELECT DISTINCT ON (item_cd_s)* FROM (
  ...
  ORDER BY item_cd_s, login_ord_s asc) AS order_code_middle_QB
	...
	, dataequipOrder AS (
   SELECT
       detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09,
  ...
  --医疗材料多选情况下复杂排序
	 equip_order as(
SELECT  detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09, ''医疗材料''::text as aa
FROM dataequipOrder
ORDER BY 
...
med_order as(
SELECT
    detail_id, e01, e02, e03, e04, e05, e06, e07, e08, e09,''药剂''::text as  aa
FROM
    dataAndOrder
ORDER BY 
...
select * from med_order
 union all
 select * from  data_commmon where e09 >= ( case when (SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE'') is null  then ''28''
  else (((SELECT value FROM dialyis_item_sort WHERE key2 = ''ITEM_MEASURE_MEDICINE'')::INTEGER +1 )::text)end)', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通：透析予約繰り返し部', '2022-11-19 05:34:21.059', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -151, "field_name": "check_user1_name", "replace_var": "@checkUser1Name"}, {"sql_cd": -151, "field_name": "check_user2_name", "replace_var": "@checkUser2Name"}]');
