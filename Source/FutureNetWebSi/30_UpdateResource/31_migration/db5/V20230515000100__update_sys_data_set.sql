...
where a.data_pic_name<>'''' and order_main.treat_date = a.event_start_date )
UNION ALL
(select a.* from input_params_expand as a where a.data_pic_name<>'''' ORDER BY a.event_start_date )) b 
LIMIT 1
