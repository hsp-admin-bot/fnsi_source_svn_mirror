DELETE FROM ntss.sys_data_set where sql_cd in (3107,3109,3104);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (3107, 'WITH taking_title_info AS (
...
WHERE
  ...
  AND (SELECT taking_title FROM taking_title_info) != '''' -- コメントのタイトルを指定済み', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の初回申し込み→依頼事項登録→観察記録(患者イベント情報)_登録', '2022-03-14 14:46:10.256',CURRENT_TIMESTAMP, NULL);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (3104, 'WITH taking_title_info AS (
  ...
  WHERE
    ...
    AND ini.facility_cd = ''@facilityCd'' 
    AND TRIM(ini_info ->> ''key1'') = ''ORDER_RECV_TAKING_TITLE'' 
    AND TRIM(ini_info ->> ''key2'') IN (SELECT
		TRIM(ini_info ->> ''key2'')
  FROM
    mst_coop_ini AS ini 
    CROSS JOIN LATERAL json_array_elements(ini.coop_ini_info ::json) AS ini_info 
  WHERE
    ini.is_del = ''0'' 
    AND ini.facility_cd = ''999998'' 
    AND TRIM(ini_info ->> ''key1'') = ''ORDER_RECV_TAKING_TITLE'' 
    AND TRIM(ini_info ->> ''value'') in (''コメント'',''観察記録'',''依頼事項''))
    AND ''@patMemoInfo.title'' IN (SELECT
		TRIM(ini_info ->> ''key2'')
  FROM
    mst_coop_ini AS ini 
    CROSS JOIN LATERAL json_array_elements(ini.coop_ini_info ::json) AS ini_info 
...
, mstInfo AS (
  SELECT
    ...
    , (''【'' || (SELECT taking_title FROM taking_title_info) || ''】'' || ''LLLの懸念あり。'') 
     AS content
  FROM
    mstInitInfo
  WHERE
...
WHERE
  ...
  AND pat_id = @patId 
  AND facility_cd = ''@facilityCd''
  AND (SELECT idx FROM mstInfo) IS NOT NULL
', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の初回申し込み→依頼事項登録→患者メモ', '2022-03-14 14:46:10.256',CURRENT_TIMESTAMP, NULL);
...
, taking_title_info AS (
  ...
  WHERE
    ...
    AND ini.facility_cd = ''@facilityCd'' 
    AND TRIM(ini_info ->> ''key1'') = ''ORDER_RECV_TAKING_TITLE'' 
    AND TRIM(ini_info ->> ''key2'') = ''VD1'' 
    AND ''@patMemoInfo.title'' = ''VD1''
  UNION 
  SELECT
    2 AS order_no
    , '''' AS taking_title
...
--     AND ini.facility_cd = ''@facilityCd'' 
--     AND TRIM(ini_info ->> ''key1'') = ''ORDER_RECV'' 
--     AND TRIM(ini_info ->> ''key2'') = ''ORDER_RECV_BBS_STAFF'' 
--   UNION 
--   SELECT
--     2 AS order_no
--     , '''' AS bbs_stuff
