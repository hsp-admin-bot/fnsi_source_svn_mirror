delete from ntss.sys_data_set where ntss.sys_data_set.sql_cd in (1013, 1012);
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1012, 'WITH name_info AS (SELECT ''@patLastName'' ::TEXT    AS patLastName
...
VALUES ( NULLIF(''@fnPatId'', '''')
       ...
       , coalesce(personal_info_encrypt((SELECT patFirstNmKana FROM new_name_info)), '''')
       ...
       , NULLIF(''@remoteMonitorUserPw'', ''''))', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)日機装)患者プロファイル(profile)(XML):患者個人情報の取得の新規', '2022-06-13 08:34:44.813', CURRENT_TIMESTAMP, '[{"sql_cd": 1001, "field_name": "dial_diff_com_info", "replace_var": "@dialDiffComInfo"}, {"sql_cd": 1010, "field_name": "check_value", "replace_var": "@patBloodRhType"}, {"sql_cd": 1011, "field_name": "check_value", "replace_var": "@patBloodAboType"}, {"sql_cd": 1014, "field_name": "check_value", "replace_var": "@patSexOut"}, {"sql_cd": 1015, "field_name": "check_value", "replace_var": "@inOutClassOut"}]');
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1013, 'WITH name_info AS (SELECT ''@patLastName'' ::TEXT    AS patLastName
                        , ''@patFirstName'' ::TEXT   AS patFirstName
                        , ''@patLastNmKana'' ::TEXT  AS patLastNmKana
                        , ''@patFirstNmKana'' ::TEXT AS patFirstNmKana)
...
--   , pat_first_name_kana = COALESCE(personal_info_encrypt(NULLIF((SELECT patFirstNmKana FROM new_name_info), '''')) , pat_first_name_kana)
  ...
  , pat_first_name_kana    = case
                                 ...
                                 when ((SELECT concat(patLastNmKana, patFirstNmKana) FROM new_name_info) !=
                                       concat(personal_info_decrypt(pat_last_name_kana),
                                              personal_info_decrypt(pat_first_name_kana)) and
                                       (''@patFirstNmKana'' not like ''% %'')) then '''' end
