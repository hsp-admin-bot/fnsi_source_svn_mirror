...
WHERE sql_cd in (-1103003,-1103002,-1103001);


INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-1103003, 'WITH RECURSIVE coop_ini_info AS (
--連携設定より取得
SELECT
...
SELECT
  ...
  (SELECT value FROM coop_ini_info WHERE key1 = ''SCM_DIALYSISSEND'' AND key2 = ''院内コード'') AS treat_convert_value
)
, auth_info AS (
...
LEFT JOIN mst_medicine AS mst_medi ON mst_medi.medicine_cd::text = t1.tre_info ->> ''cd''
  AND t1.tre_info ->> ''medicine_type''::text = ''1''
LEFT JOIN mst_medi_mix AS mst_mix ON mst_mix.mix_cd::text = t1.tre_info ->> ''cd''
...
SELECT
  ...
  ''01'' AS detail_id
FROM
  rp_series', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'セコム連携 透析実績連携 処置実績ファイル_オーダーインデックス', '2025-07-16 14:50:48.578', CURRENT_TIMESTAMP, NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
...
LEFT JOIN mst_medi_mix AS mst_mix ON mst_mix.mix_cd::text = medi_info ->> ''cd''
  AND medi_info ->> ''medicine_type''::text = ''2''
LEFT JOIN mst_procedure AS mst_pro ON mst_pro.procedure_cd::text = t1.medi_info ->> ''procedure_cd''
CROSS JOIN ini_value
WHERE
...
SELECT
  ...
  RIGHT(mi.hosp_cd, 8) AS medi_cd,
  ROUND(mi.amount, 2)::FLOAT8::TEXT AS amount,
  mi.unit AS unit
FROM
  get_items gi
INNER JOIN medi_indo AS mi ON gi.med_code = LPAD(RIGHT(mi.hosp_cd, 6), 6,'' '')
  AND gi.technique = LPAD(RIGHT(mi.pro_hosp_cd, 2), 2,'' '')
WHERE
  mi.is_shot = ''1'' and 
  mi.is_disp = ''1''
  
ORDER BY gi.rp_no, gi.med_no', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析実績連携', '2025-06-03 08:56:02.129', CURRENT_TIMESTAMP, NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-1103001, 'WITH RECURSIVE coop_ini_info AS (
--連携設定より取得
SELECT
  COALESCE(NULLIF(info ->> ''value'', ''''), info ->> ''default_v'') AS value,
  info ->> ''key1'' AS key1,
...
LEFT JOIN mst_facility_setting AS mst ON mst.facility_setting_no = ''3007''
...
FROM
  sys_facility_setting AS sys
LEFT JOIN mst_facility_setting AS mst ON mst.facility_setting_no = ''3006''
  AND mst.facility_cd = @facilityCd
WHERE
...
-- 連携設定.治療方法変換設定をテーブル化
SELECT
  split_part(t1.set_value, ''_'', 1) AS hosp_id,
  t1.set_value AS val
FROM
  (SELECT
    UNNEST(string_to_array(ini.treat_convert_value, '','')) AS set_value
  FROM
    ini_value ini
 ) t1
)
...
FROM
  ord_main_tre tre
LEFT JOIN treat_convert tc ON tc.hosp_id = tre.hosp_cd
CROSS JOIN ini_value
)
, ind_dialyzer AS (
-- ダイアライザ
...
SELECT
  ...
  CASE
    ...
    ELSE
      CASE t1.tre_info ->> ''medicine_type''
        WHEN ''1'' THEN (t1.tre_info ->> ''amount'')::NUMERIC
        WHEN ''2'' THEN
          CASE mst_mix.solvent
            WHEN ''0'' THEN
              (t1.tre_info ->> ''amount'')::NUMERIC * mst_mix.amount::NUMERIC
            WHEN ''1'' THEN
              mst_mix.amount::NUMERIC
          END
        ELSE 0
  ...
  CASE
    ...
    WHEN t1.tre_info ->> ''treat_class'' = ''3''
      ...
      AND ini_value.oxgen_medi_code <> ''''
      AND ini_value.oxgen_procedure_code <> '''' THEN
      COALESCE(ini_value.oxgen_unit_code, NULL)
    ELSE
      CASE t1.tre_info ->> ''medicine_type''
        WHEN ''1'' THEN 
          CASE
...
UNION ALL
  SELECT
    ti.temp_no AS temp_no,
    ti.medicine_type AS medicine_type,
    ti.timing_cd AS timing_cd,
    ti.procedure_cd AS procedure_cd,
    ti.interval_no AS interval_no,
    ''愁訴処置情報(手技なし）'' AS title,
    ti.mst_cd AS mst_cd,
    ti.hosp_cd AS hosp_cd,
    ti.amount AS amount,
...
LEFT JOIN mst_medi mmd ON ind_medi_table.mst_cd = mmd.medicine_cd::text
LEFT JOIN timing_order ON ind_medi_table.timing_cd = timing_order.timing_code
LEFT JOIN procedure_order ON ind_medi_table.procedure_cd = procedure_order.procedure_code
ORDER BY
  sort_num
)
, medi_union_2 AS (
...
SELECT
  ...
  MAX(pro_medi_table.unit) AS unit,
  MAX(mst.pricedure_name) AS pro_title,
  pro_medi_table.procedure_cd,
  CASE
    WHEN pro_medi_table.procedure_cd = -9999 THEN
...
ORDER BY
  RP,
  sort_key', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(送信用)セコムの透析実績連携', '2025-07-16 14:50:48.578', CURRENT_TIMESTAMP, NULL);
