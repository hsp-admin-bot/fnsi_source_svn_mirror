DELETE  FROM sys_data_set WHERE sql_cd IN (1403);
...
, data_info AS ( 
  ...
  SELECT
    ...
    (CASE WHEN NEW.ctl_no_old IS NULL THEN info ->> ''last_name'' ELSE new.last_name::TEXT END) AS last_name,
        (CASE WHEN NEW.ctl_no_old IS NULL THEN info ->> ''first_name'' ELSE new.first_name::TEXT END) AS first_name,
    (CASE WHEN NEW.ctl_no_old IS NULL THEN info ->> ''last_name_kana'' ELSE new.last_name_kana::TEXT END) AS last_name_kana,
        (CASE WHEN NEW.ctl_no_old IS NULL THEN info ->> ''first_name_kana'' ELSE new.first_name_kana::TEXT END) AS first_name_kana,
...
WHERE
    is_del = ''0'' 
    AND pat_id = @patId 
    AND facility_cd = ''@facilityCd''', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)富士通の患者プロファイル_緊急連絡先', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, '[{"sql_cd": 1006, "field_name": "relation_flg", "replace_var": "@relationFlg"}]');
