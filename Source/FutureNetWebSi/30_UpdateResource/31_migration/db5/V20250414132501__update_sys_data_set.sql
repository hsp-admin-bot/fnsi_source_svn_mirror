...
WHERE sql_cd IN (-400001, -400017);

INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-400001, ' select
 hosp_pat_id,
 ...
 is_del = ''0''
 and
 pat_id = @patId', 3, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, '{"classes": []}'::jsonb, '日機装 透析レポート', '2020-07-31 18:29:49.000', CURRENT_TIMESTAMP, '[{"sql_cd": -400017, "field_name": "out_val", "replace_var": "@outVal"}, {"sql_cd": -400017, "field_name": "in_val", "replace_var": "@inVal"}]'::jsonb);
INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-400017, 'SELECT
    (
        SELECT
            COALESCE(NULLIF (info ->> ''value'', ''''), info ->> ''default_v'') AS out_val
        FROM
            mst_coop_ini AS ini
        ...
        WHERE
            ...
            AND info ->> ''key2'' = ''DEF_CONV_INOUT_0''
        LIMIT 1
    ) AS out_val,
    (
        SELECT
            COALESCE(NULLIF (info ->> ''value'', ''''), info ->> ''default_v'') AS in_val
        FROM
            mst_coop_ini AS ini
        CROSS JOIN LATERAL jsonb_array_elements (ini.coop_ini_info) info
        WHERE
            ...
            AND COALESCE(info->>''key0'', '''') = @key0
                AND info ->> ''key1'' = ''DIALYSISSCHESEND''
                AND info ->> ''key2'' = ''DEF_CONV_INOUT_1''
            LIMIT 1
    ) AS in_val', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, '{"classes": []}'::jsonb, '日機装 透析予約 入外区分連携設定取得', '2025-04-13 01:26:02.888', CURRENT_TIMESTAMP, NULL);