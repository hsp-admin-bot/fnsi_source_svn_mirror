  ...
  UNION ALL
    ...
    WHERE
      ord.ord_no = @ordNo ) as all_cost1 where all_cost1.e01 = all_cost.e01) count
            , all_cost.e08 AS cond_info_jyun
FROM
  (
    SELECT--1次膜情報
    ''投与薬剤'' AS detail_id,
...
SELECT data_middle_all.detail_id, data_middle_all.item_cd, data_middle_all.amount, data_middle_all.unit, data_middle_all.count, data_middle_all.cond_info_jyun, mst_medicine.medicine_cd, mst_medicine.class_cd 
FROM data_middle_all 
    LEFT JOIN mst_medicine ON data_middle_all.item_cd = TRIM(mst_medicine.in_hospital_cd_1)) AS data_t
)
, order_code_F AS (
...
SELECT DISTINCT ON (item_cd_s)* FROM (
  ...
  FROM
    do_ord AS ord, do_medicine_mix_dis
    CROSS JOIN LATERAL json_array_elements(ord.rst_medi_info :: json) with ordinality as tmp(medi, json_idx)
  WHERE ord.ord_no = @ordNo
    UNION ALL
    SELECT 
     (SELECT TRIM(in_hospital_cd_1) FROM mst_medicine WHERE medicine_cd = TO_NUMBER( medi_info_cd :: text, ''999999999999'')) AS item_cd_s,
...
SELECT
    ...
    CASE WHEN (SELECT login_ord_s FROM order_code_S WHERE order_code_S.item_cd_s = item_cd) IS NULL THEN 0 ELSE (SELECT login_ord_s FROM order_code_S WHERE order_code_S.item_cd_s = item_cd) END AS login_ord,
    CASE WHEN (SELECT login_ord_medicine_mix FROM order_code_S WHERE order_code_S.item_cd_s = item_cd) IS NULL THEN 0 ELSE (SELECT login_ord_medicine_mix FROM order_code_S WHERE order_code_S.item_cd_s = item_cd) END AS login_ord_mix,       
    CASE WHEN (SELECT class_cd_f FROM order_code_F WHERE order_code_F.item_cd_f = item_cd) IS NULL THEN 0 ELSE (SELECT class_cd_f FROM order_code_F WHERE order_code_F.item_cd_f = item_cd) END AS class_cd, 
        CASE WHEN (SELECT medicine_type_s FROM order_code_S WHERE order_code_S.item_cd_s = item_cd) IS NULL THEN 0 ELSE (SELECT medicine_type_s FROM order_code_S WHERE order_code_S.item_cd_s = item_cd) END AS medicine_type, 
    CASE WHEN (SELECT medi_cd_f FROM order_code_F WHERE order_code_F.item_cd_f = item_cd) IS NULL THEN 0 ELSE (SELECT medi_cd_f FROM order_code_F WHERE order_code_F.item_cd_f = item_cd) END AS medi_cd,
...
CASE WHEN ''0'' = (SELECT case when (select staff_cd from CREATE_NUMBER_FUNCTION_cd) IS NULL or 
            ...
            END) THEN  
                                        ((a.e02::float)*100)::TEXT
                                 ELSE
                                        case when(((a.e02::float)*100)::INTEGER) > 99 then ((a.e02::float)*100)::TEXT
                                        else 
                                            case when (((a.e02::float)*100)::INTEGER)>10 then ''0''||((a.e02::float)*100)::TEXT
                                            else ''00''||((a.e02::float)*100)::TEXT
                                            end
                                        END 
                                 END as medi_amount,
2 as medi_back,
...
mmm.amount_ml*to_number(mix.value:: json ->> ''amount'',''999'')*(select mix_count from IND_COND_INFO) as e02,
mmd.unit as e03,
''1'' as e04
...
SELECT
  ...
  UNION ALL
    ...
    WHERE
      ord.ord_no = @ordNo ) as all_cost1 where all_cost1.e01 = all_cost.e01) count
            , all_cost.e09 AS cond_info_jyun
FROM
  (
  SELECT--1次膜情報
    ''投与薬剤'' AS detail_id,
...
WHERE
  all_cost.e01 IS NOT NULL
    group by all_cost.e01,all_cost.e05,all_cost.e08,all_cost.e09
)
, data_all AS (
SELECT DISTINCT ON (item_cd)* FROM (
SELECT data_middle_all.detail_id, data_middle_all.item_cd, data_middle_all.amount, data_middle_all.unit, data_middle_all.count, data_middle_all.cond_info_jyun, mst_medicine.medicine_cd, mst_medicine.class_cd 
