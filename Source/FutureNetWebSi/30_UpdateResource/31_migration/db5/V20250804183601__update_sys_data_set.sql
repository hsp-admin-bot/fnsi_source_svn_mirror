...
, target_datas AS (
  SELECT
    datas.ctl_no,
    ''----- '' || coalesce(res_file, ''NO_FILE_res_'' || ctl_no) || '' -----'' AS res_header,
    ''----- '' || coalesce(treat_file, ''NO_FILE_treat_'' || ctl_no) || '' -----'' AS treat_header,
    ''----- '' || coalesce(inj_file, ''NO_FILE_inj_'' || ctl_no) || '' -----'' AS inj_header,
    ''----- '' || coalesce(inj_detail_file, ''NO_FILE_inj_detail_'' || ctl_no) || '' -----'' AS inj_detail_header,
    ''----- '' || coalesce(med_file, ''NO_FILE_med_'' || ctl_no) || '' -----'' AS med_header
  FROM datas
)
-- data行の特定
, matched_res AS (
