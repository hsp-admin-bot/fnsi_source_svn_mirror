...
WHERE sql_cd IN (-501102,-501103,-501105);
...
select
    ...
    CASE WHEN (SELECT ora FROM medi_order_data WHERE no2 = 7) = 0 THEN cost_fin.medi_reg_order
        ...
        WHEN (SELECT ora FROM medi_order_data WHERE no2 = 7) = 6 THEN cost_fin.interval_no END,
    CASE WHEN (select value from coop_ini) = ''0'' THEN cost_fin.medicine_type ELSE 0 END, cost_fin.medi_code_order
  ),''FM9999'') as cost_no
from
(
select
...
select --投与薬剤情報(調製)セット
    ...
    mp.pricedure_name as e07,
    10000 + t.idx AS medi_reg_order,
    medi_mix_order.medi_code_order AS medi_code_order,
  ...
  LEFT JOIN medi_mix_order ON mmx.medicine_mix_cd = medi_mix_order.medi_code
  LEFT JOIN medi_class_order ON mmx.class_cd = medi_class_order.medi_class_code
  where
    medi ->> ''effect_flg'' = ''1'' and
    medi ->> ''medicine_type'' = ''2'' and
    COALESCE(mmx.in_hospital_cd_1, '''') <> '''' and
    COALESCE(mmx.in_hospital_cd_2, '''') = '''' and
    ord.ord_no = @ordNo and
...
select --処置薬剤情報(通常)
    ...
    20000 + t.idx AS medi_reg_order,
    mmd.medi_code_order AS medi_code_order,
    mmd.medi_class_code_order AS medi_class_code_order,
    1 AS medicine_type,
    null AS timing_code_order,
    pro.procedure_code_order AS procedure_code_order,
    null ::int AS interval_no
    from
      ord_main_max as ord
    cross join lateral
...
select --処置薬剤情報(調製)分解
    ...
    COALESCE((
      case mmxd ->> ''solvent''
      when ''1'' then to_char(to_number(mmxd ->> ''amount'', ''99999.99''), ''FM99990.00'')
      else to_char(TRUNC(to_number(t.tmedi ->> ''amount'',''99999.99'') * to_number(mmxd ->> ''amount'',''99999.99''), 2), ''FM99990.00'')
      end
      ),''0.00'') as e04,
    mmd.unit as e05,
    mp.in_hospital_cd_a1 as e06,
    ...
    left outer join
      mst_medicine_mix as mmx
    on
      mmx.medicine_mix_cd = TO_NUMBER (t.tmedi ->> ''treat_medicine_cd'',''999999999999'')
    cross join lateral
      json_array_elements (mmx.mix_info :: json) mmxd
    left outer join
      mst_medi as mmd
...
select --処置薬剤情報(調製)セット
    ''処置薬剤'' as detail_id,
    mmx.in_hospital_cd_1 as e01,
    t.tmedi ->> ''treat_medicine_name'' as e02,
    mmdc.class_name as e03 ,
    COALESCE((to_char(to_number(t.tmedi ->> ''amount'',''99999.99'') ,''FM99990.00'')),''0.00'') as e04,
    t.tmedi ->> ''unit'' as e05,
    mp.in_hospital_cd_a1 as e06,
    t.tmedi ->> ''procedure_name'' as e07,
    20000 + t.idx AS medi_reg_order,
...
CASE WHEN (SELECT ora FROM medi_order_data WHERE no2 = 7) = 0 THEN cost_fin.medi_reg_order
    ...
    WHEN (SELECT ora FROM medi_order_data WHERE no2 = 7) = 5 THEN cost_fin.procedure_code_order
    WHEN (SELECT ora FROM medi_order_data WHERE no2 = 7) = 6 THEN cost_fin.interval_no END,
CASE WHEN (select value from coop_ini) = ''0'' THEN cost_fin.medicine_type ELSE 0 END, cost_fin.medi_code_order', 2, '[{}]'::jsonb, '1', '{"applications": [4]}'::jsonb, NULL, 'SSI)実績）薬剤繰り返し部', '2025-03-17 09:41:58.788', CURRENT_TIMESTAMP, NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
