...
SELECT
    ...
    COALESCE((SELECT value FROM dialysis_item_procedure_tag_info WHERE key2 = mp.in_hospital_cd_a1), '''') AS e08, 
    --出力順
    CASE length(medi->>''no'') WHEN 1 THEN COALESCE((SELECT (value :: INTEGER + 1) :: text FROM item_sort_info WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-0'' || (medi->>''no'') || ''-''|| ''1'' 
    ELSE COALESCE((SELECT (value :: INTEGER + 1) :: text FROM item_sort_info WHERE key2 = ''ITEM_MEASURE_MEDICINE''), '''')|| ''-'' || (medi->>''no'') || ''-''|| ''1'' END AS e09,
    '''' as sorttag1,
    '''' as sorttag2
FROM
    ord_main ord
