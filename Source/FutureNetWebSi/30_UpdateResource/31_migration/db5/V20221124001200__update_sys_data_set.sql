...
from data_all_meq where aa = ''治医疗材料'' order by sorttag1
 ...
 sorttag::text,sortTag1::text,sortTag2::text,aa from med_order5
    ...
    union all
		select * from data_soap', 2, '[{}]', '0', '{"applications": [4]}', NULL, '富士通）実績繰り返し部', '2020-04-24 19:15:25.001', CURRENT_TIMESTAMP, '[{"sql_cd": -20, "field_name": "in_out", "replace_var": "@inOut"}, {"sql_cd": -79, "field_name": "dial_diff_cd", "replace_var": "@dial_diff_cd"}]');
