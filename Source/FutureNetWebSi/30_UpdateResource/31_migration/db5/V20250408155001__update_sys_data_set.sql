...
VALUES(1403, 'WITH do_equals_result AS (
  SELECT 
  ...
  FROM
    ...
    AND pm.is_del = ''0''
    AND (REPLACE(info ->> ''last_name'','' '','''') || REPLACE(info ->> ''first_name'','' '','''')) = REPLACE(REPLACE(''@otherContactInfo.lastName'' ::TEXT,'' '',''''),''　'','''')
)
, do_old_ordno AS (
  SELECT 
...
, check_name_update AS (
  SELECT
    CASE
      WHEN POSITION('' '' IN lastName) = 0
        AND POSITION(''　'' IN lastName) = 0
      THEN ''0''
      ELSE ''1''
...
, data_new_info AS (
  SELECT 
    ...
    --CASE WHEN COALESCE(NULLIF(''@relationFlg'', ''''), ''0'') = ''1'' THEN NULLIF(''@otherContactInfo.relationCd'', '''') ELSE null END AS relation_cd,
        NULLIF(''@otherContactInfo.relationCd'', '''') AS relation_cd,
...
, data_exists_info AS (
  ...
  FROM
    pat_personal_main pm
    CROSS JOIN LATERAL json_array_elements (personal_info_decrypt_jsonb(pm.other_contact_info) :: json ) AS info 
    INNER JOIN data_new_info AS NEW ON (REPLACE(info ->> ''last_name'' ::TEXT,'' '','''') || REPLACE(info ->> ''first_name'' ::TEXT,'' '','''')) = (REPLACE(NEW.last_name ::TEXT,'' '','''') || REPLACE(NEW.first_name ::TEXT,'' '','''')) 
  WHERE
    pm.pat_id = @patId 
    AND pm.facility_cd = ''@facilityCd'' 
    AND pm.is_del = ''0'' 
...
, data_info AS ( 
  ...
  SELECT
    ...
    (CASE WHEN NEW.ctl_no_old IS NULL OR (SELECT name_update_flg FROM check_name_update) = ''0'' THEN info ->> ''last_name'' ELSE new.last_name::TEXT END) AS last_name,
        (CASE WHEN NEW.ctl_no_old IS NULL OR (SELECT name_update_flg FROM check_name_update) = ''0'' THEN info ->> ''first_name'' ELSE new.first_name::TEXT END) AS first_name,
    (CASE WHEN NEW.ctl_no_old IS NULL OR (SELECT name_update_flg FROM check_name_update) = ''0'' THEN info ->> ''last_name_kana'' ELSE new.last_name_kana::TEXT END) AS last_name_kana,
        (CASE WHEN NEW.ctl_no_old IS NULL OR (SELECT name_update_flg FROM check_name_update) = ''0'' THEN info ->> ''first_name_kana'' ELSE new.first_name_kana::TEXT END) AS first_name_kana,
...
WHERE
    is_del = ''0'' 
    AND pat_id = @patId 
    AND facility_cd = ''@facilityCd''', 3, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)富士通の患者プロファイル_緊急連絡先', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, '[{"sql_cd": 1006, "field_name": "relation_flg", "replace_var": "@relationFlg"}]'::jsonb);
