DELETE FROM "sys_data_set" WHERE "sql_cd" in (3,4,5,6,8,12,15,45,74,75);
...
from
  ...
  WHERE
	master_physical_name = ''mst_medicine_class'') as dmed_cls on dmed_cls.medi_class_code = med.class_cd
  left join (SELECT
		index_no AS code_order,
		TO_NUMBER( order_cd ->> ''code'', ''999999999999'' ) AS medi_class_code,
		order_cd ->> ''name'' AS medi_class_code_name
  FROM
		...
		CROSS JOIN LATERAL jsonb_array_elements ( order_settings -> ''items'' ) WITH ORDINALITY AS tmp ( order_cd, index_no )
  WHERE
		master_physical_name = ''mst_medicine'') as dmed on dmed.medi_class_code = med.medicine_cd
  left join (SELECT
		index_no AS code_order,
		TO_NUMBER( order_cd ->> ''code'', ''999999999999'' ) AS medi_class_code,
		order_cd ->> ''name'' AS medi_class_code_name
  FROM
		...
		CROSS JOIN LATERAL jsonb_array_elements ( order_settings -> ''items'' ) WITH ORDINALITY AS tmp ( order_cd, index_no )
  WHERE
		master_physical_name = ''mst_medicine_mix'') as dmed_mix on dmed_mix.medi_class_code = mix.medicine_mix_cd
  left join (SELECT
		index_no AS code_order,
		TO_NUMBER( order_cd ->> ''code'', ''999999999999'' ) AS medi_class_code,
		order_cd ->> ''name'' AS medi_class_code_name
  FROM
		...
		CROSS JOIN LATERAL jsonb_array_elements ( order_settings -> ''items'' ) WITH ORDINALITY AS tmp ( order_cd, index_no )
  WHERE
		master_physical_name = ''mst_medicate_timing'') as dtim on dtim.medi_class_code = tim.medicate_timing_cd
  left join (	SELECT
		index_no AS code_order,
		TO_NUMBER( order_cd ->> ''code'', ''999999999999'' ) AS medi_class_code,
		order_cd ->> ''name'' AS medi_class_code_name
  FROM
		...
		CROSS JOIN LATERAL jsonb_array_elements ( order_settings -> ''items'' ) WITH ORDINALITY AS tmp ( order_cd, index_no )
  WHERE
		master_physical_name = ''mst_procedure'') as dpro on dpro.medi_class_code = pro.procedure_cd
    LEFT JOIN ( select
       ...
       and
             med.medicine_cd = ms.code ) a
left join g
ON
	g.ordNob=a.ord_no
			 where a.class_cd IN (@medIds)
			order by
            a.index
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (6, 'WITH mstcp_tbl AS (
	...
	CASE
			treat_class
			WHEN ''0'' THEN
			mstMedicMix.in_hospital_cd_1 ELSE mstMedic.in_hospital_cd_1
		END AS treatMdeci_in_hospital_cd_1,
	CASE
			treat_class
			WHEN ''0'' THEN
			mstMedicMix.in_hospital_cd_2 ELSE mstMedic.in_hospital_cd_2
		END AS treatMdeci_in_hospital_cd_2,
	CASE
			treat_class
			WHEN ''0'' THEN
			mstMedicMix.in_hospital_cd_3 ELSE mstMedic.in_hospital_cd_3
		END AS treatMdeci_in_hospital_cd_3,
	CASE
			treat_class
			WHEN ''0'' THEN
			'''' ELSE mstMedic.in_hospital_cd_4
		END AS treatMdeci_in_hospital_cd_4
	FROM
		mst_comp_treatment mstCpt
	...
	FROM
		ord_main
	WHERE
		rst_dialysis_state BETWEEN ''1''
		AND ''5''
		AND ord_no = @ordNo
		AND treat_date BETWEEN to_char( date_trunc( ''day'', ( @fromDate ) :: TIMESTAMP ), ''yyyymmdd'' )
	...
	FROM
		M INNER JOIN mni_monitor ON ( M.ord_no = mni_monitor.ord_no )
	GROUP BY
		M.ord_no,
		mni_monitor.data_type
	),
	...
	f AS ( SELECT e.*
	, e.経過時間 + e.残り時間_除水完了 AS 予測時間_除水, e.経過時間 + e.残り時間_透析完了 AS 予測時間_透析 FROM e ),
	G AS (
		SELECT-- 終了予定
		M.rst_start_date + e.経過時間 * INTERVAL ''1 minute'' AS ind_end_date,
-- 終了予測
	CASE

			WHEN M.rst_dialysis_state < ''3'' THEN
		NULL
			WHEN f.残り時間_除水完了 > f.残り時間_透析完了 THEN
			M.rst_start_date + f.予測時間_除水 * INTERVAL ''1 minute'' ELSE M.rst_start_date + f.予測時間_透析 * INTERVAL ''1 minute''
		END AS ind_end_date_time -- 透析開始
		,
		M.rst_start_date -- 透析終了
		,
		M.rst_end_date
		,
		M.ord_no
	...
	FROM
		M LEFT JOIN e ON M.ord_no = e.ord_no
		LEFT JOIN f ON M.ord_no = f.ord_no
	) SELECT G
	.*,
...
FROM
	...
	WHERE
		...
		AND complaint ->> ''checkFlag'' = ''1'' --↓画面上で追加している項目の追加処理
	UNION
	SELECT
		ord_no,
		rst_start_date :: TEXT AS occur_date,
		''治療開始'' AS complaint,
	NULL
	FROM
		ord_main
	WHERE
		ord_no = @ordNo
		AND rst_start_date IS NOT NULL UNION
	SELECT
		ord_no,
		rst_end_date :: TEXT AS occur_date,
		''治療終了'' AS complaint,
		''999''
	FROM
		ord_main
	WHERE
		ord_no = @ordNo
		AND rst_end_date IS NOT NULL --↑画面上で追加している項目の追加処理
	...
	CASE
			...
			WHEN treatment ->> ''treat_class'' = ''3''
			AND treatment ->> ''oxygen_start'' IS NOT NULL THEN
				concat ( ''酸素吸入開始 '', to_char( CAST ( treatment ->> ''oxygen_speed'' AS NUMERIC ), ''FM999999.00'' ), ''L/min'' )
				WHEN treatment ->> ''treat_class'' = ''3''
				AND treatment ->> ''oxygen_start'' IS NULL THEN
					concat ( ''酸素吸入終了 '', to_char( CAST ( treatment ->> ''oxygen_amount'' AS NUMERIC ), ''FM999999.00'' ), ''L'' )
					WHEN treatment ->> ''treat_class'' = ''4''
					AND treatment ->> ''electrocardiogram_start'' IS NOT NULL THEN
						''心電図測定開始''
						WHEN treatment ->> ''treat_class'' = ''4''
						AND treatment ->> ''electrocardiogram_start'' IS NULL THEN
							''心電図測定終了'' ELSE treatment ->> ''treat_name''
							END AS treat_name,
					...
					FROM
						ord_main AS ord
						CROSS JOIN LATERAL json_array_elements ( ord.rst_treatment_info :: JSON ) treatment
						LEFT JOIN mstcp_tbl ON ( mstcp_tbl.comp_treatment_cd :: TEXT = treatment ->> ''treat_cd'' )
						LEFT JOIN ord_material_save AS save ON (
							save.supplies_base_no = ord.ord_no
							AND ord.facility_cd = save.facility_cd
							AND treatment ->> ''treat_medicine_cd'' = save.supplies_cd
							AND save.supplies_source_class = ''3''
							AND save.ind_rst_class = ''2''
						)
						LEFT JOIN mst_medicine AS mstMedic ON (
							treatment ->> ''treat_medicine_cd'' = mstMedic.medicine_cd :: TEXT
					...
					WHERE
						...
						AND ord.rst_dialysis_state <> ''0''
						AND ord_no = @ordNo
						AND treatment ->> ''checkFlag'' = ''1'' UNION ALL
					SELECT
						ord_no,
					...
					FROM
						...
						FROM
							mst_machine_record
							A LEFT JOIN mst_machine_record_control b ON A.machine_record_cd = b.machine_record_cd
					...
					FROM
						ord_main AS ord
						CROSS JOIN LATERAL json_array_elements ( ord.rst_treat_staff_info :: JSON ) treat_staff
					WHERE
						ord.is_del = ''0''
						AND ord.rst_dialysis_state <> ''0''
						AND ord_no = @ordNo
					...
					ORDER BY
						...
						occur_date,
						row_no
					) C ON COALESCE ( A.ord_no, b.ord_no ) = C.ord_no
					AND COALESCE ( A.occur_date, b.occur_date ) = C.occur_date
					AND COALESCE ( A.row_no, b.row_no ) = C.row_no
					left join G
...
, b.rst_end_date
, b.ord_no as ordnob
from  b left join e on b.ord_no = e.ord_no left join f on b.ord_no = f.ord_no)	SELECT g.*,
	1 AS dis_order,
	ord.*,
...
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (75, '	with b AS (
    select ord_main.* from ord_main
     where facility_cd = @facilityCd
