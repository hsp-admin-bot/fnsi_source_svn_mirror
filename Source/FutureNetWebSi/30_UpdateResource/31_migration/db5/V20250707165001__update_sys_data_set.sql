...
WHERE sql_cd IN (-603102, -600014, 1206, -501093);
...
INSERT INTO sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(1206, 'WITH device_set_info_default AS ( 
  SELECT
    host_notification_info
  FROM
    mst_device_set_info_default
...
VALUES ( 
  ...
  , CASE ''@oldUpDate_Date'' 
    ...
    END
  , NULLIF((SELECT host_notification_info FROM device_set_info_default), ''[]'') ::JSONB
)', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)富士通の浄化申し込み・初回指示_患者基本情報の新規
', '2022-01-28 18:21:40.000', CURRENT_TIMESTAMP, '[{"sql_cd": 1002, "field_name": "pat_memo_info", "replace_var": "@patMemoInfo"}, {"sql_cd": 1003, "field_name": "infect_info", "replace_var": "@infectInfo"}, {"sql_cd": 1004, "field_name": "addition_info", "replace_var": "@additioninfo"}]'::jsonb);
...
VALUES ( 
  ...
  , CASE ''@oldUpDate_Date'' 
    ...
    END
  , NULLIF((SELECT host_notification_info FROM device_set_info_default), ''[]'') ::JSONB
)', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, '(受信用)SSIの患者プロファイル_患者基本情報の新規', '2025-06-19 10:46:04.398', CURRENT_TIMESTAMP, '[{"sql_cd": 1002, "field_name": "pat_memo_info", "replace_var": "@patMemoInfo"}, {"sql_cd": 1004, "field_name": "addition_info", "replace_var": "@additioninfo"}]'::jsonb);