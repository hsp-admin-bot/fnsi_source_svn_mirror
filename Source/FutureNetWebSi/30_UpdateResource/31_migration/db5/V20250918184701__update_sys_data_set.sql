...
WHERE sql_cd in (-504003, -504004, -504005, -504006, -504007, -504008, -504009, -504010, -504011, -504012);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-504003, '-- =====================================================
-- 複数行SQL: -504003 透析条件情報
...
content_conditions AS (
  ...
  FROM jsonb_array_elements(@content::jsonb) AS content_data(element)
)
...
xml_dispose AS (
  SELECT 
    xml_base.ctl_no,
    xml_base.is_xml_valid,
    unnest(xpath(''//DISPOSE/DISPOSE_INFO/@DISPOSE_CTL_NO'', xml_data))::text as xml_cost_no,
    unnest(xpath(''//DISPOSE/DISPOSE_INFO/TENCD/text()'', xml_data))::text as xml_tencd,
    unnest(xpath(''//DISPOSE/DISPOSE_INFO/TKJNAM/text()'', xml_data))::text as xml_tkjnam,
    unnest(xpath(''//DISPOSE/DISPOSE_INFO/AMOUNT/text()'', xml_data))::text as xml_amount,
    unnest(xpath(''//DISPOSE/DISPOSE_INFO/UNIT/text()'', xml_data))::text as xml_unit
  FROM xml_base
  WHERE xml_data IS NOT NULL
),
...
content_dispose AS (
  SELECT
    ...
    (content_data.element ->> ''row_no'')::int AS row_no
  FROM jsonb_array_elements(@content::jsonb) AS content_data(element)
)
...
WHERE
  -- XMLデータ優先：XMLが有効な場合はXMLデータのみ
  (xml_base.is_xml_valid = true AND xml_dispose.xml_cost_no IS NOT NULL)
  OR
  -- フォールバック：XMLが無効な場合はcontentデータのみ
  (xml_base.is_xml_valid = false AND content_dispose.detail_id IS NOT NULL);', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'SSI 実績送信', '2025-09-13 19:58:07.067', '2025-09-13 19:58:07.068', '[{"sql_cd": -301, "field_name": ["detail_id", "e01", "e02", "e03", "e04", "table_no", "row_no"], "replace_var": "content"}]'::jsonb);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-504005, '-- =====================================================
-- 複数行SQL: -504005 医材詳細情報
-- XMLパース第一優先、フォールバック対応
-- パラメータ: ctlNo, ordNo, facilityCd
-- =====================================================
...
xml_equip AS (
  SELECT 
    xml_base.ctl_no,
    xml_base.is_xml_valid,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/@CTL_NO'', xml_data))::text as xml_cost_no,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/EQUIP_CD/text()'', xml_data))::text as xml_equip_cd,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/EQUIP_NAME/text()'', xml_data))::text as xml_equip_name,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/EQUIP_CLASS_NAME/text()'', xml_data))::text as xml_equip_class_name,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/PUNCTURE_CLASS/text()'', xml_data))::text as xml_puncture_class,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/AMOUNT/text()'', xml_data))::text as xml_amount,
    unnest(xpath(''//DIALYSIS_EQUIP/EQUIP_INFO/UNIT/text()'', xml_data))::text as xml_unit
  FROM xml_base
  WHERE xml_data IS NOT NULL
),
...
SELECT
  ...
  CASE
    WHEN COALESCE(xml_base.is_xml_valid, false) = true THEN
      xml_equip.xml_cost_no
    ELSE
      content_equip.cost_no
  END as cost_no,

  CASE
    WHEN COALESCE(xml_base.is_xml_valid, false) = true THEN
      xml_equip.xml_equip_cd
    ELSE
      content_equip.e01
  END as e01,
  ...
  CASE
    WHEN COALESCE(xml_base.is_xml_valid, false) = true THEN
      xml_equip.xml_equip_class_name
    ELSE
      content_equip.e03
  END as e03,
...
FROM xml_base
LEFT JOIN xml_equip ON xml_equip.ctl_no = xml_base.ctl_no
LEFT JOIN content_equip ON COALESCE(xml_base.is_xml_valid, false) = false
WHERE
  -- XMLデータ優先：XMLが有効な場合はXMLデータのみ
  (xml_base.is_xml_valid = true AND xml_equip.xml_cost_no IS NOT NULL)
  OR
  -- フォールバック：XMLが無効な場合はcontentデータのみ
  (xml_base.is_xml_valid = false AND content_equip.detail_id IS NOT NULL);', 2, '[{}]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'SSI 実績送信', '2025-09-13 19:58:07.067', '2025-09-13 19:58:07.068', '[{"sql_cd": -501102, "field_name": ["detail_id", "e01", "e02", "e03", "e04", "e05", "e06", "cost_no"], "replace_var": "content"}]'::jsonb);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
VALUES(-504006, '-- =====================================================
...
xml_medicine AS (
  SELECT
    xb.ctl_no,
    TRUE AS is_xml_valid,
    (xpath(''MEDI_INFO/@CTL_NO'',               mi.n))[1]::text                           AS xml_cost_no,
    (xpath(''MEDI_INFO/MEDICINE_CD/text()'',    mi.n))[1]::text                           AS xml_medicine_cd,
    (xpath(''MEDI_INFO/MEDICINE_NAME/text()'',  mi.n))[1]::text                           AS xml_medicine_name,
    (xpath(''MEDI_INFO/MEDI_CLASS_NAME/text()'',mi.n))[1]::text                           AS xml_medi_class_name,
    (xpath(''MEDI_INFO/AMOUNT/text()'',         mi.n))[1]::text                           AS xml_amount,
    (xpath(''MEDI_INFO/UNIT/text()'',           mi.n))[1]::text                           AS xml_unit,
    (xpath(''MEDI_INFO/PROCEDURE_CD/text()'',   mi.n))[1]::text                           AS xml_procedure_cd,
    (xpath(''MEDI_INFO/PROCEDURE_NAME/text()'', mi.n))[1]::text                           AS xml_procedure_name
  FROM xml_base xb
  CROSS JOIN LATERAL (
    SELECT unnest(xpath(''//DIALYSIS_MEDI/MEDI_INFO'', xmlparse(document xb.xml_text))) AS n
  ) AS mi
...
content_medicine AS (
  ...
  FROM jsonb_array_elements(@content::jsonb) AS t(elem)
)
SELECT
  ''薬剤詳細'' AS detail_id,
...
FROM xml_base xb
LEFT JOIN xml_medicine  xm ON xm.ctl_no = xb.ctl_no
LEFT JOIN content_medicine cm
  ON xm.ctl_no IS NULL
WHERE
  (xm.is_xml_valid IS TRUE)
  OR
  (xm.ctl_no IS NULL AND cm.detail_id IS NOT NULL);
...
content_staff AS (
  ...
  SELECT
    elem ->> ''charge1_id''     AS charge1_id,
    elem ->> ''charge1_name''   AS charge1_name,
    elem ->> ''charge2_id''     AS charge2_id,
    elem ->> ''charge2_name''   AS charge2_name,
    elem ->> ''puncture1_id''   AS puncture1_id,
    elem ->> ''puncture1_name'' AS puncture1_name,
    elem ->> ''puncture2_id''   AS puncture2_id,
    elem ->> ''puncture2_name'' AS puncture2_name,
    elem ->> ''return1_id''     AS return1_id,
    elem ->> ''return1_name''   AS return1_name,
    elem ->> ''return2_id''     AS return2_id,
    elem ->> ''return2_name''   AS return2_name
  FROM src, LATERAL jsonb_array_elements(src.j) AS elem
  LIMIT 1
)
