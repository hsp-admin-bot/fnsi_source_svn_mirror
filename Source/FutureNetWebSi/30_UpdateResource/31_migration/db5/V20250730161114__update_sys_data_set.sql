...
sort_map(sort_no, seq, item_id) AS (
  VALUES
    ...
    (130, 14, 12 ), (140, 15, 13 ), (150, 16, 14 ), (160, 17,  0 ),
    (170, 18,  0 ), (180, 19,  0 ), (190, 20, 40 ), (200, 21,  0 ),
...
header_parts AS (
  (
    SELECT 0 AS seq,
           0 AS item_id,
           (CASE
              WHEN cr.treatment_weekday IS NULL
                   OR cr.treatment_weekday = ''''
              THEN ''''
           ...
           TO_CHAR(
             CASE
               ...
               WHEN LENGTH(cr.treatment_start_date) >= 19
                 AND SUBSTR(cr.treatment_start_date, 5, 1) = ''-'' THEN
                 to_date(SUBSTR(cr.treatment_start_date, 1, 10), ''YYYY-MM-DD'')
               ELSE NULL
             END,
             ''YYYY/MM/DD'') ||
           CASE
             WHEN cr.treatment_end_date IN ('''', ''99991231'', ''9999-12-31'')
             ...
             ELSE '' '' || U&''\FF5E'' || '' '' ||
                  TO_CHAR(
                    CASE
                      ...
                      WHEN LENGTH(cr.treatment_end_date) >= 19
                        AND SUBSTR(cr.treatment_end_date, 5, 1) = ''-'' THEN
                        to_date(SUBSTR(cr.treatment_end_date, 1, 10), ''YYYY-MM-DD'')
                      ELSE NULL
                    END,
                    ''YYYY/MM/DD'')
           END || '')'' || CHR(10) AS part
