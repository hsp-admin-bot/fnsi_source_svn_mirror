...
WHERE
    is_del = ''0'' 
  AND pat_id = @patId 
  AND facility_cd = ''@facilityCd''', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)日機装の患者プロファイル(薬剤アレルギー情報)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);





INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (9628, 'WITH newRecAdd as (
SELECT * FROM (
  SELECT jsonb_array_elements(taboo_allergy_info) as tt FROM pat_main WHERE pat_id = @patId AND facility_cd = ''@facilityCd''
) t
WHERE
  tt->>''memo'' like ''【分類】薬剤アレルギー%'' AND tt->>''flag'' = ''add''
),
...
CASE WHEN taboo_allergy_info != ''[]'' THEN
    CASE WHEN newRecCnt.newCnt > 0 THEN
	    CASE WHEN EXISTS(SELECT * FROM pat_main, jsonb_array_elements_text(taboo_allergy_info) WHERE pat_id = @patId AND value like ''%"memo": "【分類】薬剤アレルギー%'' AND (value like ''%"taboo_allergy_cd": ""%'' or value like concat(concat(''%"taboo_allergy_cd": "'', (select seq from mstRec)), ''"%''))) THEN
	        --薬剤アレルギーあり
	    	--薬剤以外、更新項目、追加項目
	    	COALESCE((
