delete from ntss.sys_data_set where sql_cd in (-13, -99992);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-13, 'SELECT
  ...
  COALESCE((select treatment_name from mst_treatment where treatment_cd = ord.ind_treatment_cd), '''') AS treatment_name,
  COALESCE((select in_hospital_cd_a1 from mst_treatment where treatment_cd = ord.ind_treatment_cd), '''') AS treatment_cd,
...
WHERE
  ...
  pm.pat_id = ord.pat_id', 2, '[{}]', '0', '{"applications": [4]}', NULL, '汎用）指示）透析条件', '2020-03-18 19:07:17.001', CURRENT_TIMESTAMP, NULL);
...
WHERE
  ...
  ordCoopNo.facility_cd = @facilityCd and
