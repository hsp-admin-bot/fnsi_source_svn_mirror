...
FROM
	...
	WHERE
		...
		FROM
			...
			LEFT OUTER JOIN meqcc ON emq.class_cd = meqcc.meq_code
		ORDER BY
		              ...
		              mdcc.pk_order,
								...
								FROM
									...
									WHERE
										...
										COALESCE ( mdc.class_name, '''' ) AS kind,
										md.medicine_name AS NAME,
										( to_number( medi ->> ''amount'', ''99999.99'' ) * to_number( mmxd ->> ''amount'', ''99999.99'' ) ) AS Amount,
										COALESCE ( md.unit, '''' ) AS Unit,
										md.in_hospital_cd_1,
										md.in_hospital_cd_2,
										md.in_hospital_cd_3,
