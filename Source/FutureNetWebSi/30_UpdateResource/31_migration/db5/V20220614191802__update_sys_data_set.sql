...
VALUES (1706, 'WITH data_new_info AS (
  SELECT 
    null AS ctl_no,
    ''2'' :: TEXT AS in_out, 
    null AS reason,
...
WHERE
  ...
  AND facility_cd = ''@facilityCd'' 
  AND is_del = ''0''
  AND (SELECT exists_flag FROM data_exists_info) = ''0'' ', 2, '[{}]', '0', '{"applications": [4]}', null,
        '(受信用)富士通の患者プロファイル_固有情報_入外・転入出情報(死亡)', '2020-05-25 18:21:40.841', CURRENT_TIMESTAMP, null);
