DELETE FROM "ntss"."sys_data_set" WHERE sql_cd = 45;
INSERT INTO "ntss"."sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (45, 'with ord_tbl as (
  ...
  and
	 ...
	 where
	 ord_no=@ordNo
	 and is_del = ''0'') and is_del = ''0'' and rst_dialysis_state = ''0'' and facility_cd = @facilityCd ORDER BY treat_date ASC limit 1
...
-- 指定患者の車いす情報を取得
  ...
  select
    pm.pat_id,
    mwc.wheel_chair_name,
    mwc.wheel_chair_weight
  from
    pat_main pm
    join mst_wheel_chair mwc
      on mwc.wheel_chair_cd = pm.wheel_chair_cd
  where
    pm.pat_id = @patId
  and
    mwc.is_disp = ''1''
...
-- 	treatment_tbl.treatment_name AS treatment_name1,
  ...
  round(pat_wheel_chair_tbl.wheel_chair_weight / 1000.0, 2) as wheel_chair_weight,
