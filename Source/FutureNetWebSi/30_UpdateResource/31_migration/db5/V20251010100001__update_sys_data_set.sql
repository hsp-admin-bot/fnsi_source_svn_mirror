...
SELECT
    COALESCE(
        (SELECT
            COALESCE(NULLIF(info->>''value'', ''''), NULLIF(info->>''default_v'', ''''), ''1'')
        FROM
            mst_coop_ini AS ini
...
SELECT
  ...
  CASE (SELECT VALUE FROM ssi_in_hospital_cd)
    WHEN ''1'' THEN COALESCE(mmc.in_hospital_cd_1 , '''')
    WHEN ''2'' THEN COALESCE(mmc.in_hospital_cd_2 , '''')
...
CASE (SELECT value FROM get_bed_code_conv)
  ...
  TO_CHAR(ord.rst_start_date, ''yyyy-MM-dd hh24:mi:ss'') AS start_date,--透析開始日時
  COALESCE(TO_CHAR( ord.rst_start_date, ''YYYYMMDDHH24MISS'' ), '''') AS start_date14,
  TO_CHAR(ord.rst_end_date, ''yyyy-MM-dd hh24:mi:ss'') AS end_date,--透析終了日時
