delete from "sys_data_set" where "sql_cd" = 1718;
INSERT INTO "sys_data_set" ("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1718, 'WITH exam_date_tmp AS ( 
  SELECT
    (''@physicalInfo.examDate_Date'')::TEXT AS exam_date
) 
, exam_date_info AS ( 
  SELECT
  ...
  FROM
   (
    ...
    WHERE
      ...
      AND COALESCE(ini_info->>''key0'','''')= ''@key0''
      AND ini.facility_cd = ''@facilityCd'' 
      AND TRIM(ini_info ->> ''key1'') = ''SSI_ORDER_RECV'' 
...
, data_info AS ( 
  SELECT
    ...
    , (TO_NUMBER(order_class, ''FM9'') + 1) ::TEXT AS order_class
...
WHERE
  ...
  AND (SELECT ctr_imp_flag FROM ctr_imp_flag_info) = ''1'' ', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)SSIの患者プロファイル_固有情報_身体情報', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
