DELETE FROM "ntss"."sys_data_set" WHERE "sql_cd" IN (1102,1103);
INSERT INTO "ntss"."sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (1010, 'SELECT
    CASE WHEN @patBloodTypeRh = TRIM(ini_info ->> ''value'') THEN TO_NUMBER(@patBloodTypeRh, ''FM9999999999999999'') 
       ELSE ''0''
    END AS check_value
  FROM
    mst_coop_ini AS ini 
  ...
  LIMIT 1', 2, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)連携設定詳細(血液型RH)の取得', '2022-01-17 15:02:47', '2022-05-26 10:08:29', NULL);
...
)', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報の取得の新規', '2020-05-25 18:21:40.841', '2022-02-07 08:32:43.192', '[{"sql_cd": 1001, "field_name": "dial_diff_com_info", "replace_var": "@dialDiffComInfo"}, {"sql_cd": 1010, "field_name": "check_value", "replace_var": "@patBloodRhType"}]');
...
WHERE
  ...
  AND facility_cd = ''@facilityCd''', 3, '[{}]', '0', '{"applications": [4]}', NULL, '(受信用)患者個人情報の取得の修正', '2022-01-07 18:21:46', '2022-03-28 16:21:47.522', '[{"sql_cd": 1009, "field_name": "up_base_disease_flg", "replace_var": "@upBaseDiseaseFlg"}, {"sql_cd": 1010, "field_name": "check_value", "replace_var": "@patBloodRhType"}]');
