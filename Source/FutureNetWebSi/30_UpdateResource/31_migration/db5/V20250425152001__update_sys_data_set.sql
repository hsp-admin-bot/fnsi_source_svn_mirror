...
interview_record_info as (
	SELECT
		...
		(unescape_html(REPLACE(REGEXP_REPLACE(substring(REGEXP_REPLACE(result_row.value ->> ''result_value'' , ''</[p^>]*>'', '''', ''g'') from 1 for length(REGEXP_REPLACE(result_row.value ->> ''result_value'' , ''</[p^>]*>'', '''', ''g''))), ''<[^>]*>'', '''', ''g''),''&#65279;'' ,'''')))  AS e01
	FROM
		result_row
...
INNER JOIN interview_record_info ON
	pei.pat_event_cd = interview_record_info.pat_event_cd', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'Medicom透析経過データ連携問診記録取得SQL', '2025-04-07 14:38:45.425', '2025-04-09 16:45:21.570', NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
...
', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'Medicom透析経過データ連携SOAP繰り返し取得SQL', '2025-04-07 14:38:45.425', '2025-04-09 16:45:21.570', NULL);
INSERT INTO ntss.sys_data_set
(sql_cd, "sql", db_class, detail, can_repeat, use_application, report_class, memo, reg_date, up_date, pre_sql_info)
...
INNER JOIN nursing_notes_info ON
	pei.pat_event_cd = nursing_notes_info.pat_event_cd', 2, '[]'::jsonb, '0', '{"applications": [4]}'::jsonb, NULL, 'Medicom透析経過データ連携看護メモ録取得SQL', '2025-04-07 14:38:45.425', '2025-04-09 16:45:21.570', NULL);
