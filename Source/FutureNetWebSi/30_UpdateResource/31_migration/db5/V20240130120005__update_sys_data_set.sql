...
WHERE
    ...
    AND (
        CASE
            ...
            ELSE CAST(ntss_db5_om.treat_date  as DATE) > (CURRENT_DATE - INTERVAL ''1 years'')
            END
    )
...
ORDER BY
    ...
    , ntss_db5_om.ord_no
    , ctlno;', 2, '[]'::jsonb, '1', '{"applications": [5]}'::jsonb, '{"classes": []}'::jsonb, '患者病歴情報　@facilityCd @fromDate @toDate使用 {"Mergekey": ["patid"]}', '2021-02-26 17:51:54.726', CURRENT_TIMESTAMP, NULL);
