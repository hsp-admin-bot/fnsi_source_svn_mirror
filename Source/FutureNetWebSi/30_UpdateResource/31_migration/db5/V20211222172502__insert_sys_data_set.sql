delete from "sys_data_set" where "sql_cd" in (-210,-211,-212);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-210, 'SELECT
  TO_CHAR(ord.rst_start_date, ''YYYYMMDD'') AS start_date
  , TO_CHAR(ord.rst_start_date, ''HH24MISS'') AS start_time
  , TO_CHAR(ord.rst_end_date, ''YYYYMMDD'') AS end_date
  , TO_CHAR(ord.rst_end_date, ''HH24MISS'') AS end_time 
  , COALESCE ( pat.medical_care_info ->> ''dialysis_start_date'', '''' ) AS dialysis_start_date 
  , COALESCE ( ord.rst_bed_cd, 0 ) AS rst_bed_cd
  ...
  , COALESCE ( ord.rst_treatment_cd, 0 ) AS rst_treatment_cd
...
WHERE
  ord.ord_no = @ordNo', 2, '[{}]', '0', '{"applications": [4]}', NULL, 'NECiS)透析実績の透析指示情報を取得する', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
INSERT INTO "sys_data_set"("sql_cd", "sql", "db_class", "detail", "can_repeat", "use_application", "report_class", "memo", "reg_date", "up_date", "pre_sql_info") VALUES (-211, 'WITH department_source_info AS ( 
...
, DepartmentStaffInfoSrc AS ( 
  SELECT
    -- 診療科マスタ
    ord.rst_course_cd AS course_cd
  ...
  FROM
    ...
    LEFT OUTER JOIN mst_course AS course ON course.course_cd = ord.rst_course_cd
  WHERE
    ord.ord_no = @ordNo
) 
...
FROM
  DepartmentStaffInfoNoDefault AS info', 2, '[{}]', '0', '{"applications": [4]}', NULL, 'NECiS)透析実績の(依頼科コード、依頼医コード)を取得する', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
...
, dialysis_detail AS ( 
  ...
  FROM
    pat_main AS pat
    CROSS JOIN LATERAL json_array_elements ( pat.addition_info :: json ) addition
...
FROM
  dialysis_detail 
ORDER BY
  function_id ASC, item_code ASC', 2, '[{}]', '0', '{"applications": [4]}', NULL, 'NECiS)透析実績の透析指示明細を取得する', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL);
