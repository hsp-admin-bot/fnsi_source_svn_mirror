...
public class LogAspector {
  ...
  private static ThreadLocal<String> facilityCdTL = new ThreadLocal();

  /**
   * get facility_cd from current thread
   * @return
   */
  public static String getCurrentRequestFacilityCd(){
    return facilityCdTL.get();
  }

  /**
   * At the place where facility_cd can be obtained, call this method to set the facility_cd for current thread
   * to ensure that "LogAspector.outputLog()" can get facility_cd from current thread
   * @param facilityCd
   */
  public static void setCurrentRequestFacilityCd(String facilityCd){
    facilityCdTL.set(facilityCd);
  }

  /**
   * At the end of the request, clear the FacilityCdTL in current Thread, Avoid affecting other request
   * see {@link LogAspectorFacilityClearInterceptor} and {@link WebMvcConfiguration}
   */
  public static void clearFacilityCdTL(){
    if(facilityCdTL.get() != null){
      facilityCdTL.remove();
    }
  }
  ...
  @Pointcut(value = "execution (* jp.co.nikkiso.ntss.coop_api.service..*.*(..))")
...
// #9698 アプリケーションログの内容修正 20260327 add yangxuewang start
  ...
  public Object cutDaoPoint(ProceedingJoinPoint pjp) throws Throwable {
    ...
    try {
      ...
      map.put("LogAspectorCoopApi","SQL-LOG");
      ...
      map.put("Throwable", NtssUtils.ExcetionStackTraceToString((Exception) e));
      eventLogMessage.setLogMessage(toJson(map));
...
// #9698 アプリケーションログの内容修正 20260327 mod yangxuewang end
  ...
  private void outputLog(String className, String methodName, JoinPoint joinPoint, OUTPUT_KBN outputKbn, String errMessage) {
    ...
    // add by shiyw 2023-03-17: If the facilityCd cannot be obtained from the paramNames above,Attempt to obtain facility_cd from current thread --start
    if(StringUtils.isEmpty(facilitycd)){
      String currentRequestFacilityCd = getCurrentRequestFacilityCd();
      if(!StringUtils.isEmpty(currentRequestFacilityCd)){
        facilitycd = currentRequestFacilityCd;
      }
    }
    // add by shiyw 2023-03-17: If the facilityCd cannot be obtained from the paramNames above,Attempt to obtain facility_cd from current thread --end
