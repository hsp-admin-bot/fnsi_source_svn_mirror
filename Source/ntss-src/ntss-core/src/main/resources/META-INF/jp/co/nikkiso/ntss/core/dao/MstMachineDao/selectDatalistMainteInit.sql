...
SELECT DISTINCT mm.mainte_no                                                      -- 点検結果コード
 ...
 CROSS JOIN jsonb_array_elements(ml.detail_info_1) di1                            -- 詳細検査リスト1
  LEFT JOIN mst_mainte_category mc                                                -- 日常・定期点検項目グループマスタ
    ON mc.mainte_category_cd::TEXT  = di1 ->> 'cd'                                -- 点検カテゴリコード
 WHERE mm.is_del                    = '0'                                         -- 削除フラグ
   AND mm.is_disp                   = '1'                                         -- 表示フラグ
   AND di1 ->> 'isDisp'             = 'true'                                      -- 表示フラグ
), DATA3 AS (
SELECT DISTINCT mm.mainte_no                                                      -- 点検結果コード
     , mg.group_name AS category_name                                                           -- カテゴリー名
 ...
 INNER JOIN mst_mainte_layout ml                                                  -- 日常・定期点検レイアウトマスタ
   ...
   AND ml.is_del                    = '0'                                         -- 削除フラグ
   AND ml.is_disp                   = '1'                                         -- 表示フラグ
 CROSS JOIN jsonb_array_elements(ml.detail_info_2) di2                            -- 詳細検査リスト1
  LEFT JOIN mst_mainte_category mc                                                -- 日常・定期点検項目グループマスタ
    ON mc.mainte_category_cd::TEXT  = di2 ->> 'cd'                                -- 点検カテゴリコード
 WHERE mm.is_del                    = '0'                                         -- 削除フラグ
   AND mm.is_disp                   = '1'                                         -- 表示フラグ
   AND di2 ->> 'isDisp'             = 'true'                                      -- 表示フラグ
), DATA4 AS (
SELECT * FROM DATA1
 UNION ALL
SELECT * FROM DATA2
...
SELECT DISTINCT mst.machine_type_cd                                               -- 型式コード
 ...
 INNER JOIN mnt_mainte_main mnt                                                   -- 点検結果
