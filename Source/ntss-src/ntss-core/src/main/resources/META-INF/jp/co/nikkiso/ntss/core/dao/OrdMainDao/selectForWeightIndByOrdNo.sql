...
from
  ord_main O
  left outer join pat_main P on O.pat_id = P.pat_id
  left outer join mst_treatment T_IND on O.ind_treatment_cd = T_IND.treatment_cd
  left outer join mst_treatment T_RST on O.rst_treatment_cd = T_RST.treatment_cd
  left outer join mst_bed B on O.ind_bed_cd = B.bed_cd
  left outer join mst_kur K on O.ind_kur_cd = K.kur_cd
  left outer join mst_bed M on O.rst_bed_cd = M.bed_cd
where
  O.ord_no = /*ordNo*/0
  and
  O.is_del = '0'
