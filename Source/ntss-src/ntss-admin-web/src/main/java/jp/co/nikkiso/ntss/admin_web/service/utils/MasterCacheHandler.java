 ...
 */
public class MasterCacheHandler {

  private static ThreadLocal<MasterCacheHandler> masterCache = new ThreadLocal();

  public static MasterCacheHandler get(){
    if(masterCache.get() == null){
      MasterCacheHandler masterCacheHandler = new MasterCacheHandler();
  ...
  private Map<Integer, MstEquipment>  mstEquipmentMap = new HashMap();

  // Map<equipmentClassCd,MstEquipmentClass>
  private Map<Integer, MstEquipmentClass>  mstEquipmentClassMap = new HashMap();

  // Map<medicateTimingCd,MstMedicateTiming>
  private Map<Integer, MstMedicateTiming>  mstMedicateTimingMap = new HashMap();

  // Map<medicineMixCd,MstMedicineMix>
  private Map<Integer, MstMedicineMix>  mstMedicineMixMap = new HashMap();

  // Map<medicineClassCd,MstMedicineClass>
  private Map<Integer, MstMedicineClass>  mstMedicineClassMap = new HashMap();

  // Map<procedureCd,MstProcedure>
  private Map<Integer, MstProcedure>  mstProcedureMap = new HashMap();

  // Map<procedureCd,MstMedicine>
  private Map<Integer, MstMedicine>  mstMedicineMap = new HashMap();

  // Map<facilitySettingNo,FacilitySettingInfo>
  private Map<String, FacilitySettingInfo>  facilitySettingInfoMap = new HashMap();

  // Map<userId,MstPersonalUser>
  private Map<Long, MstPersonalUser>  mstPersonalUserMap = new HashMap();

  // Map<facilitySettingNo,List<MstTreatment>>
  private Map<String, List<MstTreatment>>  mstTreatmentInfoMap = new HashMap();

  // Map<facilitySettingNo,List<MstKur>>
  private Map<String, List<MstKur>>  mstKurInfoMap = new HashMap();

  // Map<Long, OrdMain>
  private Map<Long, OrdMain>  ordMainInfoMap = new HashMap();

  //Map<String, List<MstMachine>>
  private Map<String, List<MstMachine>>  machinesInfoMap = new HashMap();

  //Map<String, ComsvMntMachineState>
  private Map<String, ComsvMntMachineState>  comsvMntMachineStateInfoMap = new HashMap();

  //Map<Long, List<MntMachineState>>
  private Map<Long, List<MntMachineState>>  mntMachineStateInfoMap = new HashMap();

  //Map<String, List<MstBed>>
  private Map<String, List<MstBed>>  mntBedInfoMap = new HashMap();
  // ------ MstEquipment info cache ----- start
// ADD 8520 患者経過総合ビューアで、医療材料にタイアライザを追加後、500エラーが現れ、共通ローダーが解除されない。zhou start
  private Map<Integer, MstDialyzer>  mstDialyzerMap = new HashMap();
// ADD 8520 患者経過総合ビューアで、医療材料にタイアライザを追加後、500エラーが現れ、共通ローダーが解除されない。zhou END
  ...
  public MstPersonalUser getMstPersonalUser(Long userId){
    MstPersonalUserDao mstPersonalUserDao = ApplicationContextUtil.getBean(MstPersonalUserDao.class);
  ...
  //public void loadEquipmentMap(List<Integer> equipmentCds,boolean isLoadEquipClassInfo){
    ...
    // add 8520 患者経過総合ビューアで、医療材料にタイアライザを追加後、500エラーが現れ、共通ローダーが解除されない。zhou end
    MstEquipmentDao mstEquipDao = ApplicationContextUtil.getBean(MstEquipmentDao.class);
    // mod 8520 患者経過総合ビューアで、医療材料にタイアライザを追加後、500エラーが現れ、共通ローダーが解除されない。zhou start
    ...
    // add 8520 患者経過総合ビューアで、医療材料にタイアライザを追加後、500エラーが現れ、共通ローダーが解除されない。zhou start
    MstDialyzerDao dialyzerDao = ApplicationContextUtil.getBean(MstDialyzerDao.class);
  ...
  public void loadEquipmentClassMap(List<Integer> classCds){
    MstEquipmentClassDao mstEquipClassDao = ApplicationContextUtil.getBean(MstEquipmentClassDao.class);
  ...
  public void loadMstMedicateMap(List<Integer> medicineCds){
    MstMedicineDao mstMedicineDao = ApplicationContextUtil.getBean(MstMedicineDao.class);
  ...
  public void loadMstMedicineMixMap(List<Integer> medicineMixCds) {
    MstMedicineMixDao mstMedicineMixDao = ApplicationContextUtil.getBean(MstMedicineMixDao.class);
  ...
  public MstMedicateTiming getMstMedicateTimingByCd(Integer medicateTimingCd){
    MstMedicateTimingDao mstMedicateTimingDao = ApplicationContextUtil.getBean(MstMedicateTimingDao.class);
  ...
  public MstMedicineClass getMstMedicineClassByCd(Integer medicineClassCd){
    ...
    MstMedicineClassDao mstMedicineClassDao = ApplicationContextUtil.getBean(MstMedicineClassDao.class);
  ...
  public MstProcedure getMstProcedureByCd(Integer procedureCd){
    MstProcedureDao mstProcedureDao = ApplicationContextUtil.getBean(MstProcedureDao.class);
  ...
  public FacilitySettingInfo getFacilitySettingInfo(String facilityCd,String facilitySettingNo){
    MstFacilitySettingDao mstFacilitySettingDao = ApplicationContextUtil.getBean(MstFacilitySettingDao.class);
  ...
  public List<MstTreatment> getMstTreatmentInfo(String facilityCd){
    MstTreatmentDao mstTreatmentDao = ApplicationContextUtil.getBean(MstTreatmentDao.class);
    if(facilityCd == null){
  ...
  public List<MstKur> getMstKurInfo(String facilityCd){
    MstKurDao mstKurDao = ApplicationContextUtil.getBean(MstKurDao.class);
    if(facilityCd == null){
      Assert.isNull(facilityCd,"facilityCd can not be null");
    }
  ...
  public OrdMain getOrdMainInfo(Long ordNo){
    OrdMainDao ordMainDao = ApplicationContextUtil.getBean(OrdMainDao.class);
  ...
  public List<MstMachine> getMachinesInfo(String facilityCd, Long bedCd){
    MstMachineDao mstMachineDao = ApplicationContextUtil.getBean(MstMachineDao.class);
    if(facilityCd == null){
      Assert.isNull(facilityCd,"facilityCd can not be null");
    }
  ...
  public ComsvMntMachineState getComsvMntMachineStateInfo(String facilityCd, String machineTypeCd, String machineSerial){
    MntMachineStateDao mntMachineStateDao = ApplicationContextUtil.getBean(MntMachineStateDao.class);
    if(facilityCd == null){
      Assert.isNull(facilityCd,"facilityCd can not be null");
    }
  ...
  public List<MstBed> getMstBedInfo(String facilityCd){
    MstBedDao mstBedDao = ApplicationContextUtil.getBean(MstBedDao.class);
    if(facilityCd == null){
      Assert.isNull(facilityCd,"facilityCd can not be null");
    }
