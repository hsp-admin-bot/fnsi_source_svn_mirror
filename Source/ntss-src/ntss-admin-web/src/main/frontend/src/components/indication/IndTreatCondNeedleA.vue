...
<template>
  ...
  <!-- mod 9664補液及び透析液仕様修正します yangqingzhe end -->
    <v-ons-col class="action-condition-column">穿刺針(A)</v-ons-col>
...
export default {
  ...
  computed: {
    ...mapGetters("pat-viewer-treat-cond", {
      ...
      // add 8204 周安寧 start
      getIsUseFlagNeedleA: "getIsUseFlagNeedleA",
      // add 8204 周安寧 end
  ...
  async mounted() {
    ...
    const selectedEditMst = this.popoverData.popoverContentDataset.find(
      // mod #10937 20260428 Ji start
      // item => item.value == this.value
      item => item.value == (this.velue ?? this.value)
      // mod #10937 20260428 Ji end
      );
  ...
  methods: {
    ...
    async createPopoverData() {
      ...
      const popover = await buildMasterPopover(
        ...
        getErrorMessage("IndTreatCondNeedleA.vue", "createPopoverData", error);
        throw error;
      });
      ...
      let contentArr = options.map(item => ({
        ...
        text:
          // mod #10937 20260428 Ji start
          rstName && (String(item.value) == String(this.localSelectedCd) || String(item.value) == String(this.velue))
          // mod #10937 20260428 Ji end
            ? rstName
            : item.text,
      }));
