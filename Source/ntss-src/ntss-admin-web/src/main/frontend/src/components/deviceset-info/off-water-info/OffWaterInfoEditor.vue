...
export default {
  ...
  data() {
    return {
      ...
      settingData: {
        ...
        propsTareOffWaterInfoFlag: 1,
        propsModalData: this.propsModalData
      }
    };
  },
  ...
  methods: {
    ...
    async updateIndInfo(structData) {
      ...
      // this._compatSet(structData, "opeCd", "004031");
      ...
      // ).catch(error => {
        //FNSI-修正 VUEのエラー場合のログ対応 xiebzh add start
        // getErrorMessage('OffWaterInfoEditor.vue', 'updateIndInfo', error);
        //FNSI-修正 VUEのエラー場合のログ対応 xiebzh add end
        // throw error;
      // });
      ...
      // }
      // mod FNSI-連携イベントの登録適正化 楊 end
    //upd by ztc 2023-02-27 [Optimize runtime No.5482] --end region
    ...
    //   ).catch(error => {
    //     getErrorMessage('OffWaterInfoEditor.vue', 'updateIndInfo', error);
    //     console.log("OffWaterInfoEditor.vue updateIndInfo throw error; this.finishLoadingScreen();");
    //     this.finishLoadingScreen();
    //     throw error;
    //   });
    //
    ...
    // del #10553 ④装置設定＞風袋・除水補正にて指示への展開保存をした場合には、変更された治療予定毎の連携イベントが必要 piao end
      console.log("OffWaterInfoEditor.vue updateIndInfo this.finishLoadingScreen();");
      this.finishLoadingScreen();
  },
     ...
     */
    reflectPatInfo() {
      this.$refs.baseEditor.updatePatInfoDefault();
    },
