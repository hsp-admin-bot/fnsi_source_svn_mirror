...
<template>
  ...
  <!-- <v-ons-row> -->
    <v-ons-row :class="getIsUseFlagAntiCoagulantOneshotAmount ? 'cell-disabled' : ''">
  <!-- mod 9664補液及び透析液仕様修正します yangqingzhe end -->
    ...
    <v-ons-col class="action-condition-data-column">
      ...
      <!-- <custom-input-number
        ...
        :min-value="0"
        :max-value="9999999.999999999"
        :disabled="isDisabled || getIsUseFlagAntiCoagulantOneshotAmount"
        :loop-flg="false"
        :initial-value-lock="true"
        class="action-condition-input ntss-custom-input-cond"
        style="width: 90px"
      ...
      <!-- <custom-input-number
        ...
        :min-value="0"
        :max-value="9999999"
        :disabled="isDisabled || getIsUseFlagAntiCoagulantOneshotAmount || !getItemAuthorized('Indication', 'default_authority')"
        :loop-flg="true"
        :initial-value-lock="true"
        class="action-condition-input ntss-custom-input-cond"
        style="width: 90px"
...
export default {
  ...
  computed: {
    ...mapGetters("pat-viewer-treat-cond", {
      ...
      unitLabel: "getAntiCoagulantUnit",
      // add FNSI-【8630】単位が表示されない対応 曲 start
      antiCoagulantUnitChangeFlag: "getAntiCoagulantUnitChangeFlag",
      // add FNSI-【8630】単位が表示されない対応 曲 end
      decPoint: "getAntiCoagulantDecPoint",
      // add 8204 周安寧 start
      getIsUseFlagAntiCoagulantOneshotAmount: "getIsUseFlagAntiCoagulantOneshotAmount"
      // add 8204 周安寧 end
    })
  },
  ...
  watch: {
    inputValue: {
      handler(data) {
        this.setAntiCoagulantOneshotAmount(data);
      },
      deep: true
    },
    unitLabel() {
    ...
    // add FNSI-【8630】単位が表示されない対応 曲 start
    antiCoagulantUnitChangeFlag() {
      if (this.antiCoagulantUnitChangeFlag) {
  ...
  methods: {
    ...
    // add #10196 数値IFのスタイル全不正 linjunfeng end
    ...mapMutations("pat-viewer-treat-cond", ["setAntiCoagulantOneshotAmount"])
  }
};
</script>
