/**
 * パターン３（カテゴリ、予実、日付）表示コンポーネント.
 */
<template>
  <div class="tree-row" @click="onClick(model)">
    <!-- mod FNSI-No.342 患者イベント、検査結果、検査予定、一般撮影検査予定、処方の表示、機能遷移に対応 李 start -->
    <!-- <div class="tree-col">
      {{ model.treatDate }} {{ model.treatmentName }} {{ model.kurName }} {{ model.treatTime }} {{ model.bedName }}
    </div> -->
    ...
    <div v-else-if="model.type && model.type == 'pat_event'" class="tree-col">
      ...
      <!--{{ model.treatDate }} {{ model.eventStartTime + ' 〜 ' }} {{ model.eventEndTime }} {{ model.subCategoryName }} -->
      {{ model.treatDate }} {{ model.eventStartTime}}
      {{ (model.eventStartTime === null || model.eventStartTime === "" ) && (model.eventEndTime === null || model.eventEndTime === "" )? '' : ' 〜 '}} {{ model.eventEndTime }} {{ model.subCategoryName }}
    ...
    <!-- <div v-else-if="model.type && model.type == 'in_schedule'" class="tree-col">
      {{ model.treatDate }} {{ model.count + '項目' }}
    </div>
    ...
    <div v-else-if="model.type && model.type == 'prescription'" class="tree-col">
      {{ model.treatDate }} {{ model.prescriptionType }} {{ model.count + '剤' }}
    </div> -->
