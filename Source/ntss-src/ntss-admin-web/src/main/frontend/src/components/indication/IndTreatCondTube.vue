...
<template>
  ...
  <!-- <v-ons-row> -->
    <v-ons-row :class="getIsUseFlagTube ? 'cell-disabled' : ''">
  <!-- mod 9664補液及び透析液仕様修正します yangqingzhe end -->
    <v-ons-col class="action-condition-column">血液回路</v-ons-col>
    <common-master-selector
      ...
      :extraParams="{ treatDate: mstExtraParams.treatDate, classType: circuitClassType, initValue: displayInputValue.initCd, actualName: rstNameForCd }"
      ...
      :btnDisabled="getIsUseFlagTube || !getItemAuthorized('Indication', 'default_authority')"
...
export default {
  ...
  computed: {
    ...
    // add 8204 周安寧 start
    ...mapGetters("pat-viewer-treat-cond", {getIsUseFlagTube: "getIsUseFlagTube"}),
    // add 8204 周安寧 end
    rstDialysisState() {
  ...
  methods: {
    ...
    async createPopoverData() {
      ...
      const extraParams = {
        ...
        classType: this.circuitClassType,
        initValue
      };
      const context = {
      ...
      const popover = await buildMasterPopover(
        ...
        getErrorMessage("IndTreatCondTube.vue", "createPopoverData", error);
        throw error;
      });
      ...
      const contentMapping = item => {
        return {
          ...
          text: rstName && item.equipmentCd == this.localSelectedCd ? rstName : getPrefix({
            ...
            normalClassType: CODES.EQUIPMENT_CLASS.BLOOD_CIRCUIT.classType,
            ...item
          }) + item.equipmentName,
          // #10659 禁忌、アレルギー、削除済み、分類不一致、期限切れ、削除済み含むの接頭文字対応 linjunfeng end
        };
