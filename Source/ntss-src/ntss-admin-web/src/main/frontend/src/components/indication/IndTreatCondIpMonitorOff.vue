...
<template>
  ...
  <!-- <v-ons-row> -->
    <v-ons-row :class="getIsUseFlagIpMonitorOff ? 'cell-disabled' : ''">
  <!-- mod 9664補液及び透析液仕様修正します yangqingzhe end -->
    <v-ons-col class="action-condition-column">IP電源OKモニタ切り</v-ons-col>
    <v-ons-col class="action-condition-data-column">
      <!-- mod 8204 周安寧 start -->
      <!-- <custom-radio
        :value="displayInputValue"
        :name="'treatCondIpMonitorOffRadio'"
        :disabled="isAntiCoagulantDisabled || !isIpUse"
        :radio-value="0"
        @change="setIpMonitorOff(isMonitorOff),changeButton()"
      >切
      </custom-radio>
      <custom-radio
        :value="displayInputValue"
      ...
      <!--   :value="displayInputValue" -->
      <!--   :name="'treatCondIpMonitorOffRadio'" -->
      <!--   :disabled="isAntiCoagulantDisabled || !isIpUse || getIsUseFlagIpMonitorOff" -->
      <!--   :radio-value="0" -->
      <!--   @change="setIpMonitorOff(isMonitorOff),changeButton()" -->
      <!-- >切 -->
      <custom-radio
...
export default {
  ...
  computed: {
    ...mapGetters("pat-viewer-treat-cond", {
      ...
      isIpUse: "isIpUse",
      // add 8204 周安寧 start
      getIsUseFlagIpMonitorOff: "getIsUseFlagIpMonitorOff"
      // add 8204 周安寧 end
    }),
  ...
  mounted() {
    this.treatItemCd = "37";
    // 初期表示時にストアのIP電源OKモニタ切りフラグを現在の値に応じて書き換える
    this.setIpMonitorOff(this.isMonitorOff);
  },
  ...
  methods: {
    ...
    ...mapMutations("pat-viewer-treat-cond", ["setIpMonitorOff"])
  }
};
</script>
