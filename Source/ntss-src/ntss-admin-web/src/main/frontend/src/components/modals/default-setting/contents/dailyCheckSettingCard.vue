  ...
  async created() {
    ...
    this.$nextTick(() => {
      ...
      if (!this.editRecord || Object.keys(this.editRecord).length === 0) {
        ...
        } else if (!this.mstBedGroup.some(bg => +bg.roomBedGroupCd === +this.editRecord[DAILY_CHECK.KEY_NAME_BED_GROUP_CD])) {
          // NOTE: マスタ削除された場合、「null : すべて」を再設定
          this.editRecord[DAILY_CHECK.KEY_NAME_BED_GROUP_CD] = null;
        }
