delete from sys_data_set where sql_cd in (-1000002,-1000003,-1000005,-1000018,-1000020,-1000022,-1000023,-1000024);
...
JOIN
    ...
    AND mst.facility_cd = @facilityCd                              -- 修正：facility_cdを絞り込む
WHERE
...
WHERE
    pat.is_del = ''0''
    AND pat.facility_cd = @facilityCd                                       -- 修正：facility_cdを絞り込む
ORDER BY
...
WITH filtered_exam AS (
    ...
    WHERE
        ...
        AND to_date(@fromDate, ''YYYY/MM/DD'') <= ex.reg_exam_date
        AND ex.result_exam_date < to_date(@toDate, ''YYYY/MM/DD'')
