...
namespace NKKAccessCardService
{
    //----------------------------------------------------------------------------------------------------
    /// <summary>
    /// NKKAccessCardServiceクラス定義
    /// </summary>
    //----------------------------------------------------------------------------------------------------
    public partial class NKKAccessCardService : ServiceBase
    {
        //----------------------------------------------------------------------------------------------------
        /// <summary>
        /// サービス名称
        ...
        //----------------------------------------------------------------------------------------------------
        /// <summary>
        /// NKKAccessCardクラスオブジェクト
        /// </summary>
        //----------------------------------------------------------------------------------------------------
        private NKKAccessCard m_NkkAccessCard = null;

        private const string TRAY_PROCESS_NAME = "FNWSiAccessCardTool";
        private const string TRAY_EXE_NAME = "FNWSiAccessCardTool.exe";
        ...
        public NKKAccessCardService()
        {
            ...
            // 各オブジェクト構築
            this.m_NkkAccessCard = new NKKAccessCard(AppDomain.CurrentDomain.BaseDirectory);
            //this.m_ViewLogServer = new TdcViewLogServer(TdcViewLogModule.GetInstance().ViewLog);
            //this.m_ViewLogServer.LogExt = "VIEW";

            // ログ記録
            log.AddLogInfo(DateTime.Now, NKKAccessCardService.SERVICE_NAME, NKKLogging.LOGGING_CLASS.INFO, "サービス構築完了");
        }
        //----------------------------------------------------------------------------------------------------
        ...
        protected override void OnStart(string[] args)
        {
            ...
            {
                // OnStart処理開始
                log.AddLogInfo(DateTime.Now, NKKAccessCardService.SERVICE_NAME, NKKLogging.LOGGING_CLASS.INFO, "OnStart処理開始");

                // 以下の処理で失敗した場合はthrowされる

                // NKKAccessCard初期化処理
                if (this.m_NkkAccessCard.Start() == true)
                {
                    TryStartTrayInActiveUserSession(log);
                }
                else
                {
                    throw (new Exception("NKKAccessCard初期化、待ち受け失敗"));
                }

                // OnStart処理終了
                log.AddLogInfo(DateTime.Now, NKKAccessCardService.SERVICE_NAME, NKKLogging.LOGGING_CLASS.INFO, "OnStart処理終了");
            }
            catch (Exception ex)
            {
                // ログ記録：エラー
                log.AddLogInfo(DateTime.Now, NKKAccessCardService.SERVICE_NAME, NKKLogging.LOGGING_CLASS.ERROR, String.Format("Error,{0}", ex.Message));
        ...
        protected override void OnStop()
        {
            ...
            {
                // OnStop処理開始
                log.AddLogInfo(DateTime.Now, NKKAccessCardService.SERVICE_NAME, NKKLogging.LOGGING_CLASS.INFO, "OnStop処理開始");

                //// Tool待ち受け処理停止
                //this.m_ViewLogServer.StopListner();

                // NKKAccessCard終了
                this.m_NkkAccessCard.Stop();

                // OnStop処理終了
                log.AddLogInfo( DateTime.Now, NKKAccessCardService.SERVICE_NAME, NKKLogging.LOGGING_CLASS.INFO, "OnStop処理終了");
            }
            catch (Exception ex)
            {
                // ログ記録：エラー
