...
using LayoutDesignerUtilityLib.Properties;
using NKKLoggingLib;
...
namespace LayoutDesignerUtilityLib
{
    ...
    /// </summary>
    public class LayoutDesignerUtility
    {
        #region メンバ定数定義
        ...
        /// </summary>
        public const string DOWNLOAD_FILE_PASSWORD = "nkk";
        ...
        /// 値の取得のみ可能です。
        /// </summary>
        public static String DataVersionPath => String.Format("{0}{1}{2}", AppDirPath, System.IO.Path.DirectorySeparatorChar, LayoutDesignerConstant.FILE_NAME_DATAVERSION_FILE);
        ...
        public static Boolean PreAppStartUp()
        {
            ...
            try
            {
                ...
                // ログ識別子
                wLogging.LogExt = $"{LayoutDesignerConstant.LOG_FILE_EXT}_{System.Net.Dns.GetHostName()}";
                // バージョン情報記録用処理登録(ログが変わった場合にログの先頭に記録するため)
                wLogging.FirstWriteEvent = VersionInfos.GetVersionInfo;

                // アプリケーション共通設定ファイル読み込み
                LayoutDesignerUtility.LoadCommonSettingFile();

                // add #12093 データとモデルのバージョンが書き換わらない 高 start
                // アプリケーション共通設定ファイル読み込み
                LayoutDesignerUtility.LoadDataVersionFile();
                // add #12093 データとモデルのバージョンが書き換わらない 高 end

                // 必要ディレクトリの存在確認と作成
                LayoutDesignerUtility.CheckAndCreateAppDirectory();

                wRet = true;
            }
            catch (Exception ex)
            {
                LayoutDesignerUtility.RecordException(ex);
            }
        ...
        public static void RecordException(Exception aEx) => LayoutDesignerUtility.RecordException(aEx, true);
        ...
        public static void RecordException(Exception aEx, bool aIsShowMsg) => LayoutDesignerUtility.RecordException(null, aEx, aIsShowMsg);
        ...
        /// <param name="aEx">発生した例外情報</param>
        public static void RecordException(System.Windows.Forms.Form aOwner, Exception aEx) => LayoutDesignerUtility.RecordException(aOwner, aEx, true);
        ...
        public static void RecordException(System.Windows.Forms.Form aOwner, Exception aEx, bool aIsShowMsg, string MSG_TITLE)
        {
            try
            {
                ...
                // 書き込みメッセージを作成
                var wLines = LayoutDesignerUtility.MakeWriteMessageString(aEx).Split(new string[] { System.Environment.NewLine }, StringSplitOptions.None);
                ...
                if (aIsShowMsg)
                {
                    ...
                    RldMessageBox.Show(
                        aOwner,
                        LayoutDesignerUtility.MakeShowMessageString(aEx),
                        MSG_TITLE,
        ...
        public static void WriteLog(NKKLogging.LOGGING_CLASS aLoggingClass, string aMessage) => LayoutDesignerUtility.WriteLog(DateTime.Now, aLoggingClass, aMessage);
        ...
        {
            NKKLogging.GetInstance().AddLogInfo(aDatetime, LayoutDesignerUtility.PRODUCT_NAME, aLoggingClass, aMessage);
        }
        ...
        /// <returns></returns>
        public static Boolean DeleteFileIfExists(String aFilePath)
        {
            Boolean wRet = false;
            System.IO.FileInfo wInfo = null;

            try
            {
                // ファイル情報を取得
                wInfo = new System.IO.FileInfo(aFilePath);
            }
            catch (Exception ex)
            {
                LayoutDesignerUtility.RecordException(
                    new System.ApplicationException(@"ファイル情報の取得に失敗しました。", ex), true);
                return false;
            }

            // ファイルが存在しない場合は成功として抜ける
            if (!wInfo.Exists)
            {
                return true;
            }
            ...
            {
                LayoutDesignerUtility.RecordException(
                    new System.ApplicationException(@"ファイルの削除に失敗しました。", ex), true);
            }
        ...
        /// <returns></returns>
        public static Boolean DeleteDirectoryIfExists(String aDirPath)
        {
            Boolean wRet = false;
            System.IO.DirectoryInfo wInfo = null;

            try
            {
                // ディレクトリ情報を取得
                wInfo = new System.IO.DirectoryInfo(aDirPath);
            }
            catch (Exception ex)
            {
                LayoutDesignerUtility.RecordException(
                    new System.ApplicationException(@"ディレクトリ情報の取得に失敗しました。", ex), true);
                return false;
            }

            // ディレクトリが存在しない場合は成功として抜ける
            if (!wInfo.Exists)
            {
                return true;
            }
            ...
            {
                LayoutDesignerUtility.RecordException(
                    new System.ApplicationException(@"ディレクトリの削除に失敗しました。", ex), true);
            }
        ...
        /// <summary>
        /// 指定されたディレクトリへのフルパスが存在するか確認し無ければ作成します。
        /// </summary>
        /// <param name="aDirPath"></param>
        /// <returns>既に存在している場合または作成に成功した場合は True 、それ以外は False 。</returns>
        public static Boolean CheckAndCreateDirectory(string aDirPath)
        {
            Boolean wRet = false;

            try
            {
                if (!System.IO.Directory.Exists(aDirPath))
                {
                    System.IO.Directory.CreateDirectory(aDirPath);
                }
                wRet = true;
            }
            catch (Exception ex)
            {
                LayoutDesignerUtility.RecordException(
                    new System.ApplicationException("フォルダの作成に失敗しました。", ex), true);
            }
            return wRet;
        }
        ...
        /// 施設情報を設定ファイル内に保存します。
        /// </summary>
        /// <param name="aFacilityCode"></param>
        /// <param name="aFacilityHash"></param>
        /// <returns></returns>
        public static Boolean SaveFacilityInfo(String aFacilityCode, String aFacilityHash)
        {
            Boolean wRet = false;

            try
            {
                // アプリケーション共通設定ファイル有無チェック
                if (!System.IO.File.Exists(LayoutDesignerUtility.ConfigFilePath))
                {
                    throw new System.IO.FileNotFoundException("アプリケーション共通設定ファイルが見つかりません。", LayoutDesignerUtility.ConfigFilePath);
                }

                // アプリケーション共通設定ファイル読み込み
                var wInfo = TdcLib.SystemSettingInfo.GetInstance();
                if (!wInfo.Load(LayoutDesignerUtility.ConfigFilePath))
                {
                    throw new System.ApplicationException("アプリケーション共通設定ファイルの読み込みに失敗しました。", wInfo.Error);
                }

                // 施設ハッシュ値
                wInfo.SetValue(CONFIG_COMMON_SECTION, "FacilityHash", aFacilityHash);
        ...
        /// <summary>
        /// アプリケーションの動作に必要なディレクトリの存在を確認し、存在しない場合は作成します。
        /// </summary>
        private static void CheckAndCreateAppDirectory()
        {
            const String ERR_TEXT_SUFFIX = "の確認・作成中にエラーが発生しました。";

            try
            {
                // アプリケーション設定ファイル保存先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.ConfigDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("設定ファイル保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // アプリケーションサンプルレイアウト保存先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.ModelDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("サンプルレイアウト保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // アプリケーション一時保存帳票ファイル保存先ディレクトリ
                try
                {
                    if (System.IO.Directory.Exists(LayoutDesignerUtility.TempDirPath))
                    {

                        String wTempBackDirPath = LayoutDesignerUtility.TempDirPath + ".bak";

                        if (!System.IO.Directory.Exists(wTempBackDirPath))
                        {
                            System.IO.Directory.Move(LayoutDesignerUtility.TempDirPath, wTempBackDirPath);
                        }
                    }
                    CheckAndCreateDirectory(LayoutDesignerUtility.TempDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("一時保存帳票保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // アプリケーション作業用ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.WorkDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("作業用ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // アプリケーションログ出力先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.LogDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("ログ保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // アプリケーションマニュアル保存先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.ManualDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("マニュアル保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // 画像保存先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.ImageDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("画像ファイル保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // アプリケーション最新バージョンファイルダウンロード先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.UpdateContentsDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("最新バージョン保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }

                // バックパップファイル保存先ディレクトリ
                try
                {
                    CheckAndCreateDirectory(LayoutDesignerUtility.BackupDirPath);
                }
                catch (Exception ex)
                {
                    throw new System.IO.IOException(String.Format("バックアップファイル保存先ディレクトリ{0}", ERR_TEXT_SUFFIX), ex);
                }
            }
            catch (Exception ex)
            {
                throw new System.Exception("アプリケーション用ディレクトリの作成中にエラーが発生しました。", ex);
            }
        }
        ...
        private static String MakeWriteMessageString(Exception aEx)
        {
            ...
            wRet = LayoutDesignerUtility.MakeWriteMessageStringFromOneException(wRet, aEx, 1);
        ...
        /// <summary>
        /// アプリケーション共通設定ファイルを読み込みます。
        /// </summary>
        private static void LoadCommonSettingFile()
        {
            try
            {
                NKKLogging wLogging = NKKLogging.GetInstance();
                ...
                // ログ格納先ディレクトリ設定
                wLogging.LogFolder = wInfo.GetSingleLineValue(CONFIG_LOG_SECTION, "Folder", String.Empty).Trim();
                // ログ保持日数
                if (Int32.TryParse(wInfo.GetSingleLineValue(CONFIG_LOG_SECTION, "KeepNumberOfDays", String.Empty).Trim(), out Int32 wDays) && wDays >= 0)
                {
                    LayoutDesignerUtility.LogFileKeepNumberOfDays = wDays;
                }
                // ログ保持ファイル数
                if (Int32.TryParse(wInfo.GetSingleLineValue(CONFIG_LOG_SECTION, "KeepNumberOfCount", String.Empty).Trim(), out Int32 wCount) && wCount >= 0)
                {
                    LayoutDesignerUtility.LogFileKeepNumberOfCount = wCount;
                }

                // ユーザID
                LayoutDesignerUtility.LoginID = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "LoginID", String.Empty);
                // パスワード
                LayoutDesignerUtility.Password = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "Password", String.Empty);
                // 施設ハッシュ値
                LayoutDesignerUtility.FacilityHash = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "FacilityHash", String.Empty);
                // 接続情報
                // mod 2022-04-22 #6860 最後のスラッシュを除く Thach start
                LayoutDesignerUtility.BaseUri = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "BaseUri", String.Empty).Trim(' ', '/');
                // mod 2022-04-22 #6860 最後のスラッシュを除く Thach end

                // add 20210820  トグルボタン表示 -- 鄭 start
                // プログラム名
                LayoutDesignerUtility.BaseName = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "BaseName", String.Empty);
                // add 20210816 トグルボタン表示 -- 鄭 end


                // Amazon S3 バケット使用フラグ
                if (Boolean.TryParse(wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "UseS3Bucket", Boolean.TrueString), out Boolean wUseS3Bucket))
                {
                    //LayoutDesignerUtility.UseS3Bucket = wUseS3Bucket;
                    LayoutDesignerUtility.UseS3Bucket = true; // Localモードはもう使わない
                }
                // Amazon S3 バケット
                //LayoutDesignerUtility.S3Bucket = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "S3Bucket", String.Empty);
                ...
                NKKWebAccessLib.NKKWebAccess.ClientCertificateSearchValue2 = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "ClientCertificateSearchValue2", string.Empty).Trim();

                // add 2020-08-10 FNSI-仕様追加 ページ追加 李 start
                // ヘルプファイル名を取得する
                HelpDocument = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "HelpDocument", "readme.pdf").Trim();
                // データのバージョン情報を取得する
                // del #12093 データとモデルのバージョンが書き換わらない 高 start
                //DataVersion = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "DataVersion", string.Empty).Trim();
                // del #12093 データとモデルのバージョンが書き換わらない 高 end
                // add 2020-08-10 FNSI-仕様追加 ページ追加 李 end

                // add FNSI-仕様追加 帳票モデルのバージョン判定 夏 start
                // 帳票モデルのバージョン情報を取得する
                // del #12093 データとモデルのバージョンが書き換わらない 高 start
                //ModelVersion = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "ModelVersion", string.Empty).Trim();
                // del #12093 データとモデルのバージョンが書き換わらない 高 end
                // add FNSI-仕様追加 帳票モデルのバージョン判定 夏 end
                ...
                // add #7297 初回リリース対象外の機能とその関連機能を隠す xiaosonglei start
                // 新規登録タブのドロップダウンリスト設定
                LayoutDesignerUtility.CreateDropCodeList = wInfo.GetSingleLineValue(CONFIG_COMMON_SECTION, "CreateDropCodeList", String.Empty);
                // add #7297 初回リリース対象外の機能とその関連機能を隠す xiaosonglei end
            }
            catch (Exception ex)
            {
                throw new System.Exception("アプリケーション共通設定ファイルの読込中にエラーが発生しました。", ex);
            }
        }
        ...
        /// アプリケーション共通設定ファイルを読み込みます。
        /// </summary>
        private static void LoadDataVersionFile()
        {
            try
            {
                NKKLogging wLogging = NKKLogging.GetInstance();
                ...
                // アプリケーション共通設定ファイル有無チェック
                if (!System.IO.File.Exists(LayoutDesignerUtility.DataVersionPath))
                {
                    throw new System.IO.FileNotFoundException(@"アプリケーション共通設定ファイルが見つかりません。", LayoutDesignerUtility.DataVersionPath);
                }

                // アプリケーション共通設定ファイル読み込み
                var wInfo = TdcLib.SystemSettingInfo.GetInstance();
                if (!wInfo.Load(LayoutDesignerUtility.DataVersionPath))
                {
                    throw new System.ApplicationException(@"アプリケーション共通設定ファイルの読み込みに失敗しました。", wInfo.Error);
                }
        ...
        /// add 2020-08-13 FNSI-仕様追加 プロファイルにデータを保存する 李 start
        /// <summary>
        /// 配置情報を設定ファイル内に保存します。
        /// </summary>
        ...
        /// <returns></returns>
        public static Boolean SaveConfigInfo(String node, String text)
        {
            Boolean wRet = false;
            ...
            {
                // アプリケーション共通設定ファイル有無チェック
                // mod #12093 データとモデルのバージョンが書き換わらない 高 start
                //if (!System.IO.File.Exists(LayoutDesignerUtility.ConfigFilePath))
                //{
                //    throw new System.IO.FileNotFoundException("アプリケーション共通設定ファイルが見つかりません。", LayoutDesignerUtility.ConfigFilePath);
                //}
                ...
                // mod #12093 データとモデルのバージョンが書き換わらない 高 end
                ...
                wInfo.SetValue(CONFIG_COMMON_SECTION, node, text);
        ...
        /// add 2020-08-13 FNSI-仕様追加 プロファイルにデータを保存する 李 end
