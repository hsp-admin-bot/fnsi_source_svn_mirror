...
do
	...
	curl ${FTP_CURL_OPTION} -u ${FTP_USER}:${FTP_PW} ftp://${FTP_URL} > ${FTP_LIST1} 2> ${FTP_LOG_FILE}
	LAST_ERROR_NO=$?
	if [ "${LAST_ERROR_NO}" != 0 ]
	then
		if [ "${LAST_ERROR_NO}" != 7 ]
	...
	curl ${FTP_CURL_OPTION} -l -u ${FTP_USER}:${FTP_PW} ftp://${FTP_URL} > ${FTP_LIST1FILES} 2> ${FTP_LOG_FILE}
	LAST_ERROR_NO=$?
	if [ "${LAST_ERROR_NO}" != 0 ]
	then
	...
	curl ${FTP_CURL_OPTION} -u ${FTP_USER}:${FTP_PW} ftp://${FTP_URL} > ${FTP_LIST2} 2> ${FTP_LOG_FILE}
	LAST_ERROR_NO=$?
	if [ "${LAST_ERROR_NO}" != 0 ]
	then
	...
	then
		...
		do
			# ダウンロード対象ファイル
			#echo ${line}
			curl ${FTP_CURL_OPTION} -u ${FTP_USER}:${FTP_PW} ftp://${FTP_URL}${line} -R -o ${FTP_FOLDER}${line} 2> ${FTP_LOG_FILE}
			LAST_ERROR_NO=$?
			if [ "${LAST_ERROR_NO}" != 0 ]
			then
