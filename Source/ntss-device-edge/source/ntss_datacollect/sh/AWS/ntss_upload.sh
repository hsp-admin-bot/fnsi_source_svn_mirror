...
#  echo "authorization_header: ${authorization_header}"
  ...
  LAST_ERROR_NO=0
  put_file="$($MKTEMP)"
  echo "#!/bin/sh\ncurl -Ssf ${url} -X PUT -T '${upload_file}' -H '${authorization_header}' -H 'Content-Type:${aws_content_type}' -H 'Content-Length:${upload_file_size}' -H 'Host:${aws_host}' -H 'x-api-key:${aws_api_key}' -H 'x-amz-acl:${aws_acl}' -H 'x-amz-content-sha256:${upload_file_hash}' -H 'x-amz-Date:${message_time}' -o /dev/null -w %{http_code}" > ${put_file}
  chmod 777 ${put_file} 
