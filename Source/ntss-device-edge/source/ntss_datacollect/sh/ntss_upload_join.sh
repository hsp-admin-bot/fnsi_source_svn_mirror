...
#  upload_path=$2
  ...
  upload_file=$(echo -n $3 | base64 -w 0)
  log1=$4
  log2=$5
...
#  echo "URL:"${url}
#  echo "path:"$2"->"${upload_path}
#  echo "file:"$3"->"${upload_file}
#  echo "log1:"${log1}
#  echo "log2:"${log2}
  ...
  LAST_ERROR_NO=0
  put_file="$($MKTEMP)"
  echo "#!/bin/sh\ncurl -Ssf --connect-timeout 10 ${url} -X POST -H 'SSECCAYEK:${seckey}' -F 'fileName=${upload_file}' -F 'filePath=${upload_path}' -o /dev/null -w %{http_code} > '${log1}' 2> '${log2}'" > ${put_file}
  chmod 777 ${put_file} 
