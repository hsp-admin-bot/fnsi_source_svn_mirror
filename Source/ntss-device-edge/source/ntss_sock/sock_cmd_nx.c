...
* @details NX通信装置に送信するコマンド作成
*/
int sock_cmd_nx(int thread_no, struct scn_data_fm *sp)
{
    int i;
    int sndlen;
	char bcd[10];
		...
		case C_CLOCK:		// 装置時計設定
    		...
    		*bp++ = sp->sno;
			if ( sp->comflg == C_OPTRD ) {
				sprintf(logMsg, "通信スレッドNX[%d] : 装置オプション読出 [%d]", thread_no, sp->dev_idx);
				...
				sprintf(logMsg, "通信スレッドNX[%d] : 装置時計設定 [%d]", thread_no, sp->dev_idx);
			    LogOutputs(NTSS_LOG_INFO, logMsg, 0, sp->deviceType, sp->devid);
				short_set(bp, 4);
				bp += 2;
				short_set(bp, 12);
